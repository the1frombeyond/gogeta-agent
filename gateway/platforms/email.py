"""
Email platform adapter for the Gogeta gateway.

Allows users to interact with Gogeta by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "gogeta-agent" "version" "{_gogeta_version}" '
            '"vendor" "NousResearch" '
            '"support-email" "noreply@gogeta.ai")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Gogeta Agent")
        ...
        msg_id = f"<gogeta-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
