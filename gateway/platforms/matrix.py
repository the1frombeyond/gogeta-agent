...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Gogeta command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Gogeta accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Gogeta ``/command`` text."""
...
# Uses get_gogeta_home() so each profile gets its own Matrix store.
from gogeta_constants import get_gogeta_dir as _get_gogeta_dir

_STORE_DIR = _get_gogeta_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("GOGETA_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("GOGETA_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    async def connect(self) -> bool:
        ...
        if self._encryption:
            ...
            try:
                ...
                _acct_id = self._user_id or "gogeta"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Gogeta Agent" become "Agent".
        """
        if not body:
            return ""
