...
There are two ways to add a platform to the Gogeta gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.gogeta/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Gogeta code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `gogeta gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `gogeta_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Gogeta core.
...
And add it to the `gogeta-gateway` composite:

```python
"gogeta-gateway": {
    "includes": [..., "gogeta-your-platform"]
}
```
...
## 12. Status Display (`gogeta_cli/status.py`)
...
## 13. Gateway Setup Wizard (`gogeta_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ gogeta_cli/ toolsets.py \
  --include="*.py" -l | sort -u
