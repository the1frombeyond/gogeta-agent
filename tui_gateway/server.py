...
from gogeta_constants import (
    ...
    get_gogeta_home_override,

    reset_gogeta_home_override,

    set_gogeta_home_override,
...
load_gogeta_dotenv(
    ...
    gogeta_home=_gogeta_home, project_env=Path(__file__).parent.parent / ".env"
...
# appends every unhandled exception to ~/.gogeta/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_gogeta_home, "logs", "tui_gateway_crash.log")
...
try:
    ...
    _slash_timeout = float(os.environ.get("GOGETA_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    ...
    _ws_orphan_reap_grace = float(
        ...
        os.environ.get("GOGETA_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
...
try:
    ...
    _rpc_pool_workers = max(
        ...
        2, int(os.environ.get("GOGETA_TUI_RPC_POOL_WORKERS") or "4")
...
class _SlashWorker:
    ...
    """Persistent GogetaCLI subprocess for slash commands."""
...
# ``profile`` on those calls; we open that profile's db and bind its GOGETA_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_gogeta_home).resolve():
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `gogeta` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # GOGETA_HOME so config/skills/model resolve to it, and hand the
            ...
            if profile_home:
                ...
                home_token = set_gogeta_home_override(profile_home)
            ...
            if home_token is not None:
                ...
                reset_gogeta_home_override(home_token)
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _gogeta_home. Cache is keyed on the resolved path, so

        # profiles don't clobber each other.

        override = get_gogeta_home_override()

        home = override if isinstance(override, str) and override else _gogeta_home
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["GOGETA_GATEWAY_SESSION"] = "1"
...
def resolve_skin() -> dict:
    ...
    try:
        ...
        from gogeta_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    ...
    env = (
        ...
        os.environ.get("GOGETA_MODEL", "")

        or os.environ.get("GOGETA_INFERENCE_MODEL", "")
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("GOGETA_TUI_PROVIDER", "").strip()
    ...
    try:
        ...
        from gogeta_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("GOGETA_INFERENCE_PROVIDER", "").strip().lower()
...
def _load_tool_progress_mode() -> str:
    ...
    env = os.environ.get("GOGETA_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    explicit = [
        ...
        for item in os.environ.get("GOGETA_TUI_TOOLSETS", "").split(",")
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                ...
                print(
                    ...
                    "[tui] GOGETA_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from gogeta_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            ...
            print(
                ...
                f"[tui] ignoring unknown GOGETA_TUI_TOOLSETS entries: {', '.join(unknown)}",
        ...
        if disabled:
            ...
            print(
                ...
                "[tui] ignoring disabled MCP servers in GOGETA_TUI_TOOLSETS "
        ...
        fallback_notice = (
            ...
            "[tui] no valid GOGETA_TUI_TOOLSETS entries; using configured CLI toolsets"
    ...
    try:
        ...
        if fallback_notice is not None:
            ...
            print(
                ...
                "[tui] no valid GOGETA_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    ...
    from gogeta_cli.model_switch import parse_model_flags, switch_model
    ...
    try:
        ...
        from gogeta_cli.config import get_compatible_custom_providers, load_config
    ...
    # We deliberately do NOT write process-global env vars (GOGETA_MODEL /

    # GOGETA_INFERENCE_MODEL / GOGETA_TUI_PROVIDER / GOGETA_INFERENCE_PROVIDER)
...
def _get_usage(agent) -> dict:
    ...
    # GOGETA_DEV_CREDITS so the payload stays clean when the flag is off.

    if is_truthy_value(os.environ.get("GOGETA_DEV_CREDITS")):
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        from gogeta_cli.banner import get_available_skills
    ...
    try:
        ...
        from gogeta_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from gogeta_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    ...
    try:
        ...
        try:
            ...
            from gogeta_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    ...
    try:
        ...
        env_max = int(os.environ.get("GOGETA_TUI_MAX_TURNS", "") or 0)
...
def _parse_tui_skills_env() -> list[str]:
    ...
    raw = os.environ.get("GOGETA_TUI_SKILLS", "")
...
def _make_agent(
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("GOGETA_TUI_CHECKPOINTS")),

        pass_session_id=is_truthy_value(os.environ.get("GOGETA_TUI_PASS_SESSION_ID")),

        skip_context_files=is_truthy_value(os.environ.get("GOGETA_IGNORE_RULES")),

        skip_memory=is_truthy_value(os.environ.get("GOGETA_IGNORE_RULES")),
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind GOGETA_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `GOGETA_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    home_token = (
        ...
        set_gogeta_home_override(str(profile_home)) if profile_home is not None else None
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds GOGETA_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # Mirror the classic CLI /save: snapshot under the Gogeta profile home

    # (~/.gogeta/sessions/saved/) rather than the project/workspace CWD, and
...
# Layout:  $GOGETA_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_gogeta_home() / "spawn-trees"
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        ...
        home_token = None  # per-turn GOGETA_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                ...
                home_token = set_gogeta_home_override(_profile_home_str)
            ...
            if images:
                ...
                try:
                    ...
                    from gogeta_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    ...
                    from gogeta_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: gogeta_cli.voice unavailable")
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from gogeta_cli.clipboard import has_clipboard_image, save_clipboard_image
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Gogeta Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Gogeta/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Gogeta Desktop. If it serves Gogeta/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("GOGETA_YOLO_MODE"))

                if current:

                    os.environ.pop("GOGETA_YOLO_MODE", None)
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        ...
        try:
            ...
            from gogeta_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_gogeta_home), "display": display_gogeta_home()})
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from gogeta_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from gogeta_cli.auth import has_usable_secret
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
...

                {
                    ...
                    "error": "No Gogeta provider is configured.",
...
def _(rid, params: dict) -> dict:
    ...
    """Re-read ``~/.gogeta/.env`` into the gateway process via

    ``gogeta_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        ...
        return "bare `gogeta` is interactive — use `/gogeta chat -q …` or run `gogeta` in another terminal"

    a0 = argv[0].lower()

    if a0 == "setup":

        return "`gogeta setup` needs a full terminal — run it outside the TUI"

    if a0 == "gateway":

        return "`gogeta gateway` is long-running — run it in another terminal"

    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":

        return "`gogeta sessions browse` is interactive — use /resume here, or run browse in another terminal"

    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":

        return "`gogeta config edit` needs $EDITOR in a real terminal"
...
def _(rid, params: dict) -> dict:
    ...
    """Run `python -m gogeta_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            ...
            [sys.executable, "-m", "gogeta_cli.main", *argv],
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from gogeta_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from gogeta_cli.config import is_managed, save_env_value
        ...
        if pconfig.auth_type != "api_key":
            ...
            return _err(
                ...
                f"run `gogeta model` to configure",
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from gogeta_cli.auth import PROVIDER_REGISTRY, clear_provider_auth

        from gogeta_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    return os.environ.get("GOGETA_VOICE", "").strip() == "1"
...
def _voice_tts_enabled() -> bool:
    ...
    return os.environ.get("GOGETA_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["GOGETA_VOICE"] = "1" if enabled else "0"
        ...
        if not enabled:
            ...
            try:
                ...
                from gogeta_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["GOGETA_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "start":
            ...
            from gogeta_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    ...
    from gogeta_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from gogeta_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                ...
                from gogeta_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        api_key = os.environ.get("GOGETA_API_KEY", "") or cfg.get("api_key", "")

        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"

        base_url = os.environ.get("GOGETA_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
...

            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_gogeta_home / "config.yaml")],
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            ...
            from gogeta_cli.skills_hub import do_install
        ...
        if action == "browse":
            ...
            from gogeta_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            ...
            from gogeta_cli.skills_hub import inspect_skill
