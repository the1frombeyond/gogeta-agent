"""Persistent slash-command worker — one GogetaCLI per TUI session.
...
def _run(cli: GogetaCLI, command: str) -> str:
...
def main():
    ...
    os.environ["GOGETA_SESSION_KEY"] = args.session_key
    os.environ["GOGETA_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = GogetaCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
