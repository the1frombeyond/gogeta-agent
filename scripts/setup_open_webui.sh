...
# Bootstrap Open WebUI against Gogeta Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.gogeta/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-gogeta.sh
...
#   GOGETA_API_PORT=8642
#   GOGETA_API_HOST=127.0.0.1
#   GOGETA_API_MODEL_NAME='Gogeta Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Gogeta Agent WebUI}"
...
GOGETA_ENV_FILE="${GOGETA_ENV_FILE:-$HOME/.gogeta/.env}"
GOGETA_API_PORT="${GOGETA_API_PORT:-8642}"
GOGETA_API_HOST="${GOGETA_API_HOST:-127.0.0.1}"
GOGETA_API_CONNECT_HOST="${GOGETA_API_CONNECT_HOST:-127.0.0.1}"
GOGETA_API_MODEL_NAME="${GOGETA_API_MODEL_NAME:-Gogeta Agent}"
GOGETA_API_BASE_URL="http://${GOGETA_API_CONNECT_HOST}:${GOGETA_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-gogeta.sh"
LOG_DIR="$HOME/.gogeta/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$GOGETA_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$GOGETA_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.gogeta.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.gogeta</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.gogeta"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.gogeta"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-gogeta.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Gogeta Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-gogeta.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.gogeta/logs/openwebui.log
StandardError=append:%h/.gogeta/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-gogeta.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$GOGETA_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Gogeta API server is configured...'
  upsert_env API_SERVER_ENABLED true "$GOGETA_ENV_FILE"
  upsert_env API_SERVER_HOST "$GOGETA_API_HOST" "$GOGETA_ENV_FILE"
  upsert_env API_SERVER_PORT "$GOGETA_API_PORT" "$GOGETA_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$GOGETA_API_MODEL_NAME" "$GOGETA_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$GOGETA_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Gogeta gateway so API server settings take effect...'
  gogeta gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${GOGETA_API_CONNECT_HOST}:${GOGETA_API_PORT}/health" >/dev/null; then
    log 'Gogeta API server did not answer on the first check. Trying to start gateway in the background...'
    nohup gogeta gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${GOGETA_API_CONNECT_HOST}:${GOGETA_API_PORT}/health" >/dev/null
  ...
  log "Gogeta API endpoint: ${GOGETA_API_BASE_URL}"
