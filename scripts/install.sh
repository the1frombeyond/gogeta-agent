#!/bin/bash
# ============================================================================
# Gogeta Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/the1frombeyond/gogeta-agent/main/scripts/install.sh | bash
#
# Or with options:
...
# Python-driven tool session (e.g. Gogeta terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:the1frombeyond/gogeta-agent.git"
REPO_URL_HTTPS="https://github.com/the1frombeyond/gogeta-agent.git"
GOGETA_HOME="${GOGETA_HOME:-$HOME/.gogeta}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${GOGETA_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$GOGETA_INSTALL_DIR"
...
#   code at /usr/local/lib/gogeta-agent, command at /usr/local/bin/gogeta,
#   data still at /root/.gogeta (GOGETA_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        # --no-venv is the default; flag kept for backward compat
        --no-venv)
            shift
            ;;
        --skip-setup)
        ...
        --postinstall)
            ...
            ;;
        -h|--help)
            echo "Gogeta Agent Installer"
            echo ""
            echo "Usage: install.sh [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  --no-venv      (default) Don't create virtual environment"
            echo "  --skip-setup   Skip interactive setup wizard"
            echo "  --skip-browser Skip Playwright/Chromium install (browser tools won't work)"
            echo "  --no-skills    Start with a blank slate — seed no bundled skills, and"
            echo "                   write \$GOGETA_HOME/.no-bundled-skills so future"
            echo "                   'gogeta update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Gogeta.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.gogeta/gogeta-agent"
            echo "                   default (root, Linux): /usr/local/lib/gogeta-agent"
            echo "  --gogeta-home PATH  Data directory (default: ~/.gogeta, or \$GOGETA_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Gogeta installs the code under"
            echo "  /usr/local/lib/gogeta-agent and links the command into"
            echo "  /usr/local/bin/gogeta (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$GOGETA_HOME"
            echo "  (default /root/.gogeta).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$GOGETA_HOME/gogeta-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs gogeta setup"
            echo "                   Does NOT clone repo"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `gogeta update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Gogeta-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Gogeta Agent","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install gogeta command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `gogeta` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $GOGETA_HOME/gogeta-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $GOGETA_HOME/gogeta-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/gogeta-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $GOGETA_HOME/gogeta-agent — then preserve it)
#
# Always no-op when the user set --dir or $GOGETA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in GOGETA_HOME.
    if is_termux; then
        INSTALL_DIR="$GOGETA_HOME/gogeta-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$GOGETA_HOME/gogeta-agent/.git" ]; then
            INSTALL_DIR="$GOGETA_HOME/gogeta-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/gogeta-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/gogeta-agent"
        ...
        # /usr/local/bin/gogeta wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/gogeta"
        log_info "  Data:    $GOGETA_HOME (unchanged)"
...
get_gogeta_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/gogeta" ]; then
        echo "$link_dir/gogeta"
    else
        echo "gogeta"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/the1frombeyond/gogeta-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Gogeta owns its own uv at $GOGETA_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (gogeta_cli/managed_uv.py) looks in the same
    # place, so install.sh and `gogeta update` stay in sync.
    local _managed_uv="$GOGETA_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $GOGETA_HOME/bin ..."
    mkdir -p "$GOGETA_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/gogeta-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/gogeta-uv-installer.$$.sh")"
    ...
    # directly into $GOGETA_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$GOGETA_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Gogeta-
...
check_node() {
    ...
    # Prefer a Gogeta-managed Node from a previous run over a too-old system one.
    if [ -x "$GOGETA_HOME/node/bin/node" ] && node_satisfies_build "$("$GOGETA_HOME/node/bin/node" --version)"; then
        export PATH="$GOGETA_HOME/node/bin:$PATH"
        log_success "Node.js $("$GOGETA_HOME/node/bin/node" --version) found (Gogeta-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Gogeta-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.gogeta/node/..."
    ...
    # Place into ~/.gogeta/node/ and symlink binaries into the same bin dir
    # the gogeta command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$GOGETA_HOME/node"
    mkdir -p "$GOGETA_HOME"
    mv "$extracted_dir" "$GOGETA_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$GOGETA_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$GOGETA_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$GOGETA_HOME/node/bin/npx"  "$node_link_dir/npx"

    export PATH="$GOGETA_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$GOGETA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.gogeta/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Gogeta install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Gogeta Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="gogeta-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Gogeta behaves unexpectedly."
                    else
...
setup_venv() {
    log_info "Skipping virtual environment (global install)"
}
...
install_deps() {
        ...
        if ! "$PIP_PYTHON" -m pip install -e '.[termux-all]'; then
            ...
            if ! "$PIP_PYTHON" -m pip install -e '.[termux]'; then
                ...
                if ! "$PIP_PYTHON" -m pip install -e '.'; then
                    ...
                    log_info "Then re-run: cd $INSTALL_DIR && python -m pip install -e '.[termux-all]'"
                    exit 1
                fi
            fi
        fi
    ...
    if [ -f "uv.lock" ]; then
        ...
        log_info "(this resolves + downloads the curated [all] set;"
        log_info " first run can take 1-5 minutes; uv prints progress below)"
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"gogeta-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up gogeta command..."

    GOGETA_BIN="$(which gogeta 2>/dev/null || echo "")"
    if [ -z "$GOGETA_BIN" ]; then
        log_warn "gogeta not found on PATH after install"
        return 0
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$GOGETA_BIN" ]; then
        log_warn "gogeta entry point not found at $GOGETA_BIN"
        log_info "This usually means the pip install didn't complete successfully."
        if [ "$DISTRO" = "termux" ]; then
            log_info "Try: cd $INSTALL_DIR && python -m pip install -e '.[termux-all]'"
        else
    ...
    # Create a user-facing shim for the gogeta command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $GOGETA_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$GOGETA_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/gogeta"
    cat > "$command_link_dir/gogeta" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$GOGETA_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/gogeta"
    log_success "Installed gogeta launcher → $command_link_display_dir/gogeta"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "gogeta command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v gogeta' \
                >/dev/null 2>&1; then
        ...
        log_info "gogeta not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Gogeta Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Gogeta Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Gogeta Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so gogeta works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.gogeta directory structure (config at top level, code in subdir)
    mkdir -p "$GOGETA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.gogeta/.env (top level, easy to find)
    if [ ! -f "$GOGETA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$GOGETA_HOME/.env"
            log_success "Created ~/.gogeta/.env from template"
        else
            touch "$GOGETA_HOME/.env"
            log_success "Created ~/.gogeta/.env"
        fi
    else
        log_info "~/.gogeta/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$GOGETA_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.gogeta/config.yaml (top level, easy to find)
    if [ ! -f "$GOGETA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$GOGETA_HOME/config.yaml"
            log_success "Created ~/.gogeta/config.yaml from template"
        fi
    else
        log_info "~/.gogeta/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$GOGETA_HOME/SOUL.md" ]; then
        cat > "$GOGETA_HOME/SOUL.md" << 'SOUL_EOF'
# Gogeta Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Gogeta communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.gogeta/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.gogeta/"

    # Seed bundled skills into ~/.gogeta/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `gogeta update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'gogeta update'." \
            > "$GOGETA_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $GOGETA_HOME/.no-bundled-skills"
        log_info "  Future 'gogeta update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.gogeta/skills/ ..."
        if "$PYTHON_PATH" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.gogeta/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$GOGETA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$GOGETA_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.gogeta/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$GOGETA_HOME/.env"
    ...
    {
        echo ""
        echo "# Gogeta Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Gogeta will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (gogeta --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `gogeta update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'gogeta setup' after install."
        return 0
    fi
    ...
    # Run gogeta setup using the Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    python -m gogeta_cli.main setup < /dev/tty
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$GOGETA_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Gogeta to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$GOGETA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'gogeta whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                GOGETA_CMD="$(get_gogeta_command_path)"
                $GOGETA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'gogeta whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'gogeta gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $GOGETA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $GOGETA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: gogeta gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: gogeta gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $GOGETA_CMD gateway > "$GOGETA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.gogeta/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: gogeta gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: gogeta gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $GOGETA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $GOGETA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $GOGETA_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}gogeta${NC}              Start chatting"
    echo -e "   ${GREEN}gogeta setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}gogeta config${NC}       View/edit configuration"
    echo -e "   ${GREEN}gogeta config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}gogeta gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}gogeta update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'gogeta' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'gogeta' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'gogeta' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$GOGETA_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$GOGETA_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$GOGETA_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$GOGETA_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$GOGETA_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Gogeta for pip install"
    ...
    GOGETA_CMD="$(command -v gogeta 2>/dev/null || echo "")"
    if [ -n "$GOGETA_CMD" ]; then
        log_info "Running gogeta setup..."
        "$GOGETA_CMD" setup
    else
        log_warn "gogeta command not found on PATH"
        log_info "Try: python -m gogeta_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Gogeta-managed Node to PATH, so
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Gogeta" ]; then
            app="$desktop_dir/release/linux-unpacked/Gogeta"
        elif [ -x "$desktop_dir/release/linux-unpacked/gogeta" ]; then
            app="$desktop_dir/release/linux-unpacked/gogeta"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Gogeta.app" \
            "$desktop_dir/release/mac/Gogeta.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Gogeta is damaged and can't be opened").
    ...
    # checkout stays clean for the next `gogeta update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Gogeta-managed Node
            # provisioned during prerequisites/node-deps (at $GOGETA_HOME/node/bin)
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$GOGETA_HOME/.install_method"
            ;;
        *)
