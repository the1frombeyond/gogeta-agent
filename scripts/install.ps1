...
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/the1frombeyond/gogeta-agent/main/scripts/install.ps1)
#
...
param(
    # NoVenv is the default; flag kept for backward compat
    [switch]$NoVenv,
    [switch]$SkipSetup,
    [string]$Branch = "main",
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$GsaHome = $(if ($env:GOGETA_HOME) { $env:GOGETA_HOME } else { "$env:LOCALAPPDATA\gogeta" }),
    [string]$InstallDir = $(if ($env:GOGETA_HOME) { "$env:GOGETA_HOME\gogeta-agent" } else { "$env:LOCALAPPDATA\gogeta\gogeta-agent" }),
    ...
    # builds apps/desktop into a launchable Gogeta.exe.
    #
    # Why opt-in:
    #   * Gogeta-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Gogeta.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Gogeta.exe
    ...
    #     `gogeta desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:the1frombeyond/gogeta-agent.git"
$RepoUrlHttps = "https://github.com/the1frombeyond/gogeta-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Gogeta Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $GsaHome)) {
        New-Item -ItemType Directory -Force -Path $GsaHome | Out-Null
    }
    $envFile = Join-Path $GsaHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $GsaHome "node"
...
function Install-Uv {
    # Gogeta owns its own uv at $GsaHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (gogeta_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `gogeta update` stay in sync.
    $managedUv = Join-Path $GsaHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $GsaHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $GsaHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $GsaHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $GsaHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and GOGETA_HOME wasn't propagated).
...
function Install-Git {
    ...
    which Gogeta uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\gogeta\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Gogeta needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Gogeta copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\gogeta\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``GOGETA_GIT_BASH_PATH`` (User scope) so Gogeta can find it in a fresh
    ...
    # Download PortableGit into $GsaHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $GsaHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Gogeta features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Gogeta needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``GOGETA_GIT_BASH_PATH`` (User env scope) so Gogeta can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $GsaHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $GsaHome\git\usr\bin\bash.exe
    $candidates += "$GsaHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$GsaHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("GOGETA_GIT_BASH_PATH", $candidate, "User")
            $env:GOGETA_GIT_BASH_PATH = $candidate
            Write-Info "Set GOGETA_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Gogeta may not find Git Bash."
    Write-Info "If needed, set GOGETA_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        ...
        $env:Path = "$GsaHome\node;$env:Path"
        Write-Success "Node.js $version found (Gogeta-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Gogeta-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $GsaHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $GsaHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\gogeta-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$GsaHome\node") { Remove-Item -Recurse -Force "$GsaHome\node" }
                Move-Item $extractedDir.FullName "$GsaHome\node"
                ...
                $version = & "$GsaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $GsaHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\gogeta-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    $stashName = "gogeta-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Gogeta behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running gogeta processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/the1frombeyond/gogeta-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/the1frombeyond/gogeta-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/the1frombeyond/gogeta-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\gogeta-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\gogeta-agent-extract"
    ...
    # next `gogeta update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    Write-Info "Skipping virtual environment (global install)"
}
...
function Install-Dependencies {
    ...
    $pythonExeForParse = & $UvCmd python find $PythonVersion 2>$null
    $allExtras = @()
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install gogeta-agent package even with no extras. Inspect the uv pip install output above."
    }

    # Verify the dashboard deps specifically -- they're the most common thing
    # users hit and lazy-import errors from `gogeta dashboard` are confusing.
    $pythonExe = (& $UvCmd python find $PythonVersion 2>$null)
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `gogeta dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `gogeta dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up gogeta command..."
    
    $gsaBin = "$InstallDir"
    
    # Add the install dir to user PATH so gogeta is globally available
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$gsaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$gsaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $gsaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set GOGETA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\gogeta instead
    # of the Unix default ~/.gogeta
    $currentGsaHome = [Environment]::GetEnvironmentVariable("GOGETA_HOME", "User")
    if (-not $currentGsaHome -or $currentGsaHome -ne $GsaHome) {
        [Environment]::SetEnvironmentVariable("GOGETA_HOME", $GsaHome, "User")
        Write-Success "Set GOGETA_HOME=$GsaHome"
    }
    $env:GOGETA_HOME = $GsaHome
    
    # Update current session
    $env:Path = "$gsaBin;$env:Path"
    
    Write-Success "gogeta command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.gogeta-bootstrap-complete which tells the Gogeta
    ...
    # Gogeta-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `gogeta update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".gogeta-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.gogeta directory structure
    New-Item -ItemType Directory -Force -Path "$GsaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GsaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GsaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GsaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GsaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GsaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GsaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GsaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GsaHome\skills" | Out-Null
    ...
    if (-not (Test-Path $envPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.gogeta/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.gogeta/.env"
        }
    } else {
        Write-Info "~/.gogeta/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$GsaHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.gogeta/config.yaml from template"
        }
    } else {
        Write-Info "~/.gogeta/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Gogeta's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.gogeta/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.gogeta/"
    
    # Seed bundled skills into ~/.gogeta/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.gogeta/skills/ ..."
    try {
        & python "$InstallDir\tools\skills_sync.py" 2>$null
        Write-Success "Skills synced to ~/.gogeta/skills/"
    } catch {
        # Fallback: simple directory copy
        $bundledSkills = "$InstallDir\skills"
        $userSkills = "$GsaHome\skills"
        if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
            Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
            Write-Success "Skills copied to ~/.gogeta/skills/"
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Gogeta-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'gogeta setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\gogeta-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\gogeta-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\gogeta-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Gogeta` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Gogeta.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_gogeta_desktop command
    # resolves apps/desktop/release/win-unpacked/Gogeta.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Gogeta-managed
    ...
    # The --dir mode produces an unpacked Gogeta.exe in
    ...
    # are NOT trying to work around). The Gogeta icon + product name are
    # stamped onto Gogeta.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\gogeta-desktop-build-$(Get-Random).log"
    ...
    try {
        ...
        if ($code -ne 0) {
            ...
            # ENOENT on the final `electron` -> `Gogeta` rename: app-builder's
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Gogeta.exe",
        "$desktopDir\release\win-arm64-unpacked\Gogeta.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Gogeta.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Gogeta icon + identity are stamped onto Gogeta.exe by the
    ...
    #    Gogeta.exe. We deliberately do NOT point them at `gogeta desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Gogeta.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Gogeta.lnk')
        )
        ...
        # with the Gogeta icon, but without this the shortcut can keep drawing
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'gogeta setup'."
        return
    }
    ...
    # Run gogeta setup using the system Python directly
...
function Start-GatewayIfConfigured {
    ...
    $whatsappSession = "$GsaHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'gogeta whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: gogeta gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$GsaHome\logs\gateway.log"
            Start-Process -FilePath $gsaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$GsaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: gogeta gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: gogeta gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$GsaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$GsaHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$GsaHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$GsaHome\gogeta-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   gogeta              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   gogeta setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   gogeta config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   gogeta config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   gogeta gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   gogeta update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Gogeta repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Gogeta-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Gogeta to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/the1frombeyond/gogeta-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
