...
VERSION_FILE = REPO_ROOT / "gogeta_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "gogeta@marian.local": "Schrotti77",
    ...
    "274096618+gogeta-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@gogeta.ai": "shannonsands",
    ...
    "gogeta@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (GOGETA_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (GOGETA_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: gogeta-eval community link)
    ...
    "gsaagent26@gmail.com": "gsaagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+gsaagent26@users.noreply.github.com": "gsaagent26",
    ...
    "clement@gogeta.ai": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@gogeta": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.gogeta@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Gogeta runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level GOGETA_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Gogeta
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"gogeta-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "gogeta@gogeta.ai"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/gogeta",
    ...
    lines.append(f"# Gogeta Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Gogeta Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Gogeta Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Gogeta Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Gogeta Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Gogeta Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Gogeta Agent v{new_version} ({calver_date})' "
