#!/usr/bin/env python3
"""Live test harness for Gogeta Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any gogeta imports.
ORIGINAL_HOME = os.environ.get("GOGETA_HOME")
ORIGINAL_AUTH = Path.home() / ".gogeta" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.gogeta/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.gogeta/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="gogeta_ts_live_"))
    gogeta_home = home_dir / ".gogeta"
    gogeta_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, gogeta_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".gogeta" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, gogeta_home / ".env")
        ...
        load_gogeta_dotenv(gogeta_home=str(Path.home() / ".gogeta"))
    ...
    (gogeta_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return gogeta_home
...
def reset_module_state():
    """Drop cached modules so the new GOGETA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "gogeta_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["GOGETA_HOME"] = str(home)
...
def main():
    ...
    # Restore original GOGETA_HOME
    if ORIGINAL_HOME is not None:
        os.environ["GOGETA_HOME"] = ORIGINAL_HOME
    else:
