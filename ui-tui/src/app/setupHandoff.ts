import type { RunExternalProcess } from '@gogeta/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`gogeta ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching gogeta: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`gogeta ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
