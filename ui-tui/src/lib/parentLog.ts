...
const logDir = join(process.env.GOGETA_HOME?.trim() || join(homedir(), '.gogeta'), 'logs')
...
// a real ~/.gogeta (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('gogeta-tui: parent lifecycle log unavailable\n')
    }
  }
}
