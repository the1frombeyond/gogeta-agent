...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.GOGETA_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Gogeta on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via GOGETA_TUI_TRUECOLOR=1.
