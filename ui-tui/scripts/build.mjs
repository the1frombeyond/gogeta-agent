...
await build({
  ...
  // Skip the prebuilt @gogeta/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@gogeta/ink': resolve(root, 'packages/gogeta-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The gogeta_cli launcher
