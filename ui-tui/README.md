...
React + Ink terminal UI for Gogeta. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `GOGETA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.gogeta/.gogeta_history` or under `GOGETA_HOME`.
...
ui-tui/
  ...
  packages/gogeta-ink/   forked Ink renderer (local dep)
  ...
  src/
    ...
    types/
      ...
      gogeta-ink.d.ts    type declarations for @gogeta/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent GogetaCLI subprocess for slash commands
