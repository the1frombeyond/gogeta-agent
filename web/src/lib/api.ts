...
// (e.g. https://mission-control.tilos.com/gogeta/). The Python backend

// injects ``window.__GOGETA_BASE_PATH__`` into index.html based on the
...
function readBasePath(): string {
  ...
  const raw = window.__GOGETA_BASE_PATH__ ?? "";
...
export const GOGETA_BASE_PATH = readBasePath();

const BASE = GOGETA_BASE_PATH;
...
declare global {
  ...
  interface Window {
    ...
    __GOGETA_SESSION_TOKEN__?: string;

    __GOGETA_BASE_PATH__?: string;
    ...
    __GOGETA_AUTH_REQUIRED__?: boolean;
...
const SESSION_HEADER = "X-Gogeta-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__GOGETA_SESSION_TOKEN__;
  ...
  if (res.status === 401) {
    ...
    // (``gogeta update``, ``gogeta gateway restart``, etc.). A tab kept

    // open across the restart holds the OLD token in

    // ``window.__GOGETA_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__GOGETA_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      ...
      try {
        ...
        alreadyReloaded =
          ...
          sessionStorage.getItem("gogeta.tokenReloadAttempted") === "1";
      ...
      if (!alreadyReloaded) {
        ...
        try {
          ...
          sessionStorage.setItem("gogeta.tokenReloadAttempted", "1");
  ...
  if (res.ok) {
    ...
    // current ``window.__GOGETA_SESSION_TOKEN__`` is valid, so the next

    // 401 — if any — should be allowed to trigger its own reload cycle.

    try {

      sessionStorage.removeItem("gogeta.tokenReloadAttempted");
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__GOGETA_SESSION_TOKEN__;
  ...
  throw new Error("Session token not available — page must be served by the Gogeta dashboard server");
 ...
 * The dashboard's gated-mode WS auth (``gogeta_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  ...
  if (window.__GOGETA_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Gogeta-Session-Token`` header.

 *  - gated OAuth: no token header (it's absent by design); the

 *    ``gogeta_session_at`` cookie rides along via ``credentials: 'include'``.
 ...
 * ``window.__GOGETA_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateGogeta: () =>
    ...
    fetchJSON<ActionResponse>("/api/gogeta/update", { method: "POST" }),

  checkGogetaUpdate: (force = false) =>

    fetchJSON<UpdateCheckResponse>(

      `/api/gogeta/update/check${force ? "?force=true" : ""}`,
...
export interface SkillHubSource {
  ...
  /** gogeta-index only: whether the centralized index loaded. */
