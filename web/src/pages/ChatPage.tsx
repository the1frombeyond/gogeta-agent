/**
 * ChatPage — embeds `gogeta --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (gogeta_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { GOGETA_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${GOGETA_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__GOGETA_SESSION_TOKEN__ &&
    !window.__GOGETA_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `gogeta dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__GOGETA_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Gogeta TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[gogeta-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__GOGETA_AUTH_REQUIRED__``) this
    ...
    // forwarding them into Gogeta. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="gogeta-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
