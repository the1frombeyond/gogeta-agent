"""
Gogeta MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Gogeta-specific extra)

Usage:
    gogeta mcp serve
    gogeta mcp serve --verbose
...
logger = logging.getLogger("gogeta.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using GOGETA_HOME."""
    try:
        from gogeta_constants import get_gogeta_home
        return get_gogeta_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("GOGETA_HOME", Path.home() / ".gogeta")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from gogeta_constants import get_gogeta_home
        directory_file = get_gogeta_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("GOGETA_HOME", Path.home() / ".gogeta")
...
class EventBridge:
    ...
    This is the Gogeta equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from gogeta_constants import get_gogeta_home
            db_file = get_gogeta_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("GOGETA_HOME", Path.home() / ".gogeta")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Gogeta MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "gogeta",
        instructions=(
            "Gogeta Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Gogeta MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
