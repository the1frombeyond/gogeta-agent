"""
Baked-in build metadata for Gogeta Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``gogeta_cli/dump.py`` and ``gogeta_cli/banner.py``).  That doesn't work inside
...
To make ``gogeta dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$GOGETA_GIT_SHA``
arg into ``<project_root>/.gogeta_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``GOGETA_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".gogeta_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.gogeta_build_sha`` if present.  The file is
    written by the Dockerfile's ``GOGETA_GIT_SHA`` build-arg and contains
