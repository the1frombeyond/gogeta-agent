...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  ...
  actual ``python -m gogeta_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Gogeta_Gateway"

_TASK_DESCRIPTION = "Gogeta Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    ...
    """Return CLI args that preserve the current Gogeta profile."""

    from gogeta_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    args = ["-m", "gogeta_cli.main", *_current_profile_cli_args(), "gateway", command]
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("GOGETA_GATEWAY_INSTALL_START_NOW")

    old_start_on_login = os.environ.get("GOGETA_GATEWAY_INSTALL_START_ON_LOGIN")

    old_handoff = os.environ.get("GOGETA_GATEWAY_ELEVATED_HANDOFF")

    try:

        if start_now is not None:

            os.environ["GOGETA_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"

        if start_on_login is not None:

            os.environ["GOGETA_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"

        os.environ["GOGETA_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ...
            ("GOGETA_GATEWAY_INSTALL_START_NOW", old_start_now),

            ("GOGETA_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),

            ("GOGETA_GATEWAY_ELEVATED_HANDOFF", old_handoff),
...
def get_task_name() -> str:
    ...
    Default profile: ``Gogeta_Gateway``

    Named profile X: ``Gogeta_Gateway_<X>``

    """

    _assert_windows()

    # Local import to avoid circular module initialization during gogeta_cli boot.

    from gogeta_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\gogeta\\gateway-service\\<task_name>.cmd``

    (or ``<GOGETA_HOME>/gateway-service/<task_name>.cmd`` so per-profile

    Gogeta installs stay self-contained).
    ...
    script_dir = Path(get_gogeta_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``GOGETA_HOME`` whenever it
    ...
    source checkout only if ``GOGETA_HOME`` cannot be resolved yet.
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports GOGETA_HOME, PYTHONIOENCODING, VIRTUAL_ENV

      - invokes ``pythonw -m gogeta_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "GOGETA_HOME={gogeta_home}"')

    lines.append('set "PYTHONIOENCODING=utf-8"')

    lines.append('set "GOGETA_GATEWAY_DETACHED=1"')

    # VIRTUAL_ENV lets the gateway's own python detection find the venv

    # if someone imports gogeta_constants-based logic during startup.
    ...
    prog_args = [pythonw_path, "-m", "gogeta_cli.main"]
...
def _write_task_script() -> Path:
    ...
    gogeta_home = str(Path(get_gogeta_home()).resolve())

    profile_arg = _profile_arg(gogeta_home)



    content = _build_gateway_cmd_script(python_path, working_dir, gogeta_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Gogeta builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "gogeta_cli.main"]
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m gogeta_cli.main gateway run``
    ...
    log_dir = Path(get_gogeta_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("GOGETA_GATEWAY_INSTALL_START_NOW")

    env_start_on_login = _install_choice_from_env("GOGETA_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from gogeta_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `gogeta -p <profile> gateway install` must be safe.
    ...
    from gogeta_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        start_cmd = f"gogeta {profile_arg} gateway start" if profile_arg else "gogeta gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: gogeta gateway start")
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            ...
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                ...
                print("✓ Launched elevated Gogeta gateway install prompt.")
                ...
                if start_now:
                    ...
                    print("  Approve the Windows UAC prompt, then run: gogeta gateway status")
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: gogeta gateway start")
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"    type {Path(get_gogeta_home()).resolve()}\\logs\\gateway.log")

        print(f"    type {Path(get_gogeta_home()).resolve()}\\logs\\gateway-stdio.log")
...
def _print_next_steps() -> None:
    ...
    gogeta_home = Path(get_gogeta_home()).resolve()
    ...
    print(f"  type {gogeta_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                ...
                if _launch_elevated_uninstall():
                    ...
                    print("✓ Launched elevated Gogeta gateway uninstall prompt.")
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_gogeta_home()).resolve()
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            ...
            print("  Run: gogeta gateway install")
        ...
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: gogeta gateway start")
...
def stop() -> None:
    ...
    from gogeta_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
