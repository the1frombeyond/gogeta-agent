...
Pure logic: walks a Gogeta config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `gogeta doctor` and a future `gogeta migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Gogeta config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Gogeta config and return retirement issues.
