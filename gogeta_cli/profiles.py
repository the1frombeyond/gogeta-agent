...
Profile management for multiple isolated Gogeta instances.



Each profile is a fully independent GOGETA_HOME directory with its own

config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.

Profiles live under ``~/.gogeta/profiles/<name>/`` by default.



The "default" profile is ``~/.gogeta`` itself — backward compatible,
...
Usage::
    ...
    gogeta profile create coder          # fresh profile + bundled skills

    gogeta profile create coder --clone  # also copy config, .env, SOUL.md, skills

    gogeta profile create coder --clone-all  # full copy of source profile

    coder chat                           # use via wrapper alias

    gogeta -p coder chat                 # or via flag

    gogeta profile use coder             # set as sticky default

    gogeta profile delete coder          # remove profile + alias + service
...
_PROFILE_DIRS = [
    ...
    # See gogeta_constants.get_subprocess_home() and issue #4426.
...
# default profile (``~/.gogeta``).  Named profiles never contain these
...
#   gogeta-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `gogeta profile create --no-skills`.  When present in

# a profile's root, callers of seed_profile_skills() (fresh-create, `gogeta
...
# `gogeta skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      ...
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ...
         Gogeta infrastructure directories that only the default profile

         (``~/.gogeta``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_gogeta_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.gogeta) profile.
...
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    ...
    "gogeta-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    ...
    "gogeta", "default", "test", "tmp", "root", "sudo",

})



# Gogeta subcommands that cannot be used as profile names/aliases

_GOGETA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the gogeta root, NOT to the current GOGETA_HOME
    ...
    In Docker/custom deployments where GOGETA_HOME points outside

    ``~/.gogeta``, profiles live under ``GOGETA_HOME/profiles/`` so

    they persist on the mounted volume.

    """

    return _get_default_gogeta_home() / "profiles"





def _get_default_gogeta_home() -> Path:

    """Return the default (pre-profile) GOGETA_HOME path.



    In standard deployments this is ``~/.gogeta``.

    In Docker/custom deployments where GOGETA_HOME is outside ``~/.gogeta``

    (e.g. ``/opt/data``), returns GOGETA_HOME directly.
...
def _get_default_gogeta_home() -> Path:
    return _get_default_gogeta_home()
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_gogeta_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``gogeta``, ``test``,

    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk

    collisions (a ``gogeta`` profile inside ``~/.gogeta/``) or get refused
    ...
    if name == "default":
        ...
        return  # special alias for ~/.gogeta
    ...
    if name in _RESERVED_NAMES:
        ...
        raise ValueError(
            ...
            f"the Gogeta installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    ...
    """Resolve a profile name to its GOGETA_HOME directory."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, gogeta subcommands, existing binaries in PATH.
    ...
    if canon in _GOGETA_SUBCOMMANDS:

        return f"'{canon}' conflicts with a gogeta subcommand"
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``gogeta -p <profile>``. When the alias name equals

    the profile name this is trivial, but a custom alias (``gogeta profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``gogeta profile list`` for an unrelated profile.
...
# ``config.yaml``. ``config.yaml`` is the user-facing Gogeta config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    default_home = _get_default_gogeta_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``gogeta update`` skips re-seeding this profile's
    ...
    if canon == "default":
        ...
        raise ValueError(
            ...
            "Cannot create a profile named 'default' — it is the built-in profile (~/.gogeta)."
    ...
    if clone_all and source_dir:
        ...
        # Full copy of source profile (exclude sibling ~/.gogeta/profiles/)
    ...
    # Write the opt-out marker so seed_profile_skills() and `gogeta update`'s
    ...
    if no_skills:
        ...
        try:
            ...
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`gogeta profile create --no-skills`).\n"
    ...
    if description and description.strip():
        ...
        try:
            ...
            pass  # non-fatal — user can describe later with `gogeta profile describe`
    ...
    # `gogeta -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches GOGETA_HOME at module level.

    Returns the sync result dict, or None on failure.



    Profiles that opted out of bundled skills (via ``gogeta profile create
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "GOGETA_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        ...
        raise ValueError(
            ...
            "Cannot delete the default profile (~/.gogeta).\n"

            "To remove everything, use: gogeta uninstall"
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from gogeta_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set GOGETA_HOME so _profile_suffix resolves correctly

    old_home = os.environ.get("GOGETA_HOME")

    try:

        os.environ["GOGETA_HOME"] = str(profile_dir)

        from gogeta_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            ...
            os.environ["GOGETA_HOME"] = old_home

        elif "GOGETA_HOME" in os.environ:

            del os.environ["GOGETA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.gogeta/active_profile``. Use ``"default"`` to clear.
    ...
    if canon != "default" and not profile_exists(canon):
        ...
        raise FileNotFoundError(
            ...
            f"Create it with: gogeta profile create {canon}"
...
def get_active_profile_name() -> str:
    ...
    """Infer the current profile name from GOGETA_HOME.



    Returns ``"default"`` if GOGETA_HOME is not set or points to ``~/.gogeta``.

    Returns the profile name if GOGETA_HOME points into ``~/.gogeta/profiles/<name>``.

    Returns ``"custom"`` if GOGETA_HOME is set to an unrecognized path.
    ...
    resolved = gogeta_home.resolve()



    default_resolved = _get_default_gogeta_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        ...
        # The default profile IS ~/.gogeta itself — its parent is ~/ and its

        # directory name is ".gogeta", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        ...
        raise ValueError(
            ...
            "Specify it explicitly: gogeta profile import <archive> --name <name>"
    ...
    # dir.  Importing as "default" would target ~/.gogeta itself — disallow
    ...
    if canon == "default":
        ...
        raise ValueError(
            ...
            "Cannot import as 'default' — that is the built-in root profile (~/.gogeta). "

            "Specify a different name: gogeta profile import <archive> --name <name>"
    ...
    with tempfile.TemporaryDirectory(prefix="gogeta_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"gogeta_{old_name}"

    legacy_old_host = f"gogeta.{old_name}"

    new_host = f"gogeta_{new_name}"
    ...
    candidates = [
        ...
        _get_default_gogeta_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            ...
            if source_host.startswith("gogeta_"):
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    """Resolve a profile name to a GOGETA_HOME path string.



    Called early in the CLI entry point, before any gogeta modules

    are imported, to set the GOGETA_HOME environment variable.
