...
Pure display functions with no GogetaCLI state dependency.
...
from gogeta_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built gogeta — no local git history to count against).

UPDATE_AVAILABLE_NO_COUNT = -1



_UPSTREAM_REPO_URL = "https://github.com/gogeta/gogeta-agent.git"
...
def _fetch_pypi_latest(package: str = "gogeta-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    """Check whether a Gogeta update is available.



    Two paths: if ``GOGETA_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = gogeta_home / ".update_check"

    embedded_rev = os.environ.get("GOGETA_REVISION") or None
    ...
    # GOGETA_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `gogeta update` correctly refuses to run in-place inside the

    # container anyway. The dashboard's REST `/api/gogeta/update/check`
    ...
    if embedded_rev:
        ...
        # $GOGETA_HOME/gogeta-agent/ may be a stale copy from --clone-all;
        ...
        if not (repo_dir / ".git").exists():
            ...
            repo_dir = gogeta_home / "gogeta-agent"
...
def _resolve_repo_dir() -> Optional[Path]:
    ...
    """Return the active Gogeta git checkout, or None if this isn't a git install.



    Prefers the running code's location over the profile-scoped path

    because ``$GOGETA_HOME/gogeta-agent/`` may be a stale copy carried
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``gogeta_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            ...
            from gogeta_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/gogeta/gogeta-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Gogeta checkout. Cached per-process. Release URL always points at the

    canonical Gogeta repo (forks don't get a link).
...
def format_banner_version_label() -> str:
    ...
    base = f"Gogeta Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else GOGETA_CADUCEUS

    except Exception:

        _bskin = None

        _hero = GOGETA_CADUCEUS
    ...
    try:
        ...
        from gogeta_cli.codex_runtime_switch import get_current_runtime

        from gogeta_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            ...
            from gogeta_cli.config import get_managed_update_command, recommended_update_command
            ...
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built gogeta; we know an update
    ...
    # Pip-install warning — `pip install gogeta-agent` is not the supported
    ...
    if term_width >= 95:
        ...
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else GOGETA_AGENT_LOGO
