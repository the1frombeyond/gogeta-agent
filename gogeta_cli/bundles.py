"""Implementation of the ``gogeta bundles`` CLI subcommand.

Mirrors the structure of ``gogeta_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `gogeta bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  gogeta bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``gogeta bundles`` argparse tree.

    Called from ``gogeta_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `gogeta bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``gogeta bundles <subcommand>`` to the right handler."""
