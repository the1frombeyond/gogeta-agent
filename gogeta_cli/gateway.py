...
Gateway subcommand for gogeta CLI.



Handles: gogeta gateway [run|start|stop|restart|status|install|uninstall|setup]
...
# display_gogeta_home is imported lazily at call sites to avoid ImportError

# when gogeta_constants is cached from a pre-update version during `gogeta update`.
...
from gogeta_cli.colors import Colors, color
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "gogeta CLI that invoked this scan" from "real gateway process".
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``gogeta gateway status`` from falsely counting the ``gogeta`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``gogeta gateway status``) is never mistaken for a running
    ...
    patterns = [
        ...
        "gogeta_cli.main gateway",

        "gogeta_cli.main --profile",

        "gogeta_cli.main -p",

        "gogeta_cli/main.py gateway",

        "gogeta_cli/main.py --profile",

        "gogeta_cli/main.py -p",
        ...
        # ``gogeta.exe --profile`` / ``gogeta.exe -p`` would also match

        # ``gogeta.exe --profile foo dashboard`` and other CLI subcommands,
    ...
    current_home = str(get_gogeta_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            ...
            return (
                ...
                or f"gogeta_home={current_home_lc}" in command_lc
        ...
        # the command doesn't advertise *some other* profile. GOGETA_HOME
        ...
        if (
            ...
            "gogeta_home=" in command_lc

            and f"gogeta_home={current_home_lc}" not in command_lc
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            ...
            profiles (the pre-7923 global behaviour).  ``gogeta update``

            needs this because a code update affects every profile.

            When ``False`` (default), only PIDs belonging to the current

            Gogeta profile are returned.
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Gogeta profiles via PID files."""
    ...
    try:
        ...
        from gogeta_cli.profiles import list_profiles
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    ...
    args = [get_python_path(), "-m", "gogeta_cli.main"]
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``gogeta update`` users

    # with no running gateway until they re-invoke ``gogeta gateway``
    ...
    from gogeta_cli._subprocess_compat import (
...
def _sync_gogeta_home_from_systemd_unit(system: bool) -> None:

    """When acting on a system-scope unit, adopt its ``GOGETA_HOME``.



    Under ``sudo``, ``GOGETA_HOME`` is stripped and ``HOME=/root``, so

    :func:`get_gogeta_home` falls back to ``/root/.gogeta`` — the wrong

    profile. The unit file pins ``GOGETA_HOME`` for the actual gateway
    ...
    unit_home = env.get("GOGETA_HOME", "").strip()

    if not unit_home:

        return

    current = os.environ.get("GOGETA_HOME", "").strip()

    if current == unit_home:

        return

    os.environ["GOGETA_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    ...
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}gogeta gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        ...
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}gogeta gateway restart{scope_flag}"
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from gogeta_constants import is_container
    ...
    if is_linux() and is_container():
        ...
        try:
            ...
            from gogeta_cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `gogeta gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``gogeta gateway status`` output so users with
    ...
    try:
        ...
        from gogeta_cli.profiles import get_active_profile_name
...
def _gateway_list() -> None:
    ...
    try:
        ...
        from gogeta_cli.profiles import list_profiles, get_active_profile_name
...
def stop_profile_gateway() -> bool:
    ...
    """Stop only the gateway for the current profile (GOGETA_HOME-scoped).
...
from gogeta_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Gogeta Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``gogeta gateway run`` must remain interruptible from

    PowerShell/CMD. Detached service-style launches opt in via

    ``GOGETA_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    ...
    detached = os.getenv("GOGETA_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "gogeta-gateway"

SERVICE_DESCRIPTION = "Gogeta Agent Gateway - Messaging Platform Integration"
...
def _profile_suffix() -> str:
    ...
    """Derive a service-name suffix from the current GOGETA_HOME.
    ...
    Works correctly in Docker (GOGETA_HOME=/opt/data) and standard deployments.
    ...
    home = get_gogeta_home().resolve()

    default = get_default_gogeta_root().resolve()
    ...
    # Fallback: short hash for arbitrary GOGETA_HOME paths
...
def _profile_arg(gogeta_home: str | None = None) -> str:

    """Return ``--profile <name>`` only when GOGETA_HOME is a named profile.



    For ``~/.gogeta/profiles/<name>``, returns ``"--profile <name>"``.
    ...
    Args:
        ...
        gogeta_home: Optional explicit GOGETA_HOME path. Defaults to the current

            ``get_gogeta_home()`` value. Should be passed when generating a
    ...
    home = Path(gogeta_home or str(get_gogeta_home())).resolve()
...
def get_service_name() -> str:
    ...
    """Derive a systemd service name scoped to this GOGETA_HOME.



    Default ``~/.gogeta`` returns ``gogeta-gateway`` (backward compatible).

    Profile ``~/.gogeta/profiles/coder`` returns ``gogeta-gateway-coder``.

    Any other GOGETA_HOME appends a short hash for uniqueness.
...
# Legacy service names from older Gogeta installs that predate the

# gogeta-gateway rename. Kept as an explicit allowlist (NOT a glob) so

# profile units (gogeta-gateway-*.service) and unrelated third-party

# "gogeta" units are never matched.

_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("gogeta.service",)
...
def _find_legacy_gogeta_units() -> list[tuple[str, Path, bool]]:

    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Gogeta gateway units.



    Detects unit files installed by older Gogeta versions that used a

    different service name (e.g. ``gogeta.service`` before the rename to

    ``gogeta-gateway.service``). When both a legacy unit and the current

    ``gogeta-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      ...
      as ``gogeta-gateway-coder.service`` and unrelated third-party

      ``gogeta-*`` services are never matched.

    * ExecStart content check — only flag units that invoke our gateway

      entrypoint. A user-created ``gogeta.service`` running an unrelated
...
def has_legacy_gogeta_units() -> bool:

    """Return True when any legacy Gogeta gateway unit files exist."""

    return bool(_find_legacy_gogeta_units())
...
def print_legacy_unit_warning() -> None:
    ...
    """Warn about legacy Gogeta gateway unit files if any are installed.
    ...
    legacy = _find_legacy_gogeta_units()

    if not legacy:

        return

    print_warning("Legacy Gogeta gateway unit(s) detected from an older install:")
    ...
    print_info("  These run alongside the current gogeta-gateway service and")

    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")

    print_info("  Remove them with:")

    print_info("    gogeta gateway migrate-legacy")





def remove_legacy_gogeta_units(
    ...
    """Stop, disable, and remove legacy Gogeta gateway unit files.



    Iterates over whatever ``_find_legacy_gogeta_units()`` returns — which is
    ...
    if not legacy:
        ...
        print("No legacy Gogeta gateway units found.")
    ...
    print("Legacy Gogeta gateway unit(s) found:")
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        ...
        print("Skipped. Run again with: gogeta gateway migrate-legacy")
    ...
    if system_units:
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo gogeta gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    gogeta gateway uninstall")

    print_info("    sudo gogeta gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "  System service install requires sudo, so Gogeta can't create it from this user session."
            ...
            if run_as_user:
                ...
                print_info(
                    ...
                    f"  After setup, run: sudo gogeta gateway install --system --run-as-user {run_as_user}"
                ...
                print_info(
                    ...
                    "  After setup, run: sudo gogeta gateway install --system --run-as-user <your-user>"

                )

            print_info("  Then start it with: sudo gogeta gateway start --system")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Gogeta often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.gogeta`` → ``ai.gogeta.gateway.plist`` (backward compatible).

    Profile ``~/.gogeta/profiles/coder`` → ``ai.gogeta.gateway-coder.plist``.

    """

    suffix = _profile_suffix()

    name = f"ai.gogeta.gateway-{suffix}" if suffix else "ai.gogeta.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.gogeta/gogeta-agent  -> /home/alice/.gogeta/gogeta-agent

      /opt/gogeta                 -> /opt/gogeta  (kept as-is)
...
def _gogeta_home_for_target_user(target_home_dir: str) -> str:

    """Remap the current GOGETA_HOME to the equivalent under a target user's home.



    When installing a system service via sudo, get_gogeta_home() resolves to

    root's home.  This translates it to the target user's equivalent path:

      /root/.gogeta                    → /home/alice/.gogeta

      /root/.gogeta/profiles/coder     → /home/alice/.gogeta/profiles/coder

      /opt/custom-gogeta               → /opt/custom-gogeta  (kept as-is)

    """

    current_gogeta = get_gogeta_home().resolve()

    current_default = (Path.home() / ".gogeta").resolve()

    target_default = Path(target_home_dir) / ".gogeta"



    # Default ~/.gogeta → remap to target user's default

    if current_gogeta == current_default:

        return str(target_default)



    # Profile or subdir of ~/.gogeta → preserve the relative structure

    try:

        relative = current_gogeta.relative_to(current_default)

        return str(target_default / relative)

    except ValueError:

        # Completely custom path (not under ~/.gogeta) — keep as-is

        return str(current_gogeta)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    gogeta_node = gogeta_home / "node" / "bin"

    if _is_dir(gogeta_node):

        candidates.append(str(gogeta_node))

    gogeta_nm = gogeta_home / "node_modules" / ".bin"

    if _is_dir(gogeta_nm):

        candidates.append(str(gogeta_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m gogeta_cli.main``, so module
    ...
    dir, or a clone that ``gogeta update`` later relocates/removes — the path
    ...
    ``GOGETA_HOME`` is the stable anchor: it is where config/state/logs live,

    it never moves, and it is guaranteed to exist whenever the gateway is

    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if GOGETA_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        gogeta_home = _gogeta_home_for_target_user(home_dir)

        profile_arg = _profile_arg(gogeta_home)
        ...
        # Anchor cwd to the target user's GOGETA_HOME (stable, always exists)

        # rather than a remapped source-checkout path that can rot. See

        # _stable_service_working_dir() for the full rationale.

        working_dir = str(gogeta_home) if gogeta_home else _remap_path_for_user(working_dir, home_dir)
...
ExecStart={python_path} -m gogeta_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="GOGETA_HOME={gogeta_home}"
...
"""
    ...
    gogeta_home = str(get_gogeta_home().resolve())
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only GOGETA_HOME is). If a test

    # exercises ``run_gateway()`` with a pytest-tmp GOGETA_HOME, the freshly

    # generated unit bakes that ``/tmp/pytest-of-.../gogeta_test`` path into

    # ``Environment="GOGETA_HOME=..."``. Writing that to the developer's
    ...
    print(
        ...
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Gogeta install"
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         gogeta gateway install")

    print_info("         gogeta gateway start")
...
def _get_restart_drain_timeout() -> float:
    ...
    raw = os.getenv("GOGETA_RESTART_DRAIN_TIMEOUT", "").strip()
...
def systemd_install(
    ...
    # Offer to remove legacy units (gogeta.service from pre-rename installs)

    # before installing the new gogeta-gateway.service. If both remain, they
    ...
    if has_legacy_gogeta_units():
        ...
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            ...
            remove_legacy_gogeta_units(interactive=False)
    ...
    print(
        ...
        f"  {'sudo ' if system else ''}gogeta gateway start{scope_flag}              # Start the service"

    )

    print(

        f"  {'sudo ' if system else ''}gogeta gateway status{scope_flag}             # Check status"
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}gogeta gateway install{scope_flag}")
...
def systemd_stop(system: bool = False):
    ...
    _sync_gogeta_home_from_systemd_unit(system=system)
    ...
    try:
        ...
        print(
            ...
            "check `gogeta gateway status` or logs for final shutdown state."
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `gogeta gateway status` or logs for final state."
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            ...
            f"  Run: {'sudo ' if system else ''}gogeta gateway restart{scope_flag}  # auto-refreshes the unit"
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}gogeta gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            ...
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}gogeta gateway restart{scope_flag}"
        ...
        print(
            ...
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}gogeta gateway start{scope_flag}"
...
def get_launchd_label() -> str:
    ...
    return f"ai.gogeta.gateway-{suffix}" if suffix else "ai.gogeta.gateway"
...
# (the documented `nohup gogeta gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    ...
    """Build the `python -m gogeta_cli.main [--profile X] gateway run --replace` argv.



    Profile-aware: honors the active GOGETA_HOME via `_profile_arg()` so the

    detached fallback launches into the same profile as the CLI invocation.

    """

    cmd = [get_python_path(), "-m", "gogeta_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup gogeta gateway run --replace`
    ...
    from gogeta_cli._subprocess_compat import windows_detach_popen_kwargs
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    from gogeta_constants import display_gogeta_home as _dhh
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: gogeta gateway stop")
    ...
    print(
        ...
        f"  Try manually: nohup gogeta gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        ...
        "↻ Updated gateway launchd service definition to match the current Gogeta install"
...
def launchd_install(force: bool = False):
    ...
    print("  gogeta gateway status             # Check status")
...
def launchd_stop():
    ...
    # `gogeta gateway start` re-bootstraps when it detects the job is unloaded.
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        ...
        print("✓ Service definition matches the current Gogeta install")

    else:

        print("⚠ Service definition is stale relative to the current Gogeta install")

        print("  Run: gogeta gateway start")
    ...
    if deep:
        ...
        log_file = get_gogeta_home() / "logs" / "gateway.log"
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if _truthy_env(os.getenv("GOGETA_ALLOW_ROOT_GATEWAY")):
    ...
    print_error(
        ...
        "Refusing to run the Gogeta gateway as root inside the official Docker image."

    )

    print(

        "  The image entrypoint normally drops privileges to the 'gogeta' user. "

        "If you override entrypoint in Docker Compose, include "

        "/opt/gogeta/docker/entrypoint.sh before the Gogeta command."
    ...
    print(
        ...
        "$GOGETA_HOME and break later non-root dashboard/gateway runs."

    )

    print(

        "  Set GOGETA_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `gogeta gateway run` still

    # needs to obey the banner's "Press Ctrl+C to stop" contract.

    # Service-style launchers set GOGETA_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `gogeta gateway restart` which already
    ...
    # `gogeta gateway start/restart` — leaving the gateway vulnerable to
    ...
    # GOGETA_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        ...
        if os.environ.get("GOGETA_GATEWAY_EXIT_DIAG", "1") != "1":

            return

        try:

            from gogeta_constants import get_gogeta_home as _ghh
...
_PLATFORMS = [
...

    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Gogeta will log in directly",
        ...
        "vars": [
...

            {
                ...
                "help": "Full Matrix user ID, e.g. @gogeta:matrix.example.org",
...

    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. gogeta) and copy the bot token",
        ...
        "vars": [
...

            {
                ...
                "help": "Channel ID where Gogeta delivers cron results and notifications.",
...

    {
        ...
        "setup_instructions": [
            ...
            "1. Use a dedicated email account for your Gogeta agent",
        ...
        "vars": [

            {
                ...
                "help": "The email address Gogeta will use (e.g., gogeta@gmail.com).",
...

    {
        ...
        "setup_instructions": [
            ...
            "5. Gogeta connects via the BlueBubbles REST API and receives",

            "   incoming messages via a local webhook",

            "6. To authorize users, use DM pairing: gogeta pairing generate bluebubbles",
...

    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Gogeta will connect automatically over WebSocket",
...
def _all_platforms() -> list[dict]:
    ...
    ``gogeta setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      ...
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run gogeta under
    ...
    # User-installed platform plugins under ~/.gogeta/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        ...
        if val and val.lower() == "true":
            ...
            session_file = get_gogeta_home() / "whatsapp" / "session" / "creds.json"
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            ...
            try:
                ...
                from gogeta_cli.telegram_managed_bot import (
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'gogeta pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        ...
                        "  Approve with: gogeta pairing approve <platform> <code>"
                    ...
                    print_info(
                        ...
                        "  Skipped — configure later with 'gogeta gateway setup'"
...
def _setup_whatsapp():
    ...
    from gogeta_cli.main import cmd_whatsapp
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            ...
            from gogeta_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: gogeta pairing approve <platform> <code>")
            ...
            print_info("  Skipped — configure later with 'gogeta gateway setup'")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from gogeta_cli import gateway_windows
...
def _setup_weixin():
    ...
    print_info("  1. Gogeta will open Tencent iLink QR login in this terminal.")

    print_info("  2. Use WeChat to scan and confirm the QR code.")

    print_info(

        "  3. Gogeta will store the returned account_id/token in ~/.gogeta/.env."
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `gogeta gateway setup`.")
    ...
    try:
        ...
        credentials = asyncio.run(qr_login(str(get_gogeta_home())))
    ...
    if access_idx == 0:
        ...
        print_info(
            ...
            "  Unknown DM users can request access and you approve them with `gogeta pairing approve`."
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_info(
            ...
            "  Unknown users can request access; approve with `gogeta pairing approve`."
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "GogetaAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``gogeta_cli.setup`` (which

    imports from this module for the remaining bespoke flows).

    """

    from gogeta_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.gogeta/plugins/
    ...
    if required:
        ...
        print_info(f"  Set these env vars in ~/.gogeta/.env: {', '.join(required)}")
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    ...
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: gogeta gateway")
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    try:
                        ...
                        print_info("  You can try manually: gogeta gateway install")

                else:

                    print_info("  Skipped start and auto-start setup.")

                    print_info("  You can install later: gogeta gateway install")

                    if supports_systemd_services():

                        print_info(

                            "  Or as a boot-time service: sudo gogeta gateway install --system"

                        )

                    print_info("  Or run in foreground:  gogeta gateway run")

            elif is_wsl():

                print_info("  WSL detected but systemd is not running.")

                print_info("  Run in foreground: gogeta gateway run")

                print_info(

                    "  For persistence:   tmux new -s gogeta 'gogeta gateway run'"
                ...
                print_info(
                    ...
                    f"  Or start it manually in the background (best effort): nohup gogeta gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
        ...
        print_info("No platforms configured. Run 'gogeta gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`gogeta_cli.service_manager` are caught and surfaced as

    actionable CLI messages (no raw ``CalledProcessError`` traceback).

    """

    from gogeta_cli.service_manager import (
    ...
    if profile is None:
        ...
        # GOGETA_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``gogeta gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``gogeta gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``gogeta gateway run`` are unaffected.

      2. ``GOGETA_S6_SUPERVISED_CHILD`` is exported by

         ``S6ServiceManager._render_run_script`` for the supervised

         process itself — i.e. when s6-supervise execs ``gogeta gateway
      ...
      3. ``--no-supervise`` (or ``GOGETA_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    if os.environ.get("GOGETA_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${GOGETA_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if GOGETA_DASHBOARD is set).\n"

        "  This is the recommended setup for the s6 container image — the\n"

        "  gateway will keep running even if it crashes.\n"

        "  Use `--no-supervise` (or GOGETA_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    # Prefer `sleep infinity` (matches the static main-gogeta service's

    # pattern in docker/s6-rc.d/main-gogeta/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: gogeta gateway")
        ...
        if supports_systemd_services():
            ...
            if is_wsl():
                ...
                print_info(
                    ...
                    "  Consider running in foreground instead: gogeta gateway run"

                )

                print_info(

                    "  Or use tmux/screen for persistence: tmux new -s gogeta 'gogeta gateway run'"
            ...
            print(
                ...
                "  gogeta gateway run                              # direct foreground"

            )

            print(

                "  tmux new -s gogeta 'gogeta gateway run'         # persistent via tmux"

            )

            print(

                "  nohup gogeta gateway run > ~/.gogeta/logs/gateway.log 2>&1 &  # background"
            ...
            if detect_service_manager() == "s6":
                ...
                print("  gogeta profile create <name>     # creates the s6 service slot")

                print("  gogeta -p <name> gateway start   # bring it up via s6")

                print("  gogeta status                    # see currently-supervised gateways")
            ...
            print("To run the gateway: gogeta gateway run")
            ...
            print("Run manually: gogeta gateway run")
        ...
        if is_termux():
            ...
            print("Stop manual runs with: gogeta gateway stop")
        ...
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print("  gogeta profile delete <name>     # tears down the s6 service slot")

                print("  gogeta -p <name> gateway stop    # stop without deleting the profile")
        ...
        # slot — start them individually via `gogeta -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print("Or run the gateway directly: gogeta gateway run")
        ...
        if os.getenv("_GOGETA_GATEWAY") == "1":
            ...
            print_error(
                ...
                "Use `gogeta gateway stop` from a shell outside the running gateway."
        ...
        if os.getenv("_GOGETA_GATEWAY") == "1":
            ...
            print_error(
                ...
                "Use `gogeta gateway restart` from a shell outside the running gateway."
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: gogeta gateway start")
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  sudo gogeta gateway install --system")
                ...
                print("  gogeta gateway run      # Run in foreground")

                if is_termux():

                    print(

                        "  nohup gogeta gateway run > ~/.gogeta/logs/gateway.log 2>&1 &  # Best-effort background start"
                    ...
                    print(
                        ...
                        "  gogeta gateway install  # Install as Windows Scheduled Task (auto-start on login)"

                    )

                else:

                    print("  gogeta gateway install  # Install as user service")

                    print(

                        "  sudo gogeta gateway install --system  # Install as boot-time system service"
        ...
        # Stop, disable, and remove legacy Gogeta gateway unit files from

        # pre-rename installs (e.g. gogeta.service). Profile units and
        ...
        remove_legacy_gogeta_units(interactive=not yes, dry_run=dry_run)
