...
``$GOGETA_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-gogeta-setup

(the stage2 hook) has chowned the volume and seeded $GOGETA_HOME, but
...
def reconcile_profile_gateways(
    ...
    *,
    ...
    (the implicit profile that lives at the top of ``$GOGETA_HOME``,

    not under ``profiles/``). The dispatcher in ``gogeta_cli.gateway``

    maps an empty profile suffix to ``gateway-default``, so this slot

    is what ``gogeta gateway start`` (no ``-p``) targets. Without it,

    bare ``gogeta gateway start`` inside the container would land on
    ...
    ``$GOGETA_HOME/gateway_state.json`` (sibling to the profile root,
    ...
    Args:
        ...
        gogeta_home: The container's GOGETA_HOME (typically /opt/data).

            Profiles live under ``<gogeta_home>/profiles/<name>/``;

            the default profile lives at ``<gogeta_home>`` itself.
    ...
    # ``gogeta gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_prior_state(gogeta_home)

    default_should_start = default_prior_state in _AUTOSTART_STATES

    if not dry_run:

        _cleanup_stale_runtime_files(gogeta_home)
    ...
    profiles_root = gogeta_home / "profiles"
    ...
    if profiles_root.is_dir():
        ...
        for entry in sorted(profiles_root.iterdir()):
            ...
            # SOUL.md is always seeded by `gogeta profile create` (config.yaml

            # is not — that comes later via `gogeta setup`). Use it as the
            ...
            # via ``gogeta -p default-named gateway start`` if they
    ...
    if not dry_run:
        ...
        _write_reconcile_log(gogeta_home, actions)
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    state_file = gogeta_home / "gateway_state.json"
    ...
    if os.environ.get("GOGETA_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and Path(args[0]).name == "gogeta":
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `gogeta -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit gogeta

        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls

        # (all dispatched as the gogeta user) won't hit EACCES. See
...
def _write_reconcile_log(
    ...
    gogeta_home: Path, actions: list[ReconcileAction],

) -> None:

    """Append one line per profile to $GOGETA_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    gogeta_home = Path(os.environ.get("GOGETA_HOME", "/opt/data"))

    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))

    actions = reconcile_profile_gateways(

        gogeta_home=gogeta_home, scandir=scandir,
