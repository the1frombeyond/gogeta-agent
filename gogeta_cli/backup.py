"""
Backup and import commands for gogeta CLI.

`gogeta backup` creates a zip archive of the entire ~/.gogeta/ directory
(excluding the gogeta-agent repo and transient files).

`gogeta import` restores from a backup zip, overlaying onto the current
GOGETA_HOME root.
"""
...
from gogeta_constants import get_default_gogeta_root, get_gogeta_home, display_gogeta_home
...
_EXCLUDED_DIRS = {
    "gogeta-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to gogeta root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside GOGETA_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Gogeta home directory."""
    gogeta_root = get_default_gogeta_root()

    if not gogeta_root.is_dir():
        print(f"Error: Gogeta home directory not found at {gogeta_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"gogeta-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"gogeta-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_gogeta_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(gogeta_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(gogeta_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(gogeta_root)
    ...
    print(f"\nRestore with: gogeta import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Gogeta backup.
    ...
    # Look for telltale files that a gogeta home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Gogeta backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.gogeta/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a gogeta dir name
        if prefix in {".gogeta", "gogeta"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_gogeta_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (gogeta_root / "config.yaml").exists()
        has_env = (gogeta_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Gogeta configuration.")
        ...
        gogeta_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = gogeta_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(gogeta_root.resolve())
            except ValueError:
        ...
        profiles_dir = gogeta_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # gogeta_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: gogeta profile list  (after installing gogeta)")

        # Guidance
        print()
        if not (gogeta_root / "gogeta-agent").is_dir():
            print("Note: The gogeta-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: gogeta update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  gogeta -p {pname} gateway install")

        print("Done. Your Gogeta configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and gogeta backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to GOGETA_HOME).
...
# — `gogeta update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(gogeta_home: Optional[Path] = None) -> Path:
    home = gogeta_home or get_gogeta_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    gogeta_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(gogeta_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside GOGETA_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``gogeta update``.
    ...
    Args:
        ...
        gogeta_home: Override for the Gogeta home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(gogeta_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for gogeta backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_gogeta_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, gogeta_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``gogeta_root`` to ``out_path``.
...
def _pre_update_backup_dir(gogeta_home: Optional[Path] = None) -> Path:
    home = gogeta_home or get_gogeta_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of GOGETA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<GOGETA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``gogeta update``) should continue even if the backup fails.
    """
    gogeta_root = gogeta_home or get_default_gogeta_root()
    if not gogeta_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(gogeta_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, gogeta_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `gogeta claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of GOGETA_HOME under ``backups/`` before a
    ``gogeta claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``gogeta import <archive>``.  Writes to
    ``<GOGETA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `gogeta import` and the
