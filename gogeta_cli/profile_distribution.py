"""Profile distributions — shareable, packaged Gogeta profiles via git.



A distribution is a Gogeta profile published as a git repository (or
...
* ``gogeta profile export/import`` — local backup / restore for a profile

  on your own machine. NOT a distribution format. Stays as-is.

* ``gogeta skills install <url>`` — the URL install pattern we're mirroring,

  but at the profile granularity.



Subcommands (all live under ``gogeta profile``, not a parallel tree):



    gogeta profile install <source> [--name N] [--alias] [--force] [--yes]

    gogeta profile update  <name>  [--force-config] [--yes]

    gogeta profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "gogeta_state.db", "response_store.db",
    ...
    "gogeta-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            gogeta_requires=str(data.get("gogeta_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.gogeta_requires:

            out["gogeta_requires"] = self.gogeta_requires
...
def check_gogeta_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        ...
        raise DistributionError(
            ...
            f"This distribution requires Gogeta {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        ...
        "# Environment variables required by this Gogeta distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``gogeta profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            ...
            raise DistributionError(
                ...
                "This repository is not a Gogeta profile distribution."
...
def plan_install(
    ...
    if manifest is None:
        ...
        raise DistributionError(
            ...
            "this source is not a Gogeta distribution."
    ...
    check_gogeta_requires(manifest.gogeta_requires, gogeta_version)
    ...
    if canon == "default":
        ...
        raise DistributionError(
            ...
            "root profile (~/.gogeta).  Pass --name <name> to install under a "
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="gogeta_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            ...
            raise DistributionError(
                ...
                "Use `gogeta profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        ...
        raise DistributionError(
            ...
            "`gogeta profile install <source> --name {canon} --force`."

        )



    with tempfile.TemporaryDirectory(prefix="gogeta_dist_update_") as tmp:
