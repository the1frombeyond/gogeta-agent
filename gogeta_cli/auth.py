...
Multi-provider authentication system for Gogeta Agent.
...
is persisted in ~/.gogeta/auth.json with cross-process file locking.
...
from gogeta_cli.config import get_gogeta_home, get_config_path, read_raw_config

from gogeta_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
DEFAULT_NOUS_PORTAL_URL = "https://portal.gogeta.ai"

DEFAULT_NOUS_INFERENCE_URL = "https://inference-api.gogeta.ai/v1"

DEFAULT_NOUS_CLIENT_ID = "gogeta-cli"
...
SPOTIFY_DOCS_URL = "https://gogeta.ai/docs/user-guide/features/spotify"
...
XAI_OAUTH_DOCS_URL = "https://gogeta.ai/docs/guides/xai-grok-oauth"

OAUTH_OVER_SSH_DOCS_URL = "https://gogeta.ai/docs/guides/oauth-over-ssh"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            ...
            from gogeta_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        ...
        # Check both os.environ and ~/.gogeta/.env file
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``gogeta auth``.
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        ...
        return f"{error} Run `gogeta model` to re-authenticate."
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    ...
    try:
        ...
        from gogeta_cli.nous_account import (
...
def _oauth_trace_enabled() -> bool:
    ...
    raw = os.getenv("GOGETA_OAUTH_TRACE", "").strip().lower()
...
# Auth Store — persistence layer for ~/.gogeta/auth.json
...
def _auth_file_path() -> Path:
    ...
    path = get_gogeta_home() / "auth.json"

    # Seat belt: if pytest is running and GOGETA_HOME resolves to the real

    # user's auth store, refuse rather than silently corrupt it. This catches

    # tests that forgot to monkeypatch GOGETA_HOME, tests invoked without the
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        real_home_auth = (Path.home() / ".gogeta" / "auth.json").resolve(strict=False)
        ...
        if resolved == real_home_auth:
            ...
            raise RuntimeError(
                ...
                "Set GOGETA_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom GOGETA_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_gogeta_root()

    except Exception:

        return None

    profile_home = get_gogeta_home()
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.gogeta/auth.json`` even when GOGETA_HOME is set to a profile

    path. The hermetic conftest does not redirect ``HOME``, so

    ``get_default_gogeta_root()`` for a profile-shaped GOGETA_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            ...
            real_root = Path(real_home_env) / ".gogeta" / "auth.json"
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``gogeta auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``gogeta auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``gogeta auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Gogeta.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    ...
    Clear auth state for a provider. Used by `gogeta logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        from gogeta_cli.config import validate_config_structure
        ...
        lines = ["Config issue detected — run 'gogeta doctor' for full diagnostics:"]
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'gogeta model' for available providers, or run 'gogeta doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `gogeta auth add openrouter`
    ...
    # `gogeta auth list` showing the credential while requests go out with no
    ...
    raise AuthError(
        ...
        "No inference provider configured. Run 'gogeta model' to choose a "

        "provider and model, or set an API key (OPENROUTER_API_KEY, "

        "OPENAI_API_KEY, etc.) in ~/.gogeta/.env.",
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: gogeta auth add nous",
...
def resolve_qwen_runtime_credentials(
    ...
    *,
    ...
    base_url = os.getenv("GOGETA_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `gogeta model` walks users into a broken Qwen setup flow.
...
# Tokens live in ~/.gogeta/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    ...
    """Return a status dict for `gogeta auth list` / `gogeta status`."""
...
# Spotify auth — PKCE tokens stored in ~/.gogeta/auth.json
...
def _spotify_client_id(
    ...
    candidates = (
        ...
        get_env_value("GOGETA_SPOTIFY_CLIENT_ID"),
    ...
    raise AuthError(
        ...
        "Spotify client_id is required. Set GOGETA_SPOTIFY_CLIENT_ID or pass --client-id.",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        ...
        get_env_value("GOGETA_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        ...
        get_env_value("GOGETA_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        ...
        get_env_value("GOGETA_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but Gogeta times out") can produce

            # actionable evidence without a code change.  Logged values are

            # fingerprints / booleans only; no actual code/state strings leak

            # into the log file.  Run with ``GOGETA_LOG_LEVEL=INFO`` (or check

            # ``~/.gogeta/logs/agent.log`` which captures INFO+ unconditionally).
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>gogeta auth add xai-oauth</code> to retry.</p>"
    ...
    *,
    ...
    if not refresh_token:
        ...
        raise AuthError(
            ...
            "Spotify refresh token missing. Run `gogeta auth spotify` again.",
    ...
    if response.status_code >= 400:
        ...
        raise AuthError(
            ...
            "Spotify token refresh failed. Run `gogeta auth spotify` again."
    ...
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            ...
            raise AuthError(
                ...
                "Spotify is not authenticated. Run `gogeta auth spotify` first.",
    ...
    if not access_token:
        ...
        raise AuthError(
            ...
            "Spotify access token missing. Run `gogeta auth spotify` again.",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.gogeta/.env, and return it.
    ...
    print("       App name:     anything (e.g. gogeta-agent)")
    ...
    # Persist so subsequent `gogeta auth spotify` runs skip the wizard.

    save_env_value("GOGETA_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        ...
        save_env_value("GOGETA_SPOTIFY_REDIRECT_URI", redirect_uri_hint)



    print()

    print("Saved GOGETA_SPOTIFY_CLIENT_ID to ~/.gogeta/.env")
...
def login_spotify_command(args) -> None:
    ...
    # with "GOGETA_SPOTIFY_CLIENT_ID is required".
    ...
    print("Open this URL to authorize Gogeta:")
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(f"Gogeta is waiting for the OAuth callback on {redirect_uri}")
...
# OpenAI Codex auth — tokens stored in ~/.gogeta/auth.json (not ~/.codex/)

#

# Gogeta maintains its own Codex OAuth session separate from the Codex CLI
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    """Read Codex OAuth tokens from Gogeta auth store (~/.gogeta/auth.json).
    ...
    if not state:
        ...
        raise AuthError(
            ...
            "No Codex credentials stored. Run `gogeta auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        ...
        raise AuthError(
            ...
            "Codex auth state is missing tokens. Run `gogeta auth` to re-authenticate.",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        ...
        raise AuthError(
            ...
            "Codex auth is missing access_token. Run `gogeta auth` to re-authenticate.",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        ...
        raise AuthError(
            ...
            "Codex auth is missing refresh_token. Run `gogeta auth` to re-authenticate.",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      ...
      OAuth flow when the user logged in via ``gogeta setup`` / the model

      picker.  Always synced with the fresh tokens.

    * ``manual:device_code`` — entries created by ``gogeta auth add openai-codex``
      ...
      ``gogeta auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    ...
    """Save Codex OAuth tokens to Gogeta auth store (~/.gogeta/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``gogeta auth add openai-codex`` created (which must not be
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Gogeta auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run gogeta auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            ...
            message = (
                ...
                "then run `gogeta auth` to re-authenticate."
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Gogeta auth store automatically.
...
def resolve_codex_runtime_credentials(
    ...
    *,
    ...
    """Resolve runtime credentials from Gogeta's own Codex token store.
    ...
    try:
        ...
        if pool_token:
            ...
            base_url = (
                ...
                os.getenv("GOGETA_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    refresh_timeout_seconds = float(os.getenv("GOGETA_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))
    ...
    if should_refresh:
        ...
        # Re-read under lock to avoid racing with other Gogeta processes
...
# xAI Grok OAuth — tokens stored in ~/.gogeta/auth.json
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        ...
        raise AuthError(
            ...
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `gogeta model`.",
    ...
    if not isinstance(tokens, dict):
        ...
        raise AuthError(
            ...
            "xAI OAuth state is missing tokens. Re-authenticate with `gogeta model`.",
    ...
    if not access_token:
        ...
        raise AuthError(
            ...
            "xAI OAuth state is missing access_token. Re-authenticate with `gogeta model`.",
    ...
    if not refresh_token:
        ...
        raise AuthError(
            ...
            "xAI OAuth state is missing refresh_token. Re-authenticate with `gogeta model`.",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.gogeta/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        ...
        raise AuthError(
            ...
            f"discovery; re-authenticate with `gogeta model` to re-fetch.",
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``GOGETA_XAI_BASE_URL``
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        ...
        raise AuthError(
            ...
            "xAI OAuth is missing refresh_token. Re-authenticate with `gogeta model`.",
    ...
    # written by an older Gogeta (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `gogeta model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``gogeta model``
...
def resolve_xai_oauth_runtime_credentials(
    ...
    *,
    ...
    refresh_timeout_seconds = float(os.getenv("GOGETA_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
...
# so a new `gogeta --profile <name> auth add nous --type oauth` can one-tap

# import instead of running the full device-code flow every time.

#

# File lives at ${GOGETA_SHARED_AUTH_DIR}/nous_auth.json, defaulting to

# ``<gogeta-root>/shared/nous_auth.json`` where ``<gogeta-root>`` is what

# ``get_default_gogeta_root()`` returns — ``~/.gogeta`` on Linux/macOS,

# ``%LOCALAPPDATA%\gogeta`` on native Windows, or the Docker/custom root.

# It is OUTSIDE any named profile's GOGETA_HOME so named profiles (which

# typically live under ``<gogeta-root>/profiles/<name>/``) all see the
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``GOGETA_SHARED_AUTH_DIR`` so tests can redirect it to a tmp

    path without touching the real user's home. Defaults to

    ``<gogeta-root>/shared/``, where ``<gogeta-root>`` is what

    :func:`gogeta_constants.get_default_gogeta_root` returns — so

    Linux/macOS classic installs land at ``~/.gogeta/shared/``, native

    Windows installs at ``%LOCALAPPDATA%\\gogeta\\shared\\``, and

    Docker / custom ``GOGETA_HOME`` deployments at

    ``<GOGETA_HOME>/shared/``. Sits outside any named profile so all

    profiles under the same root share the store.

    """

    override = os.getenv("GOGETA_SHARED_AUTH_DIR", "").strip()
    ...
    return get_default_gogeta_root() / "shared"
...
def _nous_shared_store_path() -> Path:
    ...
    # real user's Gogeta root, refuse rather than silently corrupt cross-profile

    # state. Tests must set GOGETA_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        real_home_shared = (
            ...
            get_default_gogeta_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            ...
            raise RuntimeError(
                ...
                f"{path}. Set GOGETA_SHARED_AUTH_DIR to a tmp_path in your test fixture."
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No GOGETA_HOME yet (pre-setup): fall through without locking.
...
def _refresh_access_token(
    ...
    *,
    ...
    # called POST /api/oauth/token with Gogeta's refresh_token without

    # persisting the rotated token back to auth.json — the server then

    # retires the original RT, Gogeta's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        ...
        description = (
            ...
            "custom self-heal hook, or another Gogeta install sharing "

            "~/.gogeta/auth.json) called POST /api/oauth/token with Gogeta's "

            "refresh token without persisting the rotated token back.\n"

            "Nous refresh tokens are single-use — only Gogeta may call the "

            "refresh endpoint. For health checks, use `gogeta auth status` "

            "instead.\n"

            "Re-authenticate with: gogeta auth add nous"
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            ...
            # Skip Gogeta models — they're not reliable for agentic tool-calling

            if "gogeta" in mid.lower():
    ...
    with _auth_store_lock():
        ...
        if not state:
            ...
            raise AuthError(
                ...
                "Gogeta is not logged into Nous Portal.",
        ...
        portal_base_url = (
            ...
            or os.getenv("GOGETA_PORTAL_BASE_URL")
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "gogeta-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``gogeta auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``gogeta auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `gogeta auth add nous --type oauth`. Best-
    ...
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            ...
            raise AuthError("Gogeta is not logged into Nous Portal.",
...
# which does a synchronous OAuth refresh POST to portal.gogeta.ai. That can take

# ~350ms even on the failure path, and read-only UI surfaces (`gogeta tools`, status panels,

# subscription-feature checks) call it many times per render — `gogeta tools` → "All Platforms"

# was firing the refresh ~31× during one menu paint, racking up >13s of HTTP and burning

# single-use refresh tokens. Cache the snapshot for a few seconds, keyed on the auth.json

# mtime so that `gogeta auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `gogeta auth` stores credentials),

    then falls back to the legacy provider state.

    """

    # Check credential pool first — this is where `gogeta auth` and

    # `gogeta model` store device_code tokens.
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        ...
        os.getenv("GOGETA_COPILOT_ACP_COMMAND", "").strip()
    ...
    raw_args = os.getenv("GOGETA_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        ...
        (we do NOT mint a token here; ``gogeta doctor`` runs the live
    ...
    try:
        ...
        from gogeta_cli.config import load_config, get_env_value
        ...
        try:
            ...
            if not installed:
                ...
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Gogeta' "
                ...
                info["hint"] = (
                    ...
                    "is skipped here. Run `gogeta doctor` to verify token acquisition."
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        ...
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set GOGETA_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `gogeta logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    from gogeta_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    from gogeta_cli.config import save_config, load_config
...
def login_command(args) -> None:
    ...
    """Deprecated: use 'gogeta model' or 'gogeta setup' instead."""

    print("The 'gogeta login' command has been removed.")

    print("Use 'gogeta auth' to manage credentials,")

    print("'gogeta model' to select a provider, or 'gogeta setup' for full setup.")
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.gogeta/auth.json."""
    ...
    # Check for existing Gogeta-owned credentials
    ...
    if not force_new_login:
        ...
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                ...
                print("Existing Codex credentials found in Gogeta auth store.")
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Gogeta will create its own session to avoid conflicts with Codex CLI / VS Code.")
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("GOGETA_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                ...
                print("Gogeta will keep working independently with its own session.")
    ...
    # Run a fresh device code flow — Gogeta gets its own OAuth session

    print()

    print("Signing in to OpenAI Codex...")

    print("(Gogeta creates its own session — won't affect Codex CLI or VS Code)")
    ...
    # Save tokens to Gogeta auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        ...
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                ...
                print("Existing xAI OAuth credentials found in Gogeta auth store.")
    ...
    print("(Gogeta creates its own local OAuth session)")
...
def _xai_oauth_build_authorize_url(
    ...
    *,
    ...
    # `referrer=gogeta-agent` lets xAI attribute Gogeta-originated logins
    ...
    *,
    ...
    if not code_verifier:
        ...
        raise AuthError(
            ...
            "This is a bug in Gogeta — please report at "

            "https://github.com/NousResearch/gogeta-agent/issues/26990.",
...
def _xai_oauth_loopback_login(
    ...
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize Gogeta with xAI:")
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    ...
    """Persist MiniMax OAuth state to Gogeta auth store (~/.gogeta/auth.json)."""
...
def _minimax_oauth_login(
    ...
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Gogeta login via MiniMax ({region}) OAuth...")
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            ...
            raise AuthError(
                ...
                "Not logged into MiniMax OAuth. Run `gogeta model` and select "
...
def resolve_minimax_oauth_runtime_credentials(
    ...
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``gogeta status`` that just want to know
...
def _nous_device_code_login(
    ...
    *,
    ...
    print(f"Starting Gogeta login via {pconfig.name}...")
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print("After subscribing, run `gogeta model` again to finish setup.")
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Gogeta builds still see
        ...
        if not selected_model:
            ...
            print("  Run `gogeta model` again to switch to Nous Portal.")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            ...
            print("Gogeta will use OpenRouter for inference.")

        elif should_reset_config:

            print("Run `gogeta model` or configure an API key to use Gogeta.")
