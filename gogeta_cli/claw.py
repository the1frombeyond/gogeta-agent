"""gogeta claw — OpenClaw migration commands.



Usage:

    gogeta claw migrate              # Preview then migrate (always shows preview first)

    gogeta claw migrate --dry-run    # Preview only, no changes

    gogeta claw migrate --yes        # Skip confirmation prompt

    gogeta claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets

    gogeta claw migrate --no-backup  # Skip pre-migration snapshot

    gogeta claw cleanup              # Archive leftover OpenClaw directories

    gogeta claw cleanup --dry-run    # Preview what would be archived
...
from gogeta_cli.config import get_gogeta_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Gogeta may try to use the same token, causing disconnects."
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    """Check if a Gogeta gateway is running with connected platforms.
    ...
    print_error(
        ...
        "Gogeta gateway is running with active connections: "
    ...
    print_info("Recommendation: stop the gateway first with 'gogeta gateway stop'.")
...
def _find_migration_script() -> Path | None:
    ...
    """Find the openclaw_to_gogeta.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_gogeta", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: gogeta claw <command> [options]")

        print()

        print("Commands:")

        print("  migrate          Migrate settings from OpenClaw to Gogeta")

        print("  cleanup          Archive leftover OpenClaw directories after migration")

        print()

        print("Run 'gogeta claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    """Run the OpenClaw → Gogeta migration."""
    ...
    # migrate-gogeta posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: gogeta claw migrate --source /path/to/.openclaw")
    ...
    print_info(f"Target:      {gogeta_home}")
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            ...
            "Each conflict is an item whose target already exists in ~/.gogeta/. "
    ...
    if not auto_yes:
        ...
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: gogeta claw migrate --yes")
    ...
    # ── Phase 2b: Pre-apply backup of the Gogeta home ─────────

    # Delegates to gogeta_cli.backup.create_pre_migration_backup(), which

    # shares implementation with the pre-update backup (same exclusion

    # rules, same SQLite safe-copy, zip format) so the archive is

    # restorable with `gogeta import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        ...
        try:
            ...
            from gogeta_cli.backup import create_pre_migration_backup, _format_size

            backup_archive = create_pre_migration_backup(gogeta_home=gogeta_home)
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: gogeta import {backup_archive.name}")
            ...
            print_info(
                ...
                "Re-run with --no-backup to skip, or free up disk space under the Gogeta home."
    ...
    # 'gogeta claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                ...
                print_info("Aborted. Stop OpenClaw first, then re-run: gogeta claw cleanup")
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: gogeta claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  gogeta claw migrate --preset {report.get('preset', 'full')}")
        ...
        if skipped_keys:
            ...
            print_info("  gogeta claw migrate --migrate-secrets")

            print()

            print_info("Or add your key manually:")

            print_info("  gogeta config set OPENROUTER_API_KEY sk-or-v1-...")
