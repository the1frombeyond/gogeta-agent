"""On-demand supply-chain audit for Gogeta Agent installs.

Scans three surfaces a Gogeta user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Gogeta venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.gogeta/plugins``
...
def _discover_plugins(gogeta_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.gogeta/plugins``.
    ...
    """
    plugins_dir = gogeta_home / "plugins"
...
def run_audit(
    *,
    ...
    home = gogeta_home or Path(get_gogeta_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, gogeta_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(gogeta_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `gogeta security audit`."""
    home = Path(get_gogeta_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, gogeta_home=home
    )
    if total == 0:
