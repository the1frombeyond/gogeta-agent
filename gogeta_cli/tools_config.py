...
Unified tool configuration for Gogeta Agent.



`gogeta tools` and `gogeta setup tools` both enter this module.
...
Saves per-platform tool configuration to ~/.gogeta/config.yaml under
...
from gogeta_cli.nous_subscription import (
...
from gogeta_cli.nous_account import format_nous_portal_entitlement_message
...
from gogeta_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _GOGETA_CORE_TOOLS (available at runtime if enabled),
...
# who want it opt in via `gogeta tools` → Video Generation, which walks
...
# `gogeta tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    try:
        ...
        from gogeta_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `gogeta tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``gogeta tools`` → "reconfigure existing" would
    ...
    try:
        ...
        from gogeta_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    ...
    """Return the toolset keys the ``gogeta tools`` checklist actually offers
    ...
    so ``gogeta tools`` never claims to add or remove a toolset the user was
...
from gogeta_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            ...
            "Gogeta routes X searches through xAI's built-in x_search "
        ...
        "providers": [

            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. GOGETA_CUA_DRIVER_VERSION is an
        ...
        "providers": [

            {
                ...
                "env_vars": [
                    ...
                    {"key": "GOGETA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},

                    {"key": "GOGETA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
...

            {
                ...
                "env_vars": [
                    ...
                    {"key": "GOGETA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},

                    {"key": "GOGETA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},

                    {"key": "GOGETA_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("GOGETA_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      ...
      update`` if the binary supports it). Used by ``gogeta update`` and

      by ``gogeta computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        ...
        if upgrade:
            ...
            # Silent on non-macOS — `gogeta update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            ...
            if verbose:
                ...
                _print_info("    Both must allow the terminal / Gogeta process.")
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_gogeta_home()}/gogeta-agent && npm install --workspaces=false")
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in gogeta-agent directory)")
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.gogeta/config.yaml")
        ...
        # Run the full `gogeta auth spotify` flow — if the user has no

        # client_id yet, this drops them into the interactive wizard

        # (opens the Spotify dashboard, prompts for client_id, persists

        # to ~/.gogeta/.env), then continues straight into PKCE. If they
        ...
        try:
            ...
            from gogeta_cli.auth import login_spotify_command

        except Exception as exc:

            _print_warning(f"    Could not load Spotify auth: {exc}")

            _print_info("    Run manually: gogeta auth spotify")
        ...
        try:
            ...
            # toolset enable; they can retry with `gogeta auth spotify`.

            _print_warning(f"    Spotify login did not complete: {exc}")

            _print_info("    Run later: gogeta auth spotify")
        ...
        try:
            ...
            from gogeta_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: gogeta plugins enable observability/langfuse")

        _print_info("    Restart Gogeta for tracing to take effect.")

        _print_info("    Verify: gogeta plugins list")
        ...
        try:
            ...
            _print_info("    Run later: gogeta auth add xai-oauth   (or set XAI_API_KEY)")
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `gogeta auth add xai-oauth`",
        ...
        if idx == 0:
            ...
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: gogeta auth add xai-oauth"
            ...
            if api_key:
                ...
                _print_warning(
                    ...
                    "    No API key provided. Run later: gogeta auth add xai-oauth"
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``gogeta tools post-setup``
...
def run_post_setup_command(args) -> int:
    ...
    """``gogeta tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        ...
        _print_error("Usage: gogeta tools post-setup <key>")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "gogeta-cli" (which include all _GOGETA_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[gogeta-cli, spotify]`` after enabling Spotify via ``gogeta
        ...
        # (e.g. "gogeta-cli") to individual tool names and reverse-mapping.
        ...
        # through ``gogeta tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `gogeta tools` (which flips has_explicit_config

    # to True) silently drops them.

    _plat_info = PLATFORMS.get(platform)

    _default_ts = _plat_info["default_toolset"] if _plat_info else f"gogeta-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("gogeta-")}
    ...
    # shipped as a bundled plugin but user must opt in via `gogeta tools`

    # so we don't ship 7 Spotify tool schemas to users who don't use it).

    # A plugin toolset is "known" for a platform once `gogeta tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            ...
            if pts in toolset_names:
                ...
                # New plugin not yet seen by gogeta tools — default enabled
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (gogeta-cli, gogeta-telegram, etc.)
    ...
    # Opening `gogeta tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `gogeta tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `gogeta model`).
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    """Entry point for `gogeta tools` and `gogeta setup tools`.
    ...
    print(color("⚕ Gogeta Tool Configuration", Colors.CYAN, Colors.BOLD))

    print(color("  Enable or disable tools per platform.", Colors.DIM))

    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))

    print(color("  Guide: https://gogeta.ai/docs/user-guide/features/tools", Colors.DIM))
    ...
    print(color(f"  Tool configuration saved to {display_gogeta_home()}/config.yaml", Colors.DIM))

    print(color("  Changes take effect on next 'gogeta' or gateway restart.", Colors.DIM))
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, gogeta mcp
