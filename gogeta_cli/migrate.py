"""CLI handlers for ``gogeta migrate ...``.

Currently exposes only ``gogeta migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``gogeta migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: gogeta migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `gogeta migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `gogeta doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
