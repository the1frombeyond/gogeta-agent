"""Persistent session goals — the Ralph loop for Gogeta.
...
goal satisfied by the assistant's last response?". If not, Gogeta feeds a
...
- This module has zero hard dependency on ``cli.GogetaCLI`` or the gateway
...
def _get_session_db() -> Optional[Any]:
    ...
    """Return a SessionDB instance for the current GOGETA_HOME.
    ...
    ``gogeta_home`` path so profile switches still pick up the right DB.
...
def run_kanban_goal_loop(
    ...
    *,
    ...
    (``gogeta -p <profile> chat -q "work kanban task <id>"``). The worker's
