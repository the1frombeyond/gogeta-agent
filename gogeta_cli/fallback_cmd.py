"""
gogeta fallback — manage the fallback provider chain.
...
Subcommands:
  gogeta fallback [list]   Show the current fallback chain (default when no subcommand)
  gogeta fallback add      Pick provider + model via the same picker as `gogeta model`,
                           then append the selection to the chain
  gogeta fallback remove   Pick an entry to delete from the chain
  gogeta fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.gogeta/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from gogeta_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `gogeta model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  gogeta fallback add")
        print()
        return
    ...
    print("  Docs: https://gogeta.ai/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `gogeta model`, then append the selection to the chain."""
    from gogeta_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `gogeta model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `gogeta fallback list` to view, or `gogeta fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``gogeta fallback [subcommand]``."""
