...
the existing module-level functions in gogeta_cli.gateway and

gogeta_cli.gateway_windows directly. This protocol is a thin facade
...
``gogeta gateway start/stop/restart`` when running inside a container.
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``gogeta gateway start/stop/restart``).
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged gogeta
    ...
    detection always fails. Since every Gogeta runtime call inside the

    container drops to gogeta via ``s6-setuidgid``, that silent failure
...
# in ``gogeta_cli.gateway`` (systemd/launchd) and ``gogeta_cli.gateway_windows``
...
# active (set via the ``gogeta -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    ...
    """Thin wrapper around the ``systemd_*`` functions in gogeta_cli.gateway.
    ...
    def start(self, name: str) -> None:
        ...
        from gogeta_cli.gateway import systemd_start
    ...
    def stop(self, name: str) -> None:
        ...
        from gogeta_cli.gateway import systemd_stop
    ...
    def restart(self, name: str) -> None:
        ...
        from gogeta_cli.gateway import systemd_restart
    ...
    def is_running(self, name: str) -> bool:
        ...
        from gogeta_cli.gateway import _probe_systemd_service_running
...
class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    ...
    """Thin wrapper around the ``launchd_*`` functions in gogeta_cli.gateway."""
    ...
    def start(self, name: str) -> None:
        ...
        from gogeta_cli.gateway import launchd_start
    ...
    def stop(self, name: str) -> None:
        ...
        from gogeta_cli.gateway import launchd_stop
    ...
    def restart(self, name: str) -> None:
        ...
        from gogeta_cli.gateway import launchd_restart
    ...
    def is_running(self, name: str) -> bool:
        ...
        from gogeta_cli.gateway import _probe_launchd_service_running
...
class WindowsServiceManager(_RegistrationUnsupportedMixin):
    ...
    """Thin wrapper around ``gogeta_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        ...
        from gogeta_cli.gateway import find_gateway_pids
...
# Per-profile gateways are registered dynamically when `gogeta profile create`

# runs inside the container (Phase 4). Static services (main-gogeta, dashboard)
...
# UID/GID of the in-image ``gogeta`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the gogeta user.
    ...
    (the ``gogeta`` user — UID 10000 — running every Gogeta runtime
    ...
    is dispatched as the gogeta user.
    ...
    with gogeta ownership before triggering ``s6-svscanctl -a``,
    ...
    ``svc_dir/``                           gogeta:gogeta, 0755 (parent must already exist)

    ``svc_dir/event/``                     gogeta:gogeta, 03730   (setgid + g+rwx + sticky)

    ``svc_dir/supervise/``                 gogeta:gogeta, 0755

    ``svc_dir/supervise/event/``           gogeta:gogeta, 03730

    ``svc_dir/supervise/control``          gogeta:gogeta, 0660    (FIFO)
    ...
    slot's supervise/ was gogeta-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            os.chown(path, _GOGETA_UID, _GOGETA_GID)

        except PermissionError:

            # Running as the gogeta user already — directory is gogeta-
    ...
    if not control.exists():
        ...
        try:
            ...
            os.chown(control, _GOGETA_UID, _GOGETA_GID)
    ...
    # when the parent slot's supervise/ is gogeta-owned.
    ...
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                ...
                os.chown(log_control, _GOGETA_UID, _GOGETA_GID)
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``gogeta -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`gogeta profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-gogeta, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          ...
          1. Sources GOGETA_HOME (and any extra env) via with-contenv —

             so e.g. ``-e GOGETA_HOME=/data/gogeta`` is honored at run
          ...
          4. Drops to the gogeta user and exec's

             ``gogeta -p <profile> gateway run`` (or just ``gogeta

             gateway run`` for the default profile — see below).



        Special case: ``profile == "default"`` emits ``gogeta gateway

        run`` with **no** ``-p`` flag. This is the sentinel for "the

        root GOGETA_HOME profile" (the implicit profile that exists at

        the top of $GOGETA_HOME, not under profiles/). It must be

        spelled this way because ``_profile_suffix()`` returns the

        empty string for the root profile, and the dispatcher in

        ``gogeta_cli.gateway`` maps that empty string to the

        ``gateway-default`` service slot. Passing ``-p default`` here

        would instead look up ``$GOGETA_HOME/profiles/default/`` — a
        ...
        ``gogeta_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        lines.append("export GOGETA_S6_SUPERVISED_CHILD=1")

        if profile == "default":

            gateway_cmd = "gogeta gateway run"

        else:

            gateway_cmd = f"gogeta -p {shlex.quote(profile)} gateway run"
        ...
        lines.append(f"exec s6-setuidgid gogeta {gateway_cmd}")
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${GOGETA_HOME}/logs/gateways/<profile>/``.

        CRITICAL: the GOGETA_HOME path is sourced from the runtime env

        via with-contenv — NOT Python-substituted at registration time

        — so a container started with ``-e GOGETA_HOME=/data/gogeta``

        gets its logs under /data/gogeta/logs/..., not the build-time
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``gogeta logs`` reads and what
        ...
        return (
            ...
            f': "${{GOGETA_HOME:=/opt/data}}"\n'

            f'log_dir="$GOGETA_HOME/logs/gateways/{prof}"\n'

            f'mkdir -p "$log_dir"\n'

            f'chown -R gogeta:gogeta "$log_dir" 2>/dev/null || true\n'

            # Skip the drop when already non-root (CAP_SETGID).

            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'

            f'exec s6-setuidgid gogeta s6-log 1 n10 s1000000 T "$log_dir"\n'
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``gogeta gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with gogeta ownership
            ...
            # the gogeta user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is gogeta-owned (see ``_seed_supervise_skeleton``), and on
