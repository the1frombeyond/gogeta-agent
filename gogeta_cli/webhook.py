"""gogeta webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    gogeta webhook subscribe <name> [options]
    gogeta webhook list
    gogeta webhook remove <name>
    gogeta webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.gogeta/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _gogeta_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: gogeta gateway run
"""
...
def webhook_command(args):
    """Entry point for 'gogeta webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: gogeta webhook {subscribe|list|remove|test}")
        print("Run 'gogeta webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (gogeta gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: gogeta webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from gogeta webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (gogeta gateway run)")
