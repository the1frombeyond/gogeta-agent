"""``gogeta dashboard register`` — register a self-hosted dashboard OAuth client.
...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.gogeta/.env``

as ``GOGETA_DASHBOARD_OAUTH_CLIENT_ID``.
...
This command:
  ...
  1. Resolves a fresh Nous Portal access token from the existing login
     ...
     (``~/.gogeta/auth.json``), refreshing it if needed. Fails fast with a

     "run `gogeta setup`" hint when the user isn't logged in.
  ...
  3. Writes ``GOGETA_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)

     ``GOGETA_DASHBOARD_PORTAL_URL`` into ``~/.gogeta/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      ...
      1. ``override`` — explicit ``--portal-url`` flag or
         ...
         ``GOGETA_DASHBOARD_PORTAL_URL`` env (used for testing against a
    ...
    try:
        ...
        from gogeta_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    ...
    *,
    ...
    makes re-running ``gogeta dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            ...
            raise RuntimeError(
                ...
                "Try `gogeta auth login nous` to re-authenticate."
...
def _print_post_register_hint(
    ...
    *,
    ...
    from gogeta_cli.config import get_env_path
    ...
    print("    GOGETA_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))

    if wrote_portal_url:

        print("    GOGETA_DASHBOARD_PORTAL_URL=" + str(portal_base_url))

    if public_url:

        print("    GOGETA_DASHBOARD_PUBLIC_URL=" + str(public_url))
    ...
    print(
        ...
        "  `gogeta dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    gogeta dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from gogeta_cli.auth import AuthError, resolve_nous_access_token

    from gogeta_cli.config import get_env_value, is_managed, save_env_value



    # Managed (Docker/hosted) installs get their dashboard OAuth client_id

    # stamped in by the orchestrator (NAS sets GOGETA_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        ...
        print(
            ...
            "✗ `gogeta dashboard register` is not available in a managed/hosted "
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `gogeta setup` (or `gogeta auth login nous`) first, then retry.")
    ...
    # GOGETA_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    ...
    # client_id locally (GOGETA_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        ...
        existing_client_id = get_env_value("GOGETA_DASHBOARD_OAUTH_CLIENT_ID")
    ...
    try:
        ...
        save_env_value("GOGETA_DASHBOARD_OAUTH_CLIENT_ID", client_id)

    except Exception as exc:

        print(f"✗ Failed to write GOGETA_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")

        print(f"  Set it manually:  GOGETA_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
    ...
    #      GOGETA_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    default_portal = "https://portal.gogeta.ai"

    existing_portal = None

    try:

        existing_portal = get_env_value("GOGETA_DASHBOARD_PORTAL_URL")
    ...
    if should_write_portal:
        ...
        try:
            ...
            save_env_value("GOGETA_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # the portal, e.g. https://gogeta.example.com/auth/callback. At serve time

    # the dashboard auth layer (dashboard_auth/routes._redirect_uri) reconstructs

    # that same callback by taking GOGETA_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as GOGETA_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            ...
            existing_public_url = get_env_value("GOGETA_DASHBOARD_PUBLIC_URL")
        ...
        if existing_public_url != public_url:
            ...
            try:
                ...
                save_env_value("GOGETA_DASHBOARD_PUBLIC_URL", public_url)
