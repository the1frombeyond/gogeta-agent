...
- Keep data: Remove code but keep ~/.gogeta/ (configs, sessions, logs)
...
def remove_path_from_shell_configs():
    ...
    """Remove Gogeta PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        ...
        try:
            ...
            # Remove lines containing gogeta-agent or gogeta PATH entries
            ...
            for line in content.split('\n'):
                ...
                # Skip the "# Gogeta Agent" comment and following line

                if '# Gogeta Agent' in line or '# gogeta-agent' in line:

                    skip_next = True

                    continue

                if skip_next and ('gogeta' in line.lower() and 'PATH' in line):
                ...
                # Remove any PATH line containing gogeta

                if 'gogeta' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
...
def remove_wrapper_script():
    ...
    """Remove the gogeta wrapper script if it exists."""

    wrapper_paths = [

        Path.home() / ".local" / "bin" / "gogeta",

        Path("/usr/local/bin/gogeta"),
    ...
    for wrapper in wrapper_paths:
        ...
        if wrapper.exists():
            ...
            try:
                ...
                # Check if it's our wrapper (contains gogeta_cli reference)

                content = wrapper.read_text()

                if 'gogeta_cli' in content or 'gogeta-agent' in content:
...
def remove_node_symlinks(gogeta_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``gogeta`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Gogeta home's ``node``
    ...
    node_dir = (gogeta_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``gogeta gateway run`` processes
    ...
    try:
        ...
        from gogeta_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            ...
            from gogeta_cli.gateway import get_launchd_plist_path
...
#   1. Sets User-scope env vars ``GOGETA_HOME`` and ``GOGETA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\gogeta\git\cmd``, ``%LOCALAPPDATA%\gogeta\git\bin``,

#      ``%LOCALAPPDATA%\gogeta\git\usr\bin``, ``%LOCALAPPDATA%\gogeta\node``.

#      Again not in any rc file — only accessible via the registry or the

#      .NET [Environment] API.

#   3. Downloads PortableGit to ``%LOCALAPPDATA%\gogeta\git\`` and Node to

#      ``%LOCALAPPDATA%\gogeta\node\`` as user-scoped, isolated copies.

#      These are ~200MB combined and serve no purpose after uninstall.

#   4. On the ``gogeta dashboard`` + gateway paths, drops files into

#      ``%LOCALAPPDATA%\gogeta\gateway-service\`` and sometimes
...
def _gogeta_path_markers(gogeta_home: Path) -> list[str]:

    """Path-entry substrings that identify Gogeta-owned User-PATH entries."""

    root = str(gogeta_home).rstrip("\\/")

    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)

    # all get swept.  Also match the bare gogeta-agent install dir.

    markers = [root + "\\gogeta-agent", root + "\\git", root + "\\node", root + "\\venv"]

    # Also match if GOGETA_HOME was customised to somewhere else — find-and-nuke

    # any entry whose path component contains "gogeta".  We don't want to catch

    # unrelated entries like "cgogeta-foo" or "ephermeral", so we look for

    # backslash-gogeta as a word-ish boundary.
...
def remove_path_from_windows_registry(gogeta_home: Path) -> list[str]:

    """Strip Gogeta-owned entries from User-scope PATH in the registry.
    ...
    try:
        ...
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _gogeta_path_markers(gogeta_home)
...
def remove_gogeta_env_vars_windows() -> list[str]:

    """Delete GOGETA_HOME and GOGETA_GIT_BASH_PATH from User-scope env vars."""
    ...
    try:
        ...
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
            ...
            for name in ("GOGETA_HOME", "GOGETA_GIT_BASH_PATH"):
...
def remove_portable_tooling_windows(gogeta_home: Path) -> list[Path]:

    """Delete PortableGit and Node installs the Windows installer created under

    ``%LOCALAPPDATA%\\gogeta\\``.  Only called on full uninstall; they're
...
def _is_default_gogeta_home(gogeta_home: Path) -> bool:

    """Return True when ``gogeta_home`` points at the default (non-profile) root."""

    try:

        from gogeta_constants import get_default_gogeta_root

        return gogeta_home.resolve() == get_default_gogeta_root().resolve()
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its GOGETA_HOME directory.



    We shell out to ``gogeta -p <name> gateway stop|uninstall`` because

    service names, unit paths, and plist paths are all derived from the

    current GOGETA_HOME and can't be easily switched in-process.
    ...
    #    Use `python -m gogeta_cli.main` so we don't depend on a `gogeta`

    #    wrapper that may be half-removed mid-uninstall.

    gogeta_invocation = [_sys.executable, "-m", "gogeta_cli.main", "--profile", name]
    ...
    for subcmd in ("stop", "uninstall"):
        ...
        try:
            ...
            subprocess.run(
                ...
                gogeta_invocation + ["gateway", subcmd],
    ...
    # 3. Wipe the profile's GOGETA_HOME directory.
...
def run_gui_uninstall(args):
    ...
    Mirrors ``gogeta uninstall --gui``. Removes the desktop app's built

    artifacts, the packaged app bundle (best-effort), and the Electron

    userData dir — nothing under ``$GOGETA_HOME`` config/sessions/.env, and

    never the Python agent or its venv.

    """

    from gogeta_cli.gui_uninstall import (
    ...
    summary = gui_install_summary(gogeta_home)
    ...
    print(color("│         ⚕ Gogeta Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    ...
    if not summary["gui_installed"]:
        ...
        print("No Gogeta Chat GUI installation was found.")

        print(f"  Checked: {gogeta_home}, and the standard app locations for this OS.")

        return



    print(color("This removes the Chat GUI only. The Gogeta agent stays installed.", Colors.CYAN))
    ...
    if agent_is_installed(gogeta_home):

        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))

        print(f"  • The Gogeta agent at {gogeta_home / 'gogeta-agent'}")

        print(f"  • Your config, sessions, and secrets under {gogeta_home}")
    ...
    print("The Gogeta agent is still installed. Run 'gogeta' to use the CLI,")

    print("or 'gogeta uninstall' to remove the agent too.")
...
def run_uninstall(args):
    ...
    - Full uninstall: removes code + ~/.gogeta/ (configs, data, logs)

    - Keep data: removes code but keeps ~/.gogeta/ for future reinstall
    ...
    # offer to clean them up too instead of leaving zombie GOGETA_HOMEs

    # and systemd units behind.

    is_default_profile = _is_default_gogeta_home(gogeta_home)
    ...
    # full wipe (code + ~/.gogeta data); otherwise keep-data. Named profiles
    ...
    print(color("│            ⚕ Gogeta Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    ...
    print(f"  Config:  {gogeta_home / 'config.yaml'}")

    print(f"  Secrets: {gogeta_home / '.env'}")

    print(f"  Data:    {gogeta_home / 'cron/'}, {gogeta_home / 'sessions/'}, {gogeta_home / 'logs/'}")
    ...
    # their alias wrappers, and wiping their GOGETA_HOME dirs. Otherwise
    ...
    if full_uninstall:
        ...
        print(color("⚠️  WARNING: This will permanently delete ALL Gogeta data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Gogeta code but keep your configuration and data.")
...
def _perform_uninstall(
    ...
    *,
    ...
    ``gogeta`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →

    delete the code checkout → (Windows) remove PortableGit/Node → optionally

    wipe ``$GOGETA_HOME`` data and named profiles on full uninstall.
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in gogeta_home so the marker matching is

        # against fully resolved paths — installer writes literal strings

        # like C:\Users\<u>\AppData\Local\gogeta\git\cmd, not %LOCALAPPDATA%.

        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(gogeta_home))))
        ...
        if removed_path_entries:
            ...
            log_info("No Gogeta-owned PATH entries in User environment")



        log_info("Removing GOGETA_HOME / GOGETA_GIT_BASH_PATH User env vars...")

        removed_env = remove_gogeta_env_vars_windows()
        ...
        if removed_env:
            ...
            log_info("No Gogeta-set User env vars to remove")

    

    # 3. Remove wrapper script

    log_info("Removing gogeta command...")
    ...
    #     (only when they still point into this Gogeta home's node dir, so we

    #     never clobber an existing nvm / user-managed Node).

    log_info("Removing Gogeta-managed node/npm/npx symlinks...")

    removed_node_links = remove_node_symlinks(gogeta_home)
    ...
    if removed_node_links:
        ...
        log_info("No Gogeta-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(gogeta_home) would sweep the in-tree artifacts anyway,

    #     but the packaged app + Electron userData live OUTSIDE GOGETA_HOME and
    ...
    try:
        ...
        from gogeta_cli.gui_uninstall import uninstall_gui

        gui_removed = uninstall_gui(gogeta_home)
    ...
    try:
        ...
        if project_root.exists():
            ...
            # If the install is inside ~/.gogeta/, just remove the gogeta-agent subdir

            if gogeta_home in project_root.parents or project_root.parent == gogeta_home:
    ...
    #     under GOGETA_HOME but they're install tooling, not config — safe to

    #     remove even in "keep data" mode.  If we're doing a full uninstall

    #     the step-5 rmtree(gogeta_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(gogeta_home)
    ...
    # 5. Optionally remove ~/.gogeta/ data directory (and named profiles)

    if full_uninstall:

        # 5a. Stop and remove each named profile's gateway service and

        #     alias wrapper. The profile GOGETA_HOME dirs live under
        ...
        try:
            ...
            if gogeta_home.exists():

                shutil.rmtree(gogeta_home)

                log_success(f"Removed {gogeta_home}")

        except Exception as e:

            log_warn(f"Could not fully remove {gogeta_home}: {e}")

            log_info("You may need to manually remove it")

    else:

        log_info(f"Keeping configuration and data in {gogeta_home}")
    ...
    print("Thank you for using Gogeta Agent! ⚕")
...
def main(argv=None) -> int:
    ...
    """Module entrypoint: ``python -m gogeta_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import gogeta_cli`` resolves from source while the venv is torn down.



    This module imports only stdlib + ``gogeta_constants`` + ``gogeta_cli.colors``

    (and lazily ``gogeta_cli.gui_uninstall``), so it runs fine under a bare
    ...
    parser = argparse.ArgumentParser(prog="python -m gogeta_cli.uninstall")
