"""Windows ConPTY bridge for the `gogeta dashboard` chat tab.

Drop-in counterpart to ``gogeta_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``gogeta_cli.web_server`` consumes: ``spawn``, ``read``,
