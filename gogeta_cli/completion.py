"""Shell completion script generation for gogeta CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_gogeta_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Gogeta Agent bash completion
# Add to ~/.bashrc:
#   eval "$(gogeta completion bash)"

_gogeta_profiles() {{
    local profiles_dir="$HOME/.gogeta/profiles"
...
_gogeta_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_gogeta_profiles)" -- "$cur"))
        return
    fi
...
complete -F _gogeta_completion gogeta
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef gogeta
# Gogeta Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(gogeta completion zsh)"

_gogeta_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.gogeta/profiles" ]]; then
        profiles+=($HOME/.gogeta/profiles/*(N/:t))
    fi
...
_gogeta() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_gogeta_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'gogeta command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Gogeta Agent fish completion",
        "# Add to your config:",
        "#   gogeta completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __gogeta_profiles",
        "    echo default",
        "    if test -d $HOME/.gogeta/profiles",
        "        for d in $HOME/.gogeta/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c gogeta -f -s p -l profile"
        " -d 'Profile name' -xa '(__gogeta_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__gogeta_profiles)' -d 'Profile name'"
                )
