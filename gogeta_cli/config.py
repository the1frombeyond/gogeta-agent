...
Configuration management for Gogeta Agent.



Config files are stored in ~/.gogeta/ for easy access:

- ~/.gogeta/config.yaml  - All settings (model, toolsets, terminal, etc.)

- ~/.gogeta/.env         - API keys and secrets



This module provides:

- gogeta config          - Show current configuration

- gogeta config edit     - Open config in editor

- gogeta config set      - Set a specific value

- gogeta config wizard   - Re-run setup wizard
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``gogeta config set`` (which rewrites
    ...
    ``config.yaml`` in place: gogeta never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.gogeta/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``gogeta logs``
    ...
    try:
        ...
        sys.stderr.write(f"⚠️  gogeta config: {msg}\n")
...
#   the next ``subprocess.run([...])`` Gogeta makes loads attacker code

#   before main().

# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /

#   ``PYTHONUSERBASE`` — Python interpreter init. Gogeta itself starts

#   from one of these on every restart.

# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,

#   ``gogeta update``, the TUI build.
...
#   on every plugin install / ``gogeta update``.
...
# * ``GOGETA_HOME`` / ``GOGETA_PROFILE`` / ``GOGETA_CONFIG`` /

#   ``GOGETA_ENV`` — Gogeta runtime location flags. Writing these into
...
# IMPORTANT: ``GOGETA_*`` overall is NOT blocked. Many legitimate

# integration credentials follow that prefix (GOGETA_GEMINI_CLIENT_ID,

# GOGETA_LANGFUSE_PUBLIC_KEY, GOGETA_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Gogeta runtime location — never via dashboard env writer.

    # NOT a GOGETA_* blanket: integration credentials (GOGETA_GEMINI_*,

    # GOGETA_LANGFUSE_*, GOGETA_SPOTIFY_*, ...) ARE allowed.

    "GOGETA_HOME", "GOGETA_PROFILE", "GOGETA_CONFIG", "GOGETA_ENV",
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        ...
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Gogeta runtime location "

            "(GOGETA_HOME, GOGETA_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `gogeta plugins enable

    # observability/langfuse` or `gogeta tools → Langfuse`); credentials gate

    # the plugin at runtime.

    "GOGETA_LANGFUSE_ENV",

    "GOGETA_LANGFUSE_RELEASE",

    "GOGETA_LANGFUSE_SAMPLE_RATE",

    "GOGETA_LANGFUSE_MAX_CHARS",

    "GOGETA_LANGFUSE_DEBUG",
...
from gogeta_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("GOGETA_MANAGED", "").strip()
    ...
    managed_marker = get_gogeta_home() / ".managed"
...
def is_managed() -> bool:
    ...
    """Check if Gogeta is running in package-manager-managed mode.



    Two signals: the GOGETA_MANAGED env var (set by the systemd service),

    or a .managed marker file in GOGETA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        ...
        return "brew upgrade gogeta-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    """Detect how Gogeta was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.



    Resolution order:

    1. Stamped ``~/.gogeta/.install_method`` file (written by installers)

    2. GOGETA_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``gogeta/gogeta-agent`` image stamps ``docker``
    ...
    so ``gogeta update`` bailed with "doesn't apply inside the Docker
    ...
    stamp = get_gogeta_home() / ".install_method"
...
def stamp_install_method(method: str) -> None:
    ...
    """Write the install method to ~/.gogeta/.install_method."""
...
def is_uv_tool_install() -> bool:
    ...
    """Return True when the *running* Gogeta lives in a ``uv tool`` layout.



    ``uv tool install gogeta-agent`` places the install at

    ``.../uv/tools/gogeta-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``gogeta-agent`` happens to be uv-tool-installed on the machine while

    the *active* Gogeta is a regular pip/venv install, causing

    ``gogeta update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/gogeta-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "docker":
        ...
        return "docker pull gogeta/gogeta-agent:latest"

    if method == "pip":

        if is_uv_tool_install():

            return "uv tool upgrade gogeta-agent"

        import shutil

        if shutil.which("uv"):

            return "uv pip install --upgrade gogeta-agent"

        return "pip install --upgrade gogeta-agent"

    return "gogeta update"
...
# Long-form text for ``gogeta update`` / ``--check`` when running inside the

# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in

# gogeta_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Gogeta, it doesn't update
...
✗ ``gogeta update`` doesn't apply inside the Docker container.



Gogeta Agent runs as a published image (gogeta/gogeta-agent), not a
...
pulling a fresh image and restarting your container instead:
  ...
  docker pull gogeta/gogeta-agent:latest

  # then restart whatever started the container, e.g.:

  docker compose up -d --force-recreate gogeta-agent
...
Verify the new version after restart:
  ...
  docker run --rm gogeta/gogeta-agent:latest --version
...
Notes:
  ...
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    ...
    tags at https://hub.docker.com/r/gogeta/gogeta-agent/tags

  • Your config and session history live under ``$GOGETA_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    ...
    """Return the user-facing message for ``gogeta update`` inside Docker.
...
def format_managed_message(action: str = "modify this Gogeta installation") -> str:

    """Build a user-facing error for managed installs."""

    managed_system = get_managed_system() or "a package manager"

    raw = os.getenv("GOGETA_MANAGED", "").strip().lower()
    ...
    if managed_system == "NixOS":
        ...
        return (
            ...
            f"Cannot {action}: this Gogeta installation is managed by NixOS "

            f"(GOGETA_MANAGED={env_hint}).\n"

            "Edit services.gogeta-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            ...
            f"Cannot {action}: this Gogeta installation is managed by Homebrew "

            f"(GOGETA_MANAGED={env_hint}).\n"

            "Use:\n"

            "  brew upgrade gogeta-agent"
    ...
    return (
        ...
        f"Cannot {action}: this Gogeta installation is managed by {managed_system}.\n"

        "Use your package manager to upgrade or reinstall Gogeta."
...
def get_container_exec_info() -> Optional[dict]:
    ...
    """Read container mode metadata from GOGETA_HOME/.container-mode.



    Returns a dict with keys: backend, container_name, exec_user, gogeta_bin

    or None if container mode is not active, we're already inside the

    container, or GOGETA_DEV=1 is set.
    ...
    if os.environ.get("GOGETA_DEV") == "1":
    ...
    container_mode_file = get_gogeta_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "gogeta-agent")

    exec_user = info.get("exec_user", "gogeta")

    gogeta_bin = info.get("gogeta_bin", "/data/current-package/bin/gogeta")
...
# Re-export from gogeta_constants — canonical definition lives there.

from gogeta_constants import get_gogeta_home  # noqa: F811,E402
...
def get_config_path() -> Path:
    ...
    return get_gogeta_home() / "config.yaml"
...
def _resolve_gogeta_uid_gid() -> tuple[Optional[int], Optional[int]]:

    """Read the GOGETA_UID / GOGETA_GID env vars set by Docker deployments.



    Docker containers running Gogeta commonly set these to map the in-container

    user to a host user so volume-mounted state files end up with the right

    ownership. The entrypoint chowns the top-level GOGETA_HOME once, but

    subdirectories created at runtime by ``ensure_gogeta_home()`` (especially
    ...
    uid_str = os.environ.get("GOGETA_UID", "").strip()

    gid_str = os.environ.get("GOGETA_GID", "").strip()
...
def _chown_to_gogeta_uid(path) -> None:

    """Chown ``path`` to ``GOGETA_UID:GOGETA_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_gogeta_home` on Docker deployments.

    See #34107.

    """

    uid, gid = _resolve_gogeta_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the gogeta group can

    share state with the gateway service.



    The mode can be overridden via the GOGETA_HOME_MODE environment variable

    (e.g. GOGETA_HOME_MODE=0701) for deployments where a web server (nginx,

    caddy, etc.) needs to traverse GOGETA_HOME to reach a served subdirectory.
    ...
    Also applies ``GOGETA_UID``/``GOGETA_GID``-based ownership when those env
    ...
    try:
        ...
        mode_str = os.environ.get("GOGETA_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Gogeta runs in a container with volume-mounted config files, forcing
    ...
    if os.environ.get("GOGETA_CONTAINER") or os.environ.get("GOGETA_SKIP_CHMOD"):
...
def _secure_file(path):
    ...
    permissions.  Set GOGETA_SKIP_CHMOD=1 to force-skip on other systems.
...
def _ensure_default_soul_md(home: Path) -> None:
    ...
    """Seed a default SOUL.md into GOGETA_HOME if the user doesn't have one yet."""
...
def ensure_gogeta_home():

    """Ensure ~/.gogeta directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            ...
            _ensure_gogeta_home_managed(home)
...
def _ensure_gogeta_home_managed(home: Path):
    ...
    if not home.is_dir():
        ...
        raise RuntimeError(
            ...
            f"GOGETA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Gogeta-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Gogeta
        ...
        # identity slot (SOUL.md). Empty by default. The GOGETA_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # skipped. When empty, Gogeta auto-sources ``~/.profile``,
        ...
        # When true (default), Gogeta sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Gogeta
        ...
        #  "/home/user/.gogeta/cache/documents:/output"]
        ...
        # bundled Gogeta image, which drops to the `gogeta` user via
    ...
    "browser": {
        ...
        "camofox": {
            ...
            # When true, Gogeta sends a stable profile-scoped userId to Camofox
            ...
            # app owns the visible browser and Gogeta should operate in it.
    ...
    # Opt in via ``gogeta chat --checkpoints`` or set enabled=True here.
    ...
    "checkpoints": {
        ...
        # Hard ceiling on total ``~/.gogeta/checkpoints/`` size (MB).  When
        ...
        # Auto-maintenance: gogeta sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Gogeta truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``gogeta kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``gogeta kanban decompose`` and the kanban
        ...
        # ``gogeta profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `gogeta model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `gogeta` (and `gogeta chat`) launches by default:
        ...
        # REPL and `--tui` (or GOGETA_TUI=1) forces the TUI regardless of config.

        "interface": "cli",

        # When true, `gogeta --tui` auto-resumes the most recent human-

        # facing session on launch instead of forging a fresh one.

        # Mirrors `gogeta -c` muscle memory.  Default off so existing

        # users aren't surprised.  GOGETA_TUI_RESUME=<id> always wins.

        "tui_auto_resume_recent": False,

        # When true (default), `gogeta --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/gogeta`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``GOGETA_DASHBOARD_OAUTH_CLIENT_ID`` and

        # ``GOGETA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.gogeta.ai).
        ...
        # (``GOGETA_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``GOGETA_DASHBOARD_PUBLIC_URL``).

        # When set, this is the complete authority — scheme + host +

        # optional path prefix (e.g. ``https://example.com/gogeta``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.gogeta/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.gogeta/plugins/.
    ...
    # response. If not, Gogeta feeds a continuation prompt back into the
    ...
    "goals": {
        ...
        # Max continuation turns before Gogeta auto-pauses the goal and
    ...
    # always goes to ~/.gogeta/skills/.

    "skills": {

        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]

        # Substitute ${GOGETA_SKILL_DIR} and ${GOGETA_SESSION_ID} in SKILL.md
    ...
    # See `gogeta curator status` for the last run summary.
    ...
    "curator": {
        ...
        # normally restored on every `gogeta update`, so pruning them only
        ...
        # skipped), snapshot ~/.gogeta/skills/ into

        # ~/.gogeta/skills/.curator_backups/<utc-iso>/skills.tar.gz so the

        # user can roll back with `gogeta curator rollback`.
    ...
    # This section is only needed for gogeta-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Gogeta Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (GOGETA_TUI_NO_CONFIRM=1 to
    ...
    # stored approval from ~/.gogeta/shell-hooks-allowlist.json.
    ...
    # toggleable per-invocation via --accept-hooks or GOGETA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `gogeta doctor --ack

        # <id>`; remove by editing the list directly. See

        # ``gogeta_cli/security_advisories.py`` for the catalog.

        "acked_advisories": [],

        # Allow Gogeta to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via GOGETA_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `gogeta -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `gogeta` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `gogeta kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     gogeta-agent's own python (sys.executable). Maximum isolation
    ...
    # Core Gogeta tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.gogeta/logs/.
    ...
    # update model picker lists without shipping a gogeta-agent release.
    ...
    "model_catalog": {
        ...
        "url": "https://gogeta.ai/docs/api/model-catalog.json",

        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the

        # next /model or `gogeta model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.gogeta/.env,
        ...
        # behavior: files must live under the Gogeta cache, under
        ...
        # user. Bridged to GOGETA_MEDIA_DELIVERY_STRICT.

        "strict": False,

        # Extra directories from which model-emitted bare file paths may be

        # uploaded as native gateway attachments. Files inside the Gogeta

        # cache (~/.gogeta/cache/{documents,images,audio,video,screenshots})
        ...
        # to GOGETA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # GOGETA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        ...
        # multi-tool agent turn. Bridged to GOGETA_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.gogeta/state.db.
    ...
    "sessions": {
        ...
        # default of ``gogeta sessions prune``.
        ...
        # rewrites ``~/.gogeta/sessions/session_{sid}.json`` on every turn
    ...
    # ``gogeta update`` behaviour.

    "updates": {

        # Run a full ``gogeta backup``-style zip of GOGETA_HOME before every

        # ``gogeta update``.  Backups land in ``<GOGETA_HOME>/backups/`` and

        # can be restored with ``gogeta import <path>``.  Off by default —

        # on large GOGETA_HOME directories the zip can add minutes to every
        ...
        # What `gogeta update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<GOGETA_HOME>/lsp/bin/`` on first use.
    ...
    # `gogeta tools`. These settings tune the backing Responses API call.
    ...
    # rather than storing them in ~/.gogeta/.env.
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.gogeta/.env (or your shell) and never in config.yaml.
            ...
            # ~/.gogeta/bin/ on first use.  When False you must install
            ...
            # `gogeta secrets bitwarden setup`.
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        ...
        "description": "Azure Foundry base URL (set via 'gogeta model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        ...
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. gogeta.ai -> firecrawl-gateway.gogeta.ai",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        ...
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Gogeta auth store)",
    ...
    "MATRIX_USER_ID": {
        ...
        "description": "Matrix user ID (e.g. @gogeta:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        ...
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. GOGETA_BOT)",
    ...
    "IRC_CHANNEL": {
        ...
        "description": "IRC channel to join (e.g. #gogeta)",
    ...
    "IRC_NICKNAME": {
        ...
        "description": "Bot nickname on IRC (default: gogeta-bot)",
    ...
    "API_SERVER_MODEL_NAME": {
        ...
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'gogeta-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        ...
        "description": "URL of a remote Gogeta API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",

        "prompt": "Remote Gogeta API server URL (e.g. http://192.168.1.100:8642)",
    ...
    "GATEWAY_PROXY_KEY": {
        ...
        "description": "Bearer token for authenticating with the remote Gogeta API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # GOGETA_TOOL_PROGRESS and GOGETA_TOOL_PROGRESS_MODE are deprecated —

    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).

    # Gateway falls back to these env vars for backward compatibility.

    "GOGETA_TOOL_PROGRESS": {
    ...
    "GOGETA_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.gogeta.config`` entries, then checks
    ...
    try:
        ...
        # should never break `gogeta update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Gogeta versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``gogeta_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        ...
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'gogeta setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        ...
        issues.append(ConfigIssue(
            ...
            "custom_providers defined but no 'model' section — Gogeta won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'gogeta doctor' for fix suggestions.\033[0m")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        ...
        hint_path = os.environ.get("GOGETA_HOME", "~/.gogeta")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            ...
            old_enabled = get_env_value("GOGETA_TOOL_PROGRESS")

            old_mode = get_env_value("GOGETA_TOOL_PROGRESS_MODE")

            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:

                display["tool_progress"] = "off"

                results["config_added"].append("display.tool_progress=off (from GOGETA_TOOL_PROGRESS=false)")

            elif old_mode and old_mode.lower() in {"new", "all"}:

                display["tool_progress"] = old_mode.lower()

                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from GOGETA_TOOL_PROGRESS_MODE)")
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            ...
            old_tz = os.getenv("GOGETA_TIMEZONE", "")

            if old_tz and old_tz.strip():

                config["timezone"] = old_tz.strip()

                results["config_added"].append(f"timezone={old_tz.strip()} (from GOGETA_TIMEZONE)")
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$GOGETA_HOME/plugins/`` for currently installed user plugins.

            grandfathered: List[str] = []

            try:

                user_plugins_dir = get_gogeta_home() / "plugins"
            ...
            if not quiet:
                ...
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `gogeta plugins enable <name>` to activate."
    ...
    # settings in their `config.yaml`, and `gogeta curator status` has no
    ...
    #   3. Creates `~/.gogeta/logs/curator/` if missing (belt-and-suspenders

    #      on top of ensure_gogeta_home() — old profiles that predate this
    ...
    if current_ver < 23:
        ...
        try:
            ...
            curator_dir = get_gogeta_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: gogeta config set <key> <value>")
    ...
    # metadata.gogeta.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _gogeta_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    ...
    """Read ~/.gogeta/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    ...
    """Load configuration from ~/.gogeta/config.yaml.
    ...
    (which change ``GOGETA_HOME`` and therefore ``get_config_path()``)
...
#   openai-codex (OAuth — gogeta auth) — OpenAI Codex

#   nous         (OAuth — gogeta auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    ...
    """Save configuration to ~/.gogeta/config.yaml."""
...
def load_env() -> Dict[str, str]:
    ...
    """Load environment variables from ~/.gogeta/.env.
    ...
    interactive menu render (`gogeta tools`, `gogeta setup`, status

    panels). Sanitisation is O(lines × known-keys), so re-parsing the

    same file on every call was burning ~300ms of CPU per `gogeta tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Gogeta env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    ...
    """Read, sanitize, and rewrite ~/.gogeta/.env in place.
...
def save_env_value(key: str, value: str):
    ...
    """Save or update a value in ~/.gogeta/.env."""
...
def remove_env_value(key: str) -> bool:
    ...
    """Remove a key from ~/.gogeta/.env and os.environ.
...
def reload_env() -> int:
    ...
    """Re-read ~/.gogeta/.env into os.environ. Returns count of vars updated.



    Adds/updates vars that changed and removes vars that were deleted from

    the .env file (but only vars known to Gogeta — OPTIONAL_ENV_VARS and
    ...
    # Remove known Gogeta vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    ...
    """Get a value from ~/.gogeta/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Gogeta Configuration                    │", Colors.CYAN))
    ...
    from gogeta_cli.auth import get_anthropic_key
    ...
    # Warn on stale GOGETA_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        ...
        _env_ghost = load_env().get("GOGETA_MAX_ITERATIONS")

        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():

            print(color(

                f"                ⚠ .env has stale GOGETA_MAX_ITERATIONS={_env_ghost} "

                f"(run 'gogeta doctor --fix' to remove)",
    ...
    print(color("  gogeta config edit     # Edit config file", Colors.DIM))

    print(color("  gogeta config set <key> <value>", Colors.DIM))

    print(color("  gogeta setup           # Run setup wizard", Colors.DIM))
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            ...
            print("Usage: gogeta config set <key> <value>")

            print()

            print("Examples:")

            print("  gogeta config set model anthropic/claude-sonnet-4")

            print("  gogeta config set terminal.backend docker")

            print("  gogeta config set OPENROUTER_API_KEY sk-or-...")
        ...
        if missing_config:
            ...
            print("    Run 'gogeta config migrate' to add them")
        ...
        print("  gogeta config           Show current configuration")

        print("  gogeta config edit      Open config in editor")

        print("  gogeta config set <key> <value>   Set a config value")

        print("  gogeta config check     Check for missing/outdated config")

        print("  gogeta config migrate   Update config with new options")

        print("  gogeta config path      Show config file path")

        print("  gogeta config env-path  Show .env file path")
...
# ``_inject_profile_env_vars`` surfaces them in ``gogeta config`` UI so users
