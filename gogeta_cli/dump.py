...
Dump command for gogeta CLI.



Outputs a compact, plain-text summary of the user's Gogeta setup
...
from gogeta_cli.config import get_gogeta_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.gogeta_build_sha`` by the Dockerfile's

    ``GOGETA_GIT_SHA`` build-arg (see ``gogeta_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``gogeta dump`` formats empty values as blank, not as ``"(not set)"``).
...
def _gateway_status() -> str:
    ...
    try:
        ...
        from gogeta_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(gogeta_home: Path) -> int:
...
def _cron_summary(gogeta_home: Path) -> str:

    """Return cron jobs summary."""

    jobs_file = gogeta_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"gogeta_home:      {display_gogeta_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `gogeta auth add openrouter` lives in the

        # credential pool, not as an env var — surface it so the dump doesn't

        # misleadingly read "not set" while `gogeta auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["gogeta-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(gogeta_home)}")

    lines.append(f"  skills:             {_count_skills(gogeta_home)}")
