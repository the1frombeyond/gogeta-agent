...
from gogeta_cli.dashboard_auth.base import (
...
from gogeta_cli.dashboard_auth.login_page import render_login_html
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``GOGETA_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /gogeta`` (Mission Control deploys) — we
    ...
    from gogeta_cli.dashboard_auth.prefix import (
...
def _prefix(request: Request) -> str:
    ...
    ``gogeta_cli.dashboard_auth.prefix`` for the normalisation rules.
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("gogeta_session_pkce", "")
...
async def api_auth_ws_ticket(request: Request):
    ...
    from gogeta_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
