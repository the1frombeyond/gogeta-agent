...
Profile-aware location: ``$GOGETA_HOME/logs/dashboard-auth.log``.
...
from ``gogeta_constants`` or other gogeta_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$GOGETA_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``gogeta_constants.get_gogeta_home`` semantics: env var wins,
    else ``~/.gogeta``. A local copy avoids an import cycle with the
    middleware which lives below ``gogeta_cli``.
    """
    home = os.environ.get("GOGETA_HOME") or str(Path.home() / ".gogeta")
