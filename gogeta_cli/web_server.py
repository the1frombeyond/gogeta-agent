...
Usage:
    ...
    python -m gogeta_cli.main web          # Start on http://127.0.0.1:9119

    python -m gogeta_cli.main web --port 8080
...
try:
    ...
    # running `gogeta dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["GOGETA_WEB_DIST"]) if "GOGETA_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``gogeta gateway run`` — but the

    desktop app spawns a ``gogeta dashboard`` backend, not a gateway, so a cron
    ...
    GOGETA_HOME — whichever process grabs the lock first wins the tick.
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (GOGETA_DESKTOP=1) fire cron jobs themselves,

    # since the app has no gateway running the scheduler. Server `gogeta
...
app = FastAPI(title="Gogeta Agent", version=__version__, lifespan=_lifespan)
...
# GOGETA_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
_SESSION_TOKEN = os.environ.get("GOGETA_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)

_SESSION_HEADER_NAME = "X-Gogeta-Session-Token"
...
# This list is defined in ``gogeta_cli.dashboard_auth.public_paths`` so the
...
from gogeta_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    ...
    from gogeta_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        ...
        "description": (
            ...
            "When the chat app / gateway updates Gogeta (no terminal prompt), "
...
async def get_status():
    ...
    # and which providers are registered so ``gogeta status`` and the
    ...
    try:
        ...
        from gogeta_cli.dashboard_auth import list_providers as _list_providers
    ...
    return {
        ...
        "gogeta_home": str(get_gogeta_home()),
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the gogeta home root).
    ...
    try:
        ...
        try:
            ...
            du = psutil.disk_usage(str(get_gogeta_home()))
...
async def run_curator():
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["curator", "run"], "curator-run")
...
async def get_portal_status():
    ...
    try:
        ...
        from gogeta_cli.auth import get_nous_auth_status
    ...
    try:
        ...
        from gogeta_cli.nous_subscription import get_nous_subscription_features
    ...
    return {
        ...
        "subscription_url": "https://portal.gogeta.ai/manage-subscription",
...
async def run_prompt_size():
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["prompt-size"], "prompt-size")
...
async def run_dump():
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["dump"], "dump")
...
async def run_config_migrate():
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["config", "migrate"], "config-migrate")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    from gogeta_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.gogeta/logs/<action>.log`` so
...
_ACTION_LOG_DIR: Path = get_gogeta_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "gogeta-update": "gogeta-update.log",
...
def _spawn_gogeta_action(subcommand: List[str], name: str) -> subprocess.Popen:

    """Spawn ``gogeta <subcommand>`` detached and record the Popen handle.



    Uses the running interpreter's ``gogeta_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "gogeta_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "GOGETA_NONINTERACTIVE": "1"},
...
async def restart_gateway():
    ...
    """Kick off a ``gogeta gateway restart`` in the background."""

    try:

        proc = _spawn_gogeta_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/gogeta/update")

async def update_gogeta():

    """Kick off ``gogeta update`` in the background."""
    ...
    if install_method == "docker":
        ...
        _record_completed_action("gogeta-update", message, exit_code=1)
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["update"], "gogeta-update")

    except Exception as exc:

        _log.exception("Failed to spawn gogeta update")
...
@app.get("/api/gogeta/update/check")

async def check_gogeta_update(force: bool = False):

    """Report whether a Gogeta update is available, without applying it.
    ...
    ``POST /api/gogeta/update`` actually runs ``gogeta update``.
    ...
    Returns:
        ...
        current_version: installed Gogeta version string
    ...
    try:
        ...
        if force:
            ...
            try:
                ...
                (get_gogeta_home() / ".update_check").unlink()
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.gogeta/config.yaml`` under ``tts.``.
...
async def get_profiles_sessions(
    ...
    from gogeta_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Gogeta surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Gogeta supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in gogeta_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options():
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `gogeta model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `gogeta model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        ...
        try:
            ...
            from gogeta_cli.auth import get_provider_auth_state
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.gogeta/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if scope == "main":
            ...
            # post-model-selection behaviour (gogeta_cli/main.py
            ...
            if provider.strip().lower() == "nous":
                ...
                try:
                    ...
                    from gogeta_cli.nous_subscription import apply_nous_managed_defaults
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "telegram": {
        ...
        "description": "Run Gogeta from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        ...
        "description": "Connect Gogeta to Discord DMs, channels, and threads.",
    ...
    "slack": {
        ...
        "description": "Use Gogeta from Slack via Socket Mode.",
    ...
    "mattermost": {
        ...
        "description": "Connect Gogeta to Mattermost channels and direct messages.",
    ...
    "matrix": {
        ...
        "description": "Use Gogeta in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        ...
        "description": "Use Gogeta through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        ...
        "description": "Control your smart home from Gogeta via Home Assistant.",
    ...
    "email": {
        ...
        "description": "Talk to Gogeta through an IMAP/SMTP mailbox.",

        "docs_url": "https://gogeta.ai/docs/user-guide/messaging/",
    ...
    "dingtalk": {
        ...
        "description": "Connect Gogeta to DingTalk groups (钉钉).",
    ...
    "feishu": {
        ...
        "description": "Use Gogeta inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Gogeta through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        ...
        "description": "Connect Gogeta to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        ...
        "description": "Connect Gogeta to Tencent Yuanbao.",
    ...
    "api_server": {
        ...
        "description": "Expose Gogeta as an OpenAI-compatible HTTP API for tools like Open WebUI.",
    ...
    "webhook": {
        ...
        "docs_url": "https://gogeta.ai/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.gogeta.ai"

_TELEGRAM_ONBOARDING_USER_AGENT = f"GogetaDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    ...
    bot_name = (body.bot_name or "Gogeta Agent").strip() or "Gogeta Agent"
...
# the canonical ``gogeta auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Gogeta resolves Anthropic creds in this order at runtime:

    1. ``~/.gogeta/.anthropic_oauth.json`` — Gogeta-managed PKCE flow
    ...
    try:
        ...
        read_gogeta_oauth_credentials = None  # type: ignore

        _GOGETA_OAUTH_FILE = None  # type: ignore



    gogeta_creds = None

    if read_gogeta_oauth_credentials:

        try:

            gogeta_creds = read_gogeta_oauth_credentials()

        except Exception:

            gogeta_creds = None

    if gogeta_creds and gogeta_creds.get("accessToken"):
        ...
        return {
            ...
            "source_label": f"Gogeta PKCE ({_GOGETA_OAUTH_FILE})",

            "token_preview": _truncate_token(gogeta_creds.get("accessToken")),

            "expires_at": gogeta_creds.get("expiresAt"),

            "has_refresh_token": bool(gogeta_creds.get("refreshToken")),
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Gogeta even

    when they also have a separate Gogeta-managed PKCE login.
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (

    {
        ...
        "cli_command": "gogeta auth add nous",

        "docs_url": "https://portal.gogeta.ai",
...

    {
        ...
        "cli_command": "gogeta auth add openai-codex",
...

    {
        ...
        "cli_command": "gogeta auth add qwen-oauth",
...

    {
        ...
        "cli_command": "gogeta auth add minimax-oauth",
...

    {
        ...
        "cli_command": "gogeta auth add xai-oauth",

        "docs_url": "https://gogeta.ai/docs/guides/xai-grok-oauth",
...

    {
        ...
        "cli_command": "gogeta auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        ...
        from gogeta_cli import auth as hauth
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("gogeta_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Gogeta-managed PKCE file
    ...
    if provider_id in {"anthropic", "claude-code"}:
        ...
        try:
            ...
            from agent.anthropic_adapter import _GOGETA_OAUTH_FILE

            if _GOGETA_OAUTH_FILE.exists():

                _GOGETA_OAUTH_FILE.unlink()
        ...
        try:
            ...
            from gogeta_cli.auth import clear_provider_auth
...
#          → persists to ~/.gogeta/.anthropic_oauth.json AND credential pool
...
# Guarded so gogeta web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    """Persist Anthropic PKCE creds to both Gogeta file AND credential pool.



    Mirrors what auth_commands.add_command does so the dashboard flow leaves

    the system in the same state as ``gogeta auth add anthropic``.
    ...
    _GOGETA_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)

    tmp_path = _GOGETA_OAUTH_FILE.with_name(

        f"{_GOGETA_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    ...
    try:
        ...
        os.replace(tmp_path, _GOGETA_OAUTH_FILE)

        try:

            _GOGETA_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "gogeta-dashboard/1.0",
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            ...
            os.getenv("GOGETA_PORTAL_BASE_URL")
...
# the tokens exactly like `gogeta auth add xai-oauth`.
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            ...
            os.getenv("GOGETA_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `gogeta auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from gogeta_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from gogeta_cli.auth import _save_codex_tokens
...
async def get_session_stats():
    ...
    """Session-store statistics for the Sessions page (mirrors `gogeta sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    ...
    """Delete ended sessions older than N days (mirrors `gogeta sessions prune`)."""
...
async def get_logs(
    ...
    from gogeta_cli.logs import _read_tail, LOG_FILES
    ...
    log_path = get_gogeta_home() / "logs" / log_name
    ...
    try:
        ...
        from gogeta_logging import COMPONENT_PREFIXES
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    ...
    """Resolve a profile query value to (profile_name, GOGETA_HOME)."""
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["gogeta_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process GOGETA_HOME at import time. The dashboard is a single
...
# Wraps the same config data layer the CLI uses (gogeta_cli.mcp_config), so

# servers managed here show up under `gogeta mcp list` and vice versa.  Secrets
...
async def list_mcp_servers():
    ...
    from gogeta_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate):
    ...
    from gogeta_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str):
    ...
    from gogeta_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from gogeta_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog():
    ...
    `gogeta mcp catalog` / `gogeta mcp install` read.

    """

    try:

        from gogeta_cli import mcp_catalog
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall):
    ...
    if entry.install is not None:
        ...
        try:
            ...
            proc = _spawn_gogeta_action(["mcp", "install", name], "mcp-install")
...
# Wraps the same JSON store the CLI uses (gogeta_cli.webhook); the webhook
...
async def list_webhooks():
    ...
    import gogeta_cli.webhook as wh
...
# spawn the real `gogeta gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway():
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["gateway", "start"], "gateway-start")
...
async def stop_gateway():
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["gateway", "stop"], "gateway-stop")
...
async def list_credential_pool():
    ...
    from gogeta_cli.auth import read_credential_pool
...
# `gogeta memory setup`).  The dashboard covers the common admin actions:
    ...
    if provider:
        ...
        if provider not in valid:
            ...
            raise HTTPException(
                ...
                detail=f"Unknown memory provider '{provider}'. Run `gogeta memory setup` to configure a new one.",
...
async def run_doctor():
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["doctor"], "doctor")
...
async def run_security_audit():
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["security", "audit"], "security-audit")
...
async def run_backup(body: BackupRequest):
    ...
    try:
        ...
        proc = _spawn_gogeta_action(args, "backup")
...
async def run_import(body: ImportRequest):
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["import", archive], "import")
...
async def list_hooks():
    ...
    from gogeta_cli.config import load_config as _load_config
    ...
    try:
        ...
        from gogeta_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <gogeta_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_gogeta_home() / "checkpoints"
...
async def prune_checkpoints():
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["checkpoints", "prune"], "checkpoints-prune")
...
async def install_skill_hub(body: SkillInstallRequest):
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["skills", "install", identifier], "skills-install")
...
async def uninstall_skill_hub(body: SkillUninstallRequest):
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["skills", "uninstall", name, "--yes"], "skills-uninstall")
...
async def update_skills_hub():
    ...
    try:
        ...
        proc = _spawn_gogeta_action(["skills", "update"], "skills-update")
...
# Human-readable labels for each hub source id (matches `gogeta skills search`
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        ...
        from gogeta_cli.skills_hub import _resolve_source_meta_and_bundle
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_gogeta_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "gogeta setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local GOGETA_HOME override so the write lands in the target
    ...
    from gogeta_constants import set_gogeta_home_override, reset_gogeta_home_override



    token = set_gogeta_home_override(str(profile_dir))
    ...
    try:
        ...
        reset_gogeta_home_override(token)
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``gogeta profile use`` —

    the profile new CLI invocations pick up. ``current`` is the profile

    the running dashboard/gateway is scoped to (derived from GOGETA_HOME).
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    ...
    """Set the sticky active profile (mirrors ``gogeta profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the GOGETA_HOME override.
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``gogeta profile describe
    ...
    try:
        ...
        from gogeta_cli import profile_describer
...
async def get_skills():
    ...
    from gogeta_cli.skills_config import get_disabled_skills
...
async def toggle_skill(body: SkillToggle):
    ...
    from gogeta_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def get_toolsets():
    ...
    from gogeta_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle):
    ...
    helper the CLI ``gogeta tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``gogeta tools`` picker shows
    ...
    from gogeta_cli.config import get_env_value
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``gogeta tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate):
    ...
    Writes each ``key: value`` to ``~/.gogeta/.env`` via ``save_env_value`` —

    the same store ``gogeta tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(name: str, body: ToolsetPostSetup):
    ...
    ``gogeta tools post-setup <key>`` and the frontend tails the log via
...
# The endpoint spawns the same ``gogeta --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    ...
    try:
        ...
        from gogeta_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        ...
        from gogeta_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from gogeta_cli.dashboard_auth.audit import AuditEvent, audit_log

        from gogeta_cli.dashboard_auth.ws_tickets import (
...
def _resolve_chat_argv(
    ...
    Default: whatever ``gogeta --tui`` would run.  Tests monkeypatch this
    ...
    Session resume is propagated via the ``GOGETA_TUI_RESUME`` env var —

    matching what ``gogeta_cli.main._launch_tui`` does for the CLI path.
    ...
    ``GOGETA_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    ...
    `sidecar_url` (when set) is forwarded as ``GOGETA_TUI_SIDECAR_URL`` so
    ...
    from gogeta_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("GOGETA_TUI_DISABLE_MOUSE", "1")

    env.setdefault("GOGETA_TUI_INLINE", "1")
    ...
    if resume:
        ...
        env["GOGETA_TUI_RESUME"] = resume



    if sidecar_url:

        env["GOGETA_TUI_SIDECAR_URL"] = sidecar_url



    if gateway_ws_url := _build_gateway_ws_url():

        env["GOGETA_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        ...
        from gogeta_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        ...
        await ws.send_text(
            ...
            "\x1b[33mInstall Gogeta inside WSL2 to use the dashboard's /chat "
...
# GOGETA_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`gogeta_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    from gogeta_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/gogeta/*`` -> local Caddy -> :9119), the

    proxy injects ``X-Forwarded-Prefix: /gogeta`` on every request. We

    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)

    and the SPA's runtime ``__GOGETA_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/gogeta``)
        ...
        ``__GOGETA_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            ...
            bootstrap_script = (
                ...
                f"window.__GOGETA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"

                f'window.__GOGETA_BASE_PATH__="{prefix}";'

                f"window.__GOGETA_AUTH_REQUIRED__={gated_js};"
            ...
            bootstrap_script = (
                ...
                f'<script>window.__GOGETA_SESSION_TOKEN__="{_SESSION_TOKEN}";'
    ...
    # under ``/gogeta`` they'd hit ``mission-control.tilos.com/fonts/...``

    # (the MC Pages app), not the Gogeta backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    ...
    {"name": "default",       "label": "Gogeta Teal",         "description": "Classic dark teal — the canonical Gogeta look"},

    {"name": "default-large", "label": "Gogeta Teal (Large)", "description": "Gogeta Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.gogeta/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    ...
    """Scan ~/.gogeta/dashboard-themes/*.yaml for user-created themes.
    ...
    themes_dir = get_gogeta_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.gogeta/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as gogeta_cli.plugins):

    1. User plugins:    ~/.gogeta/plugins/<name>/dashboard/manifest.json

    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)

    3. Project plugins: ./.gogeta/plugins/  (only if GOGETA_ENABLE_PROJECT_PLUGINS)
    ...
    from gogeta_cli.plugins import get_bundled_plugins_dir

    bundled_root = get_bundled_plugins_dir()

    search_dirs = [

        (get_gogeta_home() / "plugins", "user"),
    ...
    # ``gogeta_cli/plugins.py`` and the documented user contract.

    if env_var_enabled("GOGETA_ENABLE_PROJECT_PLUGINS"):

        search_dirs.append((Path.cwd() / ".gogeta" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from gogeta_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_gogeta_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        ...
                        auth_command = f"gogeta auth {name}"
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    ...
    from gogeta_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from gogeta_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    ...
    from gogeta_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    ...
    from gogeta_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.gogeta/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            ...
            _log.warning(
                ...
                "~/.gogeta/plugins/ if you trust it)",
        ...
        try:
            ...
            module_name = f"gogeta_dashboard_plugin_{plugin['name']}"
...
from gogeta_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from gogeta_cli.dashboard_auth import list_providers

        if not list_providers():

            # Surface the *specific* reason any bundled provider declined

            # to register (e.g. missing GOGETA_DASHBOARD_OAUTH_CLIENT_ID).

            # Each provider plugin that ships with Gogeta Agent exposes a
    ...
    print(f"  Gogeta Web UI → http://{host}:{port}")
