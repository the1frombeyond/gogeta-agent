...
Skills Hub CLI — Unified interface for the Gogeta Skills Hub.



Powers both:

  - `gogeta skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `gogeta skills install`.
...
def _existing_categories() -> List[str]:
    ...
    """Return sorted subdirectory names under ``~/.gogeta/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            ...
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.gogeta/skills/<name>/)[/]"
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    # into `gogeta skills install` fail.
    ...
    c.print("[dim]Use: gogeta skills inspect <identifier> to preview, "

            "gogeta skills install <identifier> to install "
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # skips the external API sources and serves everything from "gogeta-index".
    ...
    # The identifier is what you pass to `gogeta skills install`. Browse used
    ...
    c.print("[dim]Tip: 'gogeta skills inspect <identifier>' to preview, "

            "'gogeta skills install <identifier>' to install, "

            "'gogeta skills search <query>' to search deeper[/]\n")
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        ...
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]gogeta skills install {url} --name <your-name>[/]\n\n"
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            ...
            c.print(Panel(
                ...
                "It ships with gogeta-agent but is not activated by default.\n"

                "Installing will copy it to your skills directory where the agent can use it.\n\n"

                f"Files will be at: [cyan]{display_gogeta_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="gogeta skills install <id> to install"))
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    # "gogeta-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"gogeta-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying gogeta-index AND the external APIs at once
...
def do_list(source_filter: str = "all",
    ...
    config — ``gogeta -p <profile> skills list`` reads that profile's

    ``skills.disabled`` list because ``-p`` swaps ``GOGETA_HOME`` at process
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            ...
            c.print("[bold red]Error:[/] Repo required. Usage: gogeta skills tap add owner/repo\n")
        ...
        if not repo:
            ...
            c.print("[bold red]Error:[/] Repo required. Usage: gogeta skills tap remove owner/repo\n")
    ...
    if target == "github":
        ...
        if not repo:
            ...
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    ...
                    "Usage: gogeta skills publish <path> --to github --repo owner/repo\n")
        ...
        if not auth.is_authenticated():
            ...
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    ...
                    f"Set GITHUB_TOKEN in {display_gogeta_home()}/.env or run 'gh auth login'.\n")
    ...
    try:
        ...
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Gogeta Skills Hub.\n\n"

                        f"This skill was scanned by the Gogeta Skills Guard before submission.",
...
def skills_command(args) -> None:
    ...
    """Router for `gogeta skills <subcommand>` — called from gogeta_cli/main.py."""
    ...
    if action == "browse":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: gogeta skills snapshot [export|import]\n")
        ...
        if not tap_action:
            ...
            _console.print("Usage: gogeta skills tap [list|add|remove]\n")
        ...
        _console.print("Usage: gogeta skills [browse|search|install|inspect|list|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")

        _console.print("Run 'gogeta skills <command> --help' for details.\n")
