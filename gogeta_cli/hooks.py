"""gogeta hooks — inspect and manage shell-script hooks.

Usage::

    gogeta hooks list
    gogeta hooks test <event> [--for-tool X] [--payload-file F]
    gogeta hooks revoke <command>
    gogeta hooks doctor

Consent records live under ``~/.gogeta/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.gogeta/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``gogeta hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: gogeta hooks {list|test|revoke|doctor}")
        print("Run 'gogeta hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.gogeta/config.yaml.")
        print("See `gogeta hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `gogeta hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `gogeta hooks test` and `gogeta hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Gogeta wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `gogeta hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `gogeta hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `gogeta hooks doctor`.")
