"""``gogeta debug`` debug tools for Gogeta Agent.



Currently supports:

    gogeta debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.gogeta/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    ...
    "[gogeta debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    ...
    """Path to ``~/.gogeta/pastes/pending.json``.
    ...
    ran ``gogeta debug share`` repeatedly.
    ...
    ``_sweep_expired_pastes`` once per hour.  ``gogeta debug share`` also
    ...
    return get_gogeta_home() / "pastes" / "pending.json"
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``gogeta debug delete``
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``gogeta debug`` invocation with
...
⚠️  This will upload the following to a public paste service:
  ...
  • System info (OS, Python version, Gogeta version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `gogeta debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "gogeta-agent/debug-share"},
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``gogeta debug share`` invocation added ~20 MB of resident Python

    interpreters that never exited until the sleep completed.



    The replacement is stateless: we append to ``~/.gogeta/pastes/pending.json``

    and the gateway's cron ticker sweeps expired entries once per hour.

    ``gogeta debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "gogeta-agent/debug-share",
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    boundary = "----GogetaDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from gogeta_cli.logs import LOG_FILES



    filename = LOG_FILES.get(log_name)

    return (get_gogeta_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    ...
    """Run ``gogeta dump`` and return its stdout as a string."""
...
def collect_debug_report(
    ...
    *,
    ...
    dump_text
        ...
        Pre-captured dump output.  If empty, ``gogeta dump`` is run
...
def build_debug_share(
    ...
    *,
    ...
    This is the shared core behind ``gogeta debug share`` (CLI) and the
    ...
    if redact:
        ...
        logger.info(
            ...
            "gogeta debug share: applied force-mode redaction to log snapshots before upload"
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `gogeta debug share --local` to print the report instead.\n")
    ...
    print(f"To delete now:  gogeta debug delete <url>")



    print(f"\nShare these links with the Gogeta team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        ...
        print("Usage: gogeta debug delete <url> [<url> ...]")

        print("  Deletes paste.rs pastes uploaded by 'gogeta debug share'.")
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``gogeta debug`` call.
    ...
    # best-effort — any failure is swallowed so ``gogeta debug`` stays
    ...
    if subcmd == "share":
        ...
        print("Usage: gogeta debug <command>")
