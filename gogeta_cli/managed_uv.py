...
Gogeta owns its own uv binary at ``$GOGETA_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$GOGETA_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where Gogeta keeps *its* uv binary.

    ``$GOGETA_HOME/bin/uv`` on POSIX, ``$GOGETA_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``gogeta update`` runs
    the call site from the *old*, already-imported ``gogeta_cli.main`` against
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``gogeta update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$GOGETA_HOME/bin/`` instead of ``~/.local/bin/``.
    """
