"""``gogeta debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Gogeta Agent. Use 'gogeta debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    gogeta debug share              Upload debug report and print URL
    gogeta debug share --lines 500  Include more log lines
    gogeta debug share --expire 30  Keep paste for 30 days
    gogeta debug share --local      Print report locally (no upload)
    gogeta debug share --no-redact  Disable upload-time secret redaction
    gogeta debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'gogeta debug share'",
    )
