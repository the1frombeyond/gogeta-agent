"""``gogeta dashboard`` subcommand parser.

Extracted verbatim from ``gogeta_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Gogeta Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `gogeta dashboard` cmdlines and SIGTERM them directly — the same
    # path `gogeta update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running gogeta dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running gogeta dashboard processes and exit",
    )
    # Backward-compat shim: older Gogeta desktop app shells (<= 0.15.x) spawn the
    # backend as `gogeta dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Gogeta couldn't start" with no actionable cause. Accept and silently
    ...
    # `gogeta dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.gogeta/.env.
    # Nested subparser so bare `gogeta dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "GOGETA_DASHBOARD_OAUTH_CLIENT_ID into ~/.gogeta/.env, and prints "
            "how to engage the login gate. Requires being logged in (gogeta setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://gogeta.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via GOGETA_DASHBOARD_PORTAL_URL. Mainly for "
