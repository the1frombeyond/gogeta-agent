"""``gogeta mcp`` subcommand parser.

Extracted from ``gogeta_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
from gogeta_cli.subcommands._shared import add_accept_hooks_flag
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Gogeta as an MCP server",
        description=(
            "Manage MCP server connections and run Gogeta as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'gogeta mcp add' to connect to a new server, or\n"
            "'gogeta mcp serve' to expose Gogeta conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Gogeta as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `gogeta mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `gogeta mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `gogeta mcp install n8n`)",
    )
