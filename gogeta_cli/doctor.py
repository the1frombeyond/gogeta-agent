...
Doctor command for gogeta CLI.



Diagnoses issues with Gogeta Agent setup.
...
from gogeta_cli.config import get_project_root, get_gogeta_home, get_env_path
...
_DHH = display_gogeta_home()  # user-facing display path (e.g. ~/.gogeta or ~/.gogeta/profiles/coder)



# Load environment variables from ~/.gogeta/.env so API key checks work

_env_path = get_env_path()

load_gogeta_dotenv(gogeta_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")



from gogeta_cli.colors import Colors, color

from gogeta_cli.models import _GOGETA_USER_AGENT

from gogeta_constants import OPENROUTER_MODELS_URL
...
from gogeta_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    ...
    """Return True when ~/.gogeta/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("GOGETA_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        ...
        try:
            ...
            from gogeta_cli.auth import get_gemini_oauth_auth_status
    ...
    if normalized == "minimax":
        ...
        try:
            ...
            from gogeta_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    """Verify pyproject.toml version matches gogeta_cli.__version__.



    A git conflict resolution (reset/merge) can revert one file without the

    other, leaving ``gogeta --version`` reporting a stale version while
    ...
    try:
        ...
        from gogeta_cli import __version__ as init_version
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != gogeta_cli/__init__.py {init_version})",

            "Re-sync version files (e.g. run 'gogeta update', or set "

            "gogeta_cli/__init__.py __version__ to match pyproject.toml)",
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      ...
      - Whether the main-gogeta and dashboard static services are up
    ...
    for static in ("main-gogeta", "dashboard"):
    ...
    if not profiles:
        ...
        check_info("No per-profile gateways registered yet — create one with `gogeta profile create <name>`")
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            ...
            from gogeta_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `gogeta`.

    os.environ.setdefault("GOGETA_INTERACTIVE", "1")



    # Handle `gogeta doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.gogeta/config.yaml is writable.",
    ...
    print(color("│                 🩺 Gogeta Doctor                        │", Colors.CYAN))
        ...
        if fresh_hits:
            ...
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`gogeta doctor --ack {hit.advisory.id}`."
    ...
    # Detect drift between pyproject.toml and gogeta_cli/__init__.py versions
    ...
    # Check ~/.gogeta/.env (primary location for user config)

    env_path = GOGETA_HOME / '.env'
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'gogeta setup' to configure API keys")
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'gogeta setup' to configure API keys")

                fixed_count += 1

            else:

                check_info("Run 'gogeta setup' to create one")

                issues.append("Run 'gogeta setup' to create .env")

    

    # Check ~/.gogeta/config.yaml (primary) or project cli-config.yaml (fallback)

    config_path = GOGETA_HOME / 'config.yaml'
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                from gogeta_cli.config import get_compatible_custom_providers as _compatible_custom_providers

                from gogeta_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'gogeta config set model.provider <valid_provider>'"
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                ...
                try:
                    ...
                    if runtime_provider == "openrouter":
                        ...
                        from gogeta_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        ...
                        _fail_and_issue(
                            ...
                            "(check ~/.gogeta/.env or run 'gogeta setup')",

                            (

                                f"No credentials found for provider '{runtime_provider}'. "

                                f"Run 'gogeta setup' or set the provider's API key in {_DHH}/.env, "

                                f"or switch providers with 'gogeta config set model.provider <name>'"
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from gogeta_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        ...
        try:
            ...
            from gogeta_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    ...
                    try:
                        ...
                        issues.append("Run 'gogeta setup' to migrate config")

                else:

                    issues.append("Run 'gogeta doctor --fix' or 'gogeta setup' to migrate config")
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'gogeta doctor --fix'")
        ...
        # Detect stale GOGETA_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives GOGETA_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            ...
            from gogeta_cli.config import load_env, remove_env_value
            ...
            env_ghost = load_env().get("GOGETA_MAX_ITERATIONS")
            ...
            if drift:
                ...
                check_warn(
                    ...
                    f"GOGETA_MAX_ITERATIONS={env_ghost} in .env shadows "

                    f"agent.max_turns={cfg_max_turns} in config.yaml",

                    "(stale ghost from an earlier `gogeta setup` run)",

                )

                if should_fix:

                    if remove_env_value("GOGETA_MAX_ITERATIONS"):

                        check_ok(

                            "Removed stale GOGETA_MAX_ITERATIONS from .env "
                        ...
                        check_warn("Could not remove GOGETA_MAX_ITERATIONS from .env")

                        manual_issues.append(

                            "Manually delete the GOGETA_MAX_ITERATIONS line from "
                    ...
                    issues.append(
                        ...
                        "Stale GOGETA_MAX_ITERATIONS in .env shadows config.yaml — "

                        "run 'gogeta doctor --fix'"
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Gogeta' own device-code flow — the Codex CLI is
    ...
    if gogeta_home.exists():
        ...
        gogeta_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        ...
        subdir_path = gogeta_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Gogeta a custom personality)")
        ...
        if should_fix:
            ...
            soul_path.write_text(
                ...
                "<!-- Edit this file to customize how Gogeta communicates. -->\n\n"

                "You are Gogeta, a helpful AI assistant.\n",
    ...
    memories_dir = gogeta_home / "memories"
    ...
    state_db_path = gogeta_home / "state.db"
    ...
    if state_db_path.exists():
        ...
        try:
            ...
            from gogeta_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        ...
                        "state.db schema malformed — run 'gogeta doctor --fix' "

                        "(or 'gogeta sessions repair') to recover hidden sessions"
    ...
    wal_path = gogeta_home / "state.db-wal"
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'gogeta doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            ...
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "gogeta"
        ...
        _cmd_link = _cmd_link_dir / "gogeta"
        ...
        if _venv_bin is None:
            ...
            check_warn(
                ...
                "(gogeta not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    ...
                    check_ok(f"{_cmd_link_display}/gogeta → correct target")

                else:

                    check_warn(

                        f"{_cmd_link_display}/gogeta points to wrong target",
                    ...
                    if should_fix:
                        ...
                        check_ok(f"Fixed symlink: {_cmd_link_display}/gogeta → {_venv_bin}")

                        fixed_count += 1

                    else:

                        issues.append(f"Broken symlink at {_cmd_link_display}/gogeta — run 'gogeta doctor --fix'")

            elif _cmd_link.exists():

                # It's a regular file, not a symlink — possibly a wrapper script

                check_ok(f"{_cmd_link_display}/gogeta exists (non-symlink)")

            else:

                check_fail(

                    f"{_cmd_link_display}/gogeta not found",

                    "(gogeta command may not work outside the venv)"
                ...
                if should_fix:
                    ...
                    check_ok(f"Created symlink: {_cmd_link_display}/gogeta → {_venv_bin}")
                    ...
                    issues.append(f"Missing {_cmd_link_display}/gogeta symlink — run 'gogeta doctor --fix'")
    ...
    try:
        ...
        from gogeta_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            ...
            try:
                ...
                # to eagerly load in every `gogeta doctor` invocation.
            ...
            if r.status_code == 402:
                ...
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     ...
                     "Fix: run 'gogeta config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``gogeta doctor``.
    ...
    # never the legitimate source for `gogeta doctor`.
    ...
    try:
        ...
        if api_disabled:
            ...
            issues.append("Run 'gogeta setup' to configure missing API keys for full tool access")
    ...
    hub_dir = GOGETA_HOME / "skills" / ".hub"
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: gogeta skills list)")
    ...
    try:
        ...
        _mem_cfg_path = GOGETA_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: gogeta memory setup")
                ...
                _fail_and_issue(
                    ...
                    "run: gogeta memory setup",

                    "No Honcho API key — run 'gogeta memory setup'",
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run gogeta memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: gogeta memory status")

            else:

                check_warn(f"{_active_memory_provider} plugin not found", "run: gogeta memory setup")
    ...
    try:
        ...
        from gogeta_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
            ...
            if wrapper_dir.is_dir():
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "gogeta -p" in content:

                            _m = _re.search(r"gogeta -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        if not should_fix:
            ...
            print(color("  Tip: run 'gogeta doctor --fix' to auto-fix what's possible.", Colors.DIM))
