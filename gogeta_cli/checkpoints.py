"""`gogeta checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.gogeta/checkpoints/``.  Actions:

    gogeta checkpoints               # same as `status`
    gogeta checkpoints status        # total size, project count, breakdown
    gogeta checkpoints list          # per-project checkpoint counts + workdir
    gogeta checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    gogeta checkpoints clear [-f]    # nuke the entire base (asks first)
    gogeta checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    gogeta checkpoints
    gogeta checkpoints prune --retention-days 3 --max-size-mb 200
    gogeta checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: gogeta checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``gogeta checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `gogeta checkpoints` → status
