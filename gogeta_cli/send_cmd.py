"""CLI subcommand: ``gogeta send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"gogeta send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            ...
            print(f"gogeta send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"gogeta send: failed to load channel directory: {exc}", file=sys.stderr)
    ...
    try:
        ...
        print(f"gogeta send: failed to read channel directory: {exc}", file=sys.stderr)
    ...
    if platform_filter:
        ...
        if not filtered:
            ...
            print(
                ...
                f"gogeta send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `gogeta gateway setup`, or run the gateway once so")

        print("channel discovery can populate ~/.gogeta/channel_directory.json.")
...
def _load_gogeta_env() -> None:

    """Populate ``os.environ`` from ``~/.gogeta/.env`` AND bridge top-level
    ...
    startup that ``gogeta send`` must replicate when invoked standalone:



    1. ``load_dotenv(~/.gogeta/.env)`` — brings bot tokens into the env.

    2. Bridge top-level simple values from ``~/.gogeta/config.yaml`` into
       ...
       via ``gogeta config set``.



    See ``gateway/run.py`` for the canonical version of this bridge — we

    intentionally reimplement the minimum needed here so ``gogeta send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.gogeta/.env and ~/.gogeta/config.yaml into os.environ so the
    ...
    if not target:
        ...
        print(
            ...
            "gogeta send: --to PLATFORM[:channel[:thread]] is required\n"

            "Examples:\n"

            "  gogeta send --to telegram \"hello\"\n"

            "  gogeta send --to discord:#ops --file report.md\n"

            "  gogeta send --list      # list available targets",
    ...
    if message is None or not message.strip():
        ...
        print(
            ...
            "gogeta send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `gogeta send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Pipe text from any shell script to any messaging platform Gogeta "

            "is already configured for. Reuses the gateway's platform "

            "credentials (~/.gogeta/.env + ~/.gogeta/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  gogeta send --to telegram \"deploy finished\"\n"

            "  echo \"RAM 92%\" | gogeta send --to telegram:-1001234567890\n"

            "  gogeta send --to discord:#ops --file /tmp/report.md\n"

            "  gogeta send --to slack:#eng --subject \"[CI]\" --file build.log\n"

            "  gogeta send --list                  # all platforms\n"

            "  gogeta send --list telegram         # filter by platform\n"
    ...
    parser.add_argument(
        ...
        help="List available targets. Optional positional filter: `gogeta send --list telegram`.",
