...
Add, remove, or reorder entries here — both `gogeta setup` and

`gogeta` provider-selection will pick up the change automatically.
...
from gogeta_cli import __version__ as _GOGETA_VERSION
...
_GOGETA_USER_AGENT = f"gogeta-cli/{_GOGETA_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `gogeta model`

    flow without maintaining a separate static list.

    """

    from gogeta_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $GOGETA_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $GOGETA_HOME/models_dev_cache.json directly (no network) so this

    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``

    when the cache is empty or unreadable. Gogeta refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Gogeta that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Gogeta that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_gogeta_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$GOGETA_HOME/cache/nous_recommended_cache.json``) as last-known-good.
    ...
    base = (portal_base_url or "https://portal.gogeta.ai").rstrip("/")
...
# this list:  gogeta model, /model, list_authenticated_providers.
...
#   tui_desc    — longer description for the `gogeta model` interactive picker
...
class ProviderEntry(NamedTuple):
    ...
    tui_desc: str   # detailed description for `gogeta model` TUI
...
# Some vendors expose several Gogeta provider slugs (one per endpoint /

# auth method: global API, China API, OAuth coding plan, ...). Listing every

# slug as a top-level row in the interactive `gogeta model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``gogeta model``, the
...
# path. The *interactive* default (GUI onboarding / ``gogeta model``) uses the

# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``

# in gogeta_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``gogeta auth add openai-codex`` without

    ``gogeta model``, or a profile that sets ``provider`` with no ``model``).



    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the

    same model the ``gogeta model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    gogeta-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        ...
        from gogeta_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        # Hide models that don't advertise tool-calling support — gogeta-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    try:
        ...
        from gogeta_cli.model_catalog import get_curated_nous_models
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.gogeta.ai"
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    try:
        ...
        from gogeta_cli.auth import resolve_nous_runtime_credentials
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            ...
            # Nous base_url typically looks like https://inference-api.gogeta.ai/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``gogeta model``, ``/model``, etc.).
    ...
    for pid in provider_order:
        ...
        try:
            ...
            from gogeta_cli.auth import get_auth_status, has_usable_secret
...
def _resolve_static_model_alias(
    ...
    try:
        ...
        from gogeta_cli.model_switch import MODEL_ALIASES
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    """Normalize provider aliases to Gogeta' canonical provider ids.



    Note: ``"auto"`` passes through unchanged — use

    ``gogeta_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    ...
    """Return whether Gogeta should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``gogeta auth add copilot`` and by ``_seed_from_env``

         when the env var is set in ``~/.gogeta/.env``.
    ...
    try:
        ...
        from gogeta_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from gogeta_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Gogeta release.
    ...
    if normalized == "openai-codex":
        ...
        # a Gogeta release). Falls back to the hardcoded catalog if no token
        ...
        try:
            ...
            from gogeta_cli.auth import resolve_codex_runtime_credentials
    ...
    if normalized == "nous":
        ...
        try:
            ...
            from gogeta_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
        ...
        # added Portal models still surface without a Gogeta release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``gogeta model`` shows.
...
#   - One JSON file at $GOGETA_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_gogeta_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$GOGETA_HOME/auth.json`` and external
...
def copilot_default_headers() -> dict[str, str]:
    ...
    try:
        ...
        from gogeta_cli.copilot_auth import copilot_request_headers
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _GOGETA_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Gogeta' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    headers: dict[str, str] = {"User-Agent": _GOGETA_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_gogeta_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Gogeta will still save `{requested}`, but the endpoint should expose `/models` for verification."
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    "\n  MiniMax does not expose a /models endpoint, so Gogeta cannot verify the model name."
