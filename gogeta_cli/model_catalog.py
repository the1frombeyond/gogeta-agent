...
The Gogeta docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.gogeta/cache/model_catalog.json``.
...
DEFAULT_CATALOG_URL = (
    ...
    "https://gogeta.ai/docs/api/model-catalog.json"
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    ...
    "https://raw.githubusercontent.com/gogeta/gogeta-agent/main/website/static/api/model-catalog.json",
...
def _cache_path() -> Path:
    ...
    return get_gogeta_home() / "cache" / "model_catalog.json"
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``gogeta update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.gogeta/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    ...
    """Clear the in-process cache. Used by tests and ``gogeta model --refresh``."""
