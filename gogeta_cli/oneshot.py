...
configured for "cli" in `gogeta tools`.

Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.

Approvals = auto-bypassed (GOGETA_YOLO_MODE=1 is set for the call).

Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).



Model / provider selection mirrors `gogeta chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"gogeta -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            ...
            sys.stderr.write(
                ...
                "gogeta -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        ...
        sys.stderr.write(f"gogeta -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")

    if disabled:

        sys.stderr.write(

            "gogeta -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
    ...
    if not valid:
        ...
        return None, "gogeta -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to GOGETA_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("GOGETA_INFERENCE_MODEL", "").strip()

    if provider and not ((model or "").strip() or env_model_early):

        sys.stderr.write(

            "gogeta -z: --provider requires --model (or GOGETA_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"gogeta -z: agent failed: {failure}\n")
    ...
    if not (response or "").strip():
        ...
        real_stderr.write("gogeta -z: no final response was produced; treating the run as failed.\n")
...
def _create_session_db_for_oneshot():
    ...
    """Best-effort SessionDB for ``gogeta -z`` / oneshot mode.



    Oneshot bypasses ``GogetaCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when gogeta is invoked for

    # other commands (keeps top-level CLI startup cheap).

    from gogeta_cli.config import load_config

    from gogeta_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("GOGETA_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                ...
                from gogeta_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    ...
                    or os.getenv("GOGETA_INFERENCE_PROVIDER", "").strip().lower()
    ...
    agent = AIAgent(
        ...
        #                GOGETA_INTERACTIVE which we never set

        #   - shell-hook approval → auto-approved via GOGETA_ACCEPT_HOOKS=1

        #                (set above); also falls back to deny on non-tty

        #   - dangerous-command approval → bypassed via GOGETA_YOLO_MODE=1
