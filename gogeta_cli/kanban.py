"""CLI for the Gogeta Kanban board — ``gogeta kanban …`` subcommand.



Exposes the full Kanban command surface documented in the design spec

(``docs/gogeta-kanban-v1-spec.pdf``).  All DB work is delegated to
...
from gogeta_cli import kanban_db as kb

from gogeta_cli import kanban_swarm as ks

from gogeta_cli.profiles import get_active_profile_name, get_profile_dir, seed_profile_skills
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this GOGETA_HOME and its
    ...
    Used by ``gogeta kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        ...
        return (
            ...
            "standalone daemon (`gogeta kanban daemon --force`)."
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            ...
            "Durable SQLite-backed task board shared across Gogeta profiles. "

            "Tasks are claimed atomically, can depend on other tasks, and "

            "are executed by a named profile in an isolated workspace. "

            "See https://gogeta.ai/docs/user-guide/features/kanban "

            "or docs/gogeta-kanban-v1-spec.pdf for the full design."
    ...
    # GOGETA_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `gogeta kanban boards switch <slug>` or the "

            "GOGETA_KANBAN_BOARD env var). Use `gogeta kanban boards list` "
    ...
    p_list.add_argument("--mine", action="store_true",
                        ...
                        help="Filter by $GOGETA_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           ...
                           help="Author name (default: $GOGETA_PROFILE or 'user')")
    ...
    p_daemon = sub.add_parser(
        ...
        help="DEPRECATED — dispatcher now runs in the gateway. Use `gogeta gateway start`.",
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             ...
             "(union of ~/.gogeta/profiles/ and current assignees on the board)",
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             ...
             "(default: $GOGETA_PROFILE or 'specifier')",
    ...
    p_decompose.add_argument(
        ...
        help="Author name recorded on the audit comment "
             ...
             "(default: $GOGETA_PROFILE or 'decomposer')",
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    """Entry point from ``gogeta kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                ...
                "usage: gogeta kanban <action> [options]\n"

                "Run 'gogeta kanban --help' for the full list of actions.",
    ...
    # env-var pin for the duration of this call. Using GOGETA_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            ...
            print(
                ...
                f"Create it with `gogeta kanban boards create {normed}`.",
    ...
    # GOGETA_HOME. Previously only `init` and `daemon` triggered
...
def _profile_author() -> str:
    ...
    for env in ("GOGETA_PROFILE_NAME", "GOGETA_PROFILE"):
...
# Boards management (gogeta kanban boards …)
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    ...
    """Handle ``gogeta kanban boards <action>``.
    ...
    DB, so a fresh GOGETA_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        ...
        print("(no boards — create one with `gogeta kanban boards create <slug>`)")
    ...
    if len(boards) > 1:
        ...
        print("Switch boards with `gogeta kanban boards switch <slug>`.")
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `gogeta kanban boards switch {meta['slug']}` to make it current.")
...
def _cmd_boards_rm(args: argparse.Namespace) -> int:
    ...
    # When the user runs `gogeta kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # the kanban dispatcher can use them without a separate `gogeta profile
    ...
    if profiles:
        ...
        print("No profiles found under ~/.gogeta/profiles/.")

        print("Create one with `gogeta -p <name> setup` before assigning tasks.")

    print()

    print("Next step: start the gateway so ready tasks actually get picked up.")

    print("  gogeta gateway start")
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if not data:
        ...
        print("(no assignees — create a profile with `gogeta -p <name> setup`)")
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    from gogeta_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``gogeta kanban show <id>``
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    ...
    if os.environ.get("GOGETA_KANBAN_TASK") != task_id:

        return None

    raw = os.environ.get("GOGETA_KANBAN_RUN_ID")
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        ...
        print(
            ...
            "gogeta kanban daemon: DEPRECATED — the dispatcher now runs\n"

            "inside the gateway. To use kanban:\n"

            "\n"

            "    gogeta gateway start       # starts the gateway + embedded dispatcher\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                ...
                print(
                    ...
                    f"`gogeta kanban list --status ready` / "

                    f"`gogeta kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Gogeta profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from gogeta_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from gogeta_cli import kanban_decompose as decomp
