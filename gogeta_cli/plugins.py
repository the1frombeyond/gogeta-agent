...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with gogeta-agent;

   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their

   own discovery paths)

2. **User plugins**   – ``~/.gogeta/plugins/<name>/``

3. **Project plugins** – ``./.gogeta/plugins/<name>/`` (opt-in via

   ``GOGETA_ENABLE_PROJECT_PLUGINS``)

4. **Pip plugins**     – packages that expose the ``gogeta_agent.plugins``
...
from gogeta_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``GOGETA_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    ...
    env_override = os.getenv("GOGETA_BUNDLED_PLUGINS")
...
# Set ``GOGETA_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to

# stderr in addition to ~/.gogeta/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("GOGETA_PLUGINS_DEBUG", "").strip().lower() in {
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    """When GOGETA_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.



    Idempotent: only attaches the handler once per process unless ``force``

    is passed. Does not touch the root logger or other Gogeta loggers.
    ...
    logger.debug(
        ...
        "GOGETA_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
...
ENTRY_POINTS_GROUP = "gogeta_agent.plugins"



_NS_PARENT = "gogeta_plugins"
...
class PluginManifest:
    ...
    #              in ~/.gogeta/plugins/ still gated by ``plugins.enabled``
    ...
    # lookups and by ``gogeta plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``gogeta honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``gogeta <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        :class:`gogeta_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        from gogeta_cli.dashboard_auth import (
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``gogeta model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: gogeta_cli.main imports plugins indirectly

        from gogeta_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.gogeta/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        # Repo-shipped plugins live next to gogeta_cli/. Two layouts are
        ...
        # 2. User plugins (~/.gogeta/plugins/)

        user_dir = get_gogeta_home() / "plugins"
        ...
        # 3. Project plugins (./.gogeta/plugins/)

        if _env_enabled("GOGETA_ENABLE_PROJECT_PLUGINS"):

            project_dir = Path.cwd() / ".gogeta" / "plugins"
            ...
            logger.debug(
                ...
                "Project plugins disabled (set GOGETA_ENABLE_PROJECT_PLUGINS=1 to enable)"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with gogeta and must
            ...
            # for the same reason: every platform Gogeta ships must be
            ...
            if not is_enabled:
                ...
                loaded.error = (
                    ...
                    "not enabled in config (run `gogeta plugins enable {}` to activate)"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        ...
        """Import a directory-based plugin as ``gogeta_plugins.<slug>``.
        ...
        ``gogeta_plugins.image_gen__openai`` without colliding with any
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        ...
        name="gogeta-plugin-command-await",
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``gogeta tools`` TUI so plugin-provided toolsets appear
