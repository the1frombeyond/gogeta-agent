...
on Japanese locales, etc.).  Gogeta's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``gogeta_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Gogeta is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``GOGETA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("GOGETA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Gogeta-managed Git install directories so
    ...
    # already-running shells don't see the change — which means gogeta
    ...
    # install without Gogeta-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Gogeta's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Gogeta's
    ``gogeta config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Gogeta (User env var in Windows Settings, or
    export in a PowerShell profile) and Gogeta picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Gogeta-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\gogeta\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``gogeta`` from right after install) retain their old PATH.

    Any subprocess Gogeta spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Gogeta-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "gogeta", "git", "cmd"),
        os.path.join(local_appdata, "gogeta", "git", "bin"),
        os.path.join(local_appdata, "gogeta", "git", "usr", "bin"),
        # Gogeta venv Scripts directory — host of the gogeta.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes gogeta, but harmless to include.
        os.path.join(local_appdata, "gogeta", "gogeta-agent", "venv", "Scripts"),
