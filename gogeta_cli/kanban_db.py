...
``<root>`` is the **shared Gogeta root** (the parent of any active
...
``gogeta -p <profile>`` joins the same board as the dispatcher that
...
the profile's Gogeta home but are otherwise isolated: a worker spawned
...
* ``GOGETA_KANBAN_BOARD`` env var (used by the dispatcher to pin workers

  to the board their task lives on — workers cannot see other boards).

* ``GOGETA_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  ...
  the "currently selected" board. Written by ``gogeta kanban boards

  switch <slug>``. When absent, the active board is ``default``.



In standard installs ``<root>`` is ``~/.gogeta``. In Docker / custom

deployments where ``GOGETA_HOME`` points outside ``~/.gogeta`` (e.g.

``/opt/gogeta``), ``<root>`` is ``GOGETA_HOME``. Legacy env-var

overrides still work:



* ``GOGETA_KANBAN_DB`` — pin the database file path directly.

* ``GOGETA_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.

* ``GOGETA_KANBAN_HOME`` — pin the umbrella root that anchors kanban

  paths. Useful for tests and unusual deployments.



The dispatcher injects ``GOGETA_KANBAN_DB``,

``GOGETA_KANBAN_WORKSPACES_ROOT``, and ``GOGETA_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/gogeta-kanban-v1-spec.pdf`` for the full
...
# ``GOGETA_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``GOGETA_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("GOGETA_KANBAN_CLAIM_TTL_SECONDS", "").strip()
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``GOGETA_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    raw = os.environ.get("GOGETA_KANBAN_CRASH_GRACE_SECONDS", "").strip()
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``GOGETA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    raw = os.environ.get(
        ...
        "GOGETA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    ...
    "gogeta_kanban_current_board_override",
...
# enough that kebab-case names like ``atm10-server`` or ``gogeta-agent``
...
def kanban_home() -> Path:
    ...
    """Return the shared Gogeta root that anchors the kanban board.
    ...
    1. ``GOGETA_KANBAN_HOME`` env var when set and non-empty (explicit

       override for tests and unusual deployments).

    2. ``get_default_gogeta_root()``, which already returns ``<root>``

       when ``GOGETA_HOME`` is ``<root>/profiles/<name>``, and returns

       ``GOGETA_HOME`` directly for Docker / custom deployments.
    ...
    profile's ``GOGETA_HOME`` would silently fork the board per profile,

    which breaks the dispatcher / worker handoff.

    """

    override = os.environ.get("GOGETA_KANBAN_HOME", "").strip()
    ...
    return get_default_gogeta_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``gogeta kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``GOGETA_KANBAN_BOARD`` env var (set by the dispatcher on worker

       spawn, or manually for ad-hoc overrides).

    2. ``<root>/kanban/current`` on disk (set by ``gogeta kanban boards
    ...
    env = os.environ.get("GOGETA_KANBAN_BOARD", "").strip()
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``gogeta kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``GOGETA_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    override = os.environ.get("GOGETA_KANBAN_DB", "").strip()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``GOGETA_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    override = os.environ.get("GOGETA_KANBAN_WORKSPACES_ROOT", "").strip()
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``GOGETA_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    override = os.environ.get("GOGETA_KANBAN_ATTACHMENTS_ROOT", "").strip()
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``gogeta kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``GOGETA_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- inside an agent loop that propagated ``GOGETA_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    raw = os.environ.get("GOGETA_KANBAN_BUSY_TIMEOUT_MS", "").strip()
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Gogeta treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ...
      ``GOGETA_KANBAN_DB`` env → ``GOGETA_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See gogeta_state._WAL_INCOMPAT_MARKERS for detection logic.

                from gogeta_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``gogeta kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``GOGETA_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    from gogeta_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``gogeta --skills ...`` alongside the built-in
    ...
    # `gogeta --skills X,Y` comma syntax is handled in the dispatcher,
...
# Canonical sort-order mappings for ``gogeta kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `gogeta kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``gogeta kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``gogeta kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``gogeta kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``gogeta kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``GOGETA_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Gogeta' own DB, metadata, and logs, not task workspaces.



    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths

    outside Gogeta-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``gogeta kanban show <id>`` and the dashboard.
...
# without thrashing. Overridable via ``GOGETA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Gogeta
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``gogeta kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Gogeta profile.
    ...
    try:
        ...
        from gogeta_cli.profiles import profile_exists  # local import: avoids cycle
    ...
    *,
    ...
    if _default_assignee:
        ...
        try:
            ...
            from gogeta_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        # Skip ready tasks whose assignee is not a real Gogeta profile.

        # `_default_spawn` invokes ``gogeta -p <assignee>`` which fails

        # with "Profile 'X' does not exist" when the assignee names a

        # control-plane lane (e.g. an interactive Claude Code terminal

        # like ``orion-cc`` / ``orion-research``) rather than a Gogeta
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `gogeta kanban tail` — without
...
def _module_gogeta_argv() -> list[str]:

    """Return the interpreter-bound Gogeta CLI invocation."""

    # ``gogeta_cli.main`` is the console-script target declared in

    # pyproject.toml, NOT a top-level ``gogeta`` package — there is no

    # ``gogeta`` package to import.
...
def _absolute_gogeta_path(path: str) -> str:

    """Return an absolute filesystem path for a resolved Gogeta shim."""
...
def _gogeta_path_argv(path: str) -> list[str]:

    """Return argv for a resolved Gogeta executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        ...
        return _module_gogeta_argv()

    return [_absolute_gogeta_path(path)]





def _resolve_gogeta_argv() -> list[str]:

    """Resolve the ``gogeta`` invocation as argv parts for ``Popen``.
    ...
    1. ``$GOGETA_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("gogeta")`` — the console-script shim, normalized to

       an absolute path. On Windows, ``which`` can return a relative

       ``.\\gogeta.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m gogeta_cli.main`` — fallback for setups where

       Gogeta is launched from a venv and the ``gogeta`` shim is not on
    ...
    Mirrors ``gateway.run._resolve_gogeta_bin`` for the same reason. Kept

    local (not imported from gateway) because ``gogeta_cli`` sits below
    ...
    env_bin = os.environ.get("GOGETA_BIN", "").strip()

    if env_bin:

        if _looks_like_path(env_bin):

            return _gogeta_path_argv(env_bin)

        resolved_env_bin = _safe_which_no_cwd(env_bin)

        if resolved_env_bin:

            return _gogeta_path_argv(resolved_env_bin)

        return _module_gogeta_argv()



    gogeta_bin = _safe_which_no_cwd("gogeta") if _IS_WINDOWS else shutil.which("gogeta")

    if gogeta_bin:

        return _gogeta_path_argv(gogeta_bin)
...
def _kanban_worker_skill_available(gogeta_home: Optional[str]) -> bool:
    ...
    the worker activates a profile (``gogeta -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset GOGETA_HOME means the worker falls back to the default root

    # home (``~/.gogeta``), which ships the bundled skill.

    base = _Path(gogeta_home) if gogeta_home else (_Path.home() / ".gogeta")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``gogeta -p <profile> chat -q ...`` subprocess.
    ...
    ``GOGETA_KANBAN_DB`` / ``GOGETA_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject GOGETA_HOME so the worker reads the profile-scoped config.yaml

    # (fallback_providers, toolsets, agent settings, etc.) instead of the root

    # config.  Without this, `env = dict(os.environ)` copies only the parent's

    # env, and when the child process starts `gogeta -p <name>` the

    # _apply_profile_override() runs *before* gogeta_constants is imported.

    # If GOGETA_HOME is absent from the child's env, get_gogeta_home() falls

    # back to Path.home() / ".gogeta" (the DEFAULT profile root), ignoring the
    ...
    try:
        ...
        env["GOGETA_HOME"] = resolve_profile_env(profile_arg)

    except FileNotFoundError:

        # Profile dir doesn't exist — defer resolution to the CLI's

        # _apply_profile_override() via GOGETA_PROFILE (set below).

        # This only happens in test fixtures where the isolated

        # GOGETA_HOME never had profiles created.

        pass

    if task.tenant:

        env["GOGETA_TENANT"] = task.tenant

    env["GOGETA_KANBAN_TASK"] = task.id

    env["GOGETA_KANBAN_WORKSPACE"] = workspace

    if task.branch_name:

        env["GOGETA_KANBAN_BRANCH"] = task.branch_name

    if task.current_run_id is not None:

        env["GOGETA_KANBAN_RUN_ID"] = str(task.current_run_id)

    if task.claim_lock:

        env["GOGETA_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        ...
        env["GOGETA_KANBAN_GOAL_MODE"] = "1"

        if task.goal_max_turns is not None:

            env["GOGETA_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`gogeta -p <name>`

    # rewrites GOGETA_HOME), its kanban paths still match the

    # dispatcher's. Belt-and-braces with the `get_default_gogeta_root()`

    # resolution in `kanban_home()` — symmetric resolution is the norm,

    # but unusual symlink / Docker layouts are caught here too.

    env["GOGETA_KANBAN_DB"] = str(kanban_db_path(board=board))

    env["GOGETA_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    ...
    env["GOGETA_KANBAN_BOARD"] = resolved_board

    # GOGETA_PROFILE is the author the kanban_comment tool defaults to.

    # `gogeta -p <assignee>` activates the profile, but the env var is

    # what the tool reads — set it explicitly here so comments are

    # attributed correctly regardless of how the child loads config.

    env["GOGETA_PROFILE"] = profile_arg



    cmd = [

        *_resolve_gogeta_argv(),

        "-p", profile_arg,

        # Worker subprocesses switch to a profile-scoped GOGETA_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("GOGETA_HOME")):
    ...
    # `gogeta kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "`gogeta` executable not found on PATH. "

            "Install Gogeta Agent or activate its venv before running the kanban dispatcher."
...
def run_daemon(
    ...
    *,
    ...
    on SIGINT / SIGTERM so ``gogeta kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled GOGETA_PROFILE values like "gogeta-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Gogeta root exists



    Reads profile paths directly so this module has no import dependency on

    ``gogeta_cli.profiles`` (which pulls in a large chunk of the CLI startup
    ...
    try:
        ...
        default_root = get_default_gogeta_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``gogeta kanban assignees`` for the terminal.
