...
MCP Server Management CLI — ``gogeta mcp`` subcommand.



Implements ``gogeta mcp add/remove/list/test/configure`` for interactive
...
configuration in ~/.gogeta/config.yaml under the ``mcp_servers`` key.
...
from gogeta_cli.config import (
    ...
    get_gogeta_home,  # noqa: F401 — used by test mocks
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    ...
    from gogeta_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.gogeta/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``gogeta mcp login`` to distinguish a genuine authentication
    ...
    try:
        ...
        from tools.mcp_oauth import GogetaTokenStorage

        return GogetaTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # gogeta_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info('  gogeta mcp add ink --url "https://mcp.ml.ink/mcp"')

        _info('  gogeta mcp add github --command npx --args @modelcontextprotocol/server-github')

        _info('  gogeta mcp add myserver --preset mypreset')
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_gogeta_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: gogeta mcp test " + name)
    ...
    _success(f"Saved '{name}' to {display_gogeta_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `gogeta mcp test` in the same session) is evicted too.
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        _info('  gogeta mcp add <name> --url <endpoint>')

        _info('  gogeta mcp add <name> --command <cmd> --args <args...>')
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `gogeta mcp remove` + `gogeta mcp add` to reconfigure auth.")
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            _info("Then re-run `gogeta mcp login " + name + "`.")
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        ...
        print("Error: 'gogeta mcp configure' requires an interactive terminal.", file=_sys.stderr)
...
def mcp_command(args):
    ...
    """Main dispatcher for ``gogeta mcp`` subcommands."""
    ...
    if action == "picker":
        ...
        from gogeta_cli.mcp_picker import run_picker
    ...
    if action == "catalog":
        ...
        from gogeta_cli.mcp_picker import show_catalog
    ...
    if action == "install":
        ...
        from gogeta_cli.mcp_picker import install_by_name
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `gogeta plugin`.
        ...
        _info("gogeta mcp                                    Open the catalog picker (default)")

        _info("gogeta mcp catalog                            List Nous-approved MCPs")

        _info("gogeta mcp install <name>                     Install a catalog MCP")

        _info("gogeta mcp serve                              Run as MCP server")

        _info("gogeta mcp add <name> --url <endpoint>        Add a custom MCP server")

        _info("gogeta mcp add <name> --command <cmd>         Add a stdio server")

        _info("gogeta mcp add <name> --preset <preset>       Add from a known preset")

        _info("gogeta mcp remove <name>                      Remove a server")

        _info("gogeta mcp list                               List configured servers")

        _info("gogeta mcp test <name>                        Test connection")

        _info("gogeta mcp configure <name>                   Toggle tools")

        _info("gogeta mcp login <name>                       Re-authenticate OAuth")
