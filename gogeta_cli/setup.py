...
Interactive setup wizard for Gogeta Agent.
...
_DOCS_BASE = "https://gogeta.ai/docs"
...
# display_gogeta_home imported lazily at call sites (stale-module safety during gogeta update)
...
from gogeta_cli.cli_output import (  # noqa: E402
...
from gogeta_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print(color("⚕ Gogeta Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    ...
    print_info("Configure Gogeta using environment variables or config commands:")

    print_info("  gogeta config set model.provider custom")

    print_info("  gogeta config set model.base_url http://localhost:8080/v1")

    print_info("  gogeta config set model.default your-model-name")

    print()

    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")

    print_info("Run 'gogeta setup' in an interactive terminal to use the full wizard.")
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from gogeta_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'gogeta setup')")





def _print_setup_summary(config: dict, gogeta_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'gogeta setup' to configure"))
    ...
    # Video generation — opt-in via `gogeta tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from gogeta_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'gogeta setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'gogeta setup tts'"))
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'gogeta setup terminal'"))
    ...
    # Spotify (OAuth via gogeta auth spotify — check auth.json, not env vars)
    ...
    if disabled_tools:
        ...
        print_warning(
            ...
            "Some tools are disabled. Run 'gogeta setup tools' to configure them,"
    ...
    print(
        ...
        f"   {color('Data:', Colors.YELLOW)}      {gogeta_home}/cron/, sessions/, logs/"
    ...
    print(f"   {color('gogeta setup', Colors.GREEN)}          Re-run the full wizard")

    print(f"   {color('gogeta setup model', Colors.GREEN)}    Change model/provider")

    print(f"   {color('gogeta setup terminal', Colors.GREEN)} Change terminal backend")

    print(f"   {color('gogeta setup gateway', Colors.GREEN)}  Configure messaging")

    print(f"   {color('gogeta setup tools', Colors.GREEN)}    Configure tool providers")

    print()

    print(f"   {color('gogeta config', Colors.GREEN)}         View current settings")

    print(

        f"   {color('gogeta config edit', Colors.GREEN)}    Open config in your editor"

    )

    print(f"   {color('gogeta config set <key> <value>', Colors.GREEN)}")
    ...
    print(f"   {color('gogeta', Colors.GREEN)}              Start chatting")

    print(f"   {color('gogeta gateway', Colors.GREEN)}      Start messaging gateway")

    print(f"   {color('gogeta doctor', Colors.GREEN)}       Check for issues")
...
# between `gogeta tools` and `gogeta setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``gogeta model``)
    ...
    provider added to ``gogeta model`` is automatically available here.
    ...
    # Delegate to the shared gogeta model flow — handles provider picker,

    # credential prompting, model selection, and config persistence.

    from gogeta_cli.main import select_provider_and_model
    ...
    try:
        ...
        print_info("You can try again later with: gogeta model")
    ...
    # on demand via `gogeta auth add`, `gogeta setup` vision, and

    # `gogeta setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``gogeta model`` -> xAI Grok OAuth (SuperGrok / Premium+).

    """

    try:

        from gogeta_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            ...
            print_warning(
                ...
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.gogeta/.env."
    ...
    if selected == "neutts":
        ...
        # via the Gogeta auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via gogeta setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    ...
    """Standalone TTS setup (for 'gogeta setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Gogeta runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `gogeta setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `gogeta setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into GOGETA_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("GOGETA_MAX_ITERATIONS")
    ...
    print_info("  Run `gogeta setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # GOGETA_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        ...
        if "/profiles/" in gogeta_home:

            profile_name = gogeta_home.rstrip("/").rsplit("/", 1)[-1]
...
def _setup_telegram():
    ...
    print_info("📬 Home Channel: where Gogeta delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `gogeta update`)",
    ...
    print_info("   Full guide: https://gogeta.ai/docs/user-guide/messaging/slack/")
...
def _write_slack_manifest_and_instruct():
    ...
    """Generate the Slack manifest, write it under GOGETA_HOME, and print
    ...
    try:
        ...
        from gogeta_cli.slack_cli import _build_full_manifest
        ...
        manifest = _build_full_manifest(
            ...
            bot_description="Your Gogeta agent on Slack",

        )

        target = Path(get_gogeta_home()) / "slack-manifest.json"
        ...
        print_info(
            ...
            "   Re-run `gogeta slack manifest --write` anytime to refresh after "

            "Gogeta adds new commands."
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print_info("📬 Home Room: where Gogeta delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Gogeta to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from gogeta_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print_info("   Full guide: https://gogeta.ai/docs/user-guide/messaging/webhooks/")
    ...
    print_info("   https://gogeta.ai/docs/user-guide/messaging/webhooks/#configuring-routes")

    print()

    print_info("   Open config in your editor:  gogeta config edit")
...
def setup_gateway(config: dict):
    ...
    from gogeta_cli.gateway import _all_platforms, _platform_status, _configure_platform



    print_header("Messaging Platforms")

    print_info("Connect to messaging platforms to chat with Gogeta from anywhere.")
    ...
    if not selected:
        ...
        print_info("No platforms selected. Run 'gogeta setup gateway' later to configure.")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                ...
                print_info(
                    ...
                    f"     gogeta config set {plat.upper()}_HOME_CHANNEL <channel_id>"
        ...
        if supports_systemd and has_legacy_gogeta_units():
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    ...
                    print_info("  Or as a boot-time service: sudo gogeta gateway install --system")

                print_info("  Or run in foreground:  gogeta gateway")
            ...
            if is_container():
                ...
                print_info("   gogeta gateway run          # Run as container main process")
                ...
                print_info("   gogeta gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `gogeta setup tools` and `gogeta tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      ...
      * ``PROVIDER_REGISTRY`` in ``gogeta_cli.auth`` — lists every supported
    ...
    try:
        ...
        from gogeta_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from gogeta_cli.config
    ...
    if section_key == "model":
        ...
        from gogeta_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    ...
    """Load the openclaw_to_gogeta migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        ...
        "openclaw_to_gogeta", _OPENCLAW_SCRIPT
...
# Config values may have different semantics between OpenClaw and Gogeta.

# Instruction/context files (.md) can contain incompatible setup procedures.

_HIGH_IMPACT_KIND_KEYWORDS = {

    "gateway": "⚠ Gateway/messaging — this will configure Gogeta to use your OpenClaw messaging channels",

    "telegram": "⚠ Telegram — this will point Gogeta at your OpenClaw Telegram bot",

    "slack": "⚠ Slack — this will point Gogeta at your OpenClaw Slack workspace",

    "discord": "⚠ Discord — this will point Gogeta at your OpenClaw Discord bot",

    "whatsapp": "⚠ WhatsApp — this will point Gogeta at your OpenClaw WhatsApp connection",

    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Gogeta equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        ...
        print(color("  Would overwrite (conflicts with existing Gogeta config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print(color("  Note: OpenClaw config values may have different semantics in Gogeta.", Colors.YELLOW))

        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Gogeta's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(gogeta_home: Path) -> bool:
    ...
    print_info("Gogeta can preview what would be imported before making any changes.")
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        ...
        print_info(
            ...
            "Skipping migration. You can run it later with: gogeta claw migrate --dry-run"
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=gogeta_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        ...
        print_info(
            ...
            "Migration cancelled. You can run it later with: gogeta claw migrate"
    ...
    # Execute the migration — overwrite=False so existing Gogeta configs are
    ...
    try:
        ...
        migrator = mod.Migrator(
            ...
            overwrite=False,  # preserve existing Gogeta config
    ...
    if conflicts:
        ...
        print_info(f"Skipped {conflicts} item(s) that already exist in Gogeta (use gogeta claw migrate --overwrite to force).")
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``gogeta setup --portal`` and ``gogeta portal``. This is the
    ...
    Gogeta session — model selected, provider set, and web/image/tts/browser

    tools routed via their Portal sub — without being told to run

    ``gogeta setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``gogeta model`` runs

    when you pick Nous. Routing through it (instead of hand-rolling the auth +

    provider write here) means ``gogeta portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Gogeta Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    ...
    print_info("  Sign up: https://portal.gogeta.ai/manage-subscription")
    ...
    # setup calls, so `gogeta portal` == quick setup's Nous step.

    try:

        from gogeta_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `gogeta portal`.")

        return

    except Exception as exc:

        logger.debug("_model_flow_nous error during `gogeta portal`: %s", exc)
    ...
    print_info("  Run `gogeta portal info` to inspect routing.")

    print_info("  Run `gogeta` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      ...
      gogeta setup           — full or quick (auto-detected)

      gogeta setup model     — just model/provider

      gogeta setup tts       — just text-to-speech

      gogeta setup terminal  — just terminal backend

      gogeta setup gateway   — just messaging platforms

      gogeta setup tools     — just tool configuration

      gogeta setup agent     — just agent settings

    """

    from gogeta_cli.config import is_managed, managed_error
    ...
    if section:
        ...
        for key, label, func in SETUP_SECTIONS:
            ...
            if key == section:
                ...
                print(color(f"│     ⚕ Gogeta Setup — {label:<34s} │", Colors.MAGENTA))
    ...
    print(
        ...
        color(
            ...
            "│             ⚕ Gogeta Agent Setup Wizard                │", Colors.MAGENTA
    ...
    print(
        ...
        color(
            ...
            "│  Let's configure your Gogeta Agent installation.       │", Colors.MAGENTA
    ...
    if is_existing:
        ...
        if quick_requested:
            ...
            _run_quick_setup(config, gogeta_home)
        ...
        print_success("You already have Gogeta configured.")
        ...
        print_info("Tip: jump straight to a section with 'gogeta setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(gogeta_home)
        ...
        setup_mode = prompt_choice(
            ...
            "How would you like to set up Gogeta?",
        ...
        if setup_mode == 0:
            ...
            _run_first_time_quick_setup(config, gogeta_home, is_existing)
    ...
    print_info(f"Data folder:  {gogeta_home}")

    print_info(f"Install dir:  {PROJECT_ROOT}")

    print()

    print_info("You can edit these files directly or use 'gogeta config edit'")
    ...
    # Tune later with `gogeta setup agent`.
    ...
    _print_setup_summary(config, gogeta_home)





def _run_first_time_quick_setup(config: dict, gogeta_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``gogeta setup <section>``

    or switch providers with ``gogeta model``.
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'gogeta setup gateway'",
    ...
    print_info("  Configure all settings:    gogeta setup")

    if gateway_choice != 0:

        print_info("  Connect Telegram/Discord:  gogeta setup gateway")
...
def _run_quick_setup(config: dict, gogeta_home):
    ...
    if not has_anything_missing:
        ...
        print_info("Run 'gogeta setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("Connect Gogeta to messaging apps to chat from anywhere.")

        print_info("You can configure these later with 'gogeta setup gateway'.")
