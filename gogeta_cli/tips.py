...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or gogeta -c.",
    ...
    "Ctrl+Z suspends Gogeta to the background — run fg in your shell to resume.",
    ...
    "gogeta -c resumes your most recent CLI session. gogeta -c \"project name\" resumes by title.",

    "gogeta -w creates an isolated git worktree — perfect for parallel agent workflows.",

    "gogeta -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",

    "gogeta chat -t web,terminal enables only specific toolsets for a focused session.",

    "gogeta chat -s github-pr-workflow preloads a skill at launch.",

    "gogeta chat -q \"query\" runs a single non-interactive query and exits.",

    "gogeta chat --max-turns 200 overrides the default 90-iteration limit per turn.",

    "gogeta chat --checkpoints enables filesystem snapshots before every destructive file change.",

    "gogeta --yolo bypasses all dangerous command approval prompts for the entire session.",

    "gogeta chat --source telegram tags the session for filtering in gogeta sessions list.",

    "gogeta -p work chat runs under a specific profile without changing your default.",



    # --- CLI Subcommands ---

    "gogeta doctor --fix diagnoses and auto-repairs config and dependency issues.",

    "gogeta dump outputs a compact setup summary — great for bug reports.",

    "gogeta config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",

    "gogeta config edit opens config.yaml in your default editor.",

    "gogeta config check scans for missing or stale configuration options.",

    "gogeta sessions browse opens an interactive session picker with search.",

    "gogeta sessions stats shows session counts by platform and database size.",

    "gogeta sessions prune --older-than 30 cleans up old sessions.",

    "gogeta skills search react --source skills-sh searches the skills.sh public directory.",

    "gogeta skills check scans installed hub skills for upstream updates.",

    "gogeta skills tap add myorg/skills-repo adds a custom GitHub skill source.",

    "gogeta skills snapshot export setup.json exports your skill configuration for backup or sharing.",

    "gogeta mcp add github --command npx adds MCP servers from the command line.",

    "gogeta mcp serve runs Gogeta itself as an MCP server for other agents.",

    "gogeta auth add lets you add multiple API keys for credential pool rotation.",

    "gogeta completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",

    "gogeta logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",

    "gogeta backup creates a zip backup of your entire Gogeta home directory.",

    "gogeta profile create coder creates an isolated profile that becomes its own command.",

    "gogeta profile create work --clone copies your current config and keys to a new profile.",

    "gogeta update syncs new bundled skills to ALL profiles automatically.",

    "gogeta gateway install sets up Gogeta as a system service (systemd/launchd).",

    "gogeta memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",

    "gogeta webhook subscribe creates event-driven webhook routes with HMAC validation.",

    "Save money: gogeta tools disables unused tools, gogeta skills config trims skills down.",

    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",

    "gogeta models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as gogeta -w).",
    ...
    "GOGETA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "execute_code runs Python scripts that call Gogeta tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'gogeta profile create coder' creates the 'coder' command.",

    "gogeta profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Gogeta prints a resume command with session ID and stats.",

    "gogeta sessions export backup.jsonl exports all sessions for backup or analysis.",

    "gogeta -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "gogeta skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .gogeta/plans/ in the active workspace.",



    # --- Cron & Scheduling ---

    "Cron jobs can attach skills: gogeta cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "Gogeta runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",

    "gogeta gateway install sets it up as a system service that starts on boot.",

    "DingTalk uses Stream Mode — no webhooks or public URL needed.",

    "BlueBubbles brings iMessage to Gogeta via a local macOS server.",
    ...
    "Context files (.gogeta.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Gogeta loads project context from .gogeta.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "gogeta mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",

    "gogeta mcp catalog lists Nous-approved MCP servers shipped with the repo.",

    "gogeta mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — gogeta tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.gogeta/checkpoints/ — your project's .git is never touched.",
    ...
    "gogeta chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "gogeta plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "gogeta acp runs Gogeta as an ACP server for VS Code, Zed, and JetBrains integration.",

    "Custom providers: save named endpoints in config.yaml under custom_providers.",

    "GOGETA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",

    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",

    "gogeta auth add nous or gogeta auth add openai-codex sets up OAuth-based providers.",
    ...
    "gogeta status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.gogeta/scripts/ and run before the agent — perfect for data collection pipelines.",

    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",

    "SOUL.md completely replaces the agent's default identity — rewrite it to make Gogeta your own.",
    ...
    "Before a gateway session reset, Gogeta auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.gogeta/skills/.hub/audit.log tracks every install and removal operation.",

    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",

    "Each profile gets its own subprocess HOME at GOGETA_HOME/home/ — isolated git, ssh, npm, gh configs.",

    "GOGETA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",

    "Container mode: place .container-mode in GOGETA_HOME and the host CLI auto-execs into the container.",

    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",

    "Every interrupt during an agent run is logged to ~/.gogeta/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'gogeta profile create ls' would be rejected.",

    "gogeta profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "gogeta claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "GOGETA_DEV=1 bypasses container mode detection for local development.",

    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via gogeta tools.",

    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.gogeta/.env.",

    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",

    "The skills quarantine at ~/.gogeta/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Gogeta auto-continues turn after turn until a judge says done.',

    '/snapshot create [label] saves a full state snapshot of Gogeta config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Gogeta is working.',
    ...
    '/reload reloads ~/.gogeta/.env into the running session — pick up new API keys without restarting.',
    ...
    "GOGETA_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",



    # --- Gateway Hooks ---

    'Gateway hooks live under ~/.gogeta/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',

    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',

    'Drop a ~/.gogeta/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',



    # --- Curator ---

    'gogeta curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',

    "gogeta curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",

    'gogeta curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',



    # --- Credential Pools & Routing ---

    'gogeta auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'GOGETA_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',

    "GOGETA_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'gogeta dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',

    'gogeta dashboard embeds the full Gogeta TUI in your browser via xterm.js and a WebSocket PTY.',

    'Drop a YAML in ~/.gogeta/dashboard-themes/ with two palette colors to reskin the entire dashboard.',

    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.gogeta/dashboard-plugins/ — no npm build required.',
    ...
    'GOGETA_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',

    'GOGETA_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',

    'GOGETA_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',

    'GOGETA_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.gogeta/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.gogeta/skills/ so drop-in skills appear without restarting the session.',
    ...
    'gogeta -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',

    'gogeta chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',

    'gogeta chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',

    'gogeta chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',

    "gogeta chat --source tool tags programmatic chats so they don't clutter gogeta sessions list.",

    'gogeta dump --show-keys includes redacted API key fingerprints for deeper support debugging.',

    'gogeta sessions rename <ID> "new title" renames any past session; gogeta sessions delete <ID> removes one.',

    'gogeta import restores a session export or profile archive produced by sessions export or profile export.',

    'gogeta fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',

    'gogeta pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',

    'gogeta setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',

    'gogeta status --deep runs the full health sweep across every component; plain gogeta status is the quick view.',



    # --- Agent Behavior Env Vars ---

    'GOGETA_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',

    'GOGETA_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.gogeta/plugins/ — trust-gated by design.',

    "GOGETA_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",

    'GOGETA_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',

    'GOGETA_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',

    'GOGETA_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',

    'GOGETA_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',

    'GOGETA_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',

    'GOGETA_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',



    # --- Gateway Behavior Env Vars ---

    'GOGETA_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',

    'GOGETA_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',

    'GOGETA_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',

    'GOGETA_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    'security.tirith_fail_open: false makes Gogeta block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from gogeta sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.gogeta/dashboard-plugins/.',
