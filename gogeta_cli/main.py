...
Gogeta CLI - Main entry point.



Usage:

    gogeta                     # Interactive chat (default)

    gogeta chat                # Interactive chat

    gogeta gateway             # Run gateway in foreground

    gogeta gateway start       # Start gateway as service

    gogeta gateway stop        # Stop gateway service

    gogeta gateway status      # Show gateway status

    gogeta gateway install     # Install gateway service

    gogeta gateway uninstall   # Uninstall gateway service

    gogeta setup               # Interactive setup wizard

    gogeta logout              # Clear stored authentication

    gogeta status              # Show status of all components

    gogeta cron                # Manage cron jobs

    gogeta cron list           # List cron jobs

    gogeta cron status         # Check if cron scheduler is running

    gogeta doctor              # Check configuration and dependencies

    gogeta honcho setup                    # Configure Honcho AI memory integration

    gogeta honcho status                   # Show Honcho config and connection status

    gogeta honcho sessions                 # List directory → session name mappings

    gogeta honcho map <name>               # Map current directory to a session name

    gogeta honcho peer                     # Show peer names and dialectic settings

    gogeta honcho peer --user NAME         # Set user peer name

    gogeta honcho peer --ai NAME           # Set AI peer name

    gogeta honcho peer --reasoning LEVEL   # Set dialectic reasoning level

    gogeta honcho mode                     # Show current memory mode

    gogeta honcho mode [hybrid|honcho|local]  # Set memory mode

    gogeta honcho tokens                   # Show token budget settings

    gogeta honcho tokens --context N       # Set session.context() token cap

    gogeta honcho tokens --dialectic N     # Set dialectic result char cap

    gogeta honcho identity                 # Show AI peer identity representation

    gogeta honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)

    gogeta honcho migrate                  # Step-by-step migration guide: OpenClaw native → Gogeta + Honcho

    gogeta version             Show version

    gogeta update              Update to latest version

    gogeta uninstall           Uninstall Gogeta Agent

    gogeta acp                 Run as an ACP server for editor integration

    gogeta sessions browse     Interactive session picker with search



    gogeta claw migrate --dry-run  # Preview migration without changes

"""



# IMPORTANT: gogeta_bootstrap must be the very first import — it sets up
...
# Guarded against ModuleNotFoundError because ``gogeta_bootstrap`` is a

# top-level module registered via pyproject.toml's ``py-modules`` list.

# When the user upgrades code via ``git pull`` (or ``gogeta update``

# crashes between ``git reset --hard`` and ``uv pip install -e .``), the

# new code references ``gogeta_bootstrap`` but the editable install's

# ``.pth`` file still points at the old set of top-level modules.  Without

# this guard, gogeta crashes on import and the user can't run

# ``gogeta update`` to recover.  Missing the bootstrap means UTF-8 stdio

# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.

try:

    import gogeta_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    ...
    """Set the process title to 'gogeta' so tools like 'ps', 'top', and
    ...
    Strategy (try in order):
      ...
      1. ``setproctitle`` (opt-in dep — installed via ``gogeta tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``gogeta.exe``).
    ...
    try:
        ...
        setproctitle.setproctitle("gogeta")
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"gogeta", 0, 0, 0)  # PR_SET_NAME = 15

        elif system == "Darwin":

            libc = ctypes.CDLL("libc.dylib", use_errno=True)

            libc.pthread_setname_np(b"gogeta")

        # Windows: the .exe name is already ``gogeta.exe`` — nothing to do.
...
# that run *before* gogeta_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        home = os.environ.get("GOGETA_HOME")
        ...
        if home:
            ...
            cfg_path = os.path.join(os.path.expanduser("~"), ".gogeta", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``GOGETA_TUI=1``, then ``display.interface`` in config.
    ...
    if os.environ.get("GOGETA_TUI") == "1" or "--tui" in argv:
...
# `entry.tsx`; this is just the earlier cousin. ``GOGETA_TUI_NO_EARLY_DISABLE``

# escapes the behaviour for diagnostics.

def _suppress_mouse_residue_early() -> None:

    if os.environ.get("GOGETA_TUI_NO_EARLY_DISABLE") == "1":
    ...
    try:
        ...
        # Skip when stdout is redirected (`gogeta --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    ...
    from gogeta_cli import __release_date__, __version__



    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))

    print(f"Gogeta Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    ...
    """Handle ``gogeta --version`` before config/logging imports on Termux."""

    if os.environ.get("GOGETA_TERMUX_DISABLE_FAST_CLI") == "1":
...
from gogeta_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag

from gogeta_cli.subcommands.cron import build_cron_parser

from gogeta_cli.subcommands.gateway import build_gateway_parser

from gogeta_cli.subcommands.profile import build_profile_parser

from gogeta_cli.subcommands.model import build_model_parser

from gogeta_cli.subcommands.setup import build_setup_parser

from gogeta_cli.subcommands.postinstall import build_postinstall_parser

from gogeta_cli.subcommands.whatsapp import build_whatsapp_parser

from gogeta_cli.subcommands.slack import build_slack_parser

from gogeta_cli.subcommands.login import build_login_parser

from gogeta_cli.subcommands.logout import build_logout_parser

from gogeta_cli.subcommands.auth import build_auth_parser

from gogeta_cli.subcommands.status import build_status_parser

from gogeta_cli.subcommands.webhook import build_webhook_parser

from gogeta_cli.subcommands.hooks import build_hooks_parser

from gogeta_cli.subcommands.doctor import build_doctor_parser

from gogeta_cli.subcommands.security import build_security_parser

from gogeta_cli.subcommands.dump import build_dump_parser

from gogeta_cli.subcommands.debug import build_debug_parser

from gogeta_cli.subcommands.backup import build_backup_parser

from gogeta_cli.subcommands.import_cmd import build_import_cmd_parser

from gogeta_cli.subcommands.config import build_config_parser

from gogeta_cli.subcommands.version import build_version_parser

from gogeta_cli.subcommands.update import build_update_parser

from gogeta_cli.subcommands.uninstall import build_uninstall_parser

from gogeta_cli.subcommands.dashboard import build_dashboard_parser

from gogeta_cli.subcommands.gui import build_gui_parser

from gogeta_cli.subcommands.logs import build_logs_parser

from gogeta_cli.subcommands.prompt_size import build_prompt_size_parser

from gogeta_cli.subcommands.memory import build_memory_parser

from gogeta_cli.subcommands.acp import build_acp_parser

from gogeta_cli.subcommands.tools import build_tools_parser

from gogeta_cli.subcommands.insights import build_insights_parser

from gogeta_cli.subcommands.skills import build_skills_parser

from gogeta_cli.subcommands.pairing import build_pairing_parser

from gogeta_cli.subcommands.plugins import build_plugins_parser

from gogeta_cli.subcommands.mcp import build_mcp_parser

from gogeta_cli.subcommands.claw import build_claw_parser
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (gogeta tools, gogeta setup, gogeta model) use
    ...
    if not sys.stdin.isatty():
        ...
        print(
            ...
            f"Error: 'gogeta {command_name}' requires an interactive terminal.\n"
...
# Profile override — MUST happen before any gogeta module import.

#

# Many modules cache GOGETA_HOME at import time (module-level constants).

# We intercept --profile/-p from sys.argv here and set the env var so that

# every subsequent ``os.getenv("GOGETA_HOME", ...)`` resolves correctly.

# The flag is stripped from sys.argv so argparse never sees it.

# Falls back to ~/.gogeta/active_profile for sticky default.

# ---------------------------------------------------------------------------

def _apply_profile_override() -> None:

    """Pre-parse --profile/-p and set GOGETA_HOME before module imports."""
    ...
    # Mirrors gogeta_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If GOGETA_HOME is already set and no explicit flag was given, trust it

    # only when it already points to a specific profile directory.  The

    # distinguishing heuristic: a profile path has "profiles" as its immediate

    # parent directory name (e.g. ~/.gogeta/profiles/coder or

    # /opt/data/profiles/coder).  If GOGETA_HOME points to the gogeta root

    # instead (e.g. systemd hardcodes GOGETA_HOME=/root/.gogeta), we must

    # still read active_profile — the user may have switched profiles via

    # `gogeta profile use` and the gateway should honour that choice.

    # See issue #22502.

    gogeta_home_env = os.environ.get("GOGETA_HOME", "")

    if profile_name is None and gogeta_home_env:

        if Path(gogeta_home_env).parent.name == "profiles":

            return



    # 2. If no flag, check active_profile in the gogeta root

    if profile_name is None:

        try:

            from gogeta_constants import get_default_gogeta_root



            active_path = get_default_gogeta_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set GOGETA_HOME

    if profile_name is not None:

        try:

            from gogeta_cli.profiles import resolve_profile_env



            gogeta_home = resolve_profile_env(profile_name)
            ...
            # A bug in profiles.py must NEVER prevent gogeta from starting
        ...
        os.environ["GOGETA_HOME"] = gogeta_home
...
# Load .env from ~/.gogeta/.env first, then project root as dev fallback.

# User-managed env files should override stale shell exports on restart.

from gogeta_cli.config import get_gogeta_home

from gogeta_cli.env_loader import load_gogeta_dotenv



load_gogeta_dotenv(project_env=PROJECT_ROOT / ".env")



# Bridge security.redact_secrets from config.yaml → GOGETA_REDACT_SECRETS env

# var BEFORE gogeta_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_gogeta_home() / "config.yaml"
    ...
    if _cfg_path.exists():
        ...
        if "GOGETA_REDACT_SECRETS" not in os.environ:
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    ...
                    os.environ["GOGETA_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `gogeta` subcommands
...
try:
    ...
    from gogeta_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    ...
    try:
        ...
        from gogeta_constants import apply_ipv4_preference as _apply_ipv4
        ...
        pass  # best-effort — don't crash if gogeta_constants not importable yet
...
from gogeta_cli import __version__, __release_date__



# Provider model-selection wizard flows extracted to gogeta_cli/model_setup_flows.py

# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and

# existing test monkeypatches (gogeta_cli.main._model_flow_*) keep resolving unchanged.

from gogeta_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    ...
    return get_gogeta_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _termux_bundled_skills_sync_needed() -> bool:
    ...
    if os.environ.get("GOGETA_TERMUX_FORCE_SKILLS_SYNC") == "1":
...
def _termux_should_prefetch_update_check() -> bool:
    ...
    return os.environ.get("GOGETA_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from gogeta_cli.config import get_env_path, get_gogeta_home, load_config

    from gogeta_cli.auth import get_auth_status



    # Determine whether Gogeta itself has been explicitly configured (model
    ...
    from gogeta_cli.config import DEFAULT_CONFIG
    ...
    _has_gogeta_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    from gogeta_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_gogeta_home() / "auth.json"
    ...
    # Only count these if Gogeta has been explicitly configured — Claude Code

    # being installed doesn't mean the user wants Gogeta to use their tokens.

    if _has_gogeta_config:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    try:
        ...
        from gogeta_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        container_info: dict with backend, container_name, exec_user, gogeta_bin

        cli_args: the original CLI arguments (everything after 'gogeta')
    ...
    gogeta_bin = container_info["gogeta_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                ...
                print(
                    ...
                    f"Or run: sudo gogeta {' '.join(cli_args)}",
            ...
            print(
                ...
                f"The container may be running under root. Try: sudo gogeta {' '.join(cli_args)}",
...
def _print_tui_exit_summary(
    ...
    print(f"  gogeta --tui --resume {target}")

    if title:

        print(f'  gogeta --tui -c "{title}"')
...
def _tui_need_npm_install(root: Path) -> bool:
    ...
    """True when @gogeta/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@gogeta" / "ink" / "package.json"
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "packages/gogeta-ink/package.json",

    "packages/gogeta-ink/index.js",

    "packages/gogeta-ink/text-input.js",
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``GOGETA_TUI_FORCE_BUILD=1`` to force the old behaviour.

    """

    force = (os.environ.get("GOGETA_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``GOGETA_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    ...
    if os.environ.get("GOGETA_SKIP_NODE_BOOTSTRAP"):
    ...
    gogeta_home = os.environ.get("GOGETA_HOME") or str(Path.home() / ".gogeta")
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "GOGETA_HOME": gogeta_home},
    ...
    extras.extend([Path(gogeta_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(gogeta_cli_dir: Path | None = None) -> Path | None:

    """Find a pre-built TUI entry.js bundled in the wheel."""

    if gogeta_cli_dir is None:

        gogeta_cli_dir = Path(__file__).parent

    bundled = gogeta_cli_dir / "tui_dist" / "entry.js"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    """TUI: --dev → tsx src; else node dist (GOGETA_TUI_DIR prebuilt or esbuild)."""
    ...
    def _node_bin(bin: str) -> str:
        ...
        if bin == "node":
            ...
            env_node = os.environ.get("GOGETA_NODE")
        ...
        if not path and bin == "node":
            ...
            try:
                ...
                from gogeta_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("GOGETA_TUI_DIR")

    if tui_dev and ext_dir:

        print(

            f"Error: --dev is incompatible with GOGETA_TUI_DIR={ext_dir}\n"

            f"The prebuilt TUI has no source code to hot-reload.\n"

            f"Unset GOGETA_TUI_DIR (e.g. `unset GOGETA_TUI_DIR`) to use --dev from a checkout.",
    ...
    if (
        ...
        if not os.environ.get("GOGETA_QUIET"):
    ...
    if tui_dev:
        ...
        # Keep the local @gogeta/ink package exports in sync with source.

        # --dev runs src/entry.tsx directly, but @gogeta/ink resolves through

        # packages/gogeta-ink/dist/entry-exports.js. If that dist bundle is
        ...
        ink_dir = tui_dir / "packages" / "gogeta-ink"
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        ...
        from gogeta_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    try:
        ...
        from gogeta_cli.config import apply_terminal_config_to_env
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        ...
        prefix="gogeta-tui-active-session-", suffix=".json"

    )

    os.close(active_session_fd)

    env["GOGETA_TUI_ACTIVE_SESSION_FILE"] = active_session_file

    env["GOGETA_PYTHON_SRC_ROOT"] = os.environ.get(

        "GOGETA_PYTHON_SRC_ROOT", str(PROJECT_ROOT)

    )

    env.setdefault("GOGETA_PYTHON", sys.executable)

    env.setdefault("GOGETA_CWD", os.getcwd())
    ...
    if model:
        ...
        env["GOGETA_MODEL"] = model

        env["GOGETA_INFERENCE_MODEL"] = model

    if provider:

        env["GOGETA_TUI_PROVIDER"] = provider

        env["GOGETA_INFERENCE_PROVIDER"] = provider

    tui_toolsets = _normalize_tui_toolsets(toolsets)

    if tui_toolsets:

        env["GOGETA_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    ...
    if skills:
        ...
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                ...
                env["GOGETA_TUI_SKILLS"] = ",".join(flattened)
            ...
            if value:
                ...
                env["GOGETA_TUI_SKILLS"] = value

    if query:

        env["GOGETA_TUI_QUERY"] = query

    if image:

        env["GOGETA_TUI_IMAGE"] = image

    if checkpoints:

        env["GOGETA_TUI_CHECKPOINTS"] = "1"

    if pass_session_id:

        env["GOGETA_TUI_PASS_SESSION_ID"] = "1"

    if max_turns is not None:

        env["GOGETA_TUI_MAX_TURNS"] = str(max_turns)

    if verbose:

        env["GOGETA_TUI_TOOL_PROGRESS"] = "verbose"

    elif quiet:

        env["GOGETA_TUI_TOOL_PROGRESS"] = "off"

    if accept_hooks:

        env["GOGETA_ACCEPT_HOOKS"] = "1"
    ...
    # GOGETA_TUI_RESUME is an internal hand-off from the Python wrapper to the

    # Ink app.  Because we start from os.environ.copy(), an exported/stale value

    # in the user's shell would otherwise make a plain `gogeta --tui` try to
    ...
    # still set GOGETA_TUI_RESUME themselves.

    env.pop("GOGETA_TUI_RESUME", None)

    if resume_session_id:

        env["GOGETA_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `gogeta update` so
    ...
    if code == 42:
        ...
        from gogeta_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    """Pin the active kanban board into ``GOGETA_KANBAN_BOARD`` for the chat session.



    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls

    (``gogeta kanban …``) resolve the board on different paths: the env-pin if

    set, otherwise the global ``<root>/kanban/current`` file. A concurrent

    ``gogeta kanban boards switch`` from another session can flip the file
    ...
    if os.environ.get("GOGETA_KANBAN_BOARD"):

        return

    try:

        from gogeta_cli.kanban_db import get_current_board



        os.environ["GOGETA_KANBAN_BOARD"] = get_current_board()
...
def _sync_bundled_skills_quietly() -> None:
    ...
    """Seed ``~/.gogeta/skills/`` with the bundled skill library on first launch.



    Called from any CLI entrypoint that the user might use as their first

    interaction with Gogeta — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Gogeta still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``GOGETA_TUI=1`` → always TUI
    ...
    if getattr(args, "tui", False) or os.environ.get("GOGETA_TUI") == "1":
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        ...
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'gogeta sessions list' to see available sessions.")
    ...
    try:
        ...
        from gogeta_cli.xai_retirement import (
        ...
        from gogeta_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'gogeta doctor' for details.\033[0m\n\n")
    ...
    if not _has_any_provider_configured():
        ...
        print(
            ...
            "It looks like Gogeta isn't configured yet -- no API keys or providers found."

        )

        print()

        print("  Run:  gogeta setup")

        print()



        from gogeta_cli.setup import (
        ...
        print("You can run 'gogeta setup' at any time to configure.")
    ...
    if _termux_should_prefetch_update_check():
        ...
        try:
            ...
            from gogeta_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        ...
        os.environ["GOGETA_YOLO_MODE"] = "1"



    # --ignore-user-config: make load_cli_config() / load_config() skip the

    # user's ~/.gogeta/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "ignore_user_config", False):
        ...
        os.environ["GOGETA_IGNORE_USER_CONFIG"] = "1"
    ...
    if getattr(args, "ignore_rules", False):
        ...
        os.environ["GOGETA_IGNORE_RULES"] = "1"
    ...
    if getattr(args, "source", None):
        ...
        os.environ["GOGETA_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from gogeta_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from gogeta_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from gogeta_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        ...
        print("How will you use WhatsApp with Gogeta?")
    ...
    # has no creds.json.  Every subsequent `gogeta gateway` then paid a 30s
    ...
    session_dir = get_gogeta_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: gogeta gateway")
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `gogeta gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            ...
            print("    1. Start the gateway:  gogeta gateway")
            ...
            print("  Tip: Agent responses are prefixed with '⚕ Gogeta Agent'")
        ...
        print("  Or install as a service: gogeta gateway install")

    else:

        print("⚠ Pairing may not have completed. Run 'gogeta whatsapp' to try again.")
...
def cmd_setup(args):
    ...
    from gogeta_cli.setup_wizard import run as run_wizard
    run_wizard()
...
def cmd_postinstall(args):
    ...
    from gogeta_cli.config import stamp_install_method
    ...
    print("⚕ Gogeta post-install bootstrap")
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        ...
        try:
            ...
            from gogeta_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``gogeta model``) and the setup wizard
    ...
    from gogeta_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        ...
        config_provider or os.getenv("GOGETA_INFERENCE_PROVIDER") or "auto"
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                ...
                f"Unknown provider '{effective_provider}'. Check 'gogeta model' for "

                "available providers, or run 'gogeta doctor' to diagnose config "
    ...
    # PROVIDER_GROUPS in gogeta_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.gogeta/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    ...
    """Remove OPENAI_BASE_URL from ~/.gogeta/.env if the active provider is not 'custom'.
    ...
    from gogeta_cli.config import get_env_value, save_env_value, load_config
...
# Gogeta uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `gogeta model` provider picker. It does NOT re-run credential setup — it

# only routes already-authenticated providers to specific aux tasks. Users

# configure new providers through the normal `gogeta model` flow first.
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`gogeta_cli.plugins.PluginContext.register_auxiliary_task`) appear
    ...
    try:
        ...
        from gogeta_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    from gogeta_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Gogeta only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``gogeta model`` flow, then route aux tasks to them here.

    """

    from gogeta_cli.config import load_config

    from gogeta_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from gogeta_cli.auth import _prompt_model_selection

    from gogeta_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from gogeta_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    try:
        ...
        from gogeta_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    from gogeta_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        ...
        (
            ...
            "Use Gogeta URL heuristics; best for standard OpenAI-compatible endpoints.",
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        from gogeta_cli.curses_ui import curses_radiolist
...
# downstream call sites read `gogeta_cli.main._PROVIDER_MODELS` directly,

# so the symbol needs to be reachable as a module attribute. But importing

# the catalog eagerly costs ~55ms on every `gogeta` invocation — including

# fast paths like `gogeta --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        ...
        from gogeta_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    """Shared API-key entry point for ``gogeta setup`` / ``gogeta model``.
    ...
    recover from a malformed paste without editing ``~/.gogeta/.env`` by hand.
    ...
    from gogeta_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER

    from gogeta_cli.config import save_env_value
    ...
    from gogeta_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            ...
            f"  API key cleared.  Re-run `gogeta setup` to configure {pconfig.name} again."
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from gogeta_constants import display_gogeta_home as _dhh_fn



            print(

                f"    Gogeta will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
...
def cmd_login(args):
    ...
    """Authenticate Gogeta CLI with a provider."""

    from gogeta_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from gogeta_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from gogeta_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from gogeta_cli.status import show_status
...
def cmd_cron(args):
    ...
    from gogeta_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from gogeta_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``gogeta slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            ...
            "Run `gogeta slack manifest -h` for details.",
    ...
    if sub == "manifest":
        ...
        from gogeta_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from gogeta_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from gogeta_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from gogeta_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    """Dispatch `gogeta security <subcmd>`."""

    sub = getattr(args, "security_command", None)

    if sub in ("audit", None):

        from gogeta_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from gogeta_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from gogeta_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from gogeta_cli.config import config_command
...
def cmd_backup(args):
    ...
    """Back up Gogeta home directory to a zip file."""

    if getattr(args, "quick", False):

        from gogeta_cli.backup import run_quick_backup
        ...
        from gogeta_cli.backup import run_backup
...
def cmd_import(args):
    ...
    """Restore a Gogeta backup from a zip file."""

    from gogeta_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    from gogeta_cli.banner import format_banner_version_label
    ...
    try:
        ...
        from gogeta_cli.banner import check_for_updates

        from gogeta_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    """Uninstall Gogeta Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        ...
        from gogeta_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from gogeta_cli.uninstall import run_gui_uninstall
    ...
    from gogeta_cli.uninstall import run_uninstall
...
# Critical files that every ``gogeta`` invocation imports at startup. If any

# of these fail to parse after a pull, the CLI is bricked — the user can't

# even run ``gogeta update`` again to roll forward. The post-pull syntax
...
_UPDATE_CRITICAL_FILES = (
    ...
    "gogeta_cli/config.py",

    "gogeta_cli/__init__.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``gogeta`` startup; if any of them
    ...
    with tempfile.TemporaryDirectory(prefix="gogeta-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``gogeta update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``gogeta_cli/web_dist/`` (per vite.config.ts

    outDir: "../gogeta_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "gogeta_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``gogeta`` launcher present but ``gogeta_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    1. Fast path — the gogeta venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — gogeta venv python3, no nix-shell overhead
    ...
    # a gogeta venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``gogeta update`` to stash the
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               ...
               instead of a soft warning (used by ``gogeta web``).
    ...
    # build path usable on a stock `py -m gogeta_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        ...
        _say(
            ...
            + ("" if fatal else " (gogeta web will not be available)")
...
#   - ``gogeta update`` can unconditionally call ``gogeta desktop --build-only``

#     and it will skip if nothing actually changed

#   - ``gogeta desktop`` (interactive launch) skips the build when the

#     stamp matches, making repeated launches fast

#

# Stamp file: $GOGETA_HOME/desktop-build-stamp.json
...
def _desktop_stamp_path() -> Path:
    ...
    """Return the path to the desktop build stamp file under $GOGETA_HOME."""

    from gogeta_constants import get_gogeta_home

    return get_gogeta_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``gogeta update`` that pulled new source but hasn't built yet).
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        ...
        candidates = list(release_dir.glob("mac*/Gogeta.app/Contents/MacOS/Gogeta"))

    elif sys.platform == "win32":

        candidates = [

            release_dir / "win-unpacked" / "Gogeta.exe",

            release_dir / "win-ia32-unpacked" / "Gogeta.exe",

            release_dir / "win-arm64-unpacked" / "Gogeta.exe",
        ...
        candidates = [
            ...
            release_dir / "linux-unpacked" / "gogeta",

            release_dir / "linux-unpacked" / "Gogeta",

            release_dir / "linux-arm64-unpacked" / "gogeta",
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Gogeta'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Gogeta.exe`` keeps an exclusive lock on

    ``release/win-unpacked/Gogeta.exe``. electron-builder's pack then can't

    delete the stale binary and dies with ``remove …\\Gogeta.exe: Access is
    ...
    an unrelated "Gogeta" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Gogeta is damaged and can't be opened." The
    ...
    # exe = .../Gogeta.app/Contents/MacOS/Gogeta  ->  app bundle = .../Gogeta.app
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        ...
        print(f"✗ Gogeta Desktop is missing Electron's Linux sandbox helper: {sandbox}")
    ...
    if not sudo:
        ...
        print("✗ Gogeta Desktop requires sudo to configure Electron's Linux sandbox helper.")
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        ...
        from gogeta_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        ...
        env["GOGETA_DESKTOP_BOOT_FAKE"] = "1"

    if getattr(args, "ignore_existing", False):

        env["GOGETA_DESKTOP_IGNORE_EXISTING"] = "1"

    if getattr(args, "gogeta_root", None):

        env["GOGETA_DESKTOP_ROOT"] = str(Path(args.gogeta_root).expanduser().resolve())

    if getattr(args, "cwd", None):

        env["GOGETA_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  gogeta gui")
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Gogeta.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # on the final `electron` -> `Gogeta` rename: unpack-electron
                ...
                if purged:
                    ...
                    # The purge can't remove a win-unpacked tree whose Gogeta.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    ...
                    print("  If this says \"Access is denied\" on Gogeta.exe, close any")

                    print("  running Gogeta desktop window and retry.")

                print("  If the log shows Electron download retries, rebuild via a mirror:")

                print("    ELECTRON_MIRROR=<mirror-base-url> gogeta desktop --force-build")
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Gogeta is
    ...
    if source_mode:
        ...
        print("→ Launching Gogeta Desktop from source build...")
    ...
    print(f"→ Launching packaged Gogeta Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    ...
    *,
    ...
    """Return PIDs of ``gogeta dashboard`` processes other than ourselves.



    ``gogeta dashboard`` is a long-lived server process commonly started and

    forgotten.  When ``gogeta update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    This is used by the Gogeta Desktop Electron app to protect its own

    backend child process: when the desktop spawns ``gogeta dashboard`` as
    ...
    environment variable ``GOGETA_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        "gogeta_cli.main dashboard",

        "gogeta_cli/main.py dashboard",
    ...
    try:
        ...
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "gogeta.*dashboard"`) keeps us consistent

            # with `gogeta_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    ...
    """Print a short heads-up about the skill curator after `gogeta update`.
    ...
    print("  Preview now:  gogeta curator run --dry-run")

    print("  Pause it:     gogeta curator pause")

    print(

        "  Docs:         https://gogeta.ai/docs/user-guide/features/curator"
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``gogeta update`` is a high-attention surface — surface the
    ...
    Subsequent ``gogeta update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: gogeta curator status)"
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``gogeta dashboard`` processes.



    Called at the end of ``gogeta update`` (default ``reason``) and also

    from ``gogeta dashboard --stop`` (which overrides ``reason``).  The
    ...
    # When the Gogeta Desktop Electron app spawns this dashboard as a

    # backend child, it sets GOGETA_DESKTOP_CHILD_PID so that the update

    # path can skip killing the desktop-managed process.  (#37532)

    exclude: set[int] | None = None

    raw_pid = os.environ.get("GOGETA_DESKTOP_CHILD_PID")
    ...
    if killed:
        ...
        print("    gogeta dashboard --port <port>")
...
def _update_via_zip(args):
    ...
    """Update Gogeta Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `gogeta update "

            f"--branch {branch}`, or update against main with `gogeta update`."
    ...
    zip_url = (
        ...
        f"https://github.com/gogeta/gogeta-agent/archive/refs/heads/{branch}.zip"
    ...
    tmp_dir = tempfile.mkdtemp(prefix="gogeta-update-")

    try:

        zip_path = os.path.join(tmp_dir, f"gogeta-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for gogeta-agent itself
        ...
        # GitHub ZIPs extract to gogeta-agent-<branch>/

        extracted = os.path.join(tmp_dir, f"gogeta-agent-{branch}")
    ...
    from gogeta_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        from gogeta_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        ...
        "gogeta-update-autostash-%Y%m%d-%H%M%S"
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Gogeta behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes gogeta completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        ...
        print(
            ...
            "⚠ Local changes were restored, but Gogeta couldn't find the stash entry to drop."
        ...
        if drop.returncode != 0:
            ...
            print(
                ...
                "⚠ Local changes were restored, but Gogeta couldn't drop the saved stash entry."
    ...
    print("  Review `git diff` / `git status` if Gogeta behaves unexpectedly.")
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        ...
        print(
            ...
            "but Gogeta couldn't find the stash entry to drop."
    ...
    if drop.returncode != 0:
        ...
        print(
            ...
            "⚠ Configured to discard local changes, but Gogeta couldn't drop "
...
# Fork detection and upstream management for `gogeta update`
...
OFFICIAL_REPO_URLS = {
    ...
    "https://github.com/gogeta/gogeta-agent.git",

    "git@github.com:gogeta/gogeta-agent.git",

    "https://github.com/gogeta/gogeta-agent",

    "git@github.com:gogeta/gogeta-agent",

}

OFFICIAL_REPO_URL = "https://github.com/gogeta/gogeta-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_gogeta_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_gogeta_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("ℹ Your fork is not tracking the official Gogeta repository.")

        print("  This means you may miss updates from gogeta/gogeta-agent.")
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                ...
                print(
                    ...
                    "  ✓ Added upstream: https://github.com/gogeta/gogeta-agent.git"
            ...
            print(
                ...
                "  Skipped. Run 'git remote add upstream https://github.com/gogeta/gogeta-agent.git' to add later."
...
def _invalidate_update_cache():
    ...
    ``gogeta update``, every profile is now current.
    ...
    default_home = get_default_gogeta_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``gogeta update`` is still progressing even if pip/uv itself is silent.
...
def _gogeta_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    return [
        ...
        scripts_dir / "gogeta.exe",

        scripts_dir / "gogeta-gateway.exe",
...
def _detect_concurrent_gogeta_instances(
    ...
    The Gogeta Desktop Electron app spawns ``gogeta.EXE`` as a backend child,

    so during ``gogeta update`` the user-invoked process and the desktop's
    ...
    shims (``gogeta.exe`` / ``gogeta-gateway.exe``) and returns ``(pid,

    process_name)`` pairs. The caller's own PID and its entire ancestor

    chain are excluded so the running ``gogeta update`` invocation never

    reports itself — this matters on Windows where the setuptools .exe

    launcher (``gogeta.exe``) is a separate process from the Python
    ...
    for shim in _gogeta_exe_shims(scripts_dir):
    ...
    # setuptools-generated gogeta.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``gogeta update``
    ...
    #      gogeta.exe sitting *under* a non-Gogeta parent (e.g. a Gogeta
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "gogeta.exe"

    lines = ["✗ Another gogeta.exe is running:"]
    ...
    lines.append("  Close Gogeta Desktop, exit any open `gogeta` REPLs, and")

    lines.append("  stop the gateway (`gogeta gateway stop`) before retrying.")
    ...
    lines.append("  Override with `gogeta update --force` if you've already")
...
def _quarantine_running_gogeta_exe(

    scripts_dir: Path, *, max_attempts: int = 4

) -> list[tuple[Path, Path]]:

    """Pre-empt Windows file lock on the running ``gogeta.exe``.
    ...
    when ``gogeta update`` runs, ``gogeta.exe`` IS the live process, and uv

    fails with ``Access is denied. (os error 5)``.



    We rename live shims to ``gogeta.exe.old.<unix-ms>`` first. uv then writes

    fresh shims at the original paths. The ``.old`` files are cleaned up on

    the next gogeta invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Gogeta Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       ...
       Gogeta Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _gogeta_exe_shims(scripts_dir):
        ...
        print(
            ...
            "    Close Gogeta Desktop, exit other `gogeta` REPLs, stop the "

            "gateway, or pause AV scanning, then re-run `gogeta update`."
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    """Roll back ``_quarantine_running_gogeta_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``gogeta.exe`` first.



    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point

    shims, and on Windows the live ``gogeta.exe`` is the running process —

    pip can neither delete nor overwrite it, so without quarantine the shim

    is left missing and ``gogeta`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        ...
        moved = _quarantine_running_gogeta_exe(scripts_dir)
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    ...
    """Sweep ``gogeta.exe.old.*`` left by prior updates.



    Called early on every gogeta invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``gogeta update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `gogeta update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``gogeta.exe`` / ``gogeta-gateway.exe`` shims

    in the venv Scripts dir before each install attempt so uv can write fresh

    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See

    ``_quarantine_running_gogeta_exe`` for the rationale.
    ...
    # hours later inside ``gogeta update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``gogeta update``, not the venv we just wrote
    ...
    # Quarantine the running ``gogeta.exe`` first: ``--reinstall -e .``

    # rewrites the entry-point shims, and on Windows pip can't overwrite the

    # live launcher, which would leave ``gogeta`` off PATH.
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `gogeta update` output above.")
    ...
    try:
        ...
        print(
            ...
            "Run `gogeta update --force` after closing other gogeta processes."
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from gogeta_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$GOGETA_HOME/bin/uv``, but on Termux the official
    ...
    from gogeta_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `gogeta update`, so we install root-only first
...
class _UpdateOutputStream:
    ...
    """Stream wrapper used during ``gogeta update`` to survive terminal loss.
    ...
    * Every write is also mirrored to an append-only log file
      ...
      (``~/.gogeta/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``gogeta update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``gogeta update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ...
       ``~/.gogeta/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``gogeta update --gateway``) the update is already
    ...
    try:
        ...
        # gogeta_cli.config.get_gogeta_home to simulate setup failure.

        from gogeta_cli.config import get_gogeta_home as _get_gogeta_home



        logs_dir = _get_gogeta_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    """Implement ``gogeta update --check``: fetch and report without installing.
    ...
    from gogeta_cli.config import detect_install_method
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``gogeta update`` (apply

        # path) uses — telling the user to "reinstall via curl" or that

        # ".git is missing" would point them at the wrong remediation.

        from gogeta_cli.config import format_docker_update_message
    ...
    if method == "pip":
        ...
        from gogeta_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``gogeta update`` without requiring a reinstall.  The
    ...
    ``gogeta`` prints ``command not found`` even though the symlink lives

    at /usr/local/bin/gogeta.



    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system

    where ``bash -i -c 'command -v gogeta'`` already resolves.  Idempotent.
    ...
    # /usr/local/bin/gogeta, code at /usr/local/lib/gogeta-agent).

    fhs_link = Path("/usr/local/bin/gogeta")
    ...
    path_comment = (
        ...
        "# Gogeta Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
...
def _run_pre_update_backup(args) -> None:
    ...
    """Create a full zip backup of GOGETA_HOME before running the update.
    ...
    GOGETA_HOME directories.  The ``--backup`` flag on ``gogeta update``
    ...
    try:
        ...
        from gogeta_cli.backup import create_pre_update_backup
    ...
    # Render path using display_gogeta_home so the user sees ~/.gogeta/...

    try:

        from gogeta_constants import get_gogeta_home, display_gogeta_home
        ...
        try:
            ...
            display_path = f"{display_gogeta_home()}/{out_path.relative_to(home)}"
    ...
    print(f"  Restore:  gogeta import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``gogeta update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    ...
    """Update Gogeta Agent to the latest version.
    ...
    if is_managed():
        ...
        managed_error("update Gogeta Agent")
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `gogeta update --branch=<x>` would actually pull.
...
def _cmd_update_pip(args):
    ...
    """Update Gogeta via pip (for PyPI installs)."""

    from gogeta_cli import __version__

    from gogeta_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        ...
        cmd = [uv, "tool", "upgrade", "gogeta-agent"]
        ...
        cmd = [pipx, "upgrade", "gogeta-agent"]

    elif uv:

        cmd = [uv, "pip", "install", "--upgrade", "gogeta-agent"]
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "gogeta-agent"]
    ...
    print("✓ Update complete! Restart gogeta to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Gogeta Agent...")

    print()



    # On Windows, abort early if another gogeta.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            ...
            concurrent = _detect_concurrent_gogeta_instances(scripts_dir)
    ...
    try:
        ...
        # though `git pull` can't touch $GOGETA_HOME, this is cheap
        ...
        try:
            ...
            from gogeta_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in gogeta_cli/config.py bricked

        # every user who ran ``gogeta update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``gogeta
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``gogeta update`` again later once a fix lands.")
        ...
        # the old bytecode (e.g. get_gogeta_home added to gogeta_constants).
        ...
        # build.  ``gogeta desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``gogeta desktop`` shouldn't be forced into a full

        # Electron build by ``gogeta update``.
        ...
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            ...
            _desktop_build_cmd = [sys.executable, "-m", "gogeta_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                ...
                print("  ⚠ Desktop build failed (non-fatal; run `gogeta desktop` to retry)")
        ...
        # current — so copy it straight over ~/.gogeta/cache/model_catalog.json
        ...
        # modules in this process.  Reload gogeta_constants so that any lazy

        # import executed below (skills sync, gateway restart) sees new

        # attributes like display_gogeta_home() added since the last release.

        try:

            import importlib

            import gogeta_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit GOGETA_HOME so

        # it is not affected by sync_skills()'s module-level GOGETA_HOME cache,

        # which means the active profile is reliably synced regardless of whether

        # the caller's GOGETA_HOME env var points at the default or a named profile.

        try:

            from gogeta_cli.profiles import (
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'gogeta config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    ...
                    print("  ℹ API keys require manual entry: gogeta config migrate")

            else:

                print()

                print("Skipped. Run 'gogeta config migrate' later to configure.")
        ...
        try:
            ...
            from gogeta_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `gogeta curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``gogeta update`` gives users a predictable
        ...
        try:
            ...
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                ...
                from gogeta_cli.tools_config import install_cua_driver
        ...
        # When running as ``gogeta update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            ...
            _exit_code_path = get_gogeta_home() / ".update_exit_code"
        ...
        try:
            ...
            from gogeta_cli.gateway import (
            ...
            # Discover all gogeta-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            ]  # e.g. gogeta-gateway.service or gogeta-gateway-coder.service
                            ...
                            # path in `gogeta gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: gogeta gateway run")

                    if unmapped_count > 1:

                        print(

                            "    (or: gogeta -p <profile> gateway run  for each profile)"
        ...
        # Warn if legacy Gogeta gateway unit files are still installed.

        # When both gogeta.service (from a pre-rename install) and the

        # current gogeta-gateway.service are enabled, they SIGTERM-fight

        # for the same bot token (see PR #11909). Flagging here means

        # every `gogeta update` surfaces the issue until the user migrates.

        try:

            from gogeta_cli.gateway import (

                has_legacy_gogeta_units,

                _find_legacy_gogeta_units,
            ...
            if supports_systemd_services() and has_legacy_gogeta_units():

                print()

                print("⚠ Legacy Gogeta gateway unit(s) detected:")

                for name, path, is_sys in _find_legacy_gogeta_units():
                ...
                print("  These pre-rename units (gogeta.service) fight the current")

                print("  gogeta-gateway.service for the bot token and cause SIGTERM")

                print("  flap loops. Remove them with:")

                print()

                print("    gogeta gateway migrate-legacy")
        ...
        print("  gogeta model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``gogeta -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from gogeta_constants import display_gogeta_home
    ...
    if action is None:
        ...
        # Bare `gogeta profile` — show current profile status

        profile_name = get_active_profile_name()

        dhh = display_gogeta_home()
        ...
        for p in profiles:
            ...
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → gogeta -p {p.name}")
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                ...
                print(f"Switched to: default (~/.gogeta)")
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        ...
                        f"  Choose a custom alias:  gogeta profile alias {name} --name <custom>"

                    )

                    print(f"  Or access via flag:     gogeta -p {name} chat")
        ...
        from gogeta_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            ...
            try:
                ...
                if _profiles_mod.normalize_profile_name(name) == "default":
                    ...
                    from gogeta_constants import get_gogeta_home as _hh
        ...
        from gogeta_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `gogeta profile info {name}` for full manifest)")
        ...
        if alias_name:
            ...
            print(f"Alias:   {alias_name} → gogeta -p {name}  ({wrapper})")
        ...
        from gogeta_cli.profiles import profile_exists
        ...
        from gogeta_cli.profiles import rename_profile
        ...
        from gogeta_cli.profiles import export_profile
        ...
        from gogeta_cli.profiles import import_profile
        ...
        from gogeta_cli.profile_distribution import (
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="gogeta_dist_preview_") as tmp:
            ...
            if plan.has_cron:
                ...
                print(
                    ...
                    f"  Review them with:  gogeta -p {plan.manifest.name} cron list"

                )

            print(f"\n  Use with:      gogeta -p {plan.manifest.name} chat")
        ...
        from gogeta_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                ...
                print(
                    ...
                    "Only profiles installed via `gogeta profile install` can be updated."
            ...
            if plan.has_cron:
                ...
                print(
                    ...
                    f"gogeta -p {plan.manifest.name} cron list"
        ...
        from gogeta_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("gogeta_requires"):

            print(f"Requires:     Gogeta {data['gogeta_requires']}")
...
def _render_distribution_plan(plan) -> None:
    ...
    from gogeta_cli.profile_distribution import MANIFEST_FILENAME
    ...
    if mf.gogeta_requires:

        print(f"  Requires: Gogeta {mf.gogeta_requires}")
...
def _report_dashboard_status() -> int:
    ...
    """Print ``gogeta dashboard`` PIDs and return the count.



    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the

    current process is excluded, but since ``gogeta dashboard --status``
    ...
    if not pids:
        ...
        print("No gogeta dashboard processes running.")

        return 0



    print(f"{len(pids)} gogeta dashboard process(es) running:")
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        # Reuse the same SIGTERM-grace-SIGKILL path used after `gogeta update`.
    ...
    # the same logs directory as every other Gogeta surface.
    ...
    if "GOGETA_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            Path(os.environ["GOGETA_WEB_DIST"])

            if "GOGETA_WEB_DIST" in os.environ

            else PROJECT_ROOT / "gogeta_cli" / "web_dist"
    ...
    try:
        ...
        from gogeta_cli.plugins import discover_plugins
    ...
    from gogeta_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from gogeta_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_completion(args, parser=None):
    ...
    from gogeta_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from gogeta_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    """View and filter Gogeta log files."""

    from gogeta_cli.logs import tail_log, list_logs
...
# so that in ``gogeta -m gpt5 chat``, ``gpt5`` is correctly skipped as a

# flag value rather than misclassified as a subcommand. Kept in sync with

# the top-level flags declared in ``gogeta_cli/_parser.py``.
...
def _first_positional_argv() -> str | None:
    ...
    ``gogeta -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        ...
        # Bare ``gogeta`` or only flags → defaults to ``chat``.
...
def _is_tui_chat_launch(args) -> bool:
    ...
    return bool(getattr(args, "tui", False) or os.environ.get("GOGETA_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            ...
            from gogeta_cli.mcp_startup import start_background_mcp_discovery
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from gogeta_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from gogeta_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["GOGETA_DEFER_AGENT_STARTUP"] = "1"

            os.environ["GOGETA_FAST_STARTUP_BANNER"] = "1"

            if getattr(args, "accept_hooks", False):

                os.environ["GOGETA_ACCEPT_HOOKS"] = "1"
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `gogeta --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_gogeta_home() / "memories"
        ...
        if not existing:
            ...
            print(
                ...
                f"\n  Nothing to reset — no memory files found in {display_gogeta_home()}/memories/\n"
        ...
        print(f"  Files were in: {display_gogeta_home()}/memories/\n")

    else:

        from gogeta_cli.memory_setup import memory_command
...
def cmd_acp(args):
    ...
    """Launch Gogeta Agent as an ACP server."""
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        ...
        from gogeta_cli.tools_config import tools_disable_enable_command
        ...
        from gogeta_cli.tools_config import run_post_setup_command
        ...
        from gogeta_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from gogeta_cli.skills_config import skills_command as skills_config_command
        ...
        from gogeta_cli.skills_hub import skills_command
...
def cmd_pairing(args):
    ...
    from gogeta_cli.pairing import pairing_command
...
def cmd_plugins(args):
    ...
    from gogeta_cli.plugins_cmd import plugins_command
...
def cmd_mcp(args):
    ...
    from gogeta_cli.mcp_config import mcp_command
...
def cmd_claw(args):
    ...
    from gogeta_cli.claw import claw_command
...
def main():
    ...
    """Main entry point for gogeta CLI."""

    # Cosmetic: make the process show up as 'gogeta' instead of 'python3.11'
    ...
    try:
        ...
        from gogeta_cli.stdio import configure_windows_stdio
    ...
    # Sweep stale ``gogeta.exe.old.*`` quarantine files left by previous

    # ``gogeta update`` runs on Windows. Silent no-op on non-Windows or when

    # there's nothing to clean. See ``_quarantine_running_gogeta_exe``.
    ...
    # model command  (parser built in gogeta_cli/subcommands/model.py)
    ...
    from gogeta_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://gogeta.ai/docs/user-guide/features/fallback-providers"
    ...
    fallback_subparsers.add_parser(
        ...
        help="Pick a provider + model (same picker as `gogeta model`) and append to the chain",
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.gogeta/.env.  Currently supports "

            "Bitwarden Secrets Manager.  See: "

            "https://gogeta.ai/docs/user-guide/secrets/bitwarden"
    ...
    from gogeta_cli import secrets_cli as _secrets_cli
    ...
    from gogeta_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # gateway + proxy commands  (parsers built in gogeta_cli/subcommands/gateway.py)
    ...
    # setup command  (parser built in gogeta_cli/subcommands/setup.py)
    ...
    # postinstall command  (parser built in gogeta_cli/subcommands/postinstall.py)
    ...
    # whatsapp command  (parser built in gogeta_cli/subcommands/whatsapp.py)
    ...
    # slack command  (parser built in gogeta_cli/subcommands/slack.py)
    ...
    from gogeta_cli.send_cmd import register_send_subparser
    ...
    # login command  (parser built in gogeta_cli/subcommands/login.py)
    ...
    # logout command  (parser built in gogeta_cli/subcommands/logout.py)
    ...
    # auth command  (parser built in gogeta_cli/subcommands/auth.py)
    ...
    # status command  (parser built in gogeta_cli/subcommands/status.py)
    ...
    # cron command  (parser built in gogeta_cli/subcommands/cron.py)
    ...
    # webhook command  (parser built in gogeta_cli/subcommands/webhook.py)
    ...
    from gogeta_cli.portal_cli import add_parser as _add_portal_parser
    ...
    from gogeta_cli.kanban import build_parser as _build_kanban_parser
    ...
    # hooks command  (parser built in gogeta_cli/subcommands/hooks.py)
    ...
    # doctor command  (parser built in gogeta_cli/subcommands/doctor.py)
    ...
    # security command  (parser built in gogeta_cli/subcommands/security.py)
    ...
    # dump command  (parser built in gogeta_cli/subcommands/dump.py)
    ...
    # debug command  (parser built in gogeta_cli/subcommands/debug.py)
    ...
    # backup command  (parser built in gogeta_cli/subcommands/backup.py)
    ...
    checkpoints_parser = subparsers.add_parser(
        ...
        help="Inspect / prune / clear ~/.gogeta/checkpoints/",

        description="Manage the filesystem checkpoint store — the shadow git "

        "repo gogeta uses to snapshot working directories before "
    ...
    from gogeta_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    # import command  (parser built in gogeta_cli/subcommands/import_cmd.py)
    ...
    # config command  (parser built in gogeta_cli/subcommands/config.py)
    ...
    # pairing command  (parser built in gogeta_cli/subcommands/pairing.py)
    ...
    # skills command  (parser built in gogeta_cli/subcommands/skills.py)
    ...
    from gogeta_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    # plugins command  (parser built in gogeta_cli/subcommands/plugins.py)
    ...
    # subcommand — ``gogeta --help``, ``gogeta version``, ``gogeta logs``,
    ...
    if _plugin_cli_discovery_needed():
        ...
        try:
            ...
            from gogeta_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        ...
        from gogeta_cli.curator import register_cli as _register_curator_cli
    ...
    # memory command  (parser built in gogeta_cli/subcommands/memory.py)
    ...
    # tools command  (parser built in gogeta_cli/subcommands/tools.py)
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `gogeta computer-use install` to fetch and run the\n"

            "upstream cua-driver installer. This is equivalent to the\n"

            "post-setup hook that `gogeta tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: gogeta computer-use install --upgrade")

                return

            print("cua-driver: not installed")

            print("  Run: gogeta computer-use install")
    ...
    # mcp command  (parser built in gogeta_cli/subcommands/mcp.py)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            sessions_dir = get_gogeta_home() / "sessions"
            ...
            # Launch gogeta --resume <id> by replacing the current process
    ...
    # insights command  (parser built in gogeta_cli/subcommands/insights.py)
    ...
    # claw command  (parser built in gogeta_cli/subcommands/claw.py)
    ...
    # version command  (parser built in gogeta_cli/subcommands/version.py)
    ...
    # update command  (parser built in gogeta_cli/subcommands/update.py)
    ...
    # uninstall command  (parser built in gogeta_cli/subcommands/uninstall.py)
    ...
    # acp command  (parser built in gogeta_cli/subcommands/acp.py)
    ...
    # profile command  (parser built in gogeta_cli/subcommands/profile.py)
    ...
    # dashboard command  (parser built in gogeta_cli/subcommands/dashboard.py)
    ...
    # for one release. The Gogeta-Setup.exe success screen tells users to

    # run `gogeta desktop` from a terminal, so the canonical name needs
    ...
    # gui command  (parser built in gogeta_cli/subcommands/gui.py)
    ...
    # logs command  (parser built in gogeta_cli/subcommands/logs.py)
    ...
    # prompt-size command  (parser built in gogeta_cli/subcommands/prompt_size.py)
    ...
    # e.g. ``gogeta -c Pokemon Agent Dev`` → ``gogeta -c 'Pokemon Agent Dev'``
    ...
    from gogeta_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'gogeta -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (gogeta hooks list, cron
