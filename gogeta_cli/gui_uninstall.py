"""
Gogeta Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``gogeta desktop`` / ``gogeta gui``)
     Built inside the agent checkout under ``$GOGETA_HOME/gogeta-agent/``:
       ...
       - ``$GOGETA_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Gogeta.app`` or ``~/Applications/Gogeta.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Gogeta`` (NSIS per-user)
...
the app name ("Gogeta"), separate from ``$GOGETA_HOME``:
  - macOS:   ``~/Library/Application Support/Gogeta``
  - Windows: ``%APPDATA%\\Gogeta``
  - Linux:   ``$XDG_CONFIG_HOME/Gogeta`` (default ``~/.config/Gogeta``)
...
``gogeta uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(gogeta_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return gogeta_home / "gogeta-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Gogeta"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Gogeta"
...
def source_built_gui_artifacts(gogeta_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``gogeta desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``gogeta-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(gogeta_home)
    ...
    return [
        ...
        gogeta_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Gogeta" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Gogeta.app"),
            home / "Applications" / "Gogeta.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Gogeta).
            local_base / "Programs" / "Gogeta",
            # Older / alternate layout some builds used.
            local_base / "gogeta-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Gogeta")
    else:
        ...
        paths += [
            data_base / "applications" / "gogeta.desktop",
            data_base / "applications" / "Gogeta.desktop",
        ]
    return paths


def agent_is_installed(gogeta_home: Path) -> bool:
    """Return True when a usable Python agent install exists under GOGETA_HOME.
    ...
    if (agent_root / "gogeta_cli").is_dir():
        return True
...
def gui_is_installed(gogeta_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(gogeta_home):
        if p.exists():
            return True
...
def gui_install_summary(gogeta_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = gogeta_home if gogeta_home is not None else get_gogeta_home()
...
def uninstall_gui(gogeta_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``gogeta-agent/gogeta_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$GOGETA_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove gogeta' or "
            "'sudo dnf remove gogeta'). AppImage builds are a single file you "
