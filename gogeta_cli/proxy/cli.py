"""CLI handlers for the ``gogeta proxy`` subcommand."""
...
from gogeta_cli.proxy.adapters import ADAPTERS, get_adapter
from gogeta_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "gogeta proxy requires aiohttp. Install one of:\n"
        "  pip install 'gogeta-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"gogeta auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Gogeta proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Gogeta proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: gogeta proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``gogeta proxy <subcommand>``."""
    ...
    print(
        "gogeta proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  gogeta proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  gogeta proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  gogeta proxy providers\n"
