"""Migrate Gogeta' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Gogeta' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  ...
  Gogeta mcp_servers.<n>.command/args/env  → codex stdio transport

  Gogeta mcp_servers.<n>.url/headers       → codex streamable_http transport

  Gogeta mcp_servers.<n>.timeout           → codex tool_timeout_sec

  Gogeta mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec



What does NOT translate (warned + skipped):

  Gogeta-specific keys (sampling, etc.) — codex's MCP client has no
...
What's NOT migrated (intentional):
  ...
  AGENTS.md — codex respects this file natively in its cwd. Gogeta' own
...
MIGRATION_MARKER = (
    ...
    "# managed by gogeta-agent — `gogeta codex-runtime migrate` regenerates this section"

)

MIGRATION_END_MARKER = (

    "# end gogeta-agent managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Gogeta config.")
...
# Gogeta keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    ...
    # Gogeta' sampling subsection — codex MCP has no equivalent
...
def _translate_one_server(
    ...
    name: str, gogeta_cfg: dict

) -> tuple[Optional[dict], list[str]]:

    """Translate one Gogeta MCP server config to the codex inline-table dict
    ...
    if not isinstance(gogeta_cfg, dict):
    ...
    has_command = bool(gogeta_cfg.get("command"))

    has_url = bool(gogeta_cfg.get("url"))
    ...
    if has_command:
        ...
        out["command"] = str(gogeta_cfg["command"])

        args = gogeta_cfg.get("args") or []

        if args:

            out["args"] = [str(a) for a in args]

        env = gogeta_cfg.get("env") or {}
        ...
        out["url"] = str(gogeta_cfg["url"])

        headers = gogeta_cfg.get("headers") or {}

        if headers:

            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}

        # Gogeta' transport: sse hint is informational; codex auto-negotiates

        if gogeta_cfg.get("transport") == "sse":
    ...
    if "timeout" in gogeta_cfg:

        try:

            out["tool_timeout_sec"] = float(gogeta_cfg["timeout"])

        except (TypeError, ValueError):

            skipped.append("timeout (not numeric)")

    if "connect_timeout" in gogeta_cfg:

        try:

            out["startup_timeout_sec"] = float(gogeta_cfg["connect_timeout"])
    ...
    if gogeta_cfg.get("enabled") is False:
    ...
    for key in gogeta_cfg:

        if key in _KEYS_DROPPED_WITH_WARNING:

            skipped.append(f"{key} (no codex equivalent)")

        elif key not in _KNOWN_GOGETA_KEYS:

            skipped.append(f"{key} (unknown Gogeta key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (GOGETA_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        ...
        out.append("# (no MCP servers, plugins, or permissions configured by Gogeta)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    ...
    """Insert Gogeta' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Gogeta' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Gogeta
...
def _query_codex_plugins(
    ...
    try:
        ...
        with CodexAppServerClient(
            ...
            client.initialize(client_name="gogeta-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a GOGETA_HOME pointing

    at one of those paths is burned into ``~/.codex/config.toml``, every

    codex-routed gogeta-tools call fails silently once the directory is GC'd.



    We err on the side of refusing — losing a (very unlikely) real

    ``~/.gogeta`` symlink that happens to live under ``/private/var/folders``
...
def _build_gogeta_tools_mcp_entry() -> dict:

    """Build the codex stdio-transport entry that launches Gogeta' own
    ...
    so a gogeta installed under /opt/, /usr/local/, or a venv all work.

    GOGETA_HOME and PYTHONPATH are passed through so the spawned process
    ...
    # GOGETA_HOME passes through IF SET so the MCP subprocess sees the same

    # config / auth / sessions DB as the parent CLI. Read from os.environ

    # (not get_gogeta_home()) on purpose: when the env var is unset we want

    # codex's subprocess to inherit whatever GOGETA_HOME its launcher sets
    ...
    # — that would override the launcher's GOGETA_HOME and pin the subprocess
    ...
    # a sibling test's monkeypatch.setenv("GOGETA_HOME", tmp_path) would
    ...
    gogeta_home = os.environ.get("GOGETA_HOME") or ""

    if gogeta_home and _looks_like_test_tempdir(gogeta_home):

        gogeta_home = ""

    if gogeta_home:

        env["GOGETA_HOME"] = gogeta_home

    # PYTHONPATH passes through so a worktree-launched gogeta finds the
    ...
    env["GOGETA_REDACT_SECRETS"] = env.get("GOGETA_REDACT_SECRETS", "true")
    ...
    out: dict[str, Any] = {
        ...
        "args": ["-m", "agent.transports.gogeta_tools_mcp_server"],
...
def migrate(
    ...
    *,
    ...
    expose_gogeta_tools: bool = True,

) -> MigrationReport:

    """Translate Gogeta mcp_servers config + Codex curated plugins into
    ...
    Args:
        ...
        gogeta_config: full ~/.gogeta/config.yaml dict
        ...
        expose_gogeta_tools: when True (default), register Gogeta' own

            tool surface (web_search, browser_*, delegate_task, vision,

            memory, skills, etc.) as an MCP server in ~/.codex/config.toml

            so the codex subprocess can call back into Gogeta for tools
    ...
    gogeta_servers = (gogeta_config or {}).get("mcp_servers") or {}

    if not isinstance(gogeta_servers, dict):

        report.errors.append(

            "mcp_servers in Gogeta config is not a dict; cannot migrate."
    ...
    for name, cfg in gogeta_servers.items():
    ...
    # Inject Gogeta' own tool surface as an MCP server so the spawned

    # codex subprocess can call back into Gogeta for the tools codex

    # doesn't ship with — web_search, browser_*, delegate_task, vision,

    # memory, skills, session_search, image_generate, text_to_speech.

    # The server itself is agent/transports/gogeta_tools_mcp_server.py

    # and is launched on demand by codex (stdio MCP).

    if expose_gogeta_tools:

        translated["gogeta-tools"] = _build_gogeta_tools_mcp_entry()

        if "gogeta-tools" not in report.migrated:

            report.migrated.append("gogeta-tools")
