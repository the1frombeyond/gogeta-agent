"""``gogeta logs`` — view and filter Gogeta log files.
...
Usage examples::
    ...
    gogeta logs                    # last 50 lines of agent.log
    gogeta logs -f                 # follow agent.log in real time
    gogeta logs errors             # last 50 lines of errors.log
    gogeta logs gateway -n 100    # last 100 lines of gateway.log
    gogeta logs gui -f            # follow gui.log (dashboard/pty/ws)
    gogeta logs desktop -f        # follow desktop.log (Electron app boot/backend)
    gogeta logs --level WARNING    # only WARNING+ lines
    gogeta logs --session abc123   # filter by session ID substring
    gogeta logs --component tools  # only tool-related lines
    gogeta logs --since 1h         # lines from the last hour
    gogeta logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_gogeta_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Gogeta runs — try 'gogeta chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_gogeta_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_gogeta_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_gogeta_home()}/logs/")
        return

    print(f"Log files in {display_gogeta_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'gogeta chat' to generate logs)")
