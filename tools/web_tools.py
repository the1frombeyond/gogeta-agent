...
Backend is selected during ``gogeta tools`` setup (web.backend in config.yaml).
When available, Gogeta can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.gogeta/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``gogeta tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `gogeta tools` repaint.
        try:
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    ...
    return host == "gogeta.ai" or host.endswith(".gogeta.ai")
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `gogeta tools` to set one up."
                ),
            }
        else:
            logger.info(
