...
def register_credential_file(
    ...
    container_base: str = "/root/.gogeta",
    ...
    *relative_path* is relative to ``GOGETA_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside GOGETA_HOME so that a malicious
    ...
    gogeta_home = _resolve_gogeta_home()



    # Reject absolute paths — they bypass the GOGETA_HOME sandbox entirely.

    if os.path.isabs(relative_path):

        logger.warning(

            "credential_files: rejected absolute path %r (must be relative to GOGETA_HOME)",
    ...
    host_path = gogeta_home / relative_path



    # Resolve symlinks and normalise ``..`` before the containment check so

    # that traversal like ``../. ssh/id_rsa`` cannot escape GOGETA_HOME.

    from tools.path_security import validate_within_dir



    containment_error = validate_within_dir(host_path, gogeta_home)
            ...
            for item in cred_files:
                ...
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        ...
                        container_path = f"/root/.gogeta/{rel}"
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="gogeta-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching gogeta_constants.get_gogeta_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_gogeta_dir()`` for backward compatibility with old directory layouts.
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        ...
        host_dir = get_gogeta_dir(new_subpath, old_name)
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.gogeta``,

    SSH ``<remote_home>/.gogeta``, etc.) and whether translation is wanted.
