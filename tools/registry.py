"""Central registry for all gogeta-agent tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``gogeta tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``gogeta tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def register(
        ...
        ``category`` and ``metadata`` are new unified-platform fields; existing
        tool registrations that omit them work unchanged.
        """
        ...
        with self._lock:
            ...
            if existing and existing.toolset != toolset:
                ...
                if both_mcp:
                    ...
                    return
            entry_metadata = metadata or ToolMetadata(
                category=category or toolset,
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``gogeta tools enable foo``)
