...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("GOGETA_YOLO_MODE", ""))
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        from gogeta_cli.plugins import invoke_hook
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("GOGETA_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("GOGETA_SESSION_PLATFORM", "") or ""

    except Exception:

        return os.getenv("GOGETA_SESSION_PLATFORM", "") or ""
...
def _is_gateway_approval_context() -> bool:
    ...
    Legacy gateway integrations set GOGETA_GATEWAY_SESSION in process env.

    Newer concurrent gateway paths bind GOGETA_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds GOGETA_SESSION_PLATFORM via
    ...
    if env_var_enabled("GOGETA_CRON_SESSION"):

        return False

    if env_var_enabled("GOGETA_GATEWAY_SESSION"):
...
# via shell expansions like $HOME or $GOGETA_HOME.
...
_GOGETA_ENV_PATH = (
    ...
    r'(?:\$gogeta_home|\$\{gogeta_home\})/)'

    r'\.env\b'

)

# ~/.gogeta/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _GOGETA_ENV_PATH; matches the GOGETA_HOME override form as
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bgogeta\s+gateway\s+(stop|restart)\b', "stop/restart gogeta gateway (kills running agents)"),

    (r'\bgogeta\s+update\b', "gogeta update (restarts gateway, kills running agents)"),
    ...
    # that should always require user consent, just like `gogeta gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart gogeta-gateway')"),

    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart gogeta-gateway')"),

    # Self-termination protection: prevent agent from killing its own process

    (r'\b(pkill|killall)\b.*\b(gogeta|gateway|cli\.py)\b', "kill gogeta/gateway process (self-termination)"),

    # Self-termination via kill + command substitution (pgrep/pidof).

    # The name-based pattern above catches `pkill gogeta` but not

    # `kill -9 $(pgrep -f gogeta)` because the substitution is opaque
    ...
    # In-place edit of a Gogeta-managed security file (~/.gogeta/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_GOGETA_CONFIG_PATH}|{_GOGETA_ENV_PATH})', "in-place edit of Gogeta config/env"),

    (rf'\bsed\s+--in-place\b.*(?:{_GOGETA_CONFIG_PATH}|{_GOGETA_ENV_PATH})', "in-place edit of Gogeta config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_GOGETA_CONFIG_PATH}|{_GOGETA_ENV_PATH})', "in-place edit of Gogeta config/env (perl/ruby)"),
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["GOGETA_SPINNER_PAUSE"] = "1"
    ...
    try:
        ...
        if "GOGETA_SPINNER_PAUSE" in os.environ:

            del os.environ["GOGETA_SPINNER_PAUSE"]
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("GOGETA_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set GOGETA_INTERACTIVE or GOGETA_GATEWAY_SESSION to require approval.",
    ...
    if is_gateway or env_var_enabled("GOGETA_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_ask = env_var_enabled("GOGETA_EXEC_ASK")
