...
transport, discovers their tools, and registers them into the gogeta-agent
...
# per-profile log file (~/.gogeta/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            gogeta_home = os.path.expanduser(
                os.getenv(
                    "GOGETA_HOME", os.path.join(os.path.expanduser("~"), ".gogeta")
                )
            )
            candidates = [
                os.path.join(gogeta_home, "node", "bin", resolved_command),
                ...
                # image (which the Gogeta Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Gogeta MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Gogeta gateways know how to render.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Gogeta is fully
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Gogeta config file.
    ...
    ``os.environ`` (which includes ``~/.gogeta/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Gogeta' MEDIA: tag convention and out to messaging
            ...
            # Gogeta' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Gogeta's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Gogeta registry schema format.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple gogeta
