...
Skills Hub — Source adapters and hub state management for the Gogeta Skills Hub.
...
Used by gogeta_cli/skills_hub.py for CLI commands and the /skills slash command.
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            ...
            gogeta_meta = metadata.get("gogeta", {})

            if isinstance(gogeta_meta, dict):

                tags = gogeta_meta.get("tags", [])
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                ...
                gogeta_meta = meta_block.get("gogeta", {})
...
# Gogeta centralized index source

# ---------------------------------------------------------------------------



GOGETA_INDEX_URL = "https://gogeta.ai/docs/api/skills-index.json"

GOGETA_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "gogeta-index.json"

GOGETA_INDEX_TTL = 6 * 3600  # 6 hours





def _load_gogeta_index() -> Optional[dict]:
    ...
    We cache it locally for GOGETA_INDEX_TTL seconds to avoid repeated
    ...
    if GOGETA_INDEX_CACHE_FILE.exists():

        try:

            age = time.time() - GOGETA_INDEX_CACHE_FILE.stat().st_mtime

            if age < GOGETA_INDEX_TTL:

                return json.loads(GOGETA_INDEX_CACHE_FILE.read_text())
    ...
    try:
        ...
        resp = httpx.get(GOGETA_INDEX_URL, timeout=15, follow_redirects=True)

        if resp.status_code != 200:

            logger.debug("Gogeta index fetch returned %d", resp.status_code)
        ...
        logger.debug("Gogeta index fetch failed: %s", e)
    ...
    try:
        ...
        GOGETA_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)

        GOGETA_INDEX_CACHE_FILE.write_text(json.dumps(data))
...
class GogetaIndexSource(SkillSource):

    """Skill source backed by the centralized Gogeta Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        ...
        if not self._loaded:
            ...
            self._index = _load_gogeta_index()
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                ...
                bundle.source = entry.get("source", "gogeta-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        ...
        return SkillMeta(
            ...
            source=entry.get("source", "gogeta-index"),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        GogetaIndexSource(auth=auth), # Centralized index (search + resolved install paths)
