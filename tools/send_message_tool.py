    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.gogeta/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: gogeta config set {home_env} <channel_id>"
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            ...
            try:
                ...
                source_label = get_session_env("GOGETA_SESSION_PLATFORM", "cli")

                user_id = get_session_env("GOGETA_SESSION_USER_ID", "") or None
...
def _get_cron_auto_delivery_target():
    ...
    platform = get_session_env("GOGETA_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()

    chat_id = get_session_env("GOGETA_CRON_AUTO_DELIVER_CHAT_ID", "").strip()

    if not platform or not chat_id:

        return None

    thread_id = get_session_env("GOGETA_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
...
async def _send_matrix(token, extra, chat_id, message):
    ...
    try:
        ...
        txn_id = f"gogeta_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
async def _send_feishu(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        ...
        if not FEISHU_AVAILABLE:
            ...
            return {"error": "Feishu dependencies not installed. Run: pip install 'gogeta-agent[feishu]'"}
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``GOGETA_KANBAN_TASK``

    on every spawned worker, but those workers run with the assignee profile's

    ``GOGETA_HOME`` which has no ``gateway.pid``, so the gateway-running check
