...
# All skills live in ~/.gogeta/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> Dict[str, str]:
    ...
    """Load profile-scoped environment variables from GOGETA_HOME/.env."""
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set GOGETA_INTERACTIVE and register a

    # secret-capture callback that routes to a secure secret.request overlay, so

    # they fall through and actually prompt. (GOGETA_INTERACTIVE is the same flag

    # tools/approval.py uses to tell an interactive surface from a messaging one.)

    if _is_gateway_surface() and not env_var_enabled("GOGETA_INTERACTIVE"):
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("GOGETA_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    ...
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_gogeta_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.gogeta/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``GOGETA_SESSION_PLATFORM``.
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``GOGETA_PLATFORM`` environment variable

    3. ``GOGETA_SESSION_PLATFORM`` from gateway session context
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("GOGETA_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    """Recursively find all skills in ~/.gogeta/skills/ and external dirs.
    ...
    Args:
        ...
        skip_disabled: If True, return ALL skills regardless of disabled
            ...
            state (used by ``gogeta skills`` config UI). Default False
        ...
        if not SKILLS_DIR.exists():
            ...
            return json.dumps(

                {
                    ...
                    "message": f"No skills found. Skills directory created at {display_gogeta_home()}/skills/",
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from gogeta_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(

            {
                ...
                "error": (
                    ...
                    f"Re-enable with: gogeta plugins enable {namespace}"
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                ...
                _warnings.append(f"skill file is outside the trusted skills directory (~/.gogeta/skills/): {skill_md}")
            ...
            return json.dumps(

                {
                    ...
                    "error": (
                        ...
                        "Enable it with `gogeta skills` or inspect the files directly on disk."
        ...
        # Check metadata.gogeta.* first (agentskills.io convention), fall back to top-level
        ...
        if isinstance(metadata, dict):
            ...
            gogeta_meta = metadata.get("gogeta", {}) or {}



        tags = _parse_tags(gogeta_meta.get("tags") or frontmatter.get("tags", ""))

        related_skills = _parse_tags(

            gogeta_meta.get("related_skills") or frontmatter.get("related_skills", "")
