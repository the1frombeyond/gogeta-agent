...
_gogeta_config_resolved: str | None = None

_gogeta_config_resolved_loaded = False





def _get_gogeta_config_resolved() -> str | None:

    """Return the resolved absolute path of the Gogeta config file (cached)."""

    global _gogeta_config_resolved, _gogeta_config_resolved_loaded

    if _gogeta_config_resolved_loaded:

        return _gogeta_config_resolved

    _gogeta_config_resolved_loaded = True

    try:

        from gogeta_cli.config import get_config_path

        _gogeta_config_resolved = str(get_config_path().resolve())

    except Exception:

        try:

            _gogeta_config_resolved = str(Path("~/.gogeta/config.yaml").expanduser().resolve())

        except Exception:

            _gogeta_config_resolved = None
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    # Prevent agents from modifying the Gogeta config file directly.
    ...
    gogeta_config = _get_gogeta_config_resolved()

    if gogeta_config and (resolved == gogeta_config or normalized == gogeta_config):

        return (

            f"Refusing to write to Gogeta config file: {filepath}\n"

            "Agent cannot modify security-sensitive configuration. "

            "Edit ~/.gogeta/config.yaml directly or use 'gogeta config' instead."
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    ...
    """Return the container-side Gogeta mirror prefix for Docker file tools."""
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    """Return a soft-guard warning when ``filepath`` lands in another Gogeta

    profile's scoped area, a host-side sandbox-mirror of authoritative profile

    state, or the Docker container's sandbox mirror of Gogeta state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ...
      ``…/sandboxes/<backend>/<task>/home/.gogeta/…`` mirror created by a

      non-local terminal backend (Docker, Daytona, etc.), where the host

      Gogeta process never reads the mirror and the authoritative file is
    ...
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.gogeta/…`` path.



    Returns ``None`` when the write is in-scope or outside Gogeta scope.
    ...
    # in a session that cd'd into ``~/.gogeta/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Gogeta internal path guard ────────────────────────────────

        # Prevent prompt injection via catalog or hub metadata files,

        # and block credential stores under GOGETA_HOME.  Pass the

        # already-resolved path so a relative-path read against

        # TERMINAL_CWD == GOGETA_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Gogeta-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Gogeta-profile guard for
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                ...
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Gogeta profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                ...
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Gogeta profile's skills/plugins/cron/memories.",
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        ...
        return tool_error(
            ...
            "payload, or use execute_code with gogeta_tools.write_file() for very "
