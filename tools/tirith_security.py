...
it is automatically downloaded from GitHub releases to $GOGETA_HOME/bin/tirith.
...
def _get_gogeta_home() -> str:

    """Return the Gogeta home directory, respecting GOGETA_HOME env var."""

    return str(get_gogeta_home())
...
def _failure_marker_path() -> str:
    ...
    return os.path.join(_get_gogeta_home(), ".tirith-install-failed")
...
def _gogeta_bin_dir() -> str:

    """Return $GOGETA_HOME/bin, creating it if needed."""

    d = os.path.join(_get_gogeta_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    ...
    """Download and install tirith to $GOGETA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_gogeta_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $GOGETA_HOME/bin/tirith (previously auto-installed)

    3. Auto-install from GitHub releases → $GOGETA_HOME/bin/tirith
    ...
    gogeta_bin = os.path.join(_gogeta_bin_dir(), "tirith")

    if os.path.isfile(gogeta_bin) and os.access(gogeta_bin, os.X_OK):

        _resolved_path = gogeta_bin
