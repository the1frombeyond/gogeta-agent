...
Cron job management tools for Gogeta Agent.
...
#      saying "cat ~/.gogeta/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `gogeta-agent-dev` skill contains a security postmortem mentioning

#      `cat ~/.gogeta/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("GOGETA_SESSION_PLATFORM")

    origin_chat_id = get_session_env("GOGETA_SESSION_CHAT_ID")

    if origin_platform and origin_chat_id:

        thread_id = get_session_env("GOGETA_SESSION_THREAD_ID") or None
        ...
        return {
            ...
            "chat_name": get_session_env("GOGETA_SESSION_CHAT_NAME") or None,
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via gogeta model.
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within GOGETA_HOME/scripts/.
    ...
    # Only relative paths within ~/.gogeta/scripts/ are allowed.

    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):

        return (

            f"Script path must be relative to ~/.gogeta/scripts/. "

            f"Got absolute or home-relative path: {raw!r}. "

            f"Place scripts in ~/.gogeta/scripts/ and use just the filename."
    ...
    scripts_dir = get_gogeta_home() / "scripts"
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                ...
                "description": f"Optional path to a script that runs each tick. In the default mode its stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and its stdout is delivered verbatim (classic watchdog pattern). Relative paths resolve under {display_gogeta_home()}/scripts/. ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear."
            ...
            "profile": {
                ...
                "description": "Optional Gogeta profile name to run the job under. When set, the scheduler resolves that profile, applies a context-local Gogeta home override, loads that profile's config/.env for the run, and bridges GOGETA_HOME into subprocesses. Any temporary process-environment changes from profile .env loading are restored after the job exits. Use 'default' for the root Gogeta profile. Named profiles must already exist. When unset (default), preserves the scheduler's existing profile. On update, pass an empty string to clear. Jobs with profile run sequentially (not parallel) to keep profile-scoped runtime state isolated."
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        ...
        env_var_enabled("GOGETA_INTERACTIVE")

        or env_var_enabled("GOGETA_GATEWAY_SESSION")

        or env_var_enabled("GOGETA_EXEC_ASK")
