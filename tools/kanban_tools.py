...
running under the dispatcher (env var ``GOGETA_KANBAN_TASK`` set) or when

the active profile explicitly enables the ``kanban`` toolset for

orchestrator work. A normal ``gogeta chat`` session still sees **zero**

kanban tools in its schema unless configured.



Why tools instead of just shelling out to ``gogeta kanban``?



1. **Backend portability.** A worker whose terminal tool points at Docker

   / Modal / Singularity / SSH would run ``gogeta kanban complete …``

   inside the container, where ``gogeta`` isn't installed and the DB

   isn't mounted. Tools run in the agent's Python process, so they

   always reach ``~/.gogeta/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``gogeta kanban …``), the dashboard

(``gogeta dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``GOGETA_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    ...
    Humans running ``gogeta chat`` without the kanban toolset see zero
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    env_tid = os.environ.get("GOGETA_KANBAN_TASK")
...
def _stamp_worker_session_metadata(
    ...
    session_id = os.environ.get("GOGETA_SESSION_ID")
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``GOGETA_KANBAN_TASK`` set
    ...
    ``GOGETA_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``GOGETA_KANBAN_DB`` → ``GOGETA_KANBAN_BOARD`` env → current symlink

    → ``default``). Per-tool ``board`` lets a Telegram-side agent override

    the env-pinned active board without restarting Gogeta.
...
#   - No-op outside dispatcher-spawned worker context (no ``GOGETA_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      ...
      * ``GOGETA_KANBAN_TASK`` — task id (required; absence means no-op)

      * ``GOGETA_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat

        a stale run that may have already been reclaimed

      * ``GOGETA_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("GOGETA_KANBAN_TASK")
    ...
    try:
        ...
        try:
            ...
            claim_lock = os.environ.get("GOGETA_KANBAN_CLAIM_LOCK")
            ...
            run_id_raw = os.environ.get("GOGETA_KANBAN_RUN_ID")
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        ...
        return tool_error(
            ...
            "task_id is required (or set GOGETA_KANBAN_TASK in the env)"
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # GOGETA_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``gogeta-system`` and poison
    ...
    author = os.environ.get("GOGETA_PROFILE") or "worker"
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("GOGETA_TENANT")

    # Stamp the originating session id when the agent loop runs under

    # ACP (which sets GOGETA_SESSION_ID before invoking tools). NULL on

    # CLI / dashboard paths and on legacy hosts that don't set the env.

    session_id = args.get("session_id") or os.environ.get("GOGETA_SESSION_ID")

    priority = args.get("priority")

    # Resolve workspace. If the caller passed one explicitly, honor it.

    # Otherwise, a dispatcher-spawned worker (GOGETA_KANBAN_TASK set)
    ...
    # (kanban toolset, no GOGETA_KANBAN_TASK) and CLI/dashboard callers
    ...
    try:
        ...
        try:
            ...
            if _inherit_workspace:
                ...
                _self_tid = os.environ.get("GOGETA_KANBAN_TASK")
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("GOGETA_PROFILE") or "worker",
...
_DESC_TASK_ID_DEFAULT = (
    ...
    "Task id. If omitted, defaults to GOGETA_KANBAN_TASK from the env "
...
_DESC_BOARD = (
    ...
    "active board the usual way: GOGETA_KANBAN_DB env → "

    "GOGETA_KANBAN_BOARD env → the 'current' symlink under the kanban "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to GOGETA_TENANT env if set."
