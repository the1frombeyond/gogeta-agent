...
This module provides the glue:
    ...
    - ``GogetaTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      ...
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Gogeta Agent"
...
from gogeta_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__gogeta_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses GOGETA_HOME so each profile gets its own OAuth tokens.

    Layout: ``GOGETA_HOME/mcp-tokens/``
    ...
    try:
        ...
        base = Path(get_gogeta_home())

    except ImportError:

        base = Path(os.environ.get("GOGETA_HOME", str(Path.home() / ".gogeta")))
...
# GogetaTokenStorage -- persistent token/client-info on disk
...
class GogetaTokenStorage:
    ...
    File layout::
        ...
        GOGETA_HOME/mcp-tokens/<server_name>.json         -- tokens

        GOGETA_HOME/mcp-tokens/<server_name>.client.json   -- client info

        GOGETA_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        # Gogeta records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Gogeta.</p></body></html>"
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        ...
        print(
            ...
            f"  See: https://gogeta.ai/docs/guides/oauth-over-ssh\n",
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            ...
            "  OAuth skipped. Run `gogeta mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Gogeta Agent")
