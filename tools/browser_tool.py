...
# plugins drop in under ``~/.gogeta/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.gogeta/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Gogeta silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Gogeta CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-gogeta_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed gogeta processes — this way every clean gogeta exit sweeps
    ...
    # Sweep orphans from other crashed gogeta processes.  Safe even if we

    # never used the browser — uses owner_pid liveness to avoid reaping

    # daemons owned by other live gogeta processes.
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    ...
    """Record the current gogeta PID as the owner of a browser socket dir.



    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the

    orphan reaper can distinguish daemons owned by a live gogeta process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning gogeta process is no longer alive.
    ...
    Ownership detection priority:
      ...
      1. ``<session>.owner_pid`` file (written by current code) — if the
         ...
         referenced gogeta PID is alive, leave the daemon alone regardless

         of whether it's in *this* process's ``_active_sessions``.  This is

         cross-process safe: two concurrent gogeta instances won't reap each
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # Owner is alive — this session belongs to a live gogeta process.
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Gogeta-managed
    ...
    # Build an extended search PATH including Gogeta-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                ...
                gogeta_nm = str(get_gogeta_home() / "node_modules" / ".bin")

                recheck = shutil.which("agent-browser", path=gogeta_nm)

            if not recheck:

                gogeta_node_bin = str(get_gogeta_home() / "node" / "bin")

                recheck = shutil.which("agent-browser", path=gogeta_node_bin)

            if not recheck:

                gogeta_node_root = str(get_gogeta_home() / "node")

                recheck = shutil.which("agent-browser", path=gogeta_node_root)
...
def _run_browser_command(
    ...
    try:
        ...
        # Record this gogeta PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = gogeta_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Gogeta
    ...
    from gogeta_constants import get_gogeta_dir

    screenshots_dir = get_gogeta_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/gogeta/gogeta-agent:latest")
