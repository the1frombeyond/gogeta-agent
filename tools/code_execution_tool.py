...
Lets the LLM write a Python script that calls Gogeta tools via RPC,
...
Architecture (two transports):
  ...
  **Local backend (UDS):**
  ...
  1. Parent generates a `gogeta_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  ...
  1. Parent generates `gogeta_tools.py` with file-based RPC stubs
...
# available on every platform Gogeta itself runs on.
...
# match a safe prefix, the operational GOGETA_ allowlist, or (on Windows) an

# OS-essential name.

#

# NB: the broad "GOGETA_" prefix was deliberately removed (#27303) — it leaked

# GOGETA_*-named config that lacks a secret substring (e.g. GOGETA_BASE_URL,

# GOGETA_KANBAN_DB, GOGETA_*_WEBHOOK).  The child only needs the few

# location/profile vars in _GOGETA_CHILD_ALLOWED below; GOGETA_RPC_SOCKET /

# GOGETA_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational GOGETA_* vars the child legitimately needs by exact name — these

# are non-secret runtime-location flags (the same set gogeta_cli treats as the

# runtime location) that repo-root modules a sandbox script imports may read at

# import time.  None match _SECRET_SUBSTRINGS.

_GOGETA_CHILD_ALLOWED = frozenset({
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational GOGETA_* vars (_GOGETA_CHILD_ALLOWED) pass by exact name.
    ...
    # Non-secret GOGETA_* vars dropped by the tightened allowlist (#27303). The

    # broad "GOGETA_" prefix used to pass these through; now only the

    # operational set does. The drop is intentional (those vars can carry

    # config like GOGETA_KANBAN_DB / GOGETA_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if k.startswith("GOGETA_"):

            # Non-secret (secrets were already dropped above) and not in any

            # allowlist — a deliberately-dropped GOGETA_* var.

            _dropped_gogeta.append(k)

    if _dropped_gogeta:

        logger.debug(

            "execute_code: dropped %d non-allowlisted GOGETA_* var(s) from the "
...
# gogeta_tools.py code generator
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Gogeta-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Gogeta-profile soft guard."""',
...
def generate_gogeta_tools_module(enabled_tools: List[str],

                                 transport: str = "uds") -> str:

    """

    Build the source code for the gogeta_tools.py stub module.
...
"""Auto-generated Gogeta tools RPC stubs."""
...
def _connect():
    ...
    GOGETA_RPC_SOCKET can be either:
    ...
    if _sock is None:
        ...
        endpoint = os.environ["GOGETA_RPC_SOCKET"]
...
"""Auto-generated Gogeta tools RPC stubs (file-based transport)."""

import json, os, shlex, tempfile, threading, time



_RPC_DIR = os.environ.get("GOGETA_RPC_DIR") or os.path.join(tempfile.gettempdir(), "gogeta_rpc")
...
def _execute_remote(
    ...
    The script and the generated gogeta_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/gogeta_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_gogeta_tools_module(

            list(sandbox_tools), transport="file",

        )

        _ship_file_to_remote(env, f"{sandbox_dir}/gogeta_tools.py", tools_src)
        ...
        env_prefix = (
            ...
            f"GOGETA_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "

            f"PYTHONDONTWRITEBYTECODE=1"

        )

        tz = os.getenv("gogeta_timeZONE", "").strip()
...
def execute_code(
    ...
    # --- Set up temp directory with gogeta_tools.py and script.py ---

    tmpdir = tempfile.mkdtemp(prefix="gogeta_sandbox_")
    ...
    # selector from GOGETA_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"gogeta_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        ...
        # Write the auto-generated gogeta_tools module.
        ...
        tools_src = generate_gogeta_tools_module(list(sandbox_tools))

        with open(os.path.join(tmpdir, "gogeta_tools.py"), "w", encoding="utf-8") as f:
        ...
        #   connect.  GOGETA_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["GOGETA_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the gogeta-agent root is importable in the sandbox so

        # repo-root modules are available to child scripts.  We also prepend

        # the staging tmpdir so ``from gogeta_tools import ...`` resolves even

        # when the subprocess CWD is not tmpdir (project mode).

        _gogeta_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

        _existing_pp = child_env.get("PYTHONPATH", "")

        _pp_parts = [tmpdir, _gogeta_root]
        ...
        # gogeta_timeZONE is an internal Gogeta setting and must not leak

        # into child processes.

        _tz_name = os.getenv("gogeta_timeZONE", "").strip()

        if _tz_name:

            child_env["TZ"] = _tz_name

        child_env.pop("gogeta_timeZONE", None)



        # Per-profile HOME isolation: redirect system tool configs into

        # {GOGETA_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.gogeta/.env')).
...
def _load_config() -> dict:
    ...
    ``gogeta --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ...
        ``sys.executable`` (gogeta-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``gogeta tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and gogeta-agent's own python.
    ...
    if mode == "strict":
        ...
        cwd_note = (
            ...
            "(os.path.expanduser('~/.gogeta/.env')) or terminal()/read_file() for user files."
    ...
    description = (
        ...
        "Run a Python script that can call Gogeta tools programmatically. "
        ...
        f"Available via `from gogeta_tools import ...`:\n\n"
        ...
        "Also available (no import needed — built into gogeta_tools):\n"
            ...
            "properties": {
                ...
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from gogeta_tools import {import_str}` "
