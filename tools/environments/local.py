...
# Gogeta-internal env vars that should NOT leak into terminal subprocesses.

_GOGETA_PROVIDER_ENV_FORCE_PREFIX = "_GOGETA_FORCE_"



# Gogeta-managed AWS *inference* credentials for ``auth_type="aws_sdk"``

# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the

# Bedrock-specific bearer token, which is a Gogeta inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        ...
        from gogeta_cli.config import OPTIONAL_ENV_VARS
    ...
    blocked.update({
        ...
        "GOGETA_DASHBOARD_SESSION_TOKEN",
...
_GOGETA_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()





def _inject_context_gogeta_home(env: dict) -> None:

    """Bridge the context-local Gogeta home override into subprocess env."""

    try:

        from gogeta_constants import get_gogeta_home_override



        value = get_gogeta_home_override()

        if value:

            env["GOGETA_HOME"] = value
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    """Filter Gogeta-managed secrets from a subprocess environment."""
    ...
    for key, value in (base_env or {}).items():
        ...
        if key.startswith(_GOGETA_PROVIDER_ENV_FORCE_PREFIX):

            continue

        if key not in _GOGETA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
    ...
    for key, value in (extra_env or {}).items():
        ...
        if key.startswith(_GOGETA_PROVIDER_ENV_FORCE_PREFIX):
            ...
            real_key = key[len(_GOGETA_PROVIDER_ENV_FORCE_PREFIX):]

            sanitized[real_key] = value

        elif key not in _GOGETA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):

            sanitized[key] = value



    _inject_context_gogeta_home(sanitized)
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("GOGETA_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\gogeta\git\bin\bash.exe   (primary)

    #   MinGit:      %LOCALAPPDATA%\gogeta\git\usr\bin\bash.exe (legacy/32-bit fallback)

    _local_appdata = os.environ.get("LOCALAPPDATA", "")

    _gogeta_portable_git = os.path.join(_local_appdata, "gogeta", "git") if _local_appdata else ""

    if _gogeta_portable_git:

        for candidate in (

            os.path.join(_gogeta_portable_git, "bin", "bash.exe"),        # PortableGit (primary)

            os.path.join(_gogeta_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
    ...
    raise RuntimeError(
        ...
        "Git Bash not found. Gogeta Agent requires Git for Windows on Windows.\n"

        "Install it from: https://git-scm.com/download/win\n"

        "Or set GOGETA_GIT_BASH_PATH to your bash.exe location."
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        ...
        if k.startswith(_GOGETA_PROVIDER_ENV_FORCE_PREFIX):

            real_key = k[len(_GOGETA_PROVIDER_ENV_FORCE_PREFIX):]

            run_env[real_key] = v

        elif k not in _GOGETA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
    ...
    # Gogeta is driving via _find_bash (Git Bash), and Git Bash itself
    ...
    _inject_context_gogeta_home(run_env)



    # Per-profile HOME isolation: redirect system tool configs (git, ssh, gh,

    # npm …) into {GOGETA_HOME}/home/ when that directory exists.  Only the
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Gogeta trusts them.
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``GOGETA_HOME`` instead — single-word path, guaranteed to exist, same
        ...
        if _IS_WINDOWS:
            ...
            # Derive a Windows-safe temp dir under GOGETA_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_gogeta_home() / "cache" / "terminal"

            except Exception:

                cache_dir = Path(tempfile.gettempdir()) / "gogeta_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            ...
            try:
                ...
                proc._gogeta_pgid = os.getpgid(proc.pid)
        ...
        try:
            ...
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_gogeta_pgid", None)
