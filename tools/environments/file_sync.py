...
_FORCE_SYNC_ENV = "GOGETA_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.gogeta") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.gogeta to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``GOGETA_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, gogeta_home: Path | None = None) -> None:

        """Pull remote changes back to the host filesystem.



        Downloads the remote ``.gogeta/`` directory as a tar archive,
        ...
        # against an uninitialized remote .gogeta/ directory.
        ...
        lock_path = (gogeta_home or get_gogeta_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="gogeta-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.gogeta/skills/a.md`` →

        ``~/.gogeta/skills/a.md``, a new remote file at

        ``/root/.gogeta/skills/b.md`` maps to ``~/.gogeta/skills/b.md``.
