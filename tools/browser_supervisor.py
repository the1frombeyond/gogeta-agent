...
One ``CDPSupervisor`` runs per Gogeta ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "gogeta-dialog-bridge.invalid"
...
(() => {
  if (window.__gogetaDialogBridgeInstalled) return;
  window.__gogetaDialogBridgeInstalled = true;
  const ENDPOINT = "http://gogeta-dialog-bridge.invalid/";
