...
-----------------------------------------------------------------------------
    ...
    ~/.gogeta/checkpoints/
        ...
        store/                          — single bare-ish git repo
            ...
            refs/gogeta/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_gogeta_home() / "checkpoints"
...
DEFAULT_EXCLUDES = [
    ...
    # Worktrees (Gogeta convention — don't recursively snapshot siblings)
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("GOGETA_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(
    ...
    The shared store is internal Gogeta infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``gogeta checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        ...
        logger.info(
            ...
            "Clear with `gogeta checkpoints clear-legacy` when safe.",
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "gogeta@local"], store, cfg_wd)

    _run_git(["config", "user.name", "Gogeta Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``GOGETA_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at GOGETA_WORKDIR

    # (write in addition to the JSON metadata).

    try:

        (shadow_repo / "GOGETA_WORKDIR").write_text(
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for child in base.iterdir():
        ...
        if delete_orphans:
            ...
            wd_marker = child / "GOGETA_WORKDIR"
...
# Public helpers for `gogeta checkpoints` CLI
