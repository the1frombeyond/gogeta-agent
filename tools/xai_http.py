...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``gogeta tools`` repaint, tool-registration scans,
    ...
    2. ``~/.gogeta/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_gogeta_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    ...
    """Read ``name`` from ``~/.gogeta/.env`` first, then ``os.environ``.



    Wraps :func:`gogeta_cli.config.get_env_value` so tests can patch
    ...
    try:
        ...
        from gogeta_cli.config import get_env_value as _gogeta_get_env_value



        value = _gogeta_get_env_value(name)
...
def gogeta_xai_user_agent() -> str:

    """Return a stable Gogeta-specific User-Agent for xAI HTTP calls."""
    ...
    return f"Gogeta-Agent/{__version__}"
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    Prefers Gogeta-managed xAI OAuth credentials when available, then falls back

    to ``XAI_API_KEY`` resolved via ``gogeta_cli.config.get_env_value`` so keys

    stored in ``~/.gogeta/.env`` (the standard Gogeta location) are honored —
