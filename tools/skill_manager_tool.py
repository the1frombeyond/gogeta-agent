...
~/.gogeta/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `gogeta config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.gogeta/skills/ (single source of truth)
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`gogeta curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.gogeta/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Gogeta profiles.
    ...
    # Default profile (~/.gogeta/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.gogeta/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`gogeta -p "
            ...
            base += (
                ...
                f"Switch profiles (`gogeta -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_gogeta_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `gogeta curator unpin <name>`. "
