...
- Users can declare any number of named providers with ``type: command``
  ...
  under ``tts.providers.<name>`` in ``~/.gogeta/config.yaml``. Gogeta
...
Configuration is loaded from ~/.gogeta/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``gogeta_cli.config.get_env_value``
    ...
    try:
        ...
        from gogeta_cli.config import get_env_value as _get_env_value
...
from tools.xai_http import gogeta_xai_user_agent
...
def _get_default_output_dir() -> str:
    ...
    return str(get_gogeta_dir("cache/audio", "audio_cache"))
...
# Config loader -- reads tts: section from ~/.gogeta/config.yaml
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    Load TTS configuration from ~/.gogeta/config.yaml.
    ...
    try:
        ...
        logger.debug("gogeta_cli.config not available, using default TTS config")
...
# custom voice-cloning scripts, etc.) into Gogeta without any Python code
...
# Gogeta writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__GOGETA_TTS_PLACEHOLDER_{len(replacements)}__"
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        ...
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `gogeta model` or set XAI_API_KEY.")
    ...
    # output_format when Gogeta actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": gogeta_xai_user_agent(),
...
def _get_piper_voices_dir() -> Path:
    ...
    """Return the directory where Gogeta caches Piper voice models.



    Resolves to ``~/.gogeta/cache/piper-voices/`` under the active

    GOGETA_HOME so voice downloads follow profile boundaries.

    """

    from gogeta_constants import get_gogeta_dir

    root = Path(get_gogeta_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.gogeta/config.yaml (tts: section).
    ...
    platform = get_session_env("GOGETA_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             ...
                             "Run: pip install 'gogeta-agent[mistral]'"
            ...
            if not _check_neutts_available():
                ...
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             ...
                             "Run gogeta setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             ...
                             "Run 'gogeta setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             ...
                             "Run 'gogeta tools' and select Piper under TTS, or install manually: "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                ...
                "description": f"Optional custom file path to save the audio. Defaults to {display_gogeta_home()}/audio_cache/<timestamp>.mp3"
