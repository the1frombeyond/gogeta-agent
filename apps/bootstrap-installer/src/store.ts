...
export const $gsaHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, gsaHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_gogeta_home'),
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Gogeta" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Gogeta-Setup.exe --update);
...
export async function launchGsaDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_gogeta_desktop', { installRoot })
}
