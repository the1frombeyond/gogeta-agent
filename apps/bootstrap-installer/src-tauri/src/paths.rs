...
//! Mirrors `gogeta_constants.get_gogeta_home()` from the Python CLI:

//!   Windows: %LOCALAPPDATA%\gogeta

//!   macOS:   ~/.gogeta

//!   Linux:   ~/.gogeta  (override via $GOGETA_HOME)

//!

//! NOTE (macOS): Python's get_gogeta_home(), scripts/install.sh, and the

//! Electron desktop's resolveGogetaHome() ALL use ~/.gogeta on macOS — there
...
/// Returns the canonical Gogeta home directory, respecting $GOGETA_HOME if set.

pub fn gogeta_home() -> PathBuf {

    if let Ok(override_path) = std::env::var("GOGETA_HOME") {
...

    {
        ...
        // %LOCALAPPDATA%\gogeta — matches scripts/install.ps1's $GogetaHome.

        if let Some(local_app_data) = dirs::data_local_dir() {

            return local_app_data.join("gogeta");
    ...
    // macOS + Linux + fallback: ~/.gogeta (matches Python get_gogeta_home(),

    // install.sh, and the Electron desktop's resolveGogetaHome()).

    if let Some(home) = dirs::home_dir() {

        return home.join(".gogeta");
...
pub fn bootstrap_cache_dir() -> PathBuf {
    ...
    gogeta_home().join("bootstrap-cache")
...
/// GOGETA_HOME so it survives repo checkout deletion (unlike anything under

/// gogeta-agent/).

///

/// On Windows this is `%LOCALAPPDATA%\gogeta\gogeta-setup.exe`; on other
...
pub fn copy_self_to_gogeta_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to GOGETA_HOME");
...
/// We don't always know ACTIVE_GOGETA_ROOT until install.ps1 reports it, so

/// this is a probe helper, not a definitive path.

pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {

    install_root.join(".gogeta-bootstrap-complete")

}



/// Initializes tracing to bootstrap-installer.log under GOGETA_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[gogeta-setup] could not create log dir {dir:?}: {err}");
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("GOGETA_BOOTSTRAP_LOG")
...
pub fn get_gogeta_home() -> String {

    gogeta_home().to_string_lossy().into_owned()
