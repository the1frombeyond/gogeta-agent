...
//! GOGETA_HOME/logs/bootstrap-<timestamp>.log.
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for GOGETA_HOME. Tests use this; production

    /// almost always falls back to the OS default.

    pub gogeta_home: Option<String>,
...
/// Spawn the locally-built Gogeta desktop binary, then close the installer
...
pub async fn launch_gogeta_desktop(
    ...
    let exe_path = resolve_gogeta_desktop_exe(&install_root).ok_or_else(|| {

        format!(

            "Couldn't find a built Gogeta desktop at {}. The desktop build step \

             may have been skipped or failed. Run `gogeta desktop` from a \
    ...
    tracing::info!(?exe_path, "launching Gogeta desktop");



    // Detach from us — the installer is about to exit. On macOS launch the

    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Gogeta
...
pub(crate) fn resolve_gogeta_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        ...
        &[
            ...
            ("win-unpacked", "Gogeta.exe"),

            ("win-arm64-unpacked", "Gogeta.exe"),
        ...
        &[
            ...
            ("mac/Gogeta.app/Contents/MacOS", "Gogeta"),

            ("mac-arm64/Gogeta.app/Contents/MacOS", "Gogeta"),

        ]

    } else {

        &[("linux-unpacked", "gogeta")]
...
pub(crate) fn resolve_gogeta_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {

    let exe = resolve_gogeta_desktop_exe(install_root)?;

    #[cfg(target_os = "macos")]

    {

        // .../Gogeta.app/Contents/MacOS/Gogeta -> .../Gogeta.app
...
/// path so a bare re-open just opens Gogeta instead of re-running setup.

pub(crate) fn gogeta_is_installed(install_root: &std::path::Path) -> bool {

    install_root.join(".gogeta-bootstrap-complete").exists()

        && resolve_gogeta_desktop_exe(install_root).is_some()
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    ...
    let exe = resolve_gogeta_desktop_exe(install_root).ok_or_else(|| {

        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Gogeta desktop app")
...

    {
        ...
        // installer exits, mirroring launch_gogeta_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.current_dir(crate::paths::gogeta_home());
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $GogetaHome which Stage-Repository clones

    // the repo INTO at $GogetaHome\gogeta-agent. Mirrors gogeta_constants.

    let gogeta_home = args

        .gogeta_home

        .clone()

        .unwrap_or_else(|| crate::paths::gogeta_home().to_string_lossy().into_owned());

    let install_root = PathBuf::from(&gogeta_home).join("gogeta-agent");



    // Copy ourselves to GOGETA_HOME/gogeta-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_gogeta_home() {

        tracing::warn!(?err, "failed to copy installer into GOGETA_HOME (non-fatal)");
...
async fn run_install_script(
    ...
    gogeta_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, gogeta_home_override, cancel_rx)
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Gogeta"), b"#!/bin/sh\n").unwrap();

            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Gogeta.app
    ...
    // what the updater ditto's over /Applications/Gogeta.app). A regression in

    // this derivation breaks the post-update auto-relaunch, so guard it.

    #[test]

    fn resolve_gogeta_desktop_app_finds_built_bundle() {
        ...
        let resolved = resolve_gogeta_desktop_app(&root)
    ...
    fn resolve_gogeta_desktop_app_is_none_without_a_build() {
        ...
        assert!(
            ...
            resolve_gogeta_desktop_app(&root).is_none(),
