...
/// desktop app handing off via `Gogeta-Setup.exe --update`).
...
pub fn run() {
    ...
    // Tracing → bootstrap-installer.log under GOGETA_HOME/logs/ so install
    ...
    // Gogeta is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Gogeta installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Gogeta is already installed should NOT show the installer or

            // rebuild — it should just open the app, so the /Applications

            // "Gogeta" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Gogeta" shortcuts that install.ps1 creates, and a

            // reliable detached relaunch there needs the DETACHED_PROCESS +

            // startup-grace handling used by launch_gogeta_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                ...
                let install_root = paths::gogeta_home().join("gogeta-agent");

                if bootstrap::gogeta_is_installed(&install_root) {
                    ...
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        ...
                        Ok(()) => {
                            ...
                            // before we exit (mirrors launch_gogeta_desktop).

                            std::thread::sleep(std::time::Duration::from_millis(200));

                            tracing::info!(

                                "gogeta already installed — relaunched desktop; exiting installer"
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            bootstrap::launch_gogeta_desktop,

            // Diagnostics

            paths::get_log_path,

            paths::get_gogeta_home,
        ...
        .expect("error while running Gogeta Setup");
