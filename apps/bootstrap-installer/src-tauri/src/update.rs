...
//! Driven when the installer is launched as `Gogeta-Setup.exe --update` (see

//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:

//!

//!   1. wait for the old Gogeta desktop process to fully exit (so the venv

//!      shim is free; otherwise `gogeta update` aborts with exit code 2),

//!   2. run `gogeta update --yes --gateway` (Python/repo update; this does NOT

//!      rebuild apps/desktop by design — see cmd_update in gogeta_cli/main.py),

//!   3. run `gogeta desktop --build-only` (the rebuild step update skips),
...
//! Cross-platform note: `gogeta update` already handles macOS/Linux (git/pip).

//! The only OS-specific bits here are the venv shim path (resolve_gogeta) and
...
/// `gogeta update` exit code meaning "another gogeta process is holding the

/// venv shim open / dirty precondition" — see _cmd_update_impl in

/// gogeta_cli/main.py (sys.exit(2)). We surface a targeted message for this.
...
/// before giving up and letting `gogeta update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
...

    {
        ...
        let mut stages = vec![
            ...
            stage_info("update", "Updating Gogeta"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    let gogeta_home = crate::paths::gogeta_home();

    let install_root = gogeta_home.join("gogeta-agent");
    ...
    let gogeta = resolve_gogeta(&install_root).ok_or_else(|| {

        let msg = format!(

            "Could not find the gogeta CLI under {}. Is Gogeta installed? \
    ...
    // async on Windows. If it still holds the venv shim, `gogeta update`
    ...
    // ---- stage 1: gogeta update -----------------------------------------

    // Pass --branch so `gogeta update` targets the branch this installer was
    ...
    // without --branch, `gogeta update` switches the checkout to `main` (a
    ...
    // --force skips `gogeta update`'s Windows running-exe guard (which would
    ...
    // time `gogeta update` runs there is no legitimate gogeta.exe to protect,

    // and the guard would only produce a false "Gogeta is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `gogeta update` lazily imports
    ...
    // second `gogeta update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Gogeta" state
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            ...
            let msg = "Gogeta is still running. Close all Gogeta windows and try \
        ...
        other => {
            ...
            let msg = format!(
                ...
                "gogeta update failed (exit {:?}). See {} for details.",

                other,

                crate::paths::gogeta_home()
    ...
    // ---- stage 2: gogeta desktop --build-only ----------------------------

    // `gogeta update` deliberately does NOT build apps/desktop (it installs
    ...
    if rebuild.exit_code != Some(0) {
        ...
        let msg = format!(
            ...
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             ...
             applied but the app could not be rebuilt; run `gogeta desktop` \
    ...
    if let Some(target_app) = launch_target {
        ...
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            ...
            emit_log(
                ...
                &format!("[update] could not auto-launch desktop: {err}. Launch Gogeta manually."),
        ...
        crate::bootstrap::launch_gogeta_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
...
async fn wait_for_venv_free(install_root: &Path, app: &AppHandle) {
    ...
    let shim = venv_gogeta(install_root);

    let deadline = Instant::now() + DESKTOP_EXIT_WAIT;



    emit_log(app, Some("update"), LogStream::Stdout, "[update] waiting for Gogeta to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // Last resort: a backend gogeta.exe (or a grandchild it spawned)
            ...
            // every gogeta.exe except ourselves, then give the OS a beat to
            ...
            emit_log(
                ...
                "[update] Gogeta still holding the venv shim; force-killing stragglers…",

            );

            force_kill_other_gogeta();
...
/// Force-kill any `gogeta.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `gogeta.exe` image tree via
...
/// which completes BEFORE we spawn `venv\Scripts\gogeta.exe update`. At this

/// point no update-driven gogeta.exe exists yet, so the only gogeta.exe images
...
/// Spawn `gogeta <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv gogeta shim under an install root, regardless of existence.

fn venv_gogeta(install_root: &Path) -> PathBuf {

    if cfg!(target_os = "windows") {

        install_root.join("venv").join("Scripts").join("gogeta.exe")

    } else {

        install_root.join("venv").join("bin").join("gogeta")
...
/// Resolve the gogeta CLI to drive. Prefer the venv shim in the install we

/// just updated; fall back to `gogeta` on PATH.

fn resolve_gogeta(install_root: &Path) -> Option<PathBuf> {
    ...
    let exe = if cfg!(target_os = "windows") { "gogeta.exe" } else { "gogeta" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        ...
        "GOGETA_HOME".to_string(),

        gogeta_home.as_os_str().to_os_string(),

    )];

    if let Some(path) = path_with_prepended_entries(&[

        gogeta_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_gogeta_desktop_app(install_root).ok_or_else(|| {

        anyhow!(

            "desktop rebuild succeeded but no Gogeta.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.gogeta-update-new", target_app.display()));

    let old = PathBuf::from(format!("{}.gogeta-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        ...
        .current_dir(crate::paths::gogeta_home())
...
mod tests {
    ...
    fn venv_gogeta_is_under_install_root() {

        let root = Path::new("/x/gogeta-agent");

        let shim = venv_gogeta(root);
    ...
    fn parses_only_app_targets() {
        ...
        assert_eq!(
            ...
            target_app_from_args(["--update", "--target-app", "/Applications/Gogeta.app"]),

            Some(PathBuf::from("/Applications/Gogeta.app"))
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Gogeta.app");

        let tmp = base.join("Gogeta.app.gogeta-update-new");

        let old = base.join("Gogeta.app.gogeta-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let tmp = base.join("Gogeta.app.gogeta-update-new"); // intentionally absent
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let tmp = base.join("Gogeta.app.gogeta-update-new"); // absent
