...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'gogeta-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/gogeta-runner-test',
    sourceRepoRoot: null,
    gogetaHome: '/tmp/gogeta-runner-test',
    logRoot: '/tmp/gogeta-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'gogeta-agent', 'scripts')
