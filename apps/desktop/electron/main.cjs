...
const USER_DATA_OVERRIDE = process.env.GOGETA_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.GOGETA_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with GOGETA_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    ...
    `[gogeta] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        ...
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          ...
          console.warn(
            ...
            `[gogeta] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
...
if (INSTALL_STAMP) {
  ...
  console.log(
    ...
    `[gogeta] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  ...
  console.error(
    ...
    '[gogeta] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
...
// GOGETA_HOME — the user-facing root for everything Gogeta-related. Mirrors

// scripts/install.ps1's $GogetaHome and scripts/install.sh's $GOGETA_HOME.

//

// Defaults:

//   Windows: %LOCALAPPDATA%\gogeta (matches install.ps1)

//   macOS / Linux: ~/.gogeta (matches install.sh)

//

// Special case for Windows: if the user has a legacy ~/.gogeta directory

// (e.g., from a prior pip install or a manual setup) AND no

// %LOCALAPPDATA%\gogeta yet, prefer the legacy path so we don't orphan their

// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.

//

// GOGETA_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox

// GOGETA_HOME beneath the throwaway userData dir so a fresh-install run never

// touches the user's real ~/.gogeta / %LOCALAPPDATA%\gogeta.

function resolveGogetaHome() {

  if (process.env.GOGETA_HOME) return path.resolve(process.env.GOGETA_HOME)

  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'gogeta-home')

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {

    const localappdata = path.join(process.env.LOCALAPPDATA, 'gogeta')

    const legacy = path.join(app.getPath('home'), '.gogeta')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy

    // ~/.gogeta setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.gogeta')

}



const GOGETA_HOME = resolveGogetaHome()

// ACTIVE_GOGETA_ROOT — the canonical mutable Gogeta install. Same path

// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end

// up with identical layouts and can share one install.

const ACTIVE_GOGETA_ROOT = path.join(GOGETA_HOME, 'gogeta-agent')

// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.

const VENV_ROOT = path.join(ACTIVE_GOGETA_ROOT, 'venv')
...
// boots, going straight to `resolveGogetaBackend()`. Missing or stale marker
...
// We deliberately put the marker INSIDE ACTIVE_GOGETA_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_GOGETA_ROOT, '.gogeta-bootstrap-complete')
...
// active-profile.json records which Gogeta profile the desktop launches its

// local backend as. When set, startGogeta() passes `gogeta --profile <name>

// dashboard …`, which deterministically pins GOGETA_HOME (see

// _apply_profile_override in gogeta_cli/main.py) and bypasses the sticky

// ~/.gogeta/active_profile file. Unset (null) preserves the legacy behavior:

// no --profile flag, so the backend honors active_profile / default.

const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')

// Mirrors gogeta_cli.profiles._PROFILE_ID_RE so we never hand the backend a
...
// gogetaDesktop.updates.setBranch().

const DEFAULT_UPDATE_BRANCH = 'main'

// desktop.log lives under GOGETA_HOME/logs/ so it sits next to agent.log,

// errors.log, gateway.log produced by gogeta_logging.setup_logging — one log

// directory per user, regardless of which UI surface produced the line.

const DESKTOP_LOG_PATH = path.join(GOGETA_HOME, 'logs', 'desktop.log')
...
// Mirror the Python logs (gogeta_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.GOGETA_DESKTOP_BOOT_FAKE === '1'

const BOOT_FAKE_STEP_MS = (() => {

  const raw = Number.parseInt(String(process.env.GOGETA_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// Seed the native About panel with the live Gogeta version. This is refreshed

// on every open via the explicit "About" menu handler (refreshAboutPanel), so

// an in-place `gogeta update` mid-session is reflected without an app restart;
...
app.setAboutPanelOptions({
  ...
  applicationVersion: resolveGogetaVersion(),
...
const MEDIA_PROTOCOL = 'gogeta-media'

// Only audio/video may be streamed. Without this the handler would read any

// non-blocklisted local file (no size cap) for any `fetch(gogeta-media://…)`.
...
let gogetaProcess = null

let connectionPromise = null

// Additional per-profile backends, keyed by profile name. The PRIMARY backend

// (the desktop's launch profile) stays managed by gogetaProcess +

// connectionPromise + startGogeta(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.GOGETA_DESKTOP_POOL_MAX) || 3)

const POOL_IDLE_MS = Math.max(60_000, Number(process.env.GOGETA_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startGogeta() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start Gogeta backend',
...
function rememberLog(chunk) {
  ...
  const lines = text.split(/\r?\n/).map(line => `[gogeta] ${line}`)

  gogetaLog.push(...lines)

  if (gogetaLog.length > 300) {

    gogetaLog.splice(0, gogetaLog.length - 300)
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-gogeta-wsl-windows-fonts.conf')
...
function broadcastBootProgress() {
  ...
  webContents.send('gogeta:boot-progress', bootProgressState)
...
// The snapshot is queryable via the gogeta:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('gogeta:bootstrap:event', ev)
...
function isGogetaSourceRoot(root) {

  return directoryExists(root) && fileExists(path.join(root, 'gogeta_cli', 'main.py'))
...
function findPythonForRoot(root) {
  ...
  const override = process.env.GOGETA_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Gogeta deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Gogeta' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\gogeta\git\... — checked
  ...
  if (localAppData) {
    ...
    candidates.push(path.join(localAppData, 'gogeta', 'git', 'bin', 'bash.exe'))

    candidates.push(path.join(localAppData, 'gogeta', 'git', 'usr', 'bin', 'bash.exe'))
...
// install only has PortableGit under %LOCALAPPDATA%\gogeta\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    ...
    candidates.push(path.join(localAppData, 'gogeta', 'git', 'cmd', 'git.exe'))

    candidates.push(path.join(localAppData, 'gogeta', 'git', 'bin', 'git.exe'))
...
function recentGogetaLog() {

  return gogetaLog.slice(-20).join('\n')

}



// ─── Self-update (git-pull against the running backend's gogeta root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_GOGETA_ROOT.

// GOGETA_DESKTOP_ROOT always wins so devs can pin a worktree.

function resolveUpdateRoot() {

  const candidates = [

    process.env.GOGETA_DESKTOP_ROOT && path.resolve(process.env.GOGETA_DESKTOP_ROOT),

    !IS_PACKAGED && isGogetaSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,

    isGogetaSourceRoot(ACTIVE_GOGETA_ROOT) ? ACTIVE_GOGETA_ROOT : null

  ].filter(Boolean)



  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_GOGETA_ROOT
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    ...
    window.webContents.send('gogeta:updates:progress', merged)
...
// GOGETA_HOME/gogeta-setup.exe on a successful install (see

// apps/bootstrap-installer paths::copy_self_to_gogeta_home). That binary owns

// ALL repo mutation — running `gogeta update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  ...
  const name = IS_WINDOWS ? 'gogeta-setup.exe' : 'gogeta-setup'

  const candidate = path.join(GOGETA_HOME, name)
...
// Path to the venv shim whose lock decides whether `gogeta update` can write

// fresh entry points. On Windows this is the file the running backend

// `gogeta.exe` holds open; on POSIX it's never mandatory-locked.

function venvGogetaShimPath(updateRoot) {

  return IS_WINDOWS

    ? path.join(updateRoot, 'venv', 'Scripts', 'gogeta.exe')

    : path.join(updateRoot, 'venv', 'bin', 'gogeta')
...
// only signals the direct child, so on Windows a backend `gogeta.exe` that

// spawned its own grandchildren (a `gogeta` REPL, a pty terminal session, the
...
// `gogetaProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on

// Windows doesn't reap the backend's grandchildren, and quit didn't wait for

// teardown, so the updater raced a still-locked `gogeta.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a gogeta

// REPL, a pty terminal, the gateway) keep `gogeta.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  if (gogetaProcess && Number.isInteger(gogetaProcess.pid)) pids.push(gogetaProcess.pid)
  ...
  if (gogetaProcess && !gogetaProcess.killed) {

    try {

      gogetaProcess.kill('SIGTERM')
  ...
  const shim = venvGogetaShimPath(updateRoot)
...
// `gogeta update`). Instead we spawn the staged Gogeta-Setup binary with

// --update and quit, so it can run `gogeta update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // macOS/Linux drag-install: no staged Tauri gogeta-setup. Unlike Windows
      ...
      // whole update itself: `gogeta update` (backend) + `gogeta desktop
    ...
    if (!updater) {
      ...
      // `gogeta desktop`, never the Tauri installer that self-copies

      // gogeta-setup.exe into GOGETA_HOME). They DO have a working `gogeta`
      ...
      // checkout they're on — bare `gogeta update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') command = `gogeta update --branch ${branch}`

        }

      } catch {

        // Best-effort: fall back to bare `gogeta update` if branch detection fails.
      ...
      return { ok: true, manual: true, command, gogetaRoot: updateRoot }

    }



    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Gogeta updater…', percent: 100 })
    ...
    // gogeta.exe (held by the backend child / its grandchildren) and the update
    ...
    // `gogeta update` will run (the venv shim is locked while we live).
    ...
    const child = spawn(updater, updaterArgs, {
      ...
      env: {
        ...
        PATH: [path.join(GOGETA_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
...
// Resolve the gogeta CLI to drive an in-app update: prefer the venv shim in

// the install we're updating, fall back to `gogeta` on PATH.

function resolveGogetaCliBinary(updateRoot) {

  const venvGogeta = path.join(updateRoot, 'venv', 'bin', 'gogeta')

  if (fileExists(venvGogeta)) return venvGogeta

  return findOnPath('gogeta') || null
...
// macOS/Linux in-app update: backend (`gogeta update`) + OS-aware GUI rebuild

// (`gogeta desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp() {
  ...
  const gogeta = resolveGogetaCliBinary(updateRoot)

  if (!gogeta) {

    emitUpdateProgress({ stage: 'manual', message: 'gogeta update', percent: null })

    return { ok: true, manual: true, command: 'gogeta update', gogetaRoot: updateRoot }

  }



  // Put the Gogeta-managed Node and the venv on PATH so `gogeta desktop`'s

  // npm build can find them on a machine with no system Node.

  const extraPath = [path.join(GOGETA_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
  ...
  // `gogeta update` reaps stale `gogeta dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads GOGETA_DESKTOP_CHILD_PID and excludes
  ...
  if (gogetaProcess && Number.isInteger(gogetaProcess.pid)) {

    desktopChildPids.push(gogetaProcess.pid)
  ...
  if (desktopChildPids.length) {
    ...
    env.GOGETA_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Gogeta (git + dependencies)…', percent: 10 })

  const updated = await runStreamedUpdate(gogeta, ['update', '--yes', ...branchArgs], {
  ...
  if (updated.code !== 0) {
    ...
    emitUpdateProgress({ stage: 'error', message: 'gogeta update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'gogeta update failed' }
  ...
  const rebuilt = await runStreamedUpdate(gogeta, ['desktop', '--build-only'], {
  ...
  if (rebuilt.code !== 0) {
    ...
    emitUpdateProgress({
      ...
      message: 'Backend updated, but the desktop rebuild failed. Restart Gogeta to retry.',
  ...
  const rebuiltApp = [
    ...
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Gogeta.app'),

    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Gogeta.app')
  ...
  if (!rebuiltApp || !targetApp) {
    ...
    emitUpdateProgress({
      ...
      message: 'Backend updated. Restart Gogeta to load the new version.',
...
if [ "$SRC" != "$DST" ]; then
  ...
  if /usr/bin/ditto "$SRC" "$DST.gogeta-update-new"; then

    rm -rf "$DST.gogeta-update-old" 2>/dev/null || true

    mv "$DST" "$DST.gogeta-update-old" 2>/dev/null || rm -rf "$DST"

    mv "$DST.gogeta-update-new" "$DST"
...
`
  ...
  const scriptPath = path.join(app.getPath('temp'), `gogeta-desktop-update-${Date.now()}.sh`)
  ...
  try {
    ...
    emitUpdateProgress({
      ...
      message: 'Backend + app updated. Restart Gogeta to load the new version.',
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `gogeta
  ...
  return isGogetaSourceRoot(ACTIVE_GOGETA_ROOT) && fileExists(getVenvPython(VENV_ROOT))
...
function resolveWebDist() {
  ...
  const override = process.env.GOGETA_DESKTOP_WEB_DIST
...
function resolveGogetaCwd() {

  // In a packaged build, `process.cwd()` resolves to the install root (e.g.

  // `…/win-unpacked` on Windows or `/Applications/Gogeta.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.GOGETA_DESKTOP_CWD,
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    args: ['-m', 'gogeta_cli.main', ...dashboardArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_GOGETA_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    ...
    label: `Gogeta at ${ACTIVE_GOGETA_ROOT}`,
    ...
    env: {
      ...
      PYTHONPATH: [ACTIVE_GOGETA_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)

    },

    root: ACTIVE_GOGETA_ROOT,
...
function resolveGogetaBackend(dashboardArgs) {

  // 1. Explicit override -- GOGETA_DESKTOP_ROOT points at a developer

  //    checkout. Honour it as-is (no bootstrap; the user is driving).

  const overrideRoot = process.env.GOGETA_DESKTOP_ROOT && path.resolve(process.env.GOGETA_DESKTOP_ROOT)

  if (overrideRoot && isGogetaSourceRoot(overrideRoot)) {

    const backend = createPythonBackend(overrideRoot, `Gogeta source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `gogeta` on PATH so local Python edits are actually exercised.

  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isGogetaSourceRoot.)

  if (!IS_PACKAGED && isGogetaSourceRoot(SOURCE_REPO_ROOT)) {

    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Gogeta source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
  ...
  // 3. Bootstrap-complete ACTIVE_GOGETA_ROOT -- the canonical install at

  //    %LOCALAPPDATA%\gogeta\gogeta-agent (Windows) or ~/.gogeta/gogeta-agent.
  ...
  //    to spawning gogeta. Updates flow through the in-app update path

  //    (applyUpdates -> git pull) or `gogeta update` from the CLI.
  ...
  // 4. Existing `gogeta` on PATH -- installed via install.ps1 / install.sh from
  ...
  //    GOGETA_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.

  if (process.env.GOGETA_DESKTOP_IGNORE_EXISTING !== '1') {

    let gogetaCommand = null

    const gogetaOverride = process.env.GOGETA_DESKTOP_GOGETA



    if (gogetaOverride) {

      const resolvedOverride = findOnPath(gogetaOverride)

      if (resolvedOverride) {

        gogetaCommand = resolvedOverride

      } else if (!isWindowsBinaryPathInWsl(gogetaOverride, { isWsl: IS_WSL })) {

        gogetaCommand = gogetaOverride

      } else {

        rememberLog(`Ignoring Windows Gogeta override under WSL: ${gogetaOverride}`)

      }

    } else {

      gogetaCommand = findOnPath('gogeta')
    ...
    if (gogetaCommand) {
      ...
      if (looksLikeDesktopAppBinary(gogetaCommand)) {

        rememberLog(`Ignoring desktop app executable on PATH while resolving Gogeta CLI: ${gogetaCommand}`)
    ...
    if (gogetaCommand) {
      ...
      // Smoke-test the candidate before trusting it. A `gogeta` shim
      ...
      const shellForProbe = isCommandScript(gogetaCommand)

      if (verifyGogetaCli(gogetaCommand, { shell: shellForProbe })) {

        return {

          label: `existing Gogeta CLI at ${gogetaCommand}`,

          command: gogetaCommand,
      ...
      rememberLog(
        ...
        `Ignoring existing Gogeta CLI at ${gogetaCommand}: --version probe failed; falling through to bootstrap.`
  ...
  // 5. Last-ditch: pip-installed gogeta_cli module via system Python.
  ...
  if (python) {
    ...
    // having gogeta_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\gogeta\gogeta-agent\venv.

    if (canImportGogetaCli(python)) {

      return {

        kind: 'python',

        label: `installed gogeta_cli module via ${python}`,
    ...
    rememberLog(`Ignoring system Python ${python}: gogeta_cli is not importable; falling through to bootstrap.`)
  ...
  //    resolveGogetaBackend was the old "no payload" path and forced the
  ...
  return {
    ...
    label: 'Gogeta Agent not installed yet; bootstrap required',
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveGogetaBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    rememberLog('[bootstrap] no Gogeta install found; starting first-launch bootstrap')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      gogetaHome: GOGETA_HOME,

      logRoot: path.join(GOGETA_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      ...
      const cancelledError = new Error('Gogeta install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      ...
      const bootstrapError = new Error(
        ...
        `Gogeta bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +

          `${bootstrapResult.error || 'unknown error'}. ` +

          `Check ${path.join(GOGETA_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ...
      // Latch the failure so subsequent startGogeta() calls return this

      // same error without re-running install.ps1.  Cleared by the

      // gogeta:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveGogetaBackend(backend.args))
  ...
  if (!isGogetaSourceRoot(ACTIVE_GOGETA_ROOT)) {

    throw new Error(

      `Gogeta install at ${ACTIVE_GOGETA_ROOT} is missing or incomplete. ` +
  ...
  // On Windows, preflight Git Bash. Gogeta' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\gogeta\git\, which findGitBash() picks up, so for any

  // user who completed the bootstrap this is a no-op. For users who got

  // here via an external `gogeta` on PATH, this check still helps.

  if (IS_WINDOWS && !findGitBash()) {

    throw new Error(

      'Git for Windows is required for Gogeta on Windows (provides Git Bash, ' +

        "which the agent's terminal tool uses). Install it from " +

        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +

        'then relaunch Gogeta.'
  ...
  if (!fileExists(venvPython)) {
    ...
    // isGogetaSourceRoot() and the bootstrap writes the marker only after
    ...
    throw new Error(
      ...
      `Gogeta venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
  ...
  backend.label = `Gogeta at ${ACTIVE_GOGETA_ROOT} (venv: ${VENV_ROOT})`

  updateBootProgress({

    phase: 'runtime.ready',

    message: 'Gogeta runtime is ready',
...
function fetchJson(url, token, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      ...
      reject(new Error(`Unsupported Gogeta backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              ...
              new Error(
                ...
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  ...
                  'The endpoint is likely missing on the Gogeta backend.'
    ...
    req.setTimeout(timeoutMs, () => {
      ...
      req.destroy(new Error(`Timed out connecting to Gogeta backend after ${timeoutMs}ms`))
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Gogeta-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('gogeta:link-titles', { cache: false })
...
function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveGogetaCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('gogeta:preview-file-changed', payload)
...
async function waitForGogeta(baseUrl, token) {
  ...
  throw new Error(`Gogeta backend did not become ready: ${lastError?.message || 'timeout'}`)
...
function sendBackendExit(payload) {
  ...
  webContents.send('gogeta:backend-exit', payload)
...
function sendClosePreviewRequested() {
  ...
  webContents.send('gogeta:close-preview-requested')
...
// signal so the chat composer doesn't stay stuck on "Starting Gogeta...".
...
function sendPowerResume() {
  ...
  webContents.send('gogeta:power-resume')
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('gogeta:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('gogeta:window-state-changed', state)
...
// Hosted Gogeta gateways gate the dashboard behind an OAuth provider (e.g.
...
//   * REST is authed by HttpOnly session cookies (``gogeta_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / gogeta #37247).

//     Both are set as HttpOnly cookies (``gogeta_session_at`` ~15 min,

//     ``gogeta_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:gogeta-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      win = new BrowserWindow({
        ...
        title: 'Sign in to Gogeta gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Gogeta backend after ${timeoutMs}ms`))
...
// a valid stored value (pins the root GOGETA_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  return {
    ...
    // scope is never overridden by GOGETA_DESKTOP_REMOTE_URL.

    envOverride: key ? false : Boolean(process.env.GOGETA_DESKTOP_REMOTE_URL)
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (gogeta #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      ...
      const err = new Error(
        ...
        'Remote Gogeta gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    ...
    throw new Error(
      ...
      'Remote Gogeta gateway is selected, but no session token is saved. ' +
...
//   2. env override (GOGETA_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.GOGETA_DESKTOP_REMOTE_URL

  const rawEnvToken = process.env.GOGETA_DESKTOP_REMOTE_TOKEN
  ...
  if (rawEnvUrl) {
    ...
    if (!rawEnvToken) {
      ...
      throw new Error(
        ...
        'GOGETA_DESKTOP_REMOTE_URL is set but GOGETA_DESKTOP_REMOTE_TOKEN is not. ' +

          'Both must be provided to connect to a remote Gogeta backend.'
...
function globalRemoteActive() {
  ...
  if (process.env.GOGETA_DESKTOP_REMOTE_URL) {
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Gogeta
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startGogeta())
  ...
  // connect to Gogeta gateway". Mirror the renderer's connect here so the test
...
function resetGogetaConnection() {
...
// startGogeta() spawns fresh instead of racing the dying one. Shared by the

// connection-config and profile switch flows.

async function teardownPrimaryBackendAndWait() {

  // Capture the reference before resetGogetaConnection() nulls gogetaProcess.

  const dying = gogetaProcess && !gogetaProcess.killed ? gogetaProcess : null

  resetGogetaConnection()
...
// profile to startGogeta() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startGogeta() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    ...
    await waitForGogeta(remote.baseUrl, remote.token)
  ...
  // --profile wins over the inherited GOGETA_HOME env (see _apply_profile_override

  // step 3 in gogeta_cli/main.py), so the child re-homes to this profile.

  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', String(port)]

  const backend = await ensureRuntime(resolveGogetaBackend(dashboardArgs))

  const gogetaCwd = resolveGogetaCwd()

  const webDist = resolveWebDist()



  rememberLog(`Starting Gogeta backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(backend.command, backend.args, {
    ...
    env: {
      ...
      GOGETA_DASHBOARD_SESSION_TOKEN: token,
  ...
  child.once('error', error => {
    ...
    rememberLog(`Gogeta backend for profile "${profile}" failed to start: ${error.message}`)
  ...
  child.once('exit', (code, signal) => {
    ...
    rememberLog(`Gogeta backend for profile "${profile}" exited (${signal || code})`)

    backendPool.delete(profile)

    if (!ready) {

      rejectStart?.(new Error(`Gogeta backend for profile "${profile}" exited before it became ready (${signal || code}).`))
  ...
  await Promise.race([waitForGogeta(baseUrl, token), startFailed])
...
async function startGogeta() {

  // Latched-failure short-circuit: once bootstrap has failed in this

  // process, every subsequent startGogeta() call re-throws the same error
  ...
  connectionPromise = (async () => {
    ...
    await advanceBootProgress('backend.resolve', 'Resolving Gogeta backend', 8)
    ...
    if (remote) {
      ...
      await advanceBootProgress('backend.remote', `Connecting to remote Gogeta backend at ${remote.baseUrl}`, 24)
      ...
      updateBootProgress({
        ...
        message: 'Remote Gogeta backend is ready',
    ...
    // deterministic (it wins over the sticky ~/.gogeta/active_profile file) and

    // resolves GOGETA_HOME the same way `gogeta -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Gogeta runtime', 28)
    ...
    await advanceBootProgress('backend.spawn', `Starting Gogeta backend via ${backend.label}`, 84)

    rememberLog(`Starting Gogeta backend via ${backend.label}`)



    gogetaProcess = spawn(backend.command, backend.args, {
      ...
      env: {
        ...
        // Explicitly pin GOGETA_HOME for the child so Python's get_gogeta_home()

        // resolves to the SAME location our resolveGogetaHome() picked. Without

        // this pin, Python falls back to ~/.gogeta on every platform — fine on

        // mac/linux (where our default matches), but on Windows our default is

        // %LOCALAPPDATA%\gogeta, which differs from C:\Users\<u>\.gogeta.

        // Mismatch would split config / sessions / .env / logs across two

        // directories. install.ps1 sets GOGETA_HOME via setx; the desktop
    ...
    gogetaProcess.stdout.on('data', rememberLog)

    gogetaProcess.stderr.on('data', rememberLog)
    ...
    gogetaProcess.once('error', error => {

      rememberLog(`Gogeta backend failed to start: ${error.message}`)
      ...
      updateBootProgress(

        {
          ...
          message: `Gogeta backend failed to start: ${error.message}`,
    ...
    gogetaProcess.once('exit', (code, signal) => {

      rememberLog(`Gogeta backend exited (${signal || code})`)
      ...
      if (!backendReady) {
        ...
        const message = `Gogeta backend exited before it became ready (${signal || code}).`
        ...
        rejectBackendStart?.(
          ...
          new Error(
            ...
            `Gogeta backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentGogetaLog()}`
    ...
    await advanceBootProgress('backend.wait', 'Waiting for Gogeta backend to become ready', 90)

    await Promise.race([waitForGogeta(baseUrl, token), backendStartFailed])
    ...
    updateBootProgress({
      ...
      message: 'Gogeta backend is ready. Finalizing desktop startup',
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startGogeta().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('gogeta:connection', async (_event, profile) => ensureBackend(profile))

ipcMain.handle('gogeta:backend:touch', async (_event, profile) => {
...
ipcMain.handle('gogeta:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))

ipcMain.handle('gogeta:bootstrap:reset', async () => {

  // Renderer's "Reload and retry" path. Clear the latched failure and

  // reset connection state so the next startGogeta() call restarts the
...
ipcMain.handle('gogeta:bootstrap:repair', async () => {

  // Forceful repair: drop the bootstrap-complete marker so the next

  // startGogeta() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('gogeta:bootstrap:cancel', async () => {
...
ipcMain.handle('gogeta:boot-progress:get', async () => bootProgressState)

ipcMain.handle('gogeta:bootstrap:get', async () => getBootstrapState())

ipcMain.handle('gogeta:connection-config:get', async (_event, profile) =>

  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)

)

ipcMain.handle('gogeta:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))

ipcMain.handle('gogeta:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))

ipcMain.handle('gogeta:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('gogeta:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('gogeta:connection-config:save', async (_event, payload) => {
...
ipcMain.handle('gogeta:connection-config:apply', async (_event, payload) => {
...
ipcMain.handle('gogeta:profile:get', async () => ({ profile: readActiveDesktopProfile() }))

ipcMain.handle('gogeta:profile:set', async (_event, name) => {
  ...
  // new GOGETA_HOME. Pool backends keep their own homes, so only the primary
...
ipcMain.on('gogeta:previewShortcutActive', (_event, active) => {
...
ipcMain.handle('gogeta:requestMicrophoneAccess', async () => {
...
ipcMain.handle('gogeta:api', async (_event, request) => {
...
ipcMain.handle('gogeta:notify', (_event, payload) => {

  if (!Notification.isSupported()) return false

  new Notification({

    title: payload?.title || 'Gogeta',
...
ipcMain.handle('gogeta:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('gogeta:readFileText', async (_event, filePath) => {
...
ipcMain.handle('gogeta:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('gogeta:writeClipboard', (_event, text) => {
...
ipcMain.handle('gogeta:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))



ipcMain.handle('gogeta:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('gogeta:saveClipboardImage', async () => {
...
ipcMain.handle('gogeta:normalizePreviewTarget', (_event, target, baseDir) =>
...
ipcMain.handle('gogeta:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))



ipcMain.handle('gogeta:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))



ipcMain.on('gogeta:titlebar-theme', (_event, payload) => {
...
ipcMain.handle('gogeta:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolveGogetaCwd picks it up on the next

// session spawn (no app restart needed).

ipcMain.handle('gogeta:setting:defaultProjectDir:get', async () => ({

  dir: readDefaultProjectDir(),

  defaultLabel: path.join(app.getPath('home'), 'gogeta-projects')

}))



ipcMain.handle('gogeta:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('gogeta:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('gogeta:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))



ipcMain.handle('gogeta:logs:reveal', async () => {
...
ipcMain.handle('gogeta:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: gogetaLog.slice(-200) }))
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Gogeta' TUI into
...
function terminalChannel(id, suffix) {
  ...
  return `gogeta:terminal:${id}:${suffix}`
...
ipcMain.handle('gogeta:fs:readDir', async (_event, dirPath) => {
...
ipcMain.handle('gogeta:fs:gitRoot', async (_event, startPath) => {
...
ipcMain.handle('gogeta:terminal:start', async (event, payload = {}) => {

  if (!nodePty) {

    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Gogeta.')
...
ipcMain.handle('gogeta:terminal:write', (_event, id, data) => {
...
ipcMain.handle('gogeta:terminal:resize', (_event, id, size = {}) => {
...
ipcMain.handle('gogeta:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))



ipcMain.handle('gogeta:updates:check', async () =>
...
ipcMain.handle('gogeta:updates:apply', async (_event, payload) =>
...
ipcMain.handle('gogeta:updates:branch:get', async () => readDesktopUpdateConfig())



ipcMain.handle('gogeta:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Gogeta version (the one `release.py` bumps in

// gogeta_cli/__init__.py + pyproject.toml) so the desktop About panel shows the

// real Gogeta version instead of the Electron app's own package.json version,

// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()

// when the source tree can't be read (e.g. a packaged build without the repo).

function resolveGogetaVersion() {

  try {

    const root = resolveUpdateRoot()

    const initPath = path.join(root, 'gogeta_cli', '__init__.py')
...
// Re-resolve the live Gogeta version and push it into the native About panel

// just before showing it, so an in-place `gogeta update` is reflected without
...
ipcMain.handle('gogeta:version', async () => ({

  appVersion: resolveGogetaVersion(),
...
// the actual removal via `gogeta uninstall …` so the cross-platform PATH /

// registry / service / node-symlink cleanup all lives in one place

// (gogeta_cli/uninstall.py + gogeta_cli/gui_uninstall.py).
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_GOGETA_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isGogetaSourceRoot(agentRoot) && fileExists(py),
  ...
  return new Promise(resolve => {
    ...
    try {
      ...
      const child = spawn(py, ['-m', 'gogeta_cli.main', 'uninstall', '--gui-summary'], {

        cwd: agentRoot,

        env: { ...process.env, GOGETA_HOME, NO_COLOR: '1' },
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    ...
    return {
      ...
      message: `Can't run the uninstaller: no Gogeta agent venv at ${VENV_ROOT}.`
  ...
  // Python with PYTHONPATH=<agentRoot> so `import gogeta_cli` resolves from
  ...
  try {
    ...
    await releaseBackendLock(ACTIVE_GOGETA_ROOT, 'uninstall')
  ...
  try {
    ...
    if (IS_WINDOWS) {
      ...
      scriptPath = path.join(app.getPath('temp'), `gogeta-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `gogeta-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('gogeta:uninstall:summary', async () => getUninstallSummary())

ipcMain.handle('gogeta:uninstall:run', async (_event, payload) => {
