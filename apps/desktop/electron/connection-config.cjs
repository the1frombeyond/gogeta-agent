 ...
 *     `X-Gogeta-Session-Token` header; WS uses `?token=`.
...
// gogeta_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - gogeta_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - gogeta_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / gogeta #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-gogeta_session_at', '__Secure-gogeta_session_at', 'gogeta_session_at']
const RT_COOKIE_VARIANTS = ['__Host-gogeta_session_rt', '__Secure-gogeta_session_rt', 'gogeta_session_rt']
...
/**
 * True if any cookie in `cookies` is a gogeta session ACCESS-token cookie
