 ...
 * Drives apps/desktop's first-launch install of Gogeta Agent by spawning
...
function bootstrapCacheDir(gogetaHome) {

  return path.join(gogetaHome, 'bootstrap-cache')
...
// checkout under ~/.gogeta/gogeta-agent. Used as a last-resort fallback when

// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop

// app stamped to an unpushed HEAD).

function installedAgentInstallScript(gogetaHome) {

  if (!gogetaHome) return null

  const candidate = path.join(gogetaHome, 'gogeta-agent', 'scripts', installScriptName())
...
function cachedScriptPath(gogetaHome, commit) {

  return path.join(bootstrapCacheDir(gogetaHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/gogeta/gogeta-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, gogetaHome, emit, _download = downloadInstallScript }) {
  ...
  const cached = cachedScriptPath(gogetaHome, installStamp.commit)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(gogetaHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, gogetaHome } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, {
      ...
      env: {
        ...
        // Pass GOGETA_HOME through so install.ps1 respects the caller's

        // choice rather than re-computing the default.

        GOGETA_HOME: gogetaHome || process.env.GOGETA_HOME || ''
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, gogetaHome } = {}) {
...
function buildPosixPinArgs({ installStamp, activeRoot, gogetaHome }) {

  const args = ['--dir', activeRoot, '--gogeta-home', gogetaHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, gogetaHome, activeRoot, installStamp }) {

  const isPosix = installerKind === 'posix'

  const args = isPosix

    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, gogetaHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, gogetaHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ...
    ? [
        ...
        ...buildPosixPinArgs({ installStamp, activeRoot, gogetaHome })
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(gogetaHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, gogetaHome, emit })
