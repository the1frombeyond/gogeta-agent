 ...
 * the three user-facing uninstall modes to the `gogeta uninstall` CLI flags,
 ...
 * Map an uninstall mode to the `python -m gogeta_cli.uninstall` argv (after the

 * python executable). Uses the dedicated lightweight module entrypoint (not

 * `gogeta_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'gogeta_cli.uninstall', '--mode', mode]
 ...
 *   macOS:   …/Gogeta.app/Contents/MacOS/Gogeta  → …/Gogeta.app

 *   Windows: …\Gogeta\Gogeta.exe                 → …\Gogeta  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    ...
    // …/Gogeta.app/Contents/MacOS/Gogeta → strip 3 segments to the .app

    const macOsDir = p.dirname(exe) // …/Contents/MacOS

    const contents = p.dirname(macOsDir) // …/Contents

    const appBundle = p.dirname(contents) // …/Gogeta.app
  ...
  if (platform === 'win32') {
    ...
    // NSIS per-user installs Gogeta.exe directly in the install dir.

    const dir = p.dirname(exe)

    if (/[\\/]Gogeta$/i.test(dir) || /[\\/]gogeta-desktop$/i.test(dir)) return dir
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/gogeta
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import gogeta_cli`
...
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, gogetaHome }) {
  ...
  const lines = [
    ...
    `export GOGETA_HOME=${q(gogetaHome)}`
 ...
 * modes + `pythonPath`=agentRoot so `import gogeta_cli` resolves from source
...
function buildWindowsCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, gogetaHome }) {
  ...
  // a problem, but Gogeta install paths don't use them.
  ...
  const lines = [
    ...
    `set "GOGETA_HOME=${String(gogetaHome).replace(/"/g, '')}"`,
