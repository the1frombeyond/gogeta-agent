import type { ConnectionState, GatewayEvent } from '@gogeta/shared'
...
const isOpen = (gateway: GsaGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<GsaGateway | null>(null)
...
let primaryGateway: GsaGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: GsaGateway | null, profile = 'default'): void {
...
export function activeGateway(): GsaGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<GsaGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
