...
import { getProfiles, setApiRequestProfile } from '@/gogeta'
...
import type { ProfileInfo } from '@/types/gogeta'
...
// /api/profiles/active `current`). "default" is the root ~/.gogeta. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'gogeta.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'gogeta.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.gsaDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new GOGETA_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.gsaDesktop.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'gogeta.desktop.showAllProfiles'
...
// Switch to the default (root ~/.gogeta) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.gsaDesktop?.touchBackend?.(target).catch(() => undefined)
}
