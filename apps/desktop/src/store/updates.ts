...
const UPDATE_TOAST_SNOOZE_KEY = 'gogeta:update-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: { label: translateNow('notifications.updateGsa'), onClick: () => void applyUpdates() },
    durationMs: 0,
    id: SKEW_TOAST_ID,
    kind: 'warning',
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.gsaDesktop?.getVersion?.()
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  const bridge = window.gsaDesktop?.updates
  ...
  try {
    ...
    // The update check pulls the latest gogeta_cli + bundled package metadata
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `gogeta update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'gogeta update',
        command: result.command ?? 'gogeta update'
      })
    }
...
function onFocus() {
  ...
  // updated Gogeta from another window/CLI between focuses, and About should
