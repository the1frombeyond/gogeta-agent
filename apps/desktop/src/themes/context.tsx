...
const SKIN_KEY = 'gogeta-desktop-theme-v2'
const MODE_KEY = 'gogeta-desktop-mode-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.gsaTheme = skinName
  root.dataset.gsaMode = rendered
  ...
  window.gsaDesktop?.setTitleBarTheme?.({
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.gsaThemeFont = 'true'
...
/** Sync the desktop skin with the active Gogeta backend theme on connect. */
