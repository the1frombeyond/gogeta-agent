...
import { getGsaConfigRecord, type GsaConfigRecord, saveGsaConfig } from '@/gogeta'
...
export interface I18nConfigClient {
  getConfig: () => Promise<GsaConfigRecord>
  saveConfig: (config: GsaConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.gsaDesktop?.api) {
      return Promise.resolve({})
    }

    return getGsaConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.gsaDesktop?.api) {
...
export function getConfigDisplayLanguage(config: GsaConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: GsaConfigRecord, locale: Locale): GsaConfigRecord {
