...
const DESKTOP_COMMAND_META = [
  ...
  ['/profile', 'Switch the active Gogeta profile'],
  ...
  ['/version', 'Show Gogeta Agent version'],
...
export function isDesktopSlashCommand(command: string): boolean {
  ...
  return DESKTOP_COMMANDS.has(canonical) || !isKnownGsaSlashCommand(normalized)
}

/**
 * An "extension" command is anything the backend surfaces that is NOT one of
 * Gogeta' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownGsaSlashCommand(normalized)
}
...
function isKnownGsaSlashCommand(command: string): boolean {
