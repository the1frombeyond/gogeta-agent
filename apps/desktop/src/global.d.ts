...
declare global {
  interface Window {
    gsaDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<GsaConnection>
      ...
      profile: {
        ...
        // backend under the new GOGETA_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: GsaApiRequest) => Promise<T>
      notify: (payload: GsaNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<GsaReadFileTextResult>
      selectPaths: (options?: GsaSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<GsaPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<GsaPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: GsaTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<GsaReadDirResult>
      gitRoot?: (path: string) => Promise<string | null>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: GsaTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<GsaTerminalSession>
        write: (id: string, data: string) => Promise<boolean>
      }
      onClosePreviewRequested?: (callback: () => void) => () => void
      onOpenUpdatesRequested?: (callback: () => void) => () => void
      onWindowStateChanged?: (callback: (payload: GsaWindowState) => void) => () => void
      onPreviewFileChanged: (callback: (payload: GsaPreviewFileChanged) => void) => () => void
...
export interface GsaTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface GsaTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `gogeta update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  gsaRoot?: string
}
...
export interface GsaConnection {
  baseUrl: string
...
export interface GsaTitleBarTheme {
  background: string
  foreground: string
}

export interface GsaWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface GsaApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface GsaNotification {
  title?: string
  body?: string
  silent?: boolean
}

export interface GsaPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface GsaReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface GsaPreviewWatch {
  id: string
  path: string
}

export interface GsaReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface GsaReadDirResult {
  entries: GsaReadDirEntry[]
  error?: string
}

export interface GsaPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface GsaSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
