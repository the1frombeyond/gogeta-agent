...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.gsaDesktop && ['audio', 'video'].includes(mediaKind(path))) {
    return mediaStreamUrl(path)
  }

  if (!window.gsaDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.gsaDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function OpenMediaButton({ kind, path }: { kind: 'audio' | 'video'; path: string }) {
  return (
    <button
      ...
      onClick={() => void window.gsaDesktop?.openExternal(mediaExternalUrl(path))}
      type="button"
    >
      Open {kind} file
    </button>
