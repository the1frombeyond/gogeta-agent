...
const GOGETA_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type GsaRefType = (typeof GOGETA_REF_TYPES)[number]
...
const ICON_PATHS: Record<GsaRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as GsaRefType] ?? ICON_FALLBACK
...
const GOGETA_DIRECTIVE_RE = new RegExp(
...
export const gsaDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(GOGETA_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as GsaRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: GsaRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Gogeta directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => gsaDirectiveFormatter.parse(cleanedText), [cleanedText])
