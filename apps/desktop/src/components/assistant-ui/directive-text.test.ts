...
import { formatRefValue, gsaDirectiveFormatter } from './directive-text'
...
describe('gsaDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = gsaDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = gsaDirectiveFormatter.parse('@file:src/main.tsx the entry point')
