...
import { getGsaConfig, getGsaConfigDefaults } from '@/gogeta'
...
interface GsaConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useGsaConfig({ activeSessionIdRef, refreshProjectBranch }: GsaConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshGsaConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getGsaConfig(), getGsaConfigDefaults().catch(() => ({}))])
  ...
  return { refreshGsaConfig, sttEnabled, voiceMaxRecordingSeconds }
}
