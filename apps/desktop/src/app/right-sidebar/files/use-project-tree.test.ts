...
import type { GsaReadDirResult } from '@/global'

import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<GsaReadDirResult>>()

beforeEach(() => {
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { gsaDesktop: { readDir: typeof readDir } }).gsaDesktop = { readDir }
})

afterEach(() => {
  resetProjectTreeState()
  delete (window as unknown as { gsaDesktop?: unknown }).gsaDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): GsaReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: GsaReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<GsaReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.gsaDesktop is missing', async () => {
    delete (window as unknown as { gsaDesktop?: unknown }).gsaDesktop
