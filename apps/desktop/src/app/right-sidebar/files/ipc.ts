...
import type { GsaReadDirEntry, GsaReadDirResult } from '@/global'

export type ProjectTreeEntry = GsaReadDirEntry
...
async function gitRootFor(start: string) {
  ...
  if (!cached) {
    cached = window.gsaDesktop.gitRoot(key)
...
async function readGitignore(dir: string): Promise<GitignoreRule | null> {
  if (!window.gsaDesktop?.readDir || !window.gsaDesktop.readFileDataUrl) {
    return null
  }

  try {
    const listing = await window.gsaDesktop.readDir(dir)

    if (!listing.entries.some(e => e.name === '.gitignore' && !e.isDirectory)) {
      return null
    }

    const text = decodeDataUrl(await window.gsaDesktop.readFileDataUrl(`${dir}/.gitignore`))
...
function ignoredBy(rules: GitignoreRule[], entry: GsaReadDirEntry) {
...
async function filterIgnored(entries: GsaReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<GsaReadDirResult> {
  if (!window.gsaDesktop) {
    return { entries: [], error: 'no-bridge' }
  }

  const result = await window.gsaDesktop.readDir(dirPath)
