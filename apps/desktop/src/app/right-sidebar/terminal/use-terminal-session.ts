...
const GOGETA_PATHS_MIME = 'application/x-gogeta-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(GOGETA_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(GOGETA_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const terminalApi = window.gsaDesktop?.terminal
    ...
    try {
      ...
      console.warn('[gogeta-terminal] WebGL unavailable; falling back to DOM', err)
    }
