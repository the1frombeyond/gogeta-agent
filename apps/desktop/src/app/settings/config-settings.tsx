...
import type { ConfigFieldSchema, GsaConfigRecord } from '@/types/gogeta'
...
export function ConfigSettings({
  ...
  const { t } = useI18n()
  const c = t.settings.config
  const [config, setConfig] = useState<GsaConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<GsaConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getGsaConfigRecord(), getGsaConfigDefaults(), getGsaConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: GsaConfigRecord) => {
