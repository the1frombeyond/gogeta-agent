...
export const EMPTY_SELECT_VALUE = '__gogeta_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Gogeta & Nous-trained models',
    docsUrl: 'https://portal.gogeta.ai',
    priority: 0
  },
  {
    prefix: 'OPENROUTER_',
  ...
  { prefix: 'GOGETA_GEMINI_', name: 'Gemini', priority: 4 },
  {
    prefix: 'DEEPSEEK_',
    name: 'DeepSeek',
  ...
  { prefix: 'GOGETA_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Gogeta needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Gogeta stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Gogeta can read from one file request.',
  approvals: {
    mode: 'How Gogeta handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Gogeta updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
