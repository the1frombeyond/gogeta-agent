...
import type { GsaConfigRecord } from '@/types/gogeta'
...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: GsaConfigRecord = { display: { theme: 'mono' } }
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // GOGETA_QWEN_ and GOGETA_GEMINI_ both share the GOGETA_ stem.
      expect(providerGroup('GOGETA_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('GOGETA_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
