...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../gogeta'
...
import { useGsaConfig } from './session/hooks/use-gogeta-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.gsaDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.gsaDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.gsaDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshGsaConfig, sttEnabled, voiceMaxRecordingSeconds } = useGsaConfig({
    activeSessionIdRef,
