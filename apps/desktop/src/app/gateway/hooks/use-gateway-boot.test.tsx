...
// bug that drives the REAL useGatewayBoot hook + REAL GsaGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshGsaConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { gsaDesktop?: unknown }).gsaDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { gsaDesktop?: unknown }).gsaDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForGsa) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startGsa()'s remote branch awaits waitForGsa() for 45s before it
    ...
    ;(window as { gsaDesktop?: unknown }).gsaDesktop = desktop
    ...
    // After ~45s waitForGsa gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Gogeta backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
