...
import type { GsaConnection } from '@/global'
import { GsaGateway } from '@/gogeta'
...
import type { RpcEvent } from '@/types/gogeta'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.gsaDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: GsaGateway | null) => void
  refreshGsaConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: GsaConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Gogeta...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // as an opaque "Could not connect to Gogeta gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshGsaConfig().catch(() => undefined)
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshGsaConfig()
