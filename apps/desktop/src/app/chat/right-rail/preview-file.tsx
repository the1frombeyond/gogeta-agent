...
import { GOGETA_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  if (window.gsaDesktop.readFileText) {
    try {
      return await window.gsaDesktop.readFileText(filePath)
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)

      if (!message.includes("No handler registered for 'gogeta:readFileText'")) {
        throw error
      }
    }
  }
  ...
  const dataUrl = await window.gsaDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(GOGETA_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
