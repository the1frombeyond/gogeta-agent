        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.gsaDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Gogeta to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      target.kind !== 'file' ||
      !window.gsaDesktop?.watchPreviewFile ||
      !window.gsaDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.gsaDesktop.onPreviewFileChanged(payload => {
    ...
    void window.gsaDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.gsaDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.gsaDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:gogeta-preview')
