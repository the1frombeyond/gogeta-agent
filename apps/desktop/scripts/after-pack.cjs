 ...
 *
 * Stamps the Gogeta icon + identity onto the packed Windows Gogeta.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `gogeta desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Gogeta')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Gogeta'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Gogeta.exe keeps the stock Electron icon`)
  }
}
