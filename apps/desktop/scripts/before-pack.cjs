 ...
 * product name (`Gogeta`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Gogeta'
 *
 * This is a hard failure with no obvious cause for the user — `gogeta desktop`
 ...
 * (the mac-unpacked Gogeta.app bundle) and Windows (win-unpacked), so we
