...
// no longer bundles the Gogeta Agent Python payload (that's fetched at first
...
const APP = (() => {
  ...
  if (PLATFORM === 'darwin') {
    ...
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Gogeta.app')

    return {

      appPath,

      binary: path.join(appPath, 'Contents', 'MacOS', 'Gogeta'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      ...
      binary: path.join(unpacked, 'Gogeta.exe'),
  ...
  return {
    ...
    binary: path.join(unpacked, 'gogeta'),
...
// Default GOGETA_HOME for non-sandboxed runs -- matches main.cjs's

// resolveGogetaHome(). On Windows it's %LOCALAPPDATA%\gogeta; elsewhere

// it's ~/.gogeta. The fresh-install sandbox launchFresh() sets its own

// GOGETA_HOME and never touches this.

const DEFAULT_GOGETA_HOME = (() => {

  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {

    return path.join(process.env.LOCALAPPDATA, 'gogeta')

  }

  return path.join(os.homedir(), '.gogeta')

})()

const VENV_ROOT = path.join(DEFAULT_GOGETA_HOME, 'gogeta-agent', 'venv')

const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'gogeta-desktop-fresh-install')
...
function ensurePackagedApp() {
  ...
  if (process.env.GOGETA_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
...
function resolveDmgPath() {
  ...
  if (!exists(RELEASE_ROOT)) {
    ...
    return path.join(RELEASE_ROOT, `Gogeta-${PACKAGE_JSON.version}-${ARCH}.dmg`)

  }



  const prefix = `Gogeta-${PACKAGE_JSON.version}`
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Gogeta-${PACKAGE_JSON.version}-${ARCH}.dmg`)
...
function resolveNsisPath() {
  ...
  // electron-builder NSIS artifactName template is 'Gogeta-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.GOGETA_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
...
function ensureNsis() {
  ...
  if (process.env.GOGETA_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
...
function launchFresh() {
  ...
  const gogetaHome = path.join(sandbox, 'gogeta-home')
  ...
  fs.mkdirSync(gogetaHome, { recursive: true })
  ...
  env.GOGETA_DESKTOP_CWD = cwd

  env.GOGETA_DESKTOP_IGNORE_EXISTING = '1'

  env.GOGETA_DESKTOP_TEST_MODE = 'fresh-install'

  env.GOGETA_DESKTOP_USER_DATA_DIR = userDataDir

  env.GOGETA_HOME = gogetaHome

  delete env.GOGETA_DESKTOP_GOGETA

  delete env.GOGETA_DESKTOP_ROOT
  ...
  console.log(`  GOGETA_HOME: ${gogetaHome}`)

  console.log(`  cwd: ${cwd}`)



  return { runtimeRoot: path.join(gogetaHome, 'gogeta-agent', 'venv') }
...
//   - The Gogeta Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of gogeta_cli sneaks back in we want

  // to fail loudly rather than re-introduce the 400MB delta we just removed.

  const staleFactoryMarker = path.join(APP.resourcesPath, 'gogeta-agent', 'gogeta_cli', 'main.py')
...
function help() {
  ...
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Gogeta

  npm run test:desktop:fresh     # build packaged app, launch with temp userData + GOGETA_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ...
  GOGETA_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
