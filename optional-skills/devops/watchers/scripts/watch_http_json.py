...
Usage (via cron with --no-agent):
    ...
    gogeta cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$GOGETA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Gogeta-Watcher/1.0"})
