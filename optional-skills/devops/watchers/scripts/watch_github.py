...
Usage (via cron with --no-agent):
    ...
    gogeta cron create gogeta-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$GOGETA_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name gogeta-issues --repo NousResearch/gogeta --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.gogeta/.env to avoid the 60 req/hr
