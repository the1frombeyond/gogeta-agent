...
version: 0.1.0
author: Teknium (teknium1), Gogeta Agent
license: MIT
...
Invoke through the `terminal` tool from the target repo's root, then use `read_file` / `search_files` / `write_file` to produce the wiki. Default output location is `~/.gogeta/wikis/<repo-name>/`. Only write into the repo (`docs/wiki/`) when the user explicitly requests it.
...
```bash
OUTPUT_DIR="$HOME/.gogeta/wikis/$REPO_NAME"
...
1. **User input** — [`cli.py:GsaCLI.run_session`](<link>)
...
cat > "$OUTPUT_DIR/.codewiki-state.json" <<EOF
{
  ...
  "generator": "gogeta-agent code-wiki skill v0.1.0",
  "modules_documented": []
}
EOF
```
...
```
Generated wiki at ~/.gogeta/wikis/<repo-name>/:
...
- **In-repo output without asking.** Default is `~/.gogeta/wikis/`. Only write into the repo when the user explicitly requests it.
