...
Maps each role archetype to the Gogeta skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public gogeta-agent
...
## Gogeta skills relevant to video production

### Visual / rendering skills (`gogeta-agent/skills/creative/`)
...
### Design / pre-production skills (`gogeta-agent/skills/creative/`)
...
### Audio / media skills (`gogeta-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`gogeta-agent/skills/devops/`)
...
These are **not** Gogeta skills but external CLIs / APIs that profiles invoke.
...
## Built-in Gogeta tools for media review

These are native Gogeta tools — not invoked via terminal but through their own
...
key is present in `~/.gogeta/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Gogeta profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.gogeta/.env` → environment variable.
...
When a new Gogeta-public video skill ships:
