...
gogeta kanban watch --tenant <project-slug>

# Snapshot of the board
gogeta kanban list --tenant <project-slug>
gogeta kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
gogeta kanban stats --tenant <project-slug>

# Visual dashboard (browser)
gogeta dashboard

# Inspect a specific task (includes comments + events)
gogeta kanban show <task-id>

# Follow a single task's event stream
gogeta kanban tail <task-id>
```

Verify available subcommands with `gogeta kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `gogeta kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `gogeta kanban block <id>` then `gogeta kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `gogeta kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `gogeta kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `gogeta profile list` (profile exists?), `gogeta status` (gateway/dispatcher up?), and verify tenant. |
...
gogeta kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
gogeta kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(gogeta kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
gogeta kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
gogeta kanban block <task-id>
gogeta kanban archive <task-id>

# Diagnose what happened
gogeta kanban show <task-id>      # task body, comments, recent events
gogeta kanban tail <task-id>      # follow the live event stream
gogeta kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `gogeta kanban create` for the director
...
while true; do
    clear
    gogeta kanban list --tenant <slug>
    echo "---"
    gogeta kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `gogeta kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `gogeta profile list` and start
