...
- Skill helper paths:
  - `${GOGETA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${GOGETA_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${GOGETA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client gogeta
```
...
```bash
PUBLISH="${GOGETA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client gogeta
```
...
```bash
DRIVE="${GOGETA_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `gogeta`)    |
