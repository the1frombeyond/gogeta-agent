---
name: telephony
description: Give Gogeta phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
...
This optional skill gives Gogeta practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.gogeta/.env`
...
It does **not** turn Gogeta into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Gogeta memory
...
### 1) "I want Gogeta to own a real phone number"
Use **Twilio**.
...
- pairs well with Gogeta `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.gogeta/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
gogeta skills search telephony
gogeta skills install official/productivity/telephony
```
...
Then save credentials into Gogeta:
...
3. Buy it and save it into `~/.gogeta/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Gogeta calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Gogeta's existing `text_to_speech` support.

Use this when:
- you want the call to use Gogeta's configured TTS voice rather than Twilio `<Say>`
...
Recommended Gogeta TTS -> Twilio Play workflow:

1. Generate the audio with Gogeta `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Gogeta to create the message audio with `text_to_speech`
...
Important note:
- Gogeta TTS is great for prerecorded outbound messages
...
- Gogeta STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Gogeta memory.
...
- Do not store arbitrary third-party phone numbers in Gogeta memory.
...
3. persist that number to `~/.gogeta/.env`
