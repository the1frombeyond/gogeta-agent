...
- smoke-test a server with the FastMCP CLI before wiring it into Gogeta or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Gogeta. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.gogeta/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.gogeta/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Gogeta integration, either:

- configure the server in `~/.gogeta/config.yaml` using the `native-mcp` skill, or
...
### Gogeta cannot see the deployed server

The server-building part may be correct while the Gogeta config is not. Load the `native-mcp` skill and configure the server in `~/.gogeta/config.yaml`, then restart Gogeta.
