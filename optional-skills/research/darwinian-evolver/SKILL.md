...
version: 0.1.0
author: Bihruze (Asahi0x), Gogeta Agent
license: MIT
...
import upstream classes into Gogeta itself.
...
```bash
mkdir -p ~/.gogeta/cache/darwinian-evolver && cd ~/.gogeta/cache/darwinian-evolver
...
```bash
cd ~/.gogeta/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.gogeta/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.gogeta/skills/research/darwinian-evolver
DE_DIR=~/.gogeta/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Gogeta core.
   Custom driver scripts under `~/.gogeta/skills/...` are user-side and fine.
