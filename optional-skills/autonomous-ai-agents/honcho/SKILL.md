---
name: honcho
description: Configure and use Honcho memory with Gogeta -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Gogeta Agent
license: MIT
...
# Honcho Memory for Gogeta

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Gogeta profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/gogeta#running-honcho-locally-with-gogeta

### Verify

```bash
gogeta honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Gogeta profile's AI peer
...
Honcho models conversations as interactions between **peers**. Gogeta creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Gogeta instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Gogeta run |
| `global` | Single session across all directories |

Manual override: `gogeta honcho map my-project-name`
...
Each Gogeta profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
gogeta profile create coder --clone
# creates host block gogeta.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `gogeta.coder` host block in `honcho.json`
...
```bash
gogeta honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Gogeta when Honcho memory is active.
...
Config file: `$GOGETA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `gogeta honcho setup`. Ensure `memory.provider: honcho` is in `~/.gogeta/config.yaml`.

### Memory not persisting across sessions
Check `gogeta honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `gogeta profile create <name> --clone`. For existing profiles: `gogeta honcho sync`.
...
| Command | Description |
|---------|-------------|
| `gogeta honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `gogeta honcho status` | Show resolved config, connection test, peer info for active profile |
| `gogeta honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `gogeta honcho disable` | Disable Honcho for the active profile |
| `gogeta honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `gogeta honcho peers` | Show peer identities across all profiles |
| `gogeta honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `gogeta honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `gogeta honcho sessions` | List known directory-to-session-name mappings |
| `gogeta honcho map <name>` | Map current working directory to a Honcho session name |
| `gogeta honcho identity` | Seed AI peer identity or show both peer representations |
| `gogeta honcho sync` | Create host blocks for all Gogeta profiles that don't have one yet |
| `gogeta honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Gogeta + Honcho |
| `gogeta memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `gogeta memory status` | Show active memory provider and config |
| `gogeta memory off` | Disable external memory provider |
