...
version: 0.1.0
author: Matt Maximo (MattMaximo), Gogeta Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  gogeta:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, gogeta-agent]
---

# Grok Build CLI — Gogeta Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Gogeta terminal. Grok
...
1. Prepare stable input files first with Gogeta tools (`read_file`,
...
2. **Don't conflate Gogeta' xAI auth with the `grok` CLI's auth.** Gogeta'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Gogeta' xAI auth carries
   over.
