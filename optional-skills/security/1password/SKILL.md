...
version: 1.0.0
author: arceus77-7, enhanced by Gogeta Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during Gogeta terminal calls (desktop app flow only)
...
### Service Account (recommended for Gogeta)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.gogeta/.env` (the skill will prompt for this on first load).
...
## Gogeta Execution Pattern (desktop app flow)

Gogeta terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/gogeta-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/gogeta-op.sock"
