...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      gogeta-agent = self'.packages.default;
      gsaVenv = gogeta-agent.gsaVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "gogeta-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${gsaVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "gogeta-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "gogeta-build-package" { } ''
          echo "PASS: package built at ${gogeta-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "gogeta-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "gogeta-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${gogeta-agent}/bin/gogeta || (echo "FAIL: gogeta binary missing"; exit 1)
          test -x ${gogeta-agent}/bin/gogeta-agent || (echo "FAIL: gogeta-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${gogeta-agent}/bin/gogeta version 2>&1 | grep -qi "gogeta" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "gogeta-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in gogeta gogeta-agent gogeta-acp; do
            test -x ${gogeta-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "gogeta-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking gogeta --help ==="
          ${gogeta-agent}/bin/gogeta --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${gogeta-agent}/bin/gogeta --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "gogeta-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${gogeta-agent}/share/gogeta-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${gogeta-agent}/share/gogeta-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "GOGETA_BUNDLED_SKILLS" ${gogeta-agent}/bin/gogeta || \
            (echo "FAIL: GOGETA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: GOGETA_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "gogeta-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${gogeta-agent}/share/gogeta-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${gogeta-agent}/share/gogeta-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "GOGETA_BUNDLED_PLUGINS" ${gogeta-agent}/bin/gogeta || \
            (echo "FAIL: GOGETA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: GOGETA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "gogeta-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${gogeta-agent}/share/gogeta-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${gogeta-agent}/share/gogeta-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${gogeta-agent}/share/gogeta-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "GOGETA_BUNDLED_LOCALES" ${gogeta-agent}/bin/gogeta || \
            (echo "FAIL: GOGETA_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: GOGETA_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (GOGETA_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && GOGETA_BUNDLED_LOCALES=${gogeta-agent}/share/gogeta-agent/locales \
            ${gsaVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with GOGETA_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${gsaVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${gsaVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without GOGETA_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "gogeta-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${gogeta-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${gogeta-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "GOGETA_TUI_DIR" ${gogeta-agent}/bin/gogeta || \
            (echo "FAIL: GOGETA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: GOGETA_TUI_DIR set in wrapper"
        ...
        # Verify GOGETA_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        gogeta-node = pkgs.runCommand "gogeta-node-version" { } ''
          set -e
          echo "=== Checking GOGETA_NODE in wrapper ==="
          grep -q "GOGETA_NODE" ${gogeta-agent}/bin/gogeta || \
            (echo "FAIL: GOGETA_NODE not set in wrapper"; exit 1)
          echo "PASS: GOGETA_NODE present in wrapper"

          GOGETA_NODE=$(sed -n "s/^export GOGETA_NODE='\(.*\)'/\1/p" ${gogeta-agent}/bin/gogeta)
          test -x "$GOGETA_NODE" || (echo "FAIL: GOGETA_NODE=$GOGETA_NODE not executable"; exit 1)
          echo "PASS: GOGETA_NODE executable at $GOGETA_NODE"

          NODE_MAJOR=$("$GOGETA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All GOGETA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify GOGETA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "gogeta-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(GOGETA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking GOGETA_MANAGED guards ==="
          check_blocked "config set" ${gogeta-agent}/bin/gogeta config set model foo
          check_blocked "config edit" ${gogeta-agent}/bin/gogeta config edit
        ...
        extra-python-packages = let
          ...
          gsaWithExtra = gogeta-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "gogeta-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${gsaWithExtra}/bin/gogeta || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${gsaWithExtra}/bin/gogeta || \
          ...
          if grep -q "PYTHONPATH" ${gogeta-agent}/bin/gogeta; then
        ...
        extra-dependency-groups = let
          gsaWithGroups = gogeta-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "gogeta-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${gsaWithGroups}"
          echo "venv: ${gsaWithGroups.gsaVenv}"
        ...
        messaging-variant = pkgs.runCommand "gogeta-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.gsaVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "gogeta-config-roundtrip" {
          ...
          merge_and_load() {
            local gogeta_home="$1"
            export GOGETA_HOME="$gogeta_home"
            ${configMergeScript} ${nixSettings} "$gogeta_home/config.yaml"
            ${gsaVenv}/bin/python3 -c '
import json, sys
