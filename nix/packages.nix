# nix/packages.nix — Gogeta Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    { pkgs, lib, inputs', ... }:
    let
      gsaAgent = pkgs.callPackage ./gogeta-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = gsaAgent.gsaTui;
        web = gsaAgent.gsaWeb;
        desktop = gsaAgent.gsaDesktop;

        fix-lockfiles = gsaAgent.gsaNpmLib.mkFixLockfiles { attr = "tui"; };
      };
    };
}
