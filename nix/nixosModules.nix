# nix/nixosModules.nix — NixOS module for gogeta-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: gogeta runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $GOGETA_HOME/.env
# and read by gogeta at startup — no container recreation needed for env changes.
#
# Tool resolution: the gogeta wrapper uses --suffix PATH for nix store tools,
...
#
# Usage:
#   services.gogeta-agent = {
#     enable = true;
#     settings.model = "anthropic/claude-sonnet-4";
#     environmentFiles = [ config.sops.secrets."gogeta/env".path ];
#   };
#
{ inputs, ... }: {
  ...
  let
    cfg = config.services.gogeta-agent;
    effectivePackage =
    ...
    gogeta-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-gogeta-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "gogeta-config-attrs";
      description = "Gogeta YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "gogeta-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "gogeta-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'GOGETA_DOC_EOF'\n${value}\nGSA_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "gogeta-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/gogeta";
    ...
    # gogeta user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "gogeta-container-entrypoint" ''
      set -eu

      GOGETA_UID="''${GOGETA_UID:?GOGETA_UID must be set}"
      GOGETA_GID="''${GOGETA_GID:?GOGETA_GID must be set}"

      # ── Group: ensure a group with GID=$GOGETA_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$GOGETA_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="gogeta"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$GOGETA_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$GOGETA_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$GOGETA_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$GOGETA_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="gogeta"
        TARGET_HOME="/home/gogeta"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$GOGETA_UID" -g "$GOGETA_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$GOGETA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$GOGETA_UID:$GOGETA_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure GOGETA_HOME is owned by the target user.
      ...
      if [ -n "''${GOGETA_HOME:-}" ] && [ -d "$GOGETA_HOME" ]; then
        find "$GOGETA_HOME" \! -user "$GOGETA_UID" -exec chown "$GOGETA_UID:$GOGETA_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/gogeta-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/gogeta-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/gogeta ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/gogeta
        chmod 0440 /etc/sudoers.d/gogeta
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$GOGETA_UID" --regid="$GOGETA_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $GOGETA_HOME/.env.
    ...
    # Default: /var/lib/gogeta/workspace → /data/workspace.
  ...
  in {
    options.services.gogeta-agent = with lib; {
      enable = mkEnableOption "Gogeta Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = gogeta-agent;
        description = "The gogeta-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/gogeta";
        description = "State directory. Contains .gogeta/ subdir (GOGETA_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Gogeta config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $GOGETA_HOME/.env at activation time.
          Gogeta reads this file on every startup via load_gogeta_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $GOGETA_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $GOGETA_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `gogeta gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the gogeta user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the gogeta plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Gogeta discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          gogeta_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as gogeta (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in gogeta-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the gogeta CLI to environment.systemPackages and export
          GOGETA_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.gogeta symlink to the service
            stateDir. These users are automatically added to the gogeta group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.gogeta-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the gogeta CLI to system PATH and export GOGETA_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.gogeta/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.GOGETA_HOME = "${cfg.stateDir}/.gogeta";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.gogeta-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the gogeta user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.gogeta-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed gogeta
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure gogeta is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.gogeta        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gogeta/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gogeta/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gogeta/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gogeta/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.gogeta/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."gogeta-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.gogeta
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.gogeta ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.gogeta ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.gogeta -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.gogeta/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.gogeta/$_subdir"
            chmod 2770 "${cfg.stateDir}/.gogeta/$_subdir"
            find "${cfg.stateDir}/.gogeta/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.gogeta/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.gogeta/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.gogeta/config.yaml
            chmod 0640 ${cfg.stateDir}/.gogeta/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.gogeta/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.gogeta/.managed
          chmod 0644 ${cfg.stateDir}/.gogeta/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.gogeta/.container-mode <<'GOGETA_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    gogeta_bin=${containerDataDir}/current-package/bin/gogeta
    GOGETA_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.gogeta/.container-mode
            chmod 0644 ${cfg.stateDir}/.gogeta/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.gogeta/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.gogeta";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.gogeta" ]; then
                  rm -f "${symlinkPath}"
                  echo "gogeta-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.gogeta -> stateDir/.gogeta for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.gogeta";
                target = "${cfg.stateDir}/.gogeta";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "gogeta-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.gogeta/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.gogeta/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.gogeta/auth.json
              fi
            ''}
          ''}
          ...
          # Gogeta reads $GOGETA_HOME/.env at startup via load_gogeta_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.gogeta/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'GOGETA_NIX_ENV_EOF'
    ${envFileContent}
    GOGETA_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.gogeta/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.gogeta/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.gogeta/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.gogeta-agent = {
          description = "Gogeta Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            GOGETA_HOME = "${cfg.stateDir}/.gogeta";
            GOGETA_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $GOGETA_HOME/.env by the activation script. load_gogeta_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/gogeta"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the gogeta group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.gogeta-agent = {
          description = "Gogeta Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              GOGETA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              GOGETA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env GOGETA_UID="$GOGETA_UID" \
                --env GOGETA_GID="$GOGETA_GID" \
                --env GOGETA_HOME=${containerDataDir}/.gogeta \
                --env GOGETA_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/gogeta gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
