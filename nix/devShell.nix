...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      gsaNpmLib = self'.packages.default.passthru.gsaNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. gogeta-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        ...
        shellHook = ''
          echo "Gogeta Agent dev shell"
          ${combinedNonNpm}
          ${gsaNpmLib.mkNpmDevShellHook npmPackageJsonPaths fixLockfilesExe}
          echo "Ready. Run 'gogeta' to start."
        '';
      };
    };
}
