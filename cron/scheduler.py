...
Uses a file-based lock (~/.gogeta/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `gogeta update` reloads
# the module) fail with ModuleNotFoundError for gogeta_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from gogeta_constants import get_gogeta_home
from gogeta_cli._subprocess_compat import windows_hide_flags
from gogeta_cli.config import load_config, _expand_env_vars
from gogeta_time import now as _gogeta_now
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``gogeta tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from gogeta_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_gogeta_home: Path | None = None


def _get_gogeta_home() -> Path:
    """Resolve Gogeta home dynamically while preserving test monkeypatch hooks."""
    return _gogeta_home or get_gogeta_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    gogeta_home = _get_gogeta_home()
    lock_dir = gogeta_home / "cron"
    return lock_dir, lock_dir / ".tick.lock"


@contextmanager
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Gogeta profile.
    ...
    the scheduler's test/override hook and a context-local Gogeta home override
    both point at the resolved profile directory so _get_gogeta_home(),
    .env/config loading, script resolution, AIAgent construction, and downstream
    get_gogeta_home() callers agree on the same home.
    ...
    global _gogeta_home
    prior_override = _gogeta_home
    env_snapshot = os.environ.copy()

    from gogeta_cli.profiles import normalize_profile_name, resolve_profile_env
    from gogeta_constants import reset_gogeta_home_override, set_gogeta_home_override
    ...
    override_token = None
    try:
        override_token = set_gogeta_home_override(profile_home)
        _gogeta_home = profile_home
        logger.info(
            "Job '%s': using Gogeta profile '%s' (%s)",
            job_id,
            normalized_profile,
            profile_home,
        )
        ...
        _gogeta_home = prior_override
        if override_token is not None:
            reset_gogeta_home_override(override_token)
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("GOGETA_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid GOGETA_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within GOGETA_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against GOGETA_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_gogeta_home() / "scripts"
    ...
    # escape — scripts MUST reside within GOGETA_HOME/scripts/.
    try:
    ...
    run_env["GOGETA_HOME"] = str(_get_gogeta_home())
    try:
        from gogeta_constants import get_subprocess_home
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _gogeta_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_gogeta_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_gogeta_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["GOGETA_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed GOGETA_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads GOGETA_SESSION_PLATFORM
    #     and GOGETA_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via GOGETA_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     GOGETA_SESSION_PLATFORM == "telegram".
    ...
    #     GOGETA_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read GOGETA_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the GOGETA_CRON_AUTO_DELIVER_* vars set
    # below, so clearing GOGETA_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "GOGETA_CRON_AUTO_DELIVER_PLATFORM",
        "GOGETA_CRON_AUTO_DELIVER_CHAT_ID",
        "GOGETA_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_gogeta_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_gogeta_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["GOGETA_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["GOGETA_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["GOGETA_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("GOGETA_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_gogeta_home() / "config.yaml")
        ...
        prefill_file = (
            os.getenv("GOGETA_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_gogeta_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject GOGETA_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # GOGETA_HOME. When a workdir is configured, also inject project
        ...
        # override via GOGETA_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("GOGETA_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid GOGETA_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_gogeta_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_gogeta_now().strftime('%b %d %H:%M')}"
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _gogeta_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _gogeta_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set GOGETA_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("GOGETA_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid GOGETA_CRON_MAX_PARALLEL value; defaulting to unbounded")
        ...
        # Gogeta home override, scheduler _gogeta_home hook, and temporary
