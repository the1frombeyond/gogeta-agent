#!/bin/sh
# shellcheck shell=sh
# /opt/gogeta/bin/gogeta — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `gogeta` user (UID 10000). When an operator runs `docker exec <c> gogeta ...`
# the default UID is root (0), and any file the command writes under
# $GOGETA_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> gogeta login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Gogeta is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> gogeta
...
# Fix
# ---
# This shim sits at /opt/gogeta/bin/gogeta and is placed earliest on PATH.
# When invoked as root, it drops to the gogeta user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $GOGETA_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user gogeta`, kanban subagents, etc. — it short-circuits
...
# (/opt/gogeta/.venv/bin/gogeta), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set GOGETA_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the gogeta CLI. Default is to drop.

set -e

REAL=/opt/gogeta/.venv/bin/gogeta

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "gogeta-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user gogeta`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${GOGETA_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the gogeta user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "gogeta-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "gogeta-shim: re-run with --user gogeta or set GOGETA_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the gogeta user's home before dropping privileges. Without
