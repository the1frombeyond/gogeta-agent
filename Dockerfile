...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/gogeta/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when gogeta
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# gogeta process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main gogeta process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Gogeta WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via GOGETA_UID at runtime
RUN useradd -u 10000 -m -d /opt/data gogeta
...
#
# ui-tui/packages/gogeta-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/gogeta-ink/ ui-tui/packages/gogeta-ink/
...
# lock on the @gogeta/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/gogeta/.venv at first
...
# ---------- Permissions ----------
# Make install dir world-readable so any GOGETA_UID can read it at runtime.
# The venv needs to be traversable too.
# node_modules trees additionally need to be writable by the gogeta user
# so the runtime `npm install` triggered by _tui_need_npm_install() in
# gogeta_cli/main.py succeeds (see #18800). /opt/gogeta/web is build-time
# only (GOGETA_WEB_DIST points at gogeta_cli/web_dist) and is intentionally
# not chowned here.
# /opt/gogeta/gateway is runtime-writable: Python may create __pycache__ and
# gateway state artifacts beneath the package after services drop privileges,
# especially when the gogeta UID is remapped at boot (#27221).
# The .venv MUST remain gogeta-writable so lazy_deps.py can install
# remaining optional platform packages and future pin bumps at first use.
# Without this, `uv pip install` fails with EACCES and adapters silently
# fail to load.  See tools/lazy_deps.py.
USER root
RUN chmod -R a+rX /opt/gogeta && \
    chown -R gogeta:gogeta /opt/gogeta/.venv /opt/gogeta/ui-tui /opt/gogeta/gateway /opt/gogeta/node_modules
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the gogeta user via
# `s6-setuidgid gogeta` in its run script. If GOGETA_UID is unset, services
# run as the default gogeta user (UID 10000).

# ---------- Link gogeta-agent itself (editable) ----------
...
# container always returns nothing — meaning `gogeta dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the GOGETA_GIT_SHA build-arg to

# /opt/gogeta/.gogeta_build_sha at build time, and have

# gogeta_cli/build_info.py read it at runtime.  Both `gogeta dump` and
...
RUN if [ -n "${GOGETA_GIT_SHA}" ]; then \

        printf '%s\n' "${GOGETA_GIT_SHA}" > /opt/gogeta/.gogeta_build_sha && \

        chown gogeta:gogeta /opt/gogeta/.gogeta_build_sha; \

    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-gogeta + dashboard.
...
# `exec gogeta`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $GOGETA_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/gogeta/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-gogeta-setup && \
    chmod +x /etc/cont-init.d/01-gogeta-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV GOGETA_WEB_DIST=/opt/gogeta/gogeta_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/gogeta/ui-tui/dist/entry.js`) and skip the
...
ENV GOGETA_TUI_DIR=/opt/gogeta/ui-tui
ENV GOGETA_HOME=/opt/data

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> gogeta ...` they default to root, and any file the
# command writes under $GOGETA_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/gogeta/bin/gogeta, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid gogeta`
# when invoked as root. Non-root callers (supervised processes,
# `--user gogeta`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/gogeta/.venv/bin/gogeta). See the shim source for
# the opt-out env var (GOGETA_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/gogeta-exec-shim.sh /opt/gogeta/bin/gogeta

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> gogeta ...` and any
# subprocess that doesn't activate the venv first still find gogeta.
#
# /opt/gogeta/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/gogeta/bin:/opt/gogeta/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. gogeta
# subcommand vs. no-args), drops to the gogeta user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/gogeta/docker/main-wrapper.sh" ]
CMD [ ]
