...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("GOGETA_HOME", os.path.expanduser("~/.gogeta")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("GOGETA_HOME", Path.home() / ".gogeta")) / "skills" / "red-teaming" / "godmode"
...
GOGETA_HOME = Path(os.getenv("GOGETA_HOME", Path.home() / ".gogeta"))
CONFIG_PATH = GOGETA_HOME / "config.yaml"
PREFILL_PATH = GOGETA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Gogeta models — already uncensored, just needs clean prompt
    "gogeta": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "gogeta" in model_lower or "nous" in model_lower:
        return "gogeta"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Gogeta config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.gogeta/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Gogeta for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Gogeta for changes to take effect.")
