...
## 5. GODMODE FAST — Gogeta 4 405B (Zero Refusal)

**Model:** `nousresearch/gogeta-4-405b`
**Strategy:** Gogeta models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Gogeta Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.gogeta/config.yaml`:
