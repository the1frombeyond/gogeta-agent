#!/usr/bin/env python3
"""Google Workspace API CLI for Gogeta Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Gogeta-facing JSON contract and falls back to the Python client
...
from _gogeta_home import get_gogeta_home
...
def _gws_binary() -> str | None:
    override = os.getenv("GOGETA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Gogeta Agent")
