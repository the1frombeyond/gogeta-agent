...
description: "Operate the Teams meeting summary pipeline via Gogeta CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions."
version: 1.1.0
...
Everything operator-facing is a `gogeta teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `~/.gogeta/.env`:
...
```bash
gogeta teams-pipeline validate              # config snapshot — run first after any change
gogeta teams-pipeline token-health          # Graph token status
gogeta teams-pipeline token-health --force-refresh   # force a fresh token acquisition
gogeta teams-pipeline list                  # recent meeting jobs
gogeta teams-pipeline list --status failed  # only failed jobs
gogeta teams-pipeline show <job-id>         # full detail of one job
gogeta teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
gogeta teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
gogeta teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
gogeta teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
gogeta teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

gogeta teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
gogeta teams-pipeline delete-subscription <sub-id>
gogeta teams-pipeline maintain-subscriptions            # renew near-expiry ones
gogeta teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `gogeta teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `gogeta cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
