...
## 5. Gogeta Configuration

### Config location

`$GOGETA_HOME/config.yaml` (defaults to `~/.gogeta/config.yaml` when `GOGETA_HOME` is unset)
...
Restart the Gogeta session for changes to take effect. The MCP connection is
