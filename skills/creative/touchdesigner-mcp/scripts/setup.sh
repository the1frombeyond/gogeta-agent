...
GOGETA_HOME_DIR="${GOGETA_HOME:-$HOME/.gogeta}"
GOGETA_CFG="${GOGETA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Gogeta config has twozero_td MCP entry ──
if [[ ! -f "$GOGETA_CFG" ]]; then
    echo -e " ${FAIL} Gogeta config not found at ${GOGETA_CFG}"
    manual_steps+=("Create ${GOGETA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$GOGETA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Gogeta config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Gogeta config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${GOGETA_CFG} manually"); }
    manual_steps+=("Restart Gogeta session to pick up config change")
fi
