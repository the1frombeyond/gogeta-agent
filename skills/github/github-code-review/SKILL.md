...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.gogeta/.env ] && grep -q "^GITHUB_TOKEN=" ~/.gogeta/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.gogeta/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Gogeta Agent\",
    \"comments\": [
...
```bash
source "${GOGETA_HOME:-$HOME/.gogeta}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Gogeta Agent. Code looks clean — good test coverage, no security concerns."
