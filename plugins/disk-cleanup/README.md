...
Auto-tracks and cleans up ephemeral files created during Gogeta Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$GOGETA_HOME` and `/tmp/gogeta-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `GOGETA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under GOGETA_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `GOGETA_HOME` or `/tmp/gogeta-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$GOGETA_HOME/disk-cleanup/` is itself excluded
- `$GOGETA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
