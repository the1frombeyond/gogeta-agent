"""disk_cleanup — ephemeral file cleanup for Gogeta Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under GOGETA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly GOGETA_HOME and /tmp/gogeta-*
Never touches: ~/.gogeta/logs/ or any system directory.
"""
...
try:
    ...
    def get_gogeta_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("GOGETA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".gogeta").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$GOGETA_HOME/logs/``."""
    return get_gogeta_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$GOGETA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under GOGETA_HOME or ``/tmp/gogeta-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    gogeta_home = get_gogeta_home()
    try:
        path.resolve().relative_to(gogeta_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/gogeta-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("gogeta-"):
        return True
    return False
...
# Paths under $GOGETA_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so GOGETA_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        gogeta_home = get_gogeta_home()
        for parent in ("cron", "cronjobs"):
            base = gogeta_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside GOGETA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under GOGETA_HOME (but leave GOGETA_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "gogeta-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(gogeta_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == gogeta_home:
                continue
            try:
                rel_parts = dirpath.relative_to(gogeta_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    gogeta_home = get_gogeta_home()
    try:
        rel = path.resolve().relative_to(gogeta_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "gogeta-agent",
        }:
            return None
        ...
        # Path isn't under GOGETA_HOME (e.g. /tmp/gogeta-*) — fall through.
        pass
