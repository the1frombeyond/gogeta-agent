...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      GOGETA_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      GOGETA_DASHBOARD_PORTAL_URL       — defaults to
                                          ``https://portal.gogeta.ai``
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``gogeta-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``gogeta_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set GOGETA_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via GOGETA_DASHBOARD_PORTAL_URL for
# staging (portal.rewbs.uk) or a custom deployment. Contract docs name
# this as the production issuer.
_DEFAULT_PORTAL_URL = "https://portal.gogeta.ai"
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"gogeta_session_pkce\"]``
        ...
        cookie_payload = {
            "gogeta_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between GOGETA_DASHBOARD_PORTAL_URL / GOGETA_DASHBOARD_OAUTH_CLIENT_ID
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``GOGETA_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("GOGETA_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``GOGETA_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("GOGETA_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``GOGETA_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set GOGETA_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``gogeta_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            "GOGETA_DASHBOARD_OAUTH_CLIENT_ID is not set (and "
            "dashboard.oauth.client_id in config.yaml is empty). The "
            "Nous Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Gogeta Agent "
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"GOGETA_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
