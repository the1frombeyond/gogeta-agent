...
A standards-compliant OpenID Connect Relying Party for the ``gogeta dashboard``
...
:class:`~gogeta_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``gogeta_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~gogeta_cli.dashboard_auth.Session`.
...
same precedence convention as the ``nous`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          issuer: https://auth.example.com/application/o/gogeta/   # required
          client_id: gogeta-dashboard                              # required
          scopes: "openid profile email"                           # optional

    # Environment overrides (Docker/Fly secret injection)
    GOGETA_DASHBOARD_OIDC_ISSUER
    GOGETA_DASHBOARD_OIDC_CLIENT_ID
    GOGETA_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Gogeta does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``GOGETA_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "GOGETA_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "GOGETA_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("GOGETA_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(GOGETA_DASHBOARD_OIDC_ISSUER + GOGETA_DASHBOARD_OIDC_CLIENT_ID) "
