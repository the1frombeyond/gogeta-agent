...
Optional Gogeta observability plugin that maps Gogeta observer hooks to
...
not replace Gogeta Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Gogeta emit NeMo Relay lifecycle events for
the work Gogeta already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Gogeta Agent can:

- Preserve Gogeta execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
gogeta plugins enable observability/nemo_relay
```

The `GOGETA_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `GOGETA_HOME` that the
agent run will use:

```bash
env GOGETA_HOME=/tmp/gogeta-nemo-relay-test \
  gogeta plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`GOGETA_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`GOGETA_HOME` is the Gogeta profile/config home used by both
`gogeta plugins enable ...` and the later `gogeta chat ...` run. If unset,
Gogeta uses the user's default home, usually `~/.gogeta`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export GOGETA_HOME=/tmp/gogeta-nemo-relay-test
gogeta plugins enable observability/nemo_relay
gogeta chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `gogeta` command you run is built from the
...
uv run gogeta plugins enable observability/nemo_relay
uv run gogeta chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/gogeta_agent-*.whl
...
The plugin can configure exporters directly from `GOGETA_NEMO_RELAY_*`
...
```bash
export GOGETA_NEMO_RELAY_ATOF_ENABLED=1
export GOGETA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export GOGETA_NEMO_RELAY_ATIF_ENABLED=1
export GOGETA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `GOGETA_NEMO_RELAY_ATOF_FILENAME`
- `GOGETA_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `GOGETA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `GOGETA_NEMO_RELAY_ATIF_AGENT_NAME`
- `GOGETA_NEMO_RELAY_ATIF_AGENT_VERSION`
- `GOGETA_NEMO_RELAY_ATIF_MODEL_NAME`
- `GOGETA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Gogeta at it:

```bash
export GOGETA_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Gogeta Agent"
agent_version = "local"
```

When `GOGETA_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`GOGETA_NEMO_RELAY_ATOF_*` fallback setup is skipped.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Gogeta routes LLM and tool
...
For the full generic Gogeta middleware contract, see
...
export GOGETA_HOME=/tmp/gogeta-nemo-relay-docs/gogeta-home
mkdir -p "$GOGETA_HOME"

cat > "$GOGETA_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Gogeta session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export GOGETA_NEMO_RELAY_ATOF_ENABLED=1
export GOGETA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/gogeta-nemo-relay-docs/subagent/atof
export GOGETA_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export GOGETA_NEMO_RELAY_ATOF_MODE=overwrite
export GOGETA_NEMO_RELAY_ATIF_ENABLED=1
export GOGETA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/gogeta-nemo-relay-docs/subagent/atif
export GOGETA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export GOGETA_NEMO_RELAY_ATIF_AGENT_NAME='Gogeta Agent E2E'
export GOGETA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export GOGETA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"gogeta.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Gogeta Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Gogeta dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/gogeta-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/gogeta-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/gogeta-nemo-relay-docs/workdir/beta.txt
cd /tmp/gogeta-nemo-relay-docs/workdir

export GOGETA_NEMO_RELAY_ATOF_ENABLED=1
export GOGETA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/gogeta-nemo-relay-docs/parallel/atof
export GOGETA_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export GOGETA_NEMO_RELAY_ATOF_MODE=overwrite
export GOGETA_NEMO_RELAY_ATIF_ENABLED=1
export GOGETA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/gogeta-nemo-relay-docs/parallel/atif
export GOGETA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export GOGETA_NEMO_RELAY_ATIF_AGENT_NAME='Gogeta Agent E2E'
export GOGETA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Gogeta sessions map to `agent` scopes.
- Gogeta API request hooks map to `llm` scope start/end events.
- Gogeta tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Gogeta execution middleware to hand
...
Enable it for Gogeta:

```bash
export GOGETA_NEMO_RELAY_PLUGINS_TOML=/tmp/gogeta-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Gogeta routes execution
through these boundaries:

```text
Gogeta provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Gogeta provider adapter next_call(...)

Gogeta tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Gogeta tool dispatcher next_call(...)
```
...
middleware for a local Gogeta run.

```bash
pip install "nemo-relay==0.3"

export GOGETA_HOME=/tmp/gogeta-middleware-test/gogeta-home
mkdir -p "$GOGETA_HOME" /tmp/gogeta-middleware-test/nemo-relay
...
cat > /tmp/gogeta-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/gogeta-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/gogeta-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Gogeta Middleware E2E"
agent_version = "local"
...
export GOGETA_NEMO_RELAY_PLUGINS_TOML=/tmp/gogeta-middleware-test/nemo-relay/plugins.toml
