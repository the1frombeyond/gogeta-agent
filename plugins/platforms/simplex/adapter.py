"""SimpleX Chat platform adapter (Gogeta plugin).
...
This adapter ships as a Gogeta platform plugin under
``plugins/platforms/simplex/``. The Gogeta plugin loader scans the
...
discoverable and `gogeta setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Gogeta — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"gogeta-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``gogeta setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.gogeta/.env`` via ``gogeta_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("gogeta_cli.config not available; set SIMPLEX_* vars manually in ~/.gogeta/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `gogeta gateway status` without
