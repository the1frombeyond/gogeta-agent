"""
LINE Messaging API platform adapter for Gogeta Agent.
...
to Gogeta Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``GOGETA_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            gogeta_home = Path.home().joinpath(".gogeta").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``gogeta status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``gogeta status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``gogeta setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.gogeta/.env`` via ``gogeta_cli.config``.
    """
    print()
    ...
    try:
        from gogeta_cli.config import get_env_var, set_env_var
    except ImportError:
        print("gogeta_cli.config not available; set LINE_* vars manually in ~/.gogeta/.env")
        return
