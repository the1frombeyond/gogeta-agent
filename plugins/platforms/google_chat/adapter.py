...
# ``model_tools`` import time, so ``gogeta status``, ``gogeta chat``, etc.
...
# ``gogeta_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Gogeta is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a gogeta MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${GOGETA_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _gogeta_home = _get_gogeta_home()
        except (ModuleNotFoundError, ImportError):
            _gogeta_home = _Path.home() / ".gogeta"
        self._thread_count_store = _ThreadCountStore(
            _gogeta_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Gogeta is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("GOGETA_HOME", str(_Path.home() / ".gogeta"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${GOGETA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a gogeta MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Gogeta is thinking…' marker message.
        ...
        "Gogeta is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Gogeta is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Gogeta is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Gogeta is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Gogeta is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``gogeta setup``.

    The setup wizard at ``gogeta_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.gogeta/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. gogeta-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.gogeta/.env")
    print_info("Restart the gateway: gogeta gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``gogeta cron`` running as a
    separate process from ``gogeta gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Gogeta plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'gogeta-agent[google_chat]'",
        ...
        # the "Gogeta is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Gogeta is thinking…' marker message "
