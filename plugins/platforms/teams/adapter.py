"""
Microsoft Teams platform adapter for Gogeta Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Gogeta"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        gogeta_action = data.get("gogeta_action", "")
        session_key = data.get("session_key", "")

        if not gogeta_action or not session_key:
        ...
        choice = choice_map.get(gogeta_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="gogeta_approve",
                    data={**btn_data_base, "gogeta_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="gogeta_approve",
                    data={**btn_data_base, "gogeta_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="gogeta_approve",
                    data={**btn_data_base, "gogeta_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="gogeta_approve",
                    data={**btn_data_base, "gogeta_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Gogeta\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.gogeta/.env")
