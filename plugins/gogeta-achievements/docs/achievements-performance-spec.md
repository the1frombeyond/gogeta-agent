# Gogeta Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: gogeta-achievements plugin
...
### `GET /api/plugins/gogeta-achievements/achievements`
...
### `POST /api/plugins/gogeta-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/gogeta-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/gogeta-achievements/overview`
...
- Persist plugin data under:
  - `~/.gogeta/plugins/gogeta-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.gogeta/plugins/gogeta-achievements/scan_checkpoint.json`
...
- `~/.gogeta/plugins/gogeta-achievements/`
