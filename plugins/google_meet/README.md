...
Let the gogeta agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where gogeta runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `gogeta meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `gogeta meet` CLI |
...
| `cli.py` | `gogeta meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$GOGETA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `gogeta meet node {run,list,approve,remove,status,ping}` |
...
gogeta meet install                                      # pip + Chromium
gogeta meet setup                                        # preflight
gogeta meet auth                                         # optional
gogeta meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
gogeta meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.gogeta/.env
gogeta meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
gogeta meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
gogeta meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.gogeta/.env
gogeta meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, gogeta will **not** switch your system audio input automatically — the
...
gogeta plugins enable google_meet
gogeta meet node run --display-name my-mac --host 0.0.0.0 --port 18789
