...
session-scoped state file under ``$GOGETA_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $GOGETA_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_gogeta_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this gogeta install, leave it first —
    ...
    env = os.environ.copy()
    env["GOGETA_MEET_URL"] = url
    env["GOGETA_MEET_OUT_DIR"] = str(out)
    env["GOGETA_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["GOGETA_MEET_HEADED"] = "1"
    if auth_state:
        env["GOGETA_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["GOGETA_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if GOGETA_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["GOGETA_MEET_MODE"] = mode
    if realtime_model:
        env["GOGETA_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["GOGETA_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["GOGETA_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["GOGETA_MEET_REALTIME_KEY"] = realtime_api_key
