...
```bash
gogeta memory setup    # select "byterover"
```

Or manually:
```bash
gogeta config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.gogeta/.env
```
...
Working directory: `$GOGETA_HOME/byterover/` (profile-scoped).
