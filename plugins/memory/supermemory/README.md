...
```bash
gogeta memory setup    # select "supermemory"
```

Or manually:

```bash
gogeta config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.gogeta/.env
```

## Config

Config file: `$GOGETA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `gogeta` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `gogeta-{identity}` → `gogeta-coder`). |
...
All Supermemory API calls send `x-sm-source: gogeta`, and document writes stamp
`metadata.sm_source: gogeta`. This is a **functional routing key, not telemetry**:
it groups Gogeta-written memories into a dedicated "Gogeta" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Gogeta profile:

```json
{
  "container_tag": "gogeta-{identity}"
}
```

For a profile named `coder`, this resolves to `gogeta-coder`. The default profile resolves to `gogeta-default`. Without `{identity}`, all profiles share the same container.
