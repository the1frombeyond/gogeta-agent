...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/gogeta>
...
```bash
gogeta memory setup honcho   # configure Honcho directly (works on a fresh install)
gogeta memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
gogeta config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.gogeta/.env
```

> `gogeta honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `gogeta memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$GOGETA_HOME/honcho.json` | Profile-local (isolated Gogeta instances) |
| 2 | `~/.gogeta/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Gogeta profile: `gogeta` (default) or `gogeta_<profile>`.
...
**Deployment shapes** (`gogeta memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Gogeta to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Gogeta session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`gogeta-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`gogeta`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-gogeta-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening gogeta in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Gogeta session ID (timestamp + hex). Every `gogeta` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Gogeta profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`gogeta`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Gogeta profile: `gogeta` (default) or `gogeta_<profile>` (e.g. `gogeta -p coder` -> host key `gogeta_coder`). Older `gogeta.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `GOGETA_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `gogeta memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `gogeta honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `gogeta memory setup`) |
| `gogeta honcho status` | Show resolved config for active profile |
| `gogeta honcho enable` / `disable` | Toggle Honcho for active profile |
| `gogeta honcho mode <mode>` | Change recall or observation mode |
| `gogeta honcho peer --user <name>` | Update user peer name |
| `gogeta honcho peer --ai <name>` | Update AI peer name |
| `gogeta honcho tokens --context <N>` | Set context token budget |
| `gogeta honcho tokens --dialectic <N>` | Set dialectic max chars |
| `gogeta honcho map <name>` | Map current directory to a session name |
| `gogeta honcho sync` | Create host blocks for all Gogeta profiles |
