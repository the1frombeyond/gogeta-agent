...
```bash
gogeta memory setup    # select "holographic"
```

Or manually:
```bash
gogeta config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.gogeta-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$GOGETA_HOME/memory_store.db` | SQLite database path |
