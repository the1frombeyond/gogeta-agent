/**
 * Gogeta Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.gogeta/plugins/kanban/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__GOGETA_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__GOGETA_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "gogeta-kanban-dot-triage",
    todo: "gogeta-kanban-dot-todo",
    ready: "gogeta-kanban-dot-ready",
    running: "gogeta-kanban-dot-running",
    blocked: "gogeta-kanban-dot-blocked",
    done: "gogeta-kanban-dot-done",
    archived: "gogeta-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-gogeta-task";
  ...
  const DOCS_URL = "https://gogeta.ai/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://gogeta.ai/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "gogeta.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="gogeta-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "gogeta-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "gogeta-kanban-md",
  ...
  // standard `drop` event and our `gogeta-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("gogeta-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("gogeta-kanban-column--drop");
          if (target) target.classList.add("gogeta-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("gogeta-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("gogeta-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("gogeta-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__GOGETA_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "gogeta-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "gogeta-kanban-attention",
        "gogeta-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "gogeta-kanban-attention-bar" },
        h("span", { className: "gogeta-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "gogeta-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "gogeta-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "gogeta-kanban-attention-row",
                  "gogeta-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "gogeta-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "gogeta-kanban-attention-row-id" }, task.id),
                h("span", { className: "gogeta-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "gogeta-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "gogeta-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "gogeta-kanban-diag-action-btn",
      action.suggested ? "gogeta-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " gogeta-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".gogeta-kanban-drawer-comment-row input, .gogeta-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "gogeta-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("gogeta-kanban-diag", sevClass) },
      h("div", { className: "gogeta-kanban-diag-header" },
        h("span", { className: "gogeta-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "gogeta-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "gogeta-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "gogeta-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "gogeta-kanban-diag-data-row" },
                  h("span", { className: "gogeta-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "gogeta-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "gogeta-kanban-diag-data-row" },
                h("span", { className: "gogeta-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "gogeta-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "gogeta-kanban-diag-reassign-row" },
            h("span", { className: "gogeta-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "gogeta-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "gogeta-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "gogeta-kanban-diag-msg",
              msg.ok ? "gogeta-kanban-diag-msg--ok" : "gogeta-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "gogeta-kanban-section" },
      h("div", { className: "gogeta-kanban-section-head-row" },
        h("span", { className: "gogeta-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "gogeta-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "gogeta-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "gogeta-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "gogeta-kanban-docs-link",
      title: "Open Gogeta Kanban docs in a new tab",
      "aria-label": "Gogeta Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `gogeta kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "gogeta-kanban-msg-ok" : "gogeta-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "gogeta-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "gogeta-kanban-boardswitcher" },
      h("div", { className: "gogeta-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "gogeta-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "gogeta-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "gogeta-kanban-dialog-title" },
        ...
        h("div", { className: "gogeta-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Gogeta profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "gogeta-kanban-bulk" },
      h("span", { className: "gogeta-kanban-bulk-count" },
      ...
      h("div", { className: "gogeta-kanban-bulk-priority",
      ...
      h("div", { className: "gogeta-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Gogeta profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "gogeta-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("gogeta-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("gogeta-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "gogeta-kanban-trash",
        dragOver ? "gogeta-kanban-trash--drop" : "",
        props.draggingTaskId ? "gogeta-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "gogeta-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "gogeta-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".gogeta-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "gogeta-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("gogeta-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("gogeta-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "gogeta-kanban-column",
        dragOver ? "gogeta-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "gogeta-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "gogeta-kanban-col-check",
        ...
        h("span", { className: cn("gogeta-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "gogeta-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "gogeta-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "gogeta-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "gogeta-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "gogeta-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "gogeta-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "gogeta-kanban-lane" },
                  h("div", { className: "gogeta-kanban-lane-head" },
                    h("span", { className: "gogeta-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "gogeta-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "gogeta-kanban-card--stale-red";
    if (age >= tier.amber) return "gogeta-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".gogeta-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "gogeta-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "gogeta-kanban-card",
        props.selected ? "gogeta-kanban-card--selected" : "",
        props.failed ? "gogeta-kanban-card--failed" : "",
        props.draggingSource ? "gogeta-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "gogeta-kanban-card-content" },
          h("div", { className: "gogeta-kanban-card-row" },
            h("label", {
              className: "gogeta-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "gogeta-kanban-card-check",
            ...
            h("span", { className: "gogeta-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or gogeta kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "gogeta-kanban-warning-badge",
                    "gogeta-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "gogeta-kanban-needs-assignee",
          ...
          h("div", { className: "gogeta-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "gogeta-kanban-card-row gogeta-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "gogeta-kanban-assignee",
                            title: `Assigned to Gogeta profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "gogeta-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "gogeta-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "gogeta-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "gogeta-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Gogeta profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Gogeta profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "gogeta-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "gogeta-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "gogeta-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "gogeta-kanban-drawer-close",
        ...
        data ? h("div", { className: "gogeta-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "gogeta-kanban-section" },
      h("div", { className: "gogeta-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "gogeta-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "gogeta-kanban-drawer-body" },
      h("div", { className: "gogeta-kanban-drawer-title" },
        h("span", { className: cn("gogeta-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "gogeta-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "gogeta-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "gogeta-kanban-section" },
        h("div", { className: "gogeta-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "gogeta-kanban-section" },
        h("div", { className: "gogeta-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "gogeta-kanban-comment" },
                h("div", { className: "gogeta-kanban-comment-head" },
                  h("span", { className: "gogeta-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "gogeta-kanban-comment-ago" },
      ...
      h("div", { className: "gogeta-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "gogeta-kanban-event",
              isDiag ? "gogeta-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "gogeta-kanban-event-header" },
                  h("span", { className: "gogeta-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "gogeta-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "gogeta-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "gogeta-kanban-event-header-plain" },
                  h("span", { className: "gogeta-kanban-event-kind" }, e.kind),
                  h("span", { className: "gogeta-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "gogeta-kanban-event-phantom-row" },
                  h("span", { className: "gogeta-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "gogeta-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "gogeta-kanban-event-payload" },
    ...
    return h("div", { className: "gogeta-kanban-section" },
      h("div", { className: "gogeta-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "gogeta-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `gogeta-kanban-run--${r.outcome || r.status || "ended"}`
          : "gogeta-kanban-run--active";
        return h("div", { key: r.id, className: cn("gogeta-kanban-run", outcomeClass) },
          h("div", { className: "gogeta-kanban-run-head" },
            h("span", { className: "gogeta-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "gogeta-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "gogeta-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "gogeta-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "gogeta-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "gogeta-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "gogeta-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "gogeta-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "gogeta-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "gogeta-kanban-pre gogeta-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "gogeta-kanban-meta-row" },
      h("span", { className: "gogeta-kanban-meta-label" }, props.label),
      h("span", { className: "gogeta-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "gogeta-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "gogeta-kanban-meta-row" },
        h("span", { className: "gogeta-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "gogeta-kanban-meta-value gogeta-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "gogeta-kanban-meta-row" },
        h("span", { className: "gogeta-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "gogeta-kanban-meta-value gogeta-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "gogeta-kanban-section" },
      h("div", { className: "gogeta-kanban-section-head-row" },
        h("span", { className: "gogeta-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "gogeta-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "gogeta-kanban-section" },
      h("div", { className: "gogeta-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "gogeta-kanban-deps-row" },
        h("span", { className: "gogeta-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "gogeta-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "gogeta-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "gogeta-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "gogeta-kanban-dep-chip-x",
      ...
      h("div", { className: "gogeta-kanban-deps-row" },
        h("span", { className: "gogeta-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "gogeta-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "gogeta-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "gogeta-kanban-msg-ok"
          : "gogeta-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "gogeta-kanban-section" },
      ...
      h("div", { className: "gogeta-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "gogeta-kanban-home-sub gogeta-kanban-home-sub--on"
              : "gogeta-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__GOGETA_PLUGINS__ && typeof window.__GOGETA_PLUGINS__.register === "function") {
    window.__GOGETA_PLUGINS__.register("kanban", KanbanPage);
  }
})();
