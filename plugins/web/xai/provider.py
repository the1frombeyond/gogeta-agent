...
``{title, url, description, position}`` rows every other Gogeta web
provider produces.
...
prefers Gogeta-managed xAI Grok OAuth (via ``gogeta auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.gogeta/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``gogeta tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `gogeta auth` to sign in with "
