# OpenAI-Compatible API Server for Gogeta Agent
...
SSE streaming. By exposing this endpoint, gogeta-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  gogeta-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.gogeta/.env`
2. Run `gogeta gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with gogeta-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns gogeta-agent as a model) |
...
Authorization: Bearer gogeta-api-key-here
...
But gogeta-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "gogeta-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured gogeta-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of gogeta-agent's core prompt
...
Once implemented, gogeta-agent works as a drop-in backend for:
