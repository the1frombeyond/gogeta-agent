"""CLI entry point for the gogeta-agent ACP adapter.

Loads environment variables from ``~/.gogeta/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from GOGETA_HOME (default ``~/.gogeta``)."""
    from gogeta_cli.env_loader import load_gogeta_dotenv

    gogeta_home = get_gogeta_home()
    loaded = load_gogeta_dotenv(gogeta_home=gogeta_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", gogeta_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="gogeta-acp",
        description="Run Gogeta Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Gogeta version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Gogeta provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.gogeta/node/ "
...
def _print_version() -> None:
    ...
    print(gogeta_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import GsaACPAgent  # noqa: F401

    print("Gogeta ACP check OK")


def _run_setup() -> None:
    from gogeta_cli.main import main as gogeta_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "gogeta", "model"]
        gogeta_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``gogeta postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting gogeta-agent ACP adapter")
    ...
    import acp
    from .server import GsaACPAgent
