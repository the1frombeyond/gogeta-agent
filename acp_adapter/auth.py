"""ACP auth helpers — detect and advertise Gogeta authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "gogeta-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Gogeta runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Gogeta can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Gogeta.
    ...
    not have Gogeta provider credentials configured yet, so Gogeta always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Gogeta using the currently configured "
