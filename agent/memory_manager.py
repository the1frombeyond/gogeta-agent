...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_GOGETA_CORE_TOOLS)
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``gogeta_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_gogeta_home()`` themselves.
        """
        if "gogeta_home" not in kwargs:
            from gogeta_constants import get_gogeta_home
            kwargs["gogeta_home"] = str(get_gogeta_home())
        for provider in self._providers:
            try:
