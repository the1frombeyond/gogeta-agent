...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Gogeta starts and finds ``~/.openclaw/``.

    Points users at ``gogeta claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``gogeta claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Gogeta, run "
        "`gogeta claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `gogeta claw cleanup` (renames it to ~/.openclaw.pre-migration — "
