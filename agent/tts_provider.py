...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Gogeta with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.gogeta/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Gogeta
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``gogeta tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``gogeta setup`` for
