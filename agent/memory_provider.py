...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - gogeta_home (str): The active GOGETA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.gogeta``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "gogeta").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'gogeta memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], gogeta_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'gogeta memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``gogeta_home`` is the active GOGETA_HOME directory path.
