...
Plugins built on Gogeta Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Gogeta
...
:class:`~gogeta_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Gogeta' main config
...
handles every provider, fallback chain, and per-task override Gogeta
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`gogeta_cli.plugins.PluginContext`
