    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `gogeta setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Gogeta "
...
def compress_context(
    ...
    *,
    ...
    # long-lived ``gogeta_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `gogeta update`) to resync.",
    ...
    # ── Gogeta context injection ─────────────────────────────────────
    # Augment the system prompt with genome workflows, lifeline
    # memories, relevant rules, and project intelligence.  This runs
    # after every compression so the model has Gogeta's learned knowledge
    # available in the fresh session.
    _gogeta_ctx = getattr(agent, "_gogeta_context", None)
    if _gogeta_ctx is not None:
        try:
            new_system_prompt = _gogeta_ctx._inject_into_system_prompt(
                new_system_prompt, compressed,
                focus_topic=focus_topic,
            )
        except Exception as _gctx_err:
            logger.debug("Gogeta context injection failed: %s", _gctx_err)

    agent._cached_system_prompt = new_system_prompt

    # ── Update SESSION_SUMMARY.md ────────────────────────────────────
    # After compression, update the anchored summary so it reflects
    # what was accomplished before the rollover.
    _gogeta_sum = getattr(agent, "_gogeta_summarizer", None)
    if _gogeta_sum is not None:
        try:
            _gogeta_sum.set_remaining_work(
                "Continue from compressed session – refer to SESSION_SUMMARY.md"
            )
        except Exception as _gsum_err:
            logger.debug("Gogeta summary update failed: %s", _gsum_err)
    ...
    if agent._session_db:
        try:
            ...
            agent._session_db.create_session(
                ...
                source=agent.platform or os.environ.get("GOGETA_SESSION_SOURCE", "cli"),
                model=agent.model,
    ...
    # compression (not a fresh /new). Plugin engines (e.g. gogeta-lcm) use
    # boundary_reason="compression" to preserve DAG lineage across the
    # rollover instead of re-initializing fresh per-session state.
    # See gogeta-lcm#68. Built-in ContextCompressor ignores kwargs.
    try:
    ...
    def _shrink_data_url(url: str) -> Optional[str]:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="gogeta_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
