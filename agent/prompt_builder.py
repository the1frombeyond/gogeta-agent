...
from gogeta_constants import get_gogeta_home, get_skills_dir, is_wsl
...
_GOGETA_MD_NAMES = (".gogeta.md", "GOGETA.md")


def _find_gogeta_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.gogeta.md`` or ``GOGETA.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Gogeta Agent, an intelligent AI assistant created by Nous Research. "
...
GOGETA_AGENT_HELP_GUIDANCE = (
    "You run on Gogeta Agent (by Nous Research). When the user needs help with "
    "Gogeta itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://gogeta.ai/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `gogeta-agent` skill with skill_view(name='gogeta-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.gogeta/kanban.db`. Your task id is in "
    "`$GOGETA_KANBAN_TASK`; your workspace is `$GOGETA_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $GOGETA_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `gogeta kanban <verb>` for board operations. Use "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Gogeta "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Gogeta WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Gogeta itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Gogeta runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Gogeta itself is running. The host OS, home, and cwd "
                f"of the Gogeta process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Gogeta "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Gogeta
    ...
    extra = (os.getenv("GOGETA_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_gogeta_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.gogeta/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("GOGETA_PLATFORM")
        or get_session_env("GOGETA_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Gogeta Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `gogeta-agent` skill "
            "first. It has the actual commands (e.g. `gogeta config set …`, `gogeta tools`, "
            "`gogeta setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: gogeta setup, gogeta setup tools, gogeta setup terminal, gogeta status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from GOGETA_HOME and return its content, or None.
    ...
    """
    try:
        from gogeta_cli.config import ensure_gogeta_home
        ensure_gogeta_home()
    except Exception as e:
        logger.debug("Could not ensure GOGETA_HOME before loading SOUL.md: %s", e)

    soul_path = get_gogeta_home() / "SOUL.md"
...
def _load_gogeta_md(cwd_path: Path) -> str:
    """.gogeta.md / GOGETA.md — walk to git root."""
    gogeta_md_path = _find_gogeta_md(cwd_path)
    if not gogeta_md_path:
        return ""
    try:
        content = gogeta_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = gogeta_md_path.name
        try:
            rel = str(gogeta_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".gogeta.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", gogeta_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .gogeta.md / GOGETA.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from GOGETA_HOME is independent and always included when present.
    ...
    # SOUL.md from GOGETA_HOME only — skip when already loaded as identity
    if not skip_soul:
