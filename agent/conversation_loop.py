...
from gogeta_constants import PARTIAL_STREAM_STUB_ID
from gogeta_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Gogeta tool use: "
    ...
    return (
        ...
        f"context, but Gogeta needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Gogeta config, set `model.ollama_num_ctx: 65536` "
...
def _is_nous_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (
        base_url_host_matches(base, "inference-api.gogeta.ai")
        or base_url_host_matches(base, "inference.gogeta.ai")
    )
...
def _try_refresh_nous_paid_entitlement_credentials(agent) -> bool:
    ...
    try:
        from gogeta_cli.nous_account import get_nous_portal_account_info
...
def run_conversation(
    ...
    # ``gogeta logs --session <id>`` can filter a single conversation.
    ...
    # are reused.  The system prompt is Gogeta's territory; plugins
    ...
    agent._knowledge_gap_research_passes = 0  # Reset per-turn research counter
    ...
    # all run inside Codex). Default Gogeta path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Gogeta internals.
        #
        # Gogeta invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Gogeta tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from gogeta_cli.middleware import apply_llm_request_middleware
                ...
                if env_var_enabled("GOGETA_DUMP_REQUESTS"):
                ...
                from gogeta_cli.middleware import run_llm_execution_middleware
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: gogeta auth add nous")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.gogeta.ai")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `gogeta doctor` for credential-chain diagnostics, or")
                    ...
                    from gogeta_constants import display_gogeta_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Gogeta-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: gogeta config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: gogeta config set ANTHROPIC_API_KEY \"\"")
                ...
                # providers (DeepSeek, Kimi, MiMo, Gogeta).  A 429 can
                ...
                # which Gogeta' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Gogeta' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`gogeta",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `gogeta auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `gogeta model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: gogeta portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.gogeta.ai", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: gogeta setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        gogeta fallback add   (interactive picker — same as `gogeta model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        _policy_response = (
                            ...
                            f"(not a Gogeta/gateway failure).\n\n"
                            f"Provider message: {_summary}\n\n"
                            f"Try rephrasing the request, narrowing the context, or "
                            f"adding a fallback provider with `gogeta fallback add`."
                        )
                        return {
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                # Before finalizing, run a lightweight LLM judge to detect
                # unsupported claims. If gaps are found below threshold,
                # execute grounding tools and feed results back to the
                # model for a corrected answer.
                _kg_research_passes = getattr(agent, "_knowledge_gap_research_passes", 0)
                if _kg_research_passes < 2 and final_response.strip():
                    try:
                        from agent.auxiliary_client import call_llm as _kg_call_llm
                        from agent.knowledge_gap import (
                            KnowledgeGapDetector,
                            build_research_context,
                        )

                        _kg_detector: KnowledgeGapDetector = getattr(
                            agent, "_knowledge_gap_detector", None
                        )
                        if _kg_detector is None:
                            _kg_detector = KnowledgeGapDetector(
                                call_llm=_kg_call_llm,
                            )
                            agent._knowledge_gap_detector = _kg_detector

                        _kg_result = _kg_detector.analyze(
                            response=final_response,
                            messages=messages,
                        )

                        if _kg_result.research_performed and _kg_result.research_results:
                            agent._knowledge_gap_research_passes = _kg_research_passes + 1
                            _research_ctx = build_research_context(_kg_result.research_results)

                            _turn_exit_reason = (
                                f"knowledge_gap_research("
                                f"pass={_kg_research_passes + 1}, "
                                f"gaps={len(_kg_result.gaps)}, "
                                f"confidence={_kg_result.confidence_score:.2f})"
                            )

                            if not agent.quiet_mode:
                                agent._safe_print(
                                    f"  🔍 Knowledge gaps detected "
                                    f"({len(_kg_result.gaps)} gaps, "
                                    f"confidence={_kg_result.confidence_score:.0%})"
                                    f" — researching..."
                                )

                            # Emit event for subscribers (genome, lifeline, etc.)
                            ...
                            event_bus.emit(Event(
                                ...
                                data={
                                    "gap_count": len(_kg_result.gaps),
                                    "research_count": len(_kg_result.research_results),
                                    "confidence": _kg_result.confidence_score,
                                    "pass": _kg_research_passes + 1,
                                },
                            ))

                            # Emit confidence-low event if severely low
                            if _kg_result.confidence_score < 0.5:
                                event_bus.emit(Event(
                                    type=EventType.CONFIDENCE_LOW,
                                    source="conversation_loop",
                                    data={
                                        "confidence": _kg_result.confidence_score,
                                        "gap_count": len(_kg_result.gaps),
                            ...
                            # Inject research results as a system-level hint
                            # routed through a tool-style message so the model
                            # sees verified facts on the next turn.
                            messages.append({
                                "role": "user",
                                "content": (
                                    "[System: Knowledge Gap Research Results]\n\n"
                                    f"{_research_ctx}\n\n"
                                    "Please review the verified facts above and "
                                    "revise your previous response accordingly."
                                ),
                                "_knowledge_gap_research": True,
                            })
                        ...
                        logger.warning(
                            "Knowledge gap detection failed: %s", _kg_exc,
                        )
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("GOGETA_KANBAN_TASK")
        if _kanban_task:
            try:
