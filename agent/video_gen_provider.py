...
as ``kind: backend``) or ``~/.gogeta/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``gogeta tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$GOGETA_HOME/cache/videos/``, creating parents as needed."""
    from gogeta_constants import get_gogeta_home

    path = get_gogeta_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$GOGETA_HOME/cache/videos/``.
