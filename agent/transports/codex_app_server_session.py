...
Owns one Codex thread per Gogeta session. Drives `turn/start`, consumes
...
# Gogeta' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_GOGETA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Gogeta/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Gogeta session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _GOGETA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("GOGETA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="gogeta",
            client_title="Gogeta Agent",
            client_version=_get_gogeta_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Gogeta' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Gogeta' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own gogeta-tools callback we
            # auto-accept — the user already approved Gogeta' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Gogeta approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Gogeta returns 'once', 'session', 'always', or 'deny'.
...
def _get_gogeta_version() -> str:
    """Best-effort Gogeta version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("gogeta-agent")
