...
from gogeta_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: gogeta config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.gogeta/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from gogeta_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind GOGETA_DEV_CREDITS.
    ...
    # both live under ~/.gogeta/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from gogeta_logging import setup_logging, setup_verbose_logging
    setup_logging(gogeta_home=_ra()._gogeta_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that gogeta_logging.setup_logging() does not
        ...
        # handler level inside gogeta_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from gogeta_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from gogeta_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `gogeta model` to "
                        "select a provider, or run `gogeta setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff GOGETA_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["GOGETA_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.gogeta/sessions/ alongside gateway sessions
    gogeta_home = get_gogeta_home()
    agent.logs_dir = gogeta_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.gogeta/sessions/session_{sid}.json)
    ...
    try:
        from gogeta_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from gogeta_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "gogeta"
                    except Exception:
                        pass
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from gogeta_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from gogeta_cli.plugins import get_plugin_context_engine
    ...
    # ── Gogeta context engine ────────────────────────────────────────
    # Initialize the ContextBuilder (genome/lifeline injection into
    # system prompt after compression) and SessionSummarizer (anchored
    # SESSION_SUMMARY.md).  Both are set to None on import failure so
    # the compression path is non-fatal.
    try:
        from agent.context import ContextBuilder
        agent._gogeta_context = ContextBuilder()
    except Exception as _gctx_err:
        _ra().logger.debug("Gogeta ContextBuilder init failed: %s", _gctx_err)
        agent._gogeta_context = None
    try:
        from agent.context import SessionSummarizer
        agent._gogeta_summarizer = SessionSummarizer()
    except Exception as _gsum_err:
        _ra().logger.debug("Gogeta SessionSummarizer init failed: %s", _gsum_err)
        agent._gogeta_summarizer = None
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Gogeta Agent.  Choose a model with at least "
