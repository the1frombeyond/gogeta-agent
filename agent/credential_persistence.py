...
``auth.json``.  They intentionally have no dependency on ``gogeta_cli.auth`` so
...
# Sources Gogeta owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Gogeta-owned OAuth/device-code state)
