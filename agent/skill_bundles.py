...
Storage
-------
Bundles live in ``~/.gogeta/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``GOGETA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``gogeta bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under GOGETA_HOME.

    Honors ``GOGETA_BUNDLES_DIR`` for tests; falls back to
    ``<GOGETA_HOME>/skill-bundles``.
    """
    override = os.environ.get("GOGETA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_gogeta_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `gogeta bundles` CLI subcommand.
# ---------------------------------------------------------------------------
