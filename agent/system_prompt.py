...
upstream prefix cache warm.  See ``gogeta-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Gogeta never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want GOGETA_HOME persona while keeping
    ...
    # Pointer to the gogeta-agent skill + docs for user questions about Gogeta itself.
    stable_parts.append(GOGETA_AGENT_HELP_GUIDANCE)
    ...
    # GOGETA_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Active-profile hint — names the Gogeta profile the agent is running
    # under so it doesn't conflate ~/.gogeta/skills/ (default profile) with
    # ~/.gogeta/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Gogeta profile: default. Other profiles (if any) live "
            "under ~/.gogeta/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Gogeta profile: {active_profile}. This session reads "
            f"and writes ~/.gogeta/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.gogeta/skills/, ~/.gogeta/plugins/, "
            f"~/.gogeta/cron/, ~/.gogeta/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Gogeta never rebuilds or reinjects parts of it
