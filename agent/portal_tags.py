...
Every Gogeta request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Gogeta Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=gogeta-agent",
        "client=gogeta-client-v<__version__>",
    ]

The version is sourced live from ``gogeta_cli.__version__`` so it auto-aligns
...
``gogeta_cli.__version__`` is the canonical source of truth.
"""
...
def _gogeta_version() -> str:
    """Return the current Gogeta release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``gogeta_cli`` cannot be imported (should
...
def gogeta_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=gogeta-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=gogeta-client-v{_gogeta_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=gogeta-agent", gogeta_client_tag()]
