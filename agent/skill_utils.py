...
from gogeta_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``GOGETA_KANBAN_TASK`` / ``GOGETA_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("GOGETA_KANBAN_TASK") or os.getenv("GOGETA_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Gogeta Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``GOGETA_PLATFORM`` or
            ``GOGETA_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``gogeta`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.gogeta/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``gogeta`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against GOGETA_HOME, not cwd
        if not p.is_absolute():
            p = (gogeta_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.gogeta/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    gogeta = metadata.get("gogeta") or {}
    if not isinstance(gogeta, dict):
        gogeta = {}
    return {
        "fallback_for_toolsets": gogeta.get("fallback_for_toolsets", []),
        "requires_toolsets": gogeta.get("requires_toolsets", []),
        "fallback_for_tools": gogeta.get("fallback_for_tools", []),
        "requires_tools": gogeta.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    gogeta = metadata.get("gogeta")
    if not isinstance(gogeta, dict):
        return []
    raw = gogeta.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Gogeta metadata, VCS, virtualenv/dependency, and cache
