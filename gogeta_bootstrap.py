"""Windows UTF-8 bootstrap for Gogeta entry points.
...
should be imported at the very top of every Gogeta entry point
(``gogeta``, ``gogeta-agent``, ``gogeta-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import gogeta_bootstrap``
# (or ``from gogeta_bootstrap import apply_windows_utf8_bootstrap``) at
