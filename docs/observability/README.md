# Gogeta Observer Hooks

Gogeta observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Gogeta' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Gogeta catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Gogeta only builds
