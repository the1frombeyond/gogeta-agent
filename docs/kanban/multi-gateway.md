...
Gogeta supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.gogeta/config.yaml`:
...
Or set the env var: `GOGETA_KANBAN_DISPATCH_IN_GATEWAY=false`
