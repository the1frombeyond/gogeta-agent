...
When running Gogeta inside Docker, the default `network_mode: host` gives the
...
The Gogeta [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   GOGETA_UID=$(id -u) GOGETA_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=gogeta,gogeta-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://gogeta-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Gogeta trust model and vulnerability reporting
