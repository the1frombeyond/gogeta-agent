...
Gogeta middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Gogeta'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Gogeta calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Gogeta retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Gogeta guardrails,
...
- `telemetry_schema_version`: currently `gogeta.observer.v1`
- `middleware_schema_version`: currently `gogeta.middleware.v1`
...
Gogeta stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Gogeta runs
them as a nested chain in registration order. Middleware failures are fail-open:
Gogeta logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Gogeta applies middleware in this order:
...
For each tool call, Gogeta applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Gogeta pre-execution path against the effective arguments:
...
```bash
gogeta plugins enable <plugin-name>
```

For isolated local testing, use one `GOGETA_HOME` for plugin enablement and the
agent run:

```bash
export GOGETA_HOME=/tmp/gogeta-middleware-test
mkdir -p "$GOGETA_HOME"
gogeta plugins enable <plugin-name>
gogeta chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run gogeta plugins enable <plugin-name>
uv run gogeta chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["gogeta_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/gogeta-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Gogeta expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Gogeta treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Gogeta preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Gogeta does not convert downstream
