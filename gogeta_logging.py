"""Centralized logging setup for Gogeta Agent.
...
``~/.gogeta/logs/`` (profile-aware via ``get_gogeta_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``gogeta_cli.web_server``,

    ``gogeta_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from gogeta_constants import get_config_path, get_gogeta_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_gogeta_session_injector", False):
    ...
    _session_record_factory._gogeta_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``gogeta logs --component``.

COMPONENT_PREFIXES = {

    "gateway": ("gateway", "gogeta_plugins"),
...
def setup_logging(
    ...
    *,
    ...
    """Configure the Gogeta logging subsystem.
    ...
    *force* is ``True``.
    ...
    gogeta_home
        ...
        Override for the Gogeta home directory.  Falls back to

        ``get_gogeta_home()`` (profile-aware).
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        ...
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            ...
            if getattr(h, "_gogeta_verbose", False):
    ...
    handler._gogeta_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        ...
        inherit the gogeta group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        ...
        from gogeta_cli.config import is_managed
