...
The root ``package.json`` is installed by ``gogeta update`` on every user,
...
* ``@askjo/camofox-browser`` is NOT eager. It is an explicit opt-in
  ...
  ``gogeta tools`` → Browser Automation → Camofox, and only used at
  runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
  Firefox-fork binary, which silently blocked ``gogeta update`` for
...
guidance in the ``gogeta-agent-dev`` skill before changing the
expectations.
"""
...
def test_camofox_is_not_in_root_dependencies() -> None:
    ...
    assert "@askjo/camofox-browser" not in deps, (
        ...
        "Camofox post_setup handler in gogeta_cli/tools_config.py so it "
        "only installs when the user explicitly selects Camofox via "
        "`gogeta tools` → Browser Automation → Camofox."
    )
...
def test_agent_browser_stays_eager() -> None:
    ...
    assert "agent-browser" in deps, (
        ...
        "after `gogeta setup` / `gogeta update` without an explicit "
        "post_setup step."
    )
