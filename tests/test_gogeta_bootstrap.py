"""Tests for gogeta_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Gogeta entry point
(gogeta, gogeta-agent, gogeta-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Gogeta entry point imports gogeta_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported gogeta_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("gogeta_bootstrap", None)
    import gogeta_bootstrap  # noqa: WPS433
    return gogeta_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Gogeta CLI
...
class TestEntryPointsImportBootstrap:
    """Every Gogeta entry point must import gogeta_bootstrap as its
    ...
    # Entry points that invoke Gogeta as a process.  Each one must
    # import gogeta_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "gogeta_cli/main.py",   # gogeta CLI (console_script)
        "run_agent.py",          # gogeta-agent (console_script)
        "acp_adapter/entry.py",  # gogeta-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import gogeta_bootstrap' and that
        ...
        half-finished ``gogeta update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``gogeta_bootstrap`` as a top-level module) leaves gogeta
        ...
        """
        # Resolve relative to the gogeta-agent repo root.  Tests live
        # at tests/test_gogeta_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # gogeta start even when gogeta_bootstrap hasn't been
        ...
        assert first_import_name == "gogeta_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'gogeta_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import gogeta_bootstrap' line to be the first import."
        )
