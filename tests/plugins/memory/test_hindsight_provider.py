...
def provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_gogeta_home", lambda: tmp_path
    )

    p = HindsightMemoryProvider()
    p.initialize(session_id="test-session", gogeta_home=str(tmp_path), platform="cli")
...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:gogeta-agent, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:gogeta-agent",
    ]


def test_normalize_retain_tags_accepts_json_array_string():
    value = json.dumps(["agent:fakeassistantname", "source_system:gogeta-agent"])
    assert _normalize_retain_tags(value) == ["agent:fakeassistantname", "source_system:gogeta-agent"]
...
class TestConfig:
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Gogeta Agent and the User"
    ...
    def test_custom_config_values(self, provider_with_config):
        ...
        assert p._retain_source == "gogeta"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_gogeta_home",
        ...
        assert cfg["banks"]["gogeta"]["bankId"] == "env-bank"
        assert cfg["banks"]["gogeta"]["budget"] == "high"
...
class TestPostSetup:
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        gogeta_home = tmp_path / "gogeta-home"
        user_home = tmp_path / "user-home"
        user_home.mkdir()
        monkeypatch.setenv("HOME", str(user_home))

        selections = iter([1, 0])  # local_embedded, openai
        monkeypatch.setattr("gogeta_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-local-test")
        saved_configs = []
        monkeypatch.setattr("gogeta_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(gogeta_home), {"memory": {}})

        assert saved_configs[-1]["memory"]["provider"] == "hindsight"
        env_text = (gogeta_home / ".env").read_text()
        assert "HINDSIGHT_LLM_API_KEY=sk-local-test\n" in env_text
        assert "HINDSIGHT_TIMEOUT=120\n" in env_text
        assert "HINDSIGHT_IDLE_TIMEOUT=300\n" in env_text

        profile_env = user_home / ".hindsight" / "profiles" / "gogeta.env"
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(gogeta_home))
        provider.post_setup(str(gogeta_home), {"memory": {}})

        coder_env = user_home / ".hindsight" / "profiles" / "coder.env"
        gogeta_env = user_home / ".hindsight" / "profiles" / "gogeta.env"
        assert coder_env.exists()
        assert not gogeta_env.exists()
    ...
    def test_local_embedded_setup_blank_inputs_preserve_existing_config(self, tmp_path, monkeypatch):
        ...
        user_home.mkdir()
        monkeypatch.setenv("HOME", str(user_home))
        monkeypatch.setattr("plugins.memory.hindsight.get_gogeta_home", lambda: gogeta_home)
        ...
        provider = HindsightMemoryProvider()
        provider.save_config(existing_config, str(gogeta_home))
        ...
        monkeypatch.setattr("gogeta_cli.memory_setup._curses_select", lambda *args, **kwargs: kwargs.get("default", 0))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "")
        ...
        saved = json.loads((gogeta_home / "hindsight" / "config.json").read_text())
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        assert item["context"] == "conversation between Gogeta Agent and the User"
        ...
        assert item["metadata"]["source"] == "gogeta"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between Gogeta Agent and the User"
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_gogeta_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", gogeta_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", gogeta_home=str(tmp_path), platform="cli")
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("gogeta") == "gogeta"
    ...
    def test_resolve_empty_template_uses_fallback(self):
        result = _resolve_bank_id_template(
            "", fallback="gogeta", profile="coder"
        )
        assert result == "gogeta"

    def test_resolve_with_profile(self):
        result = _resolve_bank_id_template(
            "gogeta-{profile}", fallback="gogeta",
            profile="coder", workspace="", platform="", user="", session="",
        )
        assert result == "gogeta-coder"
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "gogeta-{user}" becomes "gogeta-" -> trimmed to "gogeta"
        result = _resolve_bank_id_template(
            "gogeta-{user}", fallback="default",
    ...
    def test_resolve_invalid_template_returns_fallback(self):
        ...
        result = _resolve_bank_id_template(
            "gogeta-{unknown}", fallback="gogeta",
            profile="", workspace="", platform="", user="", session="",
        )
        assert result == "gogeta"
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "gogeta-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="gogeta",
        )
        assert p._bank_id == "gogeta-coder"
        assert p._bank_id_template == "gogeta-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "gogeta-" which collapses to "gogeta"
        p.initialize(session_id="s1", gogeta_home=str(tmp_path), platform="cli")
        assert p._bank_id == "gogeta"
