...
Run from the PR worktree:
    ...
    cd ~/.gogeta/gogeta-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.gogeta/gogeta-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.gogeta/gogeta-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a gogeta-agent checkout"
)
...
# Isolated GOGETA_HOME for the config write.
