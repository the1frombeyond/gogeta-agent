...
def server():
    ...
    with patch.dict("sys.modules", {
        ...
        "gogeta_constants": MagicMock(get_gogeta_home=MagicMock(return_value="/tmp/gogeta_test")),

        "gogeta_cli.env_loader": MagicMock(),

        "gogeta_cli.banner": MagicMock(),

        "gogeta_state": MagicMock(),
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    ...
    """End-to-end: setting `GOGETA_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("GOGETA_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("GOGETA_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/gogeta-agent-dev": {"name": "gogeta-agent-dev", "description": "Dev workflow"}}
    ...
    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        ...
        resp = server.handle_request({
            ...
            "params": {"command": "gogeta-agent-dev", "session_id": sid},
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        ...
        "gogeta_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            ...
            "params": {"name": "gogeta-agent-dev", "session_id": sid},
    ...
    assert result["name"] == "gogeta-agent-dev"
