...
def test_gogeta_src_root_inserted_at_front():

    original = sys.path[:]

    try:

        fake_root = "/fake/gogeta/src"

        with patch.dict(os.environ, {"GOGETA_PYTHON_SRC_ROOT": fake_root}):
...
def test_no_src_root_env_does_not_crash():
    ...
    try:
        ...
        env = {k: v for k, v in os.environ.items() if k != "GOGETA_PYTHON_SRC_ROOT"}
