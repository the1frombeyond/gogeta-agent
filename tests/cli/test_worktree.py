...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("gogeta/gogeta-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned gogeta/* and pr-* branches."""

    def test_prunes_orphaned_gogeta_branch(self, git_repo):
        """gogeta/gogeta-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "gogeta/gogeta-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "gogeta/gogeta-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "gogeta/gogeta-deadbeef" in result.stdout
        ...
        assert "gogeta/gogeta-deadbeef" in orphaned
        ...
        assert "gogeta/gogeta-deadbeef" not in result.stdout
