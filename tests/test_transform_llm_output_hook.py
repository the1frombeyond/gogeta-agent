...
import gogeta_cli.plugins as plugins_mod
from gogeta_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(gogeta_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <gogeta_home>/plugins/<name> and opt it in."""
    plugin_dir = gogeta_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    gogeta_home = tmp_path / "gogeta_test"
    gogeta_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        gogeta_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("GOGETA_HOME", str(tmp_path / "gogeta_empty"))
