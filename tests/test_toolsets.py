...
class TestToolsetConsistency:
    ...
    def test_gogeta_platforms_share_core_tools(self):
        """All gogeta-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on gogeta-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["gogeta-cli", "gogeta-telegram", "gogeta-discord", "gogeta-whatsapp", "gogeta-slack", "gogeta-signal", "gogeta-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_gogeta_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("gogeta-whatsapp")

    def test_gogeta_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("gogeta-api-server")
