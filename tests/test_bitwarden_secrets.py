...
binary download are exercised manually by `gogeta secrets bitwarden
...
def gogeta_home(tmp_path, monkeypatch):
    """Point Gogeta at an isolated home directory."""
    home = tmp_path / ".gogeta"
    home.mkdir()
    monkeypatch.setenv("GOGETA_HOME", str(home))
    # Some modules cache get_gogeta_home; clear if needed.
    import gogeta_constants
    if hasattr(gogeta_constants, "_GOGETA_HOME_CACHE"):
        gogeta_constants._GOGETA_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(gogeta_home, monkeypatch):
...
def test_install_bws_happy_path(gogeta_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(gogeta_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(gogeta_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from gogeta_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under gogeta_home/cache/."""
    home = tmp_path / ".gogeta"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
