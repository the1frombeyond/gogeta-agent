"""Tests for ACP Registry metadata shipped with Gogeta."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "gogeta-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Gogeta Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/gogeta"
    assert data["website"].startswith("https://gogeta.ai/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"gogeta-agent[acp]=={data['version']}"
    assert uvx["args"] == ["gogeta-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"gogeta-agent[acp]=={_pyproject_version()}"
    )
