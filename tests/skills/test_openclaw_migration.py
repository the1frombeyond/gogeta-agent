...
"""
    ...
    assert "Tyler Williams > Active Projects: Gogeta Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `gogeta cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # gogeta_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.gogeta/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "gogeta_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Gogeta``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Gogeta prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Gogeta to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Gogeta config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Gogeta uses tools well"
    # All-lowercase matches → lowercase ``gogeta``; this preserves the
    # real filesystem path ``~/.gogeta`` (Gogeta home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "gogeta should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Gogeta remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "gogeta prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Gogeta was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "gogeta uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Gogeta said to ask Gogeta about Gogeta settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.gogeta`` (the real Gogeta home), not the broken
    ``.Gogeta``.
    ...
    rewritten to ``~/.Gogeta/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.gogeta/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .gogeta directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.gogeta'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "gogeta config path: ~/.gogeta/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Gogeta" in result
