...
class TestGsaConstantsFallback:
    """Tests for _gogeta_home.py fallback when gogeta_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_gogeta_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _gogeta_home.py with gogeta_constants blocked."""
        monkeypatch.setitem(sys.modules, "gogeta_constants", None)
        spec = importlib.util.spec_from_file_location("_gogeta_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_gogeta_home_env_var(self, monkeypatch, tmp_path):
        """When gogeta_constants is missing, GOGETA_HOME comes from env var."""
        monkeypatch.setenv("GOGETA_HOME", str(tmp_path / "custom-gogeta"))
        module = self._load_helper(monkeypatch)
        assert module.get_gogeta_home() == tmp_path / "custom-gogeta"

    def test_fallback_defaults_to_dot_gogeta(self, monkeypatch):
        """When gogeta_constants is missing and GOGETA_HOME unset, default to ~/.gogeta."""
        monkeypatch.delenv("GOGETA_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_gogeta_home() == Path.home() / ".gogeta"

    def test_fallback_ignores_empty_gogeta_home(self, monkeypatch):
        """Empty/whitespace GOGETA_HOME is treated as unset."""
        monkeypatch.setenv("GOGETA_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_gogeta_home() == Path.home() / ".gogeta"

    def test_fallback_display_gogeta_home_shortens_path(self, monkeypatch):
        """Fallback display_gogeta_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("GOGETA_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_gogeta_home() == "~/.gogeta"

    def test_fallback_display_gogeta_home_profile_path(self, monkeypatch):
        """Fallback display_gogeta_home() handles profile paths under ~/."""
        monkeypatch.setenv("GOGETA_HOME", str(Path.home() / ".gogeta/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_gogeta_home() == "~/.gogeta/profiles/coder"

    def test_fallback_display_gogeta_home_custom_path(self, monkeypatch):
        """Fallback display_gogeta_home() returns full path for non-home locations."""
        monkeypatch.setenv("GOGETA_HOME", "/opt/gogeta-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_gogeta_home() == "/opt/gogeta-custom"

    def test_delegates_to_gogeta_constants_when_available(self):
        """When gogeta_constants IS importable, _gogeta_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_gogeta_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import gogeta_constants
        assert module.get_gogeta_home is gogeta_constants.get_gogeta_home
        assert module.display_gogeta_home is gogeta_constants.display_gogeta_home
...
class TestInstallDeps:
    ...
    Regression coverage for the Gogeta Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "gogeta-agent[google]" in out
