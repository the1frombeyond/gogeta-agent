...
Override the image with ``GOGETA_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("GOGETA_TEST_IMAGE", "gogeta-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``GOGETA_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("GOGETA_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged gogeta user
# ---------------------------------------------------------------------------
#
# Background: every Gogeta runtime path inside the container drops to UID
# 10000 (the ``gogeta`` user) via ``s6-setuidgid gogeta``. ``docker exec``
...
# fails (PermissionError swallowed) for gogeta, so a test that ran as root
...
# helpers below run every probe under ``-u gogeta`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: gogeta).
