...
$GOGETA_HOME/profiles/<name>/gateway_state.json on every boot and
...
def restart_container(request, built_image: str):
    ...
    preserves $GOGETA_HOME/profiles/."""
    safe = request.node.name.replace("[", "_").replace("]", "_")
    name = f"gogeta-restart-{safe}"
    volume = f"gogeta-restart-vol-{safe}"
    _docker("rm", "-f", name)
...
def test_running_gateway_survives_container_restart(restart_container: str) -> None:
    ...
    r = _exec(container, "gogeta", "profile", "create", "coder")
    assert r.returncode == 0, f"profile create failed: {r.stderr}"

    r = _exec(container, "gogeta", "-p", "coder", "gateway", "start", timeout=60)
...
def test_stopped_gateway_stays_stopped_after_restart(restart_container: str) -> None:
    ...
    _exec(container, "gogeta", "profile", "create", "writer").check_returncode()
...
def test_stale_gateway_pid_cleaned_up_on_restart(restart_container: str) -> None:
    ...
    _exec(container, "gogeta", "profile", "create", "ghost").check_returncode()
