...
def _fake_switch_result():
    ...
    from gogeta_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "gogeta_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_gogeta_home() / config.yaml`` — point it here.
    monkeypatch.setattr("gogeta_constants.get_gogeta_home", lambda: gogeta_home)
    monkeypatch.setattr("gogeta_cli.config.get_gogeta_home", lambda: gogeta_home)
    return cfg_path
