...
GOGETA_GATEWAY_BOOTSTRAP_STATE on first boot, so a freshly-provisioned
...
GOGETA_GATEWAY_BOOTSTRAP_STATE=running; stage2-hook.sh (installed as

/etc/cont-init.d/01-gogeta-setup, which runs lexicographically BEFORE
...
This mirrors the existing GOGETA_AUTH_JSON_BOOTSTRAP env-seed pattern: it seeds
...
def _seed_block(text: str) -> str:
    ...
    """Extract the ``if [ ! -f "$GOGETA_HOME/gateway_state.json" ] && … fi``

    block that seeds the gateway state file from the bootstrap env var."""

    m = re.search(

        r'(if \[ ! -f "\$GOGETA_HOME/gateway_state\.json" \] && \\\n'
    ...
    assert m, (
        ...
        "guarded on GOGETA_GATEWAY_BOOTSTRAP_STATE"
...
def test_seed_block_present_and_guarded(stage2_text: str) -> None:
    ...
    assert '[ ! -f "$GOGETA_HOME/gateway_state.json" ]' in block, (

        "seed must be guarded by [ ! -f ] so persisted state wins on restart"

    )

    assert "GOGETA_GATEWAY_BOOTSTRAP_STATE" in block
...
def _run_seed(
    ...
    """Run the extracted seed block in a sandbox $GOGETA_HOME.



    ``env_value`` is the GOGETA_GATEWAY_BOOTSTRAP_STATE value (None = unset).
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env_line = (
            ...
            f'export GOGETA_GATEWAY_BOOTSTRAP_STATE="{env_value}"\n'

            if env_value is not None

            else "unset GOGETA_GATEWAY_BOOTSTRAP_STATE\n"
...
def test_no_seed_when_env_unset(stage2_text: str) -> None:
    ...
    assert out is None, "seed must not run when GOGETA_GATEWAY_BOOTSTRAP_STATE is unset"
