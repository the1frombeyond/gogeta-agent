...
class TestGogetaTokenStorage:

    def test_roundtrip_tokens(self, tmp_path, monkeypatch):

        monkeypatch.setenv("GOGETA_HOME", str(tmp_path))

        storage = GogetaTokenStorage("test-server")
    ...
    def test_token_file_created_with_0o600(self, tmp_path, monkeypatch):
        ...
        storage = GogetaTokenStorage("perm-test-server")
    ...
    def test_has_cached_tokens(self, tmp_path, monkeypatch):
        ...
        storage = GogetaTokenStorage("my-server")
    ...
    def test_corrupt_tokens_returns_none(self, tmp_path, monkeypatch):
        ...
        storage = GogetaTokenStorage("bad-server")
...
class TestPathTraversal:
    ...
    def test_path_traversal_blocked(self, tmp_path, monkeypatch):
        ...
        storage = GogetaTokenStorage("../../.ssh/config")
    ...
    def test_dots_and_slashes_sanitized(self, tmp_path, monkeypatch):
        ...
        storage = GogetaTokenStorage("../../../etc/passwd")
    ...
    def test_normal_name_unchanged(self, tmp_path, monkeypatch):
        ...
        storage = GogetaTokenStorage("my-mcp-server")
    ...
    def test_special_chars_sanitized(self, tmp_path, monkeypatch):
        ...
        storage = GogetaTokenStorage("server@host:8080/path")
...
def test_build_oauth_auth_preserves_server_url_path():
    ...
    ``OAuthContext.get_authorization_base_url``; Gogeta must not pre-strip.
    ...
    with patch.object(mcp_oauth, "_OAUTH_AVAILABLE", True), \
         ...
         patch.object(mcp_oauth, "GogetaTokenStorage") as mock_storage_cls:
...
class TestPasteCallbackSkipToken:
    ...
    def test_skip_message_printed(self, monkeypatch, capsys):
        ...
        assert "gogeta mcp login" in err
