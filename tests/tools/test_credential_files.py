...
class TestRegisterCredentialFiles:
    ...
    def test_dict_with_path_key(self, tmp_path):
        ...
        (gogeta_home / "token.json").write_text("{}")
        ...
        assert mounts[0]["host_path"] == str(gogeta_home / "token.json")

        assert mounts[0]["container_path"] == "/root/.gogeta/token.json"
    ...
    def test_dict_with_name_key_fallback(self, tmp_path):
        ...
        (gogeta_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        ...
        (gogeta_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        (gogeta_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    ...
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.gogeta/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        ...
        (gogeta_home / "skills").mkdir(parents=True)



        with patch.dict(os.environ, {"GOGETA_HOME": str(gogeta_home)}):

            mounts = get_skills_directory_mount(container_base="/home/user/.gogeta")



        assert mounts[0]["container_path"] == "/home/user/.gogeta/skills"
...
class TestIterSkillsFiles:
    ...
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.gogeta/skills/cat/myskill/SKILL.md" in paths

        assert "/root/.gogeta/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        """'../sensitive' must not escape GOGETA_HOME."""

        monkeypatch.setenv("GOGETA_HOME", str(tmp_path / ".gogeta"))

        (tmp_path / ".gogeta").mkdir()



        # Create a sensitive file one level above gogeta_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("GOGETA_HOME", str(gogeta_home))



        # Create a fake sensitive file outside gogeta_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        ...
        """Normal files inside GOGETA_HOME must still be registered."""
        ...
        (gogeta_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_gogeta_home_allowed(self, tmp_path, monkeypatch):

        """Files in subdirectories of GOGETA_HOME must be allowed."""

        gogeta_home = tmp_path / ".gogeta"

        gogeta_home.mkdir()

        subdir = gogeta_home / "creds"
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        """A symlink inside GOGETA_HOME pointing outside must be rejected."""
        ...
        # Create a sensitive file outside gogeta_home
        ...
        # Create a symlink inside gogeta_home pointing outside

        symlink = gogeta_home / "evil_link.json"
        ...
        # The resolved path escapes GOGETA_HOME — must be rejected
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, gogeta_home: Path, cred_files: list):
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        """'../secret' in config.yaml must not escape GOGETA_HOME."""
        ...
        self._write_config(gogeta_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        self._write_config(gogeta_home, [str(sensitive)])
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        ...
        """Normal files inside GOGETA_HOME via config must still mount."""
        ...
        (gogeta_home / "oauth.json").write_text("{}")

        self._write_config(gogeta_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        (gogeta_home / "cache" / "documents").mkdir(parents=True)

        (gogeta_home / "cache" / "audio").mkdir(parents=True)
        ...
        assert "/root/.gogeta/cache/documents" in paths

        assert "/root/.gogeta/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        assert mounts[0]["container_path"] == "/root/.gogeta/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        # Use legacy dir name — get_gogeta_dir prefers old if it exists

        (gogeta_home / "document_cache").mkdir()

        (gogeta_home / "image_cache").mkdir()
        ...
        assert str(gogeta_home / "document_cache") in host_paths

        assert str(gogeta_home / "image_cache") in host_paths

        # Container paths always use the new layout

        container_paths = {m["container_path"] for m in mounts}

        assert "/root/.gogeta/cache/documents" in container_paths

        assert "/root/.gogeta/cache/images" in container_paths



    def test_empty_gogeta_home(self, tmp_path, monkeypatch):
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        ...
        img_dir = gogeta_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.gogeta/cache/images/generated.png"
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            ...
            map_cache_path_to_container(host_path, container_base="/home/agent/.gogeta")

            == "/home/agent/.gogeta/cache/images/remote.png"
    ...
    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        ...
        (gogeta_home / "cache" / "images").mkdir(parents=True)
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(gogeta_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        doc_dir = gogeta_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        ss_dir = gogeta_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.gogeta/cache/screenshots/session_abc/screen1.png"
