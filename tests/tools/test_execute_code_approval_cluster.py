...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad GOGETA_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: GOGETA_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("GOGETA_GATEWAY_SESSION", "1")
    monkeypatch.delenv("GOGETA_INTERACTIVE", raising=False)
    monkeypatch.delenv("GOGETA_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("GOGETA_CRON_SESSION", "1")
    monkeypatch.delenv("GOGETA_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_gogeta_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "GOGETA_HOME": "/h", "GOGETA_PROFILE": "p",
        "GOGETA_CONFIG": "/c.yaml", "GOGETA_ENV": "/e",
        # other GOGETA_* → dropped (broad prefix removed)
        "GOGETA_BASE_URL": "https://x", "GOGETA_INTERACTIVE": "1",
        "GOGETA_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("GOGETA_HOME", "GOGETA_PROFILE", "GOGETA_CONFIG", "GOGETA_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "GOGETA_BASE_URL", "GOGETA_INTERACTIVE", "GOGETA_KANBAN_DB",
...
def test_env_scrub_logs_dropped_gogeta_vars(caplog):
    """Dropping a non-allowlisted, non-secret GOGETA_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset GOGETA_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "GOGETA_HOME": "/h",          # allowlisted → kept, not logged
        "GOGETA_BASE_URL": "https://x",   # dropped → logged
        "GOGETA_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "GOGETA_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "GOGETA_HOME" in out and "PATH" in out
    assert "GOGETA_BASE_URL" not in out and "GOGETA_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "GOGETA_BASE_URL" in msgs and "GOGETA_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "GOGETA_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped GOGETA_* vars."""
    import logging
