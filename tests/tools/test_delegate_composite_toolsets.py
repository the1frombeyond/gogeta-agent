...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_gogeta_cli_expands_web(self):
        """gogeta-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"gogeta-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("gogeta-cli", expanded)
    ...
    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['gogeta-cli'] yields ['web']."""
        parent_toolsets = {"gogeta-cli"}
