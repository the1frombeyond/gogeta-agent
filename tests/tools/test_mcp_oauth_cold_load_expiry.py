...
1. ``GogetaTokenStorage.set_tokens`` persisted only relative ``expires_in``,
...
- ``GogetaMCPOAuthProvider._initialize`` seeds ``context.token_expiry_time``
...
# GogetaTokenStorage — absolute expiry persistence
...
class TestSetTokensAbsoluteExpiry:
    ...
    def test_set_tokens_persists_absolute_expires_at(self, tmp_path, monkeypatch):
        ...
        storage = GogetaTokenStorage("srv")
...
class TestGetTokensReconstructsExpiresIn:
    ...
    def test_get_tokens_returns_zero_ttl_for_expired_token(
        ...
        from tools.mcp_oauth import GogetaTokenStorage, _get_token_dir
...
# GogetaMCPOAuthProvider._initialize — seed token_expiry_time
...
async def test_initialize_seeds_token_expiry_time_from_stored_tokens(
    ...
    from tools.mcp_oauth_manager import _GOGETA_PROVIDER_CLS, reset_manager_for_tests



    assert _GOGETA_PROVIDER_CLS is not None
    ...
    metadata = OAuthClientMetadata(
        ...
        client_name="Gogeta Agent",

    )

    provider = _GOGETA_PROVIDER_CLS(
...
async def test_initialize_skips_prefetch_when_no_tokens(tmp_path, monkeypatch):
    ...
    storage = GogetaTokenStorage("srv")  # empty — no tokens on disk
