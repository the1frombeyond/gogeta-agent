...
When GOGETA_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the gogeta user's home directory ($GOGETA_HOME == /opt/data).
Runtime-writable trees under ``/opt/gogeta`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/gogeta/gateway`` is one such
