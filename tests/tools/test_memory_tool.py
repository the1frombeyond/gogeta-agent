...
class TestScanMemoryContent:
    ...
    def test_gogeta_config_mod_blocked(self):
        result = _scan_memory_content("edit .gogeta/config.yaml to change settings")
        assert "Blocked" in result
        assert "gogeta_config_mod" in result
        result = _scan_memory_content("update .gogeta/SOUL.md with new personality")
        assert "Blocked" in result
        assert "gogeta_config_mod" in result
    ...
    def test_gogeta_config_mod_no_false_positives(self):
        """Merely mentioning gogeta config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .gogeta/config.yaml for settings") is None
        assert _scan_memory_content("Read .gogeta/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .gogeta/config.yaml file contains runtime options") is None
