...
class TestWriteDenyExactPaths:
    ...
    def test_gogeta_env(self):

        # ``.env`` under the active GOGETA_HOME (profile-aware, not just

        # ``~/.gogeta``) must be write-denied. The hermetic test conftest

        # points GOGETA_HOME at a tempdir — resolve via get_gogeta_home()

        # to match the denylist.

        from gogeta_constants import get_gogeta_home

        path = str(get_gogeta_home() / ".env")

        assert _is_write_denied(path) is True



    def test_gogeta_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.gogeta/.env`` (whose credentials are inherited by every profile)
        ...
        # Sanity check: GOGETA_HOME does point to the profile dir, not the root.

        from gogeta_constants import get_gogeta_home, get_default_gogeta_root

        assert get_gogeta_home() == profile_home

        assert get_default_gogeta_root() == root
...
class TestWriteAllowed:
    ...
    def test_gogeta_config_not_env(self):

        path = os.path.join(str(Path.home()), ".gogeta", "config.yaml")
