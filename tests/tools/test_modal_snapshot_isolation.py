...
def _restore_tool_modules():
    ...
    original_gogeta_home = os.environ.get("GOGETA_HOME")
    ...
    try:
        ...
        if original_gogeta_home is None:

            os.environ.pop("GOGETA_HOME", None)

        else:

            os.environ["GOGETA_HOME"] = original_gogeta_home

        _reset_modules(("tools", "gogeta_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    gogeta_cli = types.ModuleType("gogeta_cli")

    gogeta_cli.__path__ = []  # type: ignore[attr-defined]

    sys.modules["gogeta_cli"] = gogeta_cli

    gogeta_home = tmp_path / "gogeta-home"

    os.environ["GOGETA_HOME"] = str(gogeta_home)

    sys.modules["gogeta_cli.config"] = types.SimpleNamespace(

        get_gogeta_home=lambda: gogeta_home,
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        ...
        return types.SimpleNamespace(name="gogeta-agent", create_if_missing=create_if_missing)
    ...
    return {
        ...
        "snapshot_store": gogeta_home / "modal_snapshots.json",
