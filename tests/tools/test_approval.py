...
class TestApprovalModeParsing:
    ...
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        ...
        with mock_patch("gogeta_cli.config.load_config", return_value={"approvals": {"mode": False}}):
    ...
    def test_string_off_still_maps_to_off(self):
        ...
        with mock_patch("gogeta_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
...
class TestSessionKeyContext:
    ...
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            ...
            with mock_patch.dict("os.environ", {"GOGETA_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_gogeta_env(self):

        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.gogeta/.env")
    ...
    def test_tee_custom_gogeta_home_env(self):

        dangerous, key, desc = detect_dangerous_command("echo x | tee $GOGETA_HOME/.env")
    ...
    def test_tee_quoted_custom_gogeta_home_env(self):
...
class TestGogetaConfigWriteProtection:

    """Terminal-side pairing for the file_tools write_file/patch deny on

    ~/.gogeta/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        ...
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.gogeta/config.yaml")
    ...
    def test_append(self):
        ...
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.gogeta/config.yaml")
    ...
    def test_tee(self):
        ...
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.gogeta/config.yaml")
    ...
    def test_cp_over_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.gogeta/config.yaml")
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.gogeta/config.yaml")

        assert dangerous is True

        assert "gogeta config" in desc.lower() or "in-place" in desc.lower()



    def test_sed_in_place_long_flag(self):

        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.gogeta/config.yaml")

        assert dangerous is True



    def test_custom_gogeta_home(self):

        dangerous, key, desc = detect_dangerous_command("echo x | tee $GOGETA_HOME/config.yaml")
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            ...
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.gogeta/config.yaml"
    ...
    def test_ruby_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            ...
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.gogeta/config.yaml"
    ...
    def test_perl_in_place_env(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            ...
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.gogeta/.env"
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            ...
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.gogeta/config.yaml"
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            ...
            "perl -i.bak -pe 's/x/y/' ~/.gogeta/config.yaml"
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            ...
            "perl -wne 'print' ~/.gogeta/config.yaml"
    ...
    def test_read_is_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command("cat ~/.gogeta/config.yaml")
    ...
    def test_normal_yaml_write_safe(self):
        ...
        # A non-Gogeta config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_gogeta_home_env(self):

        dangerous, key, desc = detect_dangerous_command("echo x > $GOGETA_HOME/.env")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        ...
        cmd = "kill 1605 && cd ~/.gogeta/gogeta-agent && source venv/bin/activate && python -m gogeta_cli.main gateway run --replace &disown; echo done"
    ...
    def test_gateway_run_with_ampersand_detected(self):
        ...
        cmd = "python -m gogeta_cli.main gateway run --replace &"
    ...
    def test_gateway_run_with_nohup_detected(self):
        ...
        cmd = "nohup python -m gogeta_cli.main gateway run --replace"
    ...
    def test_gateway_run_with_setsid_detected(self):
        ...
        cmd = "gogeta_cli.main gateway run --replace &disown"
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart gogeta-gateway"
    ...
    def test_pkill_gogeta_detected(self):

        """pkill targeting gogeta/gateway processes must be caught."""
    ...
    def test_killall_gogeta_detected(self):
...
class TestPgrepKillExpansion:
    ...
    """kill -9 $(pgrep gogeta) bypasses the pkill/killall name-matching
    ...
    def test_kill_dollar_pgrep_detected(self):
        ...
        cmd = 'kill -9 $(pgrep -f "gogeta.*gateway")'
    ...
    def test_pkill_gogeta_still_detected(self):
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            ...
            for k in ("GOGETA_GATEWAY_SESSION", "GOGETA_CRON_SESSION",

                      "GOGETA_YOLO_MODE",

                      "GOGETA_SESSION_KEY", "GOGETA_INTERACTIVE")

        }

        os.environ.pop("GOGETA_YOLO_MODE", None)

        os.environ.pop("GOGETA_INTERACTIVE", None)

        # GOGETA_CRON_SESSION takes priority over GOGETA_GATEWAY_SESSION in
        ...
        os.environ.pop("GOGETA_CRON_SESSION", None)

        os.environ["GOGETA_GATEWAY_SESSION"] = "1"

        os.environ["GOGETA_SESSION_KEY"] = self.SESSION_KEY
