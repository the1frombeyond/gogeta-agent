...
def fake_gogeta(tmp_path, monkeypatch):
    ...
    """Build a two-profile Gogeta layout and point GOGETA_HOME at

    the gogeta-security profile (matching the original-incident shape).
    ...
    monkeypatch.setenv("GOGETA_HOME", str(sec_home))
    ...
    monkeypatch.setattr(fs, "_gogeta_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    ...
    def test_in_profile_write_allowed(self, fake_gogeta):

        from tools.file_tools import write_file_tool

        target = fake_gogeta["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_gogeta):
        ...
        target = fake_gogeta["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "gogeta-security" in result["error"]
    ...
    def test_cross_profile_True_bypass(self, fake_gogeta):
    ...
    def test_non_gogeta_path_unaffected(self, fake_gogeta, tmp_path):
...
class TestPatchCrossProfileGuard:
    ...
    def test_cross_profile_patch_blocked(self, fake_gogeta):
    ...
    def test_cross_profile_patch_bypass(self, fake_gogeta):
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_gogeta):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        self._make_skill_in_profile(fake_gogeta["root"], "default-only-skill")



        # Re-import the module so SKILLS_DIR picks up GOGETA_HOME (set in
        ...
        assert "not found in active profile 'gogeta-security'" in err
    ...
    def test_error_names_multiple_profiles(self, fake_gogeta, monkeypatch):

        """When the skill exists in TWO other profiles, both should be named."""

        self._make_skill_in_profile(fake_gogeta["root"], "everywhere-skill")

        self._make_skill_in_profile(fake_gogeta["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    ...
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.gogeta/profiles/<name>/."""

        # Don't set GOGETA_HOME — falls back to default.

        import agent.file_safety as fs

        monkeypatch.setattr(fs, "_gogeta_home_path", lambda: tmp_path / "fake")

        monkeypatch.setattr(fs, "_gogeta_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_gogeta):

        """When active profile is 'gogeta-security', the prompt warns
        ...
        assert "Active Gogeta profile" in src

        assert "cross_profile=True" in src

        assert "~/.gogeta/profiles/" in src

        # Both branches present (default and named profile).

        assert "Active Gogeta profile: default" in src

        assert "Active Gogeta profile: {active_profile}" in src
