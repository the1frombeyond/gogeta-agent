...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             ...
             patch("tools.tirith_security._gogeta_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert path == str(gogeta_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(gogeta_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_gogeta_bin(self):

        """After _INSTALL_FAILED, manual install in GOGETA_HOME/bin is picked up."""
        ...
        gogeta_bin = os.path.join(tmpdir, "tirith")

        # Create a fake executable

        with open(gogeta_bin, "w") as f:

            f.write("#!/bin/sh\n")

        os.chmod(gogeta_bin, 0o755)
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             ...
             patch("tools.tirith_security._gogeta_bin_dir", return_value=tmpdir), \

             patch("tools.tirith_security._clear_install_failed") as mock_clear:

            result = _resolve_tirith_path("tirith")

            assert result == gogeta_bin

            assert _tirith_mod._resolved_path == gogeta_bin
...
# GOGETA_HOME isolation

# ---------------------------------------------------------------------------



class TestGogetaHomeIsolation:

    def test_gogeta_bin_dir_respects_gogeta_home(self):

        """_gogeta_bin_dir must use GOGETA_HOME, not hardcoded ~/.gogeta."""

        from tools.tirith_security import _gogeta_bin_dir

        import tempfile

        tmpdir = tempfile.mkdtemp()

        with patch.dict(os.environ, {"GOGETA_HOME": tmpdir}):

            result = _gogeta_bin_dir()
    ...
    def test_failure_marker_respects_gogeta_home(self):

        """_failure_marker_path must use GOGETA_HOME, not hardcoded ~/.gogeta."""

        from tools.tirith_security import _failure_marker_path

        with patch.dict(os.environ, {"GOGETA_HOME": "/custom/gogeta"}):

            result = _failure_marker_path()

        assert result == "/custom/gogeta/.tirith-install-failed"



    def test_conftest_isolation_prevents_real_home_writes(self):

        """The conftest autouse fixture sets GOGETA_HOME; verify it's active."""

        gogeta_home = os.getenv("GOGETA_HOME")

        assert gogeta_home is not None, "GOGETA_HOME should be set by conftest"

        assert "gogeta_test" in gogeta_home, "Should point to test temp dir"



    def test_get_gogeta_home_fallback(self):

        """Without GOGETA_HOME set, falls back to the active OS home."""

        from tools.tirith_security import _get_gogeta_home

        with patch.dict(os.environ, {}, clear=True):

            # Remove GOGETA_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".gogeta")

            result = _get_gogeta_home()
