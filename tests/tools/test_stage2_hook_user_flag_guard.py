...
guidance, while still allowing:
  ...
  - `--user <gogeta-uid>` (the supported non-root start, #34648 / #34837)
...
root, and the baked image dirs are owned by the gogeta build UID, so an

arbitrary pinned UID can't write them (EACCES on a bind mount, hard crash on a

named volume). The supported path is root start + GOGETA_UID/GOGETA_GID (or the

PUID/PGID aliases), which remaps the gogeta user and chowns the volume.



The guard fires only when the current UID is neither root NOR the gogeta UID,

so the #34648 `--user 10000:10000` case (pinning to the gogeta UID itself) is
...
def _guard_block(text: str) -> str:
    ...
    assert m, "expected the --user guard block (cur_uid + non-root/non-gogeta check)"
...
def test_guard_present_and_mentions_remediation(path: Path) -> None:
    ...
    # Must check non-root AND non-gogeta-uid (so --user 10000:10000 is allowed).

    assert '"$cur_uid" != 0' in block

    assert '"$cur_uid" != "$(id -u gogeta)"' in block

    assert "exit 1" in block

    # Must point users at the supported env vars.

    assert "GOGETA_UID" in block and "GOGETA_GID" in block
...
def _run_guard(text: str, *, cur_uid: int, gogeta_uid: int = 10000) -> subprocess.CompletedProcess:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        script = (
            ...
            # Stub `id`: `id -u` -> cur_uid; `id -u gogeta` -> gogeta_uid.

            f'id() {{ if [ "$2" = gogeta ]; then echo {gogeta_uid}; else echo {cur_uid}; fi; }}\n'
...
def test_arbitrary_user_uid_is_rejected() -> None:
    ...
    """An arbitrary host UID (1000), neither root nor gogeta, is rejected."""

    for text in (_read(STAGE2_HOOK), _read(MAIN_WRAPPER)):

        proc = _run_guard(text, cur_uid=1000, gogeta_uid=10000)
...
def test_root_start_passes() -> None:
    ...
    for text in (_read(STAGE2_HOOK), _read(MAIN_WRAPPER)):
        ...
        proc = _run_guard(text, cur_uid=0, gogeta_uid=10000)
...
def test_user_pinned_to_gogeta_uid_passes() -> None:

    """`--user 10000:10000` (the gogeta UID itself) is the supported non-root

    start from #34648 / #34837 and must NOT be blocked."""

    for text in (_read(STAGE2_HOOK), _read(MAIN_WRAPPER)):

        proc = _run_guard(text, cur_uid=10000, gogeta_uid=10000)
...
def test_user_pinned_to_remapped_gogeta_uid_passes() -> None:

    """After a GOGETA_UID remap the gogeta UID is e.g. 4242; a container pinned

    to that same UID must still pass (cur_uid == gogeta_uid)."""

    for text in (_read(STAGE2_HOOK), _read(MAIN_WRAPPER)):

        proc = _run_guard(text, cur_uid=4242, gogeta_uid=4242)
