...
Verifies:
  ...
  - Tools are gated on GOGETA_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    ...
    """Normal `gogeta chat` sessions (no GOGETA_KANBAN_TASK) must have

    zero kanban_* tools in their schema."""

    monkeypatch.delenv("GOGETA_KANBAN_TASK", raising=False)
    ...
    schema = registry.get_definitions(set(resolve_toolset("gogeta-cli")), quiet=True)
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("GOGETA_KANBAN_TASK", "t_fake")
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the GOGETA_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    ...
    """Simulate being a worker: GOGETA_HOME isolated, GOGETA_KANBAN_TASK set
    ...
    monkeypatch.setenv("GOGETA_PROFILE", "test-worker")

    monkeypatch.delenv("GOGETA_SESSION_ID", raising=False)
    ...
    monkeypatch.setenv("GOGETA_KANBAN_TASK", tid)
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        ...
        "changed_files": ["gogeta_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("GOGETA_SESSION_ID", "session-trusted")
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to GOGETA_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``GOGETA_PROFILE`` so a worker can't forge a comment

    under an authoritative-looking name like ``gogeta-system`` and
    ...
    out = kt._handle_comment({
        ...
        "task_id": worker_env, "body": "hi", "author": "gogeta-system",
    ...
    try:
        ...
        # Author comes from GOGETA_PROFILE in the fixture, not the

        # caller-supplied "gogeta-system" override.
...
def test_create_inherits_worker_dir_workspace(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("GOGETA_KANBAN_TASK", self_tid)
...
def test_create_no_worker_task_stays_scratch(monkeypatch, worker_env):
    ...
    """Orchestrator/CLI callers (no GOGETA_KANBAN_TASK) still default to
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("GOGETA_SESSION_ID", "from-env")
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    ...
    """A normal chat session (no GOGETA_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    ...
    """A worker session (GOGETA_KANBAN_TASK set) MUST have the full
...
# A worker process has GOGETA_KANBAN_TASK set to its own task id. The
...
# Orchestrator profiles (no GOGETA_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # GOGETA_PROFILE — never to a caller-controlled string.
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    import gogeta_cli.kanban_db as _kb
    ...
    monkeypatch.setenv("GOGETA_KANBAN_CRASH_GRACE_SECONDS", "0")
    ...
    monkeypatch.setenv("GOGETA_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("GOGETA_KANBAN_RUN_ID", str(run2.id))
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    ...
    """Orchestrator profiles (no GOGETA_KANBAN_TASK) can still complete
...
# The dispatcher pins the active board via GOGETA_KANBAN_BOARD env var,
...
# restarting Gogeta. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    ...
    """Isolated Gogeta home with two distinct kanban boards seeded.
    ...
    GOGETA_KANBAN_TASK is pinned (orchestrator context) — workers test
    ...
    # Make sure neither GOGETA_KANBAN_DB nor GOGETA_KANBAN_BOARD pin a

    # board — the test is specifically about the per-call override.

    monkeypatch.delenv("GOGETA_KANBAN_DB", raising=False)

    monkeypatch.delenv("GOGETA_KANBAN_BOARD", raising=False)

    monkeypatch.delenv("GOGETA_KANBAN_TASK", raising=False)

    monkeypatch.setenv("GOGETA_PROFILE", "test-orchestrator")
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning GOGETA_KANBAN_TASK
    ...
    monkeypatch.setenv("GOGETA_PROFILE", "alt-worker")
