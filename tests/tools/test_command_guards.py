...
def _clean_state():
    ...
    saved = {}
    for k in ("GOGETA_INTERACTIVE", "GOGETA_GATEWAY_SESSION", "GOGETA_EXEC_ASK", "GOGETA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["GOGETA_INTERACTIVE"] = "1"
        session_key = os.getenv("GOGETA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No GOGETA_INTERACTIVE or GOGETA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
