"""Regression test for the ``GogetaMCPOAuthProvider.async_auth_flow`` bidirectional
...
async def test_gogeta_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    assert _GOGETA_PROVIDER_CLS is not None, "SDK OAuth types must be available"
...
async def test_gogeta_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
