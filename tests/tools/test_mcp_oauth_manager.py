...
def test_gogeta_provider_subclass_exists():
    """GogetaMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _GOGETA_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _GOGETA_PROVIDER_CLS is not None
    assert issubclass(_GOGETA_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_gogeta_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns GogetaMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _GOGETA_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("GOGETA_HOME", str(tmp_path))
    ...
    assert _GOGETA_PROVIDER_CLS is not None
    assert isinstance(provider, _GOGETA_PROVIDER_CLS)
    assert provider._gogeta_server_name == "srv"
