...
the volume.  GOGETA_UID/GOGETA_GID must still take precedence when both are
...
to docker/stage2-hook.sh, which is installed as /etc/cont-init.d/01-gogeta-setup
...
def _alias_lines(text: str) -> list[str]:
    ...
    """The stage2 hook lines that resolve GOGETA_UID/GOGETA_GID from aliases."""
    ...
    return [
        ...
        if line.strip().startswith(("GOGETA_UID=", "GOGETA_GID="))
...
def test_stage2_hook_resolves_puid_pgid_aliases(stage2_text: str) -> None:
    ...
    assert any("PUID" in line for line in alias_lines), (
        ...
        "docker/stage2-hook.sh must resolve GOGETA_UID from a PUID alias; see #15290"

    )

    assert any("PGID" in line for line in alias_lines), (

        "docker/stage2-hook.sh must resolve GOGETA_GID from a PGID alias; see #15290"
...
def _resolve(stage2_text: str, env: dict[str, str]) -> str:
    ...
    resolved ``GOGETA_UID:GOGETA_GID`` pair."""
...
def test_puid_pgid_populate_gogeta_uid_gid(stage2_text: str) -> None:
...
def test_gogeta_uid_gid_take_precedence_over_aliases(stage2_text: str) -> None:

    resolved = _resolve(

        stage2_text,

        {"GOGETA_UID": "2000", "GOGETA_GID": "2001", "PUID": "1000", "PGID": "10"},
...
def test_no_uid_vars_leaves_values_empty(stage2_text: str) -> None:
    ...
    # An empty resolution means the stage2 hook keeps the default gogeta user.
...
def test_stage2_hook_runs_config_migration_as_gogeta(stage2_text: str) -> None:

    assert "scripts/docker_config_migrate.py" in stage2_text

    assert 's6-setuidgid gogeta "$INSTALL_DIR/.venv/bin/python"' in stage2_text
...
def test_stage2_hook_documents_config_migration_opt_out(stage2_text: str) -> None:
    ...
    assert "GOGETA_SKIP_CONFIG_MIGRATION" in stage2_text
