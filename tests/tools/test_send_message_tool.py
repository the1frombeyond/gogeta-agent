...
class TestSendMessageTool:
    ...
    def test_ntfy_topic_target_bypasses_channel_directory(self):
        ...
        ntfy_cfg = SimpleNamespace(enabled=True, token=None, extra={"topic": "gogeta-in"})
    ...
    def test_cron_duplicate_target_is_skipped_and_explained(self):
        ...
        with patch.dict(
...

            {
                ...
                "GOGETA_CRON_AUTO_DELIVER_PLATFORM": "telegram",

                "GOGETA_CRON_AUTO_DELIVER_CHAT_ID": "-1001",
    ...
    def test_mirror_receives_current_session_user_id(self):
        ...
        with patch("gateway.config.load_gateway_config", return_value=config), \
            ...
            get_session_env_mock.side_effect = lambda name, default="": {
                ...
                "GOGETA_SESSION_USER_ID": "user-123",
...
class TestSendToPlatformChunking:
    ...
    def test_slack_messages_are_formatted_before_send(self, monkeypatch):
        ...
        with patch("tools.send_message_tool._send_slack", send):
            ...
            result = asyncio.run(
                ...
                _send_to_platform(
                    ...
                    "**hello** from [Gogeta](<https://example.com>)",
        ...
        send.assert_awaited_once_with(
            ...
            "*hello* from <https://example.com|Gogeta>",
...
class TestSendToPlatformWhatsapp:
    ...
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        async_mock.assert_awaited_once_with({"bridge_port": 3000}, chat_id, "hello from gogeta")
...
class TestParseTargetRefMatrix:
    ...
    def test_matrix_user_mxid_is_explicit(self):
        ...
        chat_id, thread_id, is_explicit = _parse_target_ref("matrix", "@gogeta:matrix.org")

        assert chat_id == "@gogeta:matrix.org"
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``gogeta cron``

    runs separately from ``gogeta gateway``), ``_send_via_adapter`` should
...
class TestCheckSendMessage:
    ...
    1. ``GOGETA_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher

       — parent gateway is by definition running, but the worker's

       ``GOGETA_HOME`` may be a profile dir without a ``gateway.pid``).

    2. ``GOGETA_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
    ...
    3. ``is_gateway_running()`` returns True (CLI / orchestrator profile with
       ...
       a live gateway colocated under the same ``GOGETA_HOME``).
    ...
    def test_kanban_task_env_grants_access(self, monkeypatch):
        ...
        """Workers spawned by the dispatcher (GOGETA_KANBAN_TASK set) must be
        ...
        monkeypatch.setenv("GOGETA_KANBAN_TASK", "t_abc12345")
    ...
    def test_kanban_task_env_short_circuits_before_gateway_check(self, monkeypatch):
        ...
        """Honoring GOGETA_KANBAN_TASK must not depend on importing or calling

        gateway.status — the worker may run with a GOGETA_HOME that has no
        ...
        with patch("gateway.session_context.get_session_env",
                   ...
                   side_effect=AssertionError("session_context not consulted "
                                              ...
                                              "when GOGETA_KANBAN_TASK is set")), \
    ...
    def test_local_platform_falls_through_to_gateway_check(self, monkeypatch):
        ...
        """``GOGETA_SESSION_PLATFORM=local`` means CLI-style — must defer to
