...
def _isolate(tmp_path, monkeypatch):
    """Redirect GOGETA_HOME and clear module caches."""
    gogeta_home = tmp_path / ".gogeta"
    gogeta_home.mkdir()
    monkeypatch.setenv("GOGETA_HOME", str(gogeta_home))
    # Write a minimal config so load_config doesn't fail
    (gogeta_home / "config.yaml").write_text("model:\n  default: test-model\n")


def _write_config(tmp_path, config_dict):
    """Write a config.yaml to the test GOGETA_HOME."""
    import yaml
...
class TestResolveProviderClientMainAlias:
    ...
    def test_main_resolves_github_copilot_alias(self, tmp_path):
        ...
        with (
            patch("gogeta_cli.auth.resolve_api_key_provider_credentials", return_value={
