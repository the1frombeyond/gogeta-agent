...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate GOGETA_HOME under tmp_path."""
    monkeypatch.setenv("GOGETA_HOME", str(tmp_path / ".gogeta"))
    (tmp_path / ".gogeta").mkdir()

    # Force the constants/image cache helpers to re-read GOGETA_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("gogeta_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_gogeta_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under GOGETA_HOME — gateway cleanup
