...
def isolated_home(monkeypatch):
    """Temp GOGETA_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="gogeta_test_31179_")
    gogeta_home = os.path.join(test_home, ".gogeta")
    os.makedirs(gogeta_home)
    monkeypatch.setenv("GOGETA_HOME", gogeta_home)
...
def _fresh_modules():
    """Drop cached gogeta modules so each test reloads against current env."""
