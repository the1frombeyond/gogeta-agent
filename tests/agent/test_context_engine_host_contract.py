...
engine plugins (e.g. gogeta-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``gogeta plugins`` picker."""
    from gogeta_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "gogeta_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "gogeta_cli.plugins.get_plugin_context_engine",
