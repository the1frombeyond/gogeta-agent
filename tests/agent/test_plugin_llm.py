...
""",
        ...
        from gogeta_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        gogeta_home = tmp_path / ".gogeta"
        gogeta_home.mkdir()
        (gogeta_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("GOGETA_HOME", str(gogeta_home))
        from gogeta_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from gogeta_cli.plugins import PluginContext, PluginManifest, PluginManager
