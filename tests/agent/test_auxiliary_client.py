...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("nous", "gogeta-4", "https://inference-api.gogeta.ai/v1"),
...
class TestReadCodexAccessToken:
    def test_valid_auth_store(self, tmp_path, monkeypatch):
        ...
        (gogeta_home / "auth.json").write_text(json.dumps({
            "version": 1,
            "providers": {
                "openai-codex": {
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(self, tmp_path, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("gogeta_cli.auth._read_codex_tokens", return_value={
    ...
    def test_missing_returns_none(self, tmp_path, monkeypatch):
        gogeta_home = tmp_path / "gogeta"
        gogeta_home.mkdir(parents=True, exist_ok=True)
        (gogeta_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
        monkeypatch.setenv("GOGETA_HOME", str(gogeta_home))
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``gogeta auth status`` already reports these as logged in; compression
        ...
        monkeypatch.setenv("GOGETA_HOME", str(gogeta_home))
        monkeypatch.delenv("GOGETA_XAI_BASE_URL", raising=False)
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("GOGETA_HOME", str(gogeta_home))
        monkeypatch.setenv("GOGETA_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``gogeta model``, no per-task override is
...
class TestExpiredCodexFallback:
    ...
    def test_gogeta_oauth_file_sets_oauth_flag(self, monkeypatch):
        """OAuth-style tokens should get is_oauth=*** (token is not sk-ant-api-*)."""
        # Mock resolve_anthropic_token to return an OAuth-style token
        with patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat-gogeta-token"), \
             patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from agent.auxiliary_client import _try_anthropic
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("gogeta_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
...
class TestAuxiliaryPoolAwareness:
    def test_try_nous_uses_pool_entry(self):
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=_Pool()),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("gogeta_cli.models.get_nous_recommended_aux_model", return_value=None),
        ):
    ...
    def test_try_nous_uses_portal_recommendation_for_text(self):
        ...
        fresh_base = "https://inference-api.gogeta.ai/v1"
        with (
            ...
            patch("gogeta_cli.models.get_nous_recommended_aux_model", return_value="minimax/minimax-m2.7") as mock_rec,
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
        ):
            from agent.auxiliary_client import _try_nous
    ...
    def test_try_nous_uses_portal_recommendation_for_vision(self):
        ...
        with (
            patch("agent.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("gogeta_cli.models.get_nous_recommended_aux_model", return_value="google/gemini-3-flash-preview") as mock_rec,
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_nous
    ...
    def test_try_nous_falls_back_when_recommendation_lookup_raises(self):
        ...
        with (
            patch("agent.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("gogeta_cli.models.get_nous_recommended_aux_model", side_effect=RuntimeError("portal down")),
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_nous
            client, model = _try_nous()
    ...
    def test_call_llm_retries_nous_after_401(self):
        ...
        stale_client.base_url = "https://inference-api.gogeta.ai/v1"
        stale_client.chat.completions.create.side_effect = _Auth401("stale nous key")

        fresh_client = MagicMock()
        fresh_client.base_url = "https://inference-api.gogeta.ai/v1"
        ...
        with (
            ...
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", "https://inference-api.gogeta.ai/v1")),
        ):
            result = call_llm(
                task="compression",
                messages=[{"role": "user", "content": "hi"}],
    ...
    def test_call_llm_refreshes_nous_after_free_tier_block_when_account_paid(self):
        ...
        with (
            ...
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", "https://inference-api.gogeta.ai/v1")),
            patch(
                "gogeta_cli.nous_account.get_nous_portal_account_info",
    ...
    async def test_async_call_llm_retries_nous_after_401(self):
        ...
        fresh_async_client.base_url = "https://inference-api.gogeta.ai/v1"
...
class TestIsPaymentError:
    ...
    def test_404_free_tier_model_block_is_payment(self):
        exc = Exception(
            ...
            "Upgrade at https://portal.gogeta.ai or pick a free model."
        )
        exc.status_code = 404
        assert _is_payment_error(exc) is True
...
class TestRefreshNousRecommendedModel:
    ...
    def test_returns_fresh_portal_recommendation(self, monkeypatch):
        monkeypatch.setattr(
            "gogeta_cli.models.get_nous_recommended_aux_model",
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "gogeta_cli.models.get_nous_recommended_aux_model", _boom)
        out = _refresh_nous_recommended_model(
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from gogeta_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("gogeta_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("gogeta_cli.config.load_config", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/NousResearch/gogeta/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/NousResearch/gogeta/issues/18478
    """
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "GsaAgent"
