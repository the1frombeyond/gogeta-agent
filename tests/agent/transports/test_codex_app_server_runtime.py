...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Gogeta passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("GOGETA_HOME", "/users/alice/.gogeta/profiles/backend-worker")
        monkeypatch.setenv("GOGETA_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "GOGETA_KANBAN_DB",
            "/users/alice/.gogeta/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.gogeta/kanban/boards/smoke"]'
            in cmd
        )
