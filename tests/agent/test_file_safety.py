...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_gogeta_env(self):
        """Gogeta' own .env inside GOGETA_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: gogeta internal .env is in ~/.gogeta/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.gogeta/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.gogeta/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Gogeta cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        gogeta_home = tmp_path / ".gogeta"
        cache = gogeta_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._gogeta_home_path", return_value=gogeta_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Gogeta cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        gogeta_home = tmp_path / ".gogeta"
        hub = gogeta_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_gogeta_home(self, tmp_path):
        """The env basename guard does not depend on GOGETA_HOME resolution."""
        gogeta_home = tmp_path / ".gogeta"
        gogeta_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        gogeta_home = tmp_path / ".gogeta"
        cache = gogeta_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
