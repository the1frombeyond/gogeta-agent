...
def test_gogeta_client_tag_includes_current_version():
    """The client tag must reflect gogeta_cli.__version__ verbatim."""
    from gogeta_cli import __version__
    from agent.portal_tags import gogeta_client_tag

    assert gogeta_client_tag() == f"client=gogeta-client-v{__version__}"


def test_gogeta_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import gogeta_client_tag

    tag = gogeta_client_tag()
    assert tag.startswith("client=gogeta-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import gogeta_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=gogeta-agent" in tags
    assert gogeta_client_tag() in tags
    assert len(tags) == 2
