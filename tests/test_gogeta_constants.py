"""Tests for gogeta_constants module."""
...
class TestGetDefaultGsaRoot:
    """Tests for get_default_gogeta_root() — Docker/custom deployment awareness."""

    def test_no_gogeta_home_returns_native(self, tmp_path, monkeypatch):
        """When GOGETA_HOME is not set, returns ~/.gogeta."""
        monkeypatch.delenv("GOGETA_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_gogeta_root() == tmp_path / ".gogeta"

    def test_gogeta_home_is_native(self, tmp_path, monkeypatch):
        """When GOGETA_HOME = ~/.gogeta, returns ~/.gogeta."""
        native = tmp_path / ".gogeta"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GOGETA_HOME", str(native))
        assert get_default_gogeta_root() == native

    def test_gogeta_home_is_profile(self, tmp_path, monkeypatch):
        """When GOGETA_HOME is a profile under ~/.gogeta, returns ~/.gogeta."""
        native = tmp_path / ".gogeta"
        profile = native / "profiles" / "coder"
        profile.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GOGETA_HOME", str(profile))
        assert get_default_gogeta_root() == native

    def test_gogeta_home_is_docker(self, tmp_path, monkeypatch):
        """When GOGETA_HOME points outside ~/.gogeta (Docker), returns GOGETA_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GOGETA_HOME", str(docker_home))
        assert get_default_gogeta_root() == docker_home

    def test_gogeta_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any GOGETA_HOME outside ~/.gogeta is treated as the root."""
        custom = tmp_path / "my-gogeta-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GOGETA_HOME", str(custom))
        assert get_default_gogeta_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (GOGETA_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GOGETA_HOME", str(profile))
        assert get_default_gogeta_root() == docker_root

    def test_no_gogeta_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\gogeta, not ~/.gogeta."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("GOGETA_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(gogeta_constants.sys, "platform", "win32")

        assert get_default_gogeta_root() == local_appdata / "gogeta"

    def test_no_gogeta_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/gogeta without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("GOGETA_HOME", raising=False)
        ...
        assert get_default_gogeta_root() == home / "AppData" / "Local" / "gogeta"


class TestGetGsaHome:
    """Tests for get_gogeta_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When GOGETA_HOME is unset on Windows, use %LOCALAPPDATA%\\gogeta."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("GOGETA_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(gogeta_constants.sys, "platform", "win32")
        monkeypatch.setattr(gogeta_constants, "_profile_fallback_warned", False)

        assert get_gogeta_home() == local_appdata / "gogeta"
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(gogeta_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(gogeta_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".gogeta"
