...
(SQLITE_PROTOCOL — typical on NFS/SMB), Gogeta must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``GOGETA_HOME`` silently lose
...
def _reset_last_init_error():
    ...
    gogeta_state._set_last_init_error(None)
    yield
    gogeta_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    gogeta_state._wal_fallback_warned_paths.clear()
    yield
    gogeta_state._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_falls_back_to_delete_on_locking_protocol(self, tmp_path, caplog):
        ...
        with caplog.at_level("WARNING", logger="gogeta_state"):
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see gogeta_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        gogeta_state._set_last_init_error("OperationalError: locking protocol")
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("gogeta_state.sqlite3.connect", side_effect=gated_connect):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        gogeta_state._set_last_init_error("OperationalError: generic SQLite error")
