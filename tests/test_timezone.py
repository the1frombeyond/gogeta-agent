"""
Tests for timezone support (gogeta_time module + integration points).
...
def _reset_gogeta_time_cache():
    """Reset the gogeta_time module cache (replacement for removed reset_cache)."""
    gogeta_time._cached_tz = None
    gogeta_time._cached_tz_name = None
    gogeta_time._cache_resolved = False


# =========================================================================
# gogeta_time.now() — core helper
# =========================================================================

class TestGsaTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_gogeta_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["gogeta_timeZONE"] = "Asia/Kolkata"
        result = gogeta_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["gogeta_timeZONE"] = "UTC"
        result = gogeta_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["gogeta_timeZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="gogeta_time"):
            result = gogeta_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["gogeta_timeZONE"] = "Europe/London"
        tz = gogeta_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["gogeta_timeZONE"] = "Not/A/Timezone"
        tz = gogeta_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When gogeta_timeZONE is set, child process sees TZ env var.
        ...
        #   2) gogeta_timeZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("gogeta_timeZONE=" + os.environ.get("gogeta_timeZONE", "NOT_SET"))'
        )
        ...
        assert "gogeta_timeZONE=NOT_SET" in result["output"], (
            "gogeta_timeZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When gogeta_timeZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("gogeta_timeZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=gogeta_tz) which shifted
        absolute time when system-local tz != Gogeta tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_gogeta_tz(self):
        """Already-aware datetimes should be normalized to Gogeta tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Gogeta tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Gogeta tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Gogeta is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Gogeta tz
        ...
        # Use a Gogeta timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _gogeta_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["gogeta_timeZONE"] = "Pacific/Midway"  # UTC-11
        _reset_gogeta_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Gogeta timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["gogeta_timeZONE"] = "US/Eastern"
        _reset_gogeta_time_cache()
