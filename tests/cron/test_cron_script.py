...
- Path resolution (absolute, relative to GOGETA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp GOGETA_HOME."""
    gogeta_home = tmp_path / ".gogeta"
    gogeta_home.mkdir()
    (gogeta_home / "cron").mkdir()
    (gogeta_home / "cron" / "output").mkdir()
    (gogeta_home / "scripts").mkdir()
    monkeypatch.setenv("GOGETA_HOME", str(gogeta_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "GOGETA_DIR", gogeta_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", gogeta_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", gogeta_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", gogeta_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.gogeta/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("GOGETA_SESSION_PLATFORM") is None
        assert os.environ.get("GOGETA_SESSION_CHAT_ID") is None
        assert os.environ.get("GOGETA_SESSION_CHAT_NAME") is None
