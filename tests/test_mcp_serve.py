"""
Tests for mcp_serve — Gogeta MCP server.
...
def _isolate_gogeta_home(tmp_path, monkeypatch):
    """Redirect GOGETA_HOME to a temp directory."""
    monkeypatch.setenv("GOGETA_HOME", str(tmp_path))
    try:
        import gogeta_constants
        monkeypatch.setattr(gogeta_constants, "get_gogeta_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking gogeta_state schema."""
