...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._gogeta_home / "logs" / "errors.log").resolve()
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host GOGETA_REDACT_SECRETS state.
        ...
        monkeypatch.delenv("GOGETA_REDACT_SECRETS", raising=False)
...
class TestBuildApiKwargs:
    ...
    def test_reasoning_sent_for_nous_route(self, agent):
        agent.provider = "nous"
        agent.base_url = "https://inference-api.gogeta.ai/v1"
        agent.model = "minimax/minimax-m2.5"
        messages = [{"role": "user", "content": "hi"}]
        kwargs = agent._build_api_kwargs(messages)
        assert kwargs["extra_body"]["reasoning"]["effort"] == "medium"
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("gogeta_cli.plugins.invoke_hook", _capture_hook)
        monkeypatch.setattr("gogeta_cli.plugins.has_hook", lambda name: True)
...
class TestConcurrentToolExecution:
    ...
    def test_invoke_tool_agent_level_tool_emits_terminal_post_tool_hook(self, agent, monkeypatch):
        ...
        hook_calls = []
        monkeypatch.setattr(
            "gogeta_cli.plugins.get_pre_tool_call_block_message",
            lambda *args, **kwargs: None,
        )
        monkeypatch.setattr(
            "gogeta_cli.plugins.invoke_hook",
    ...
    def test_sequential_agent_level_tool_execution_middleware_wraps_inline_dispatch(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("gogeta_cli.plugins.get_plugin_manager", lambda: manager)
        monkeypatch.setattr(
            "gogeta_cli.plugins.invoke_middleware",
...
class TestRunConversation:
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for Gogeta tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "gogeta_cli.plugins.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("gogeta_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("gogeta_cli.plugins.has_hook", lambda name: False)
        monkeypatch.setattr("gogeta_cli.plugins.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("gogeta_cli.plugins.has_hook", _has_hook)
        ...
        with (
            patch("gogeta_cli.plugins.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("GOGETA_KANBAN_TASK", "t_test_task_123")
        ...
        with (
            ...
            patch("gogeta_cli.kanban_db._record_task_failure",
                  mock_record_failure),
            patch("gogeta_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when GOGETA_KANBAN_TASK
    ...
    def test_try_refresh_nous_client_credentials_rebuilds_client(
        ...
        def _fake_resolve(**kwargs):
            ...
            return {
                ...
                "base_url": "https://inference-api.gogeta.ai/v1",
            }
        ...
        monkeypatch.setattr(
            "gogeta_cli.auth.resolve_nous_runtime_credentials", _fake_resolve
        )
        ...
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.gogeta.ai/v1"
        )
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Gogeta Agent" in agent._cached_system_prompt
