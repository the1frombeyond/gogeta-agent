...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    gogeta_state = ModuleType("gogeta_state")
    gogeta_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "gogeta_state", gogeta_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Gogeta"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Gogeta"
