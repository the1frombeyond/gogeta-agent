"""Shared fixtures for the gogeta-agent test suite.
...
2. **Isolated GOGETA_HOME.** GOGETA_HOME points to a per-test tempdir so
   code reading ``~/.gogeta/*`` via ``get_gogeta_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".gogeta"`` instead of the canonical
   ``get_gogeta_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No GOGETA_SESSION_* inheritance** — the agent's current gateway
...
# GOGETA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_GOGETA_BEHAVIORAL_VARS = frozenset({
    "GOGETA_YOLO_MODE",
    "GOGETA_INTERACTIVE",
    "GOGETA_QUIET",
    "GOGETA_TOOL_PROGRESS",
    "GOGETA_TOOL_PROGRESS_MODE",
    "GOGETA_MAX_ITERATIONS",
    "GOGETA_SESSION_PLATFORM",
    "GOGETA_SESSION_CHAT_ID",
    "GOGETA_SESSION_CHAT_NAME",
    "GOGETA_SESSION_THREAD_ID",
    "GOGETA_SESSION_SOURCE",
    "GOGETA_SESSION_KEY",
    "GOGETA_GATEWAY_SESSION",
    "GOGETA_CRON_SESSION",
    "_GOGETA_GATEWAY",
    "GOGETA_PLATFORM",
    "GOGETA_MODEL",
    "GOGETA_INFERENCE_MODEL",
    "GOGETA_INFERENCE_PROVIDER",
    "GOGETA_TUI_PROVIDER",
    "GOGETA_MANAGED",
    "GOGETA_DEV",
    "GOGETA_CONTAINER",
    ...
    "gogeta_timeZONE",
    "GOGETA_REDACT_SECRETS",
    "GOGETA_BACKGROUND_NOTIFICATIONS",
    "GOGETA_EXEC_ASK",
    "GOGETA_HOME_MODE",
    "GOGETA_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.gogeta/kanban.db instead of the per-test GOGETA_HOME.
    "GOGETA_KANBAN_DB",
    "GOGETA_KANBAN_BOARD",
    "GOGETA_KANBAN_HOME",
    "GOGETA_KANBAN_WORKSPACES_ROOT",
    "GOGETA_KANBAN_LOGS_ROOT",
    "GOGETA_KANBAN_TASK",
    "GOGETA_KANBAN_WORKSPACE",
    "GOGETA_KANBAN_RUN_ID",
    "GOGETA_KANBAN_CLAIM_LOCK",
    "GOGETA_KANBAN_DISPATCH_IN_GATEWAY",
    "GOGETA_TENANT",
    ...
    "GOGETA_DASHBOARD_OAUTH_CLIENT_ID",
    "GOGETA_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and GOGETA_HOME to per-test tempdirs so code that
    reads ``~/.gogeta/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral GOGETA_* vars that could change test semantics.
    for name in _GOGETA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect GOGETA_HOME to a per-test tempdir. Code that reads
    #    ``~/.gogeta/*`` via ``get_gogeta_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.gogeta/*`` via
    #    ``Path.home() / ".gogeta"`` instead of ``get_gogeta_home()``
    #    is a bug to fix at the callsite.
    fake_gogeta_home = tmp_path / "gogeta_test"
    fake_gogeta_home.mkdir()
    (fake_gogeta_home / "sessions").mkdir()
    (fake_gogeta_home / "cron").mkdir()
    (fake_gogeta_home / "memories").mkdir()
    (fake_gogeta_home / "skills").mkdir()
    monkeypatch.setenv("GOGETA_HOME", str(fake_gogeta_home))
    ...
    #    ~/.gogeta/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import gogeta_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_gogeta_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal gogeta config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``gogeta-gateway`` process
...
# correlated with a ``tests/gogeta_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> gogeta-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart gogeta-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting gogeta/python patterns are also blocked.
    """
    ...
    def _matches_gogeta_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _GOGETA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_gogeta_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch gogeta-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m gogeta_cli.main".
                if (
                    "gogeta" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live gogeta-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting gogeta/python could hit the live gateway. "
