...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves gogeta_cli.kanban_db on its own
  ...
  - worker logs are captured to GOGETA_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            ...
            **os.environ,
            ...
            "GOGETA_KANBAN_TASK": task.id,

            "GOGETA_KANBAN_WORKSPACE": workspace,
...
def main():
    ...
    home = tempfile.mkdtemp(prefix="gogeta_e2e_")
    ...
    # Point the `gogeta` CLI child processes will run at the worktree

    # gogeta_cli.main. We do this by putting a shim on PATH.

    shim_dir = os.path.join(home, "bin")

    os.makedirs(shim_dir, exist_ok=True)

    shim_path = os.path.join(shim_dir, "gogeta")

    with open(shim_path, "w") as f:

        f.write(f"""#!/bin/sh

exec {PY} -m gogeta_cli.main "$@"
...
""")
    ...
    # Scenario A workers wrote to /tmp/gogeta_e2e_*/worker_*.log
