...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``gogeta_cli``) for the package
    itself and ``gogeta_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``gogeta_cli`` without ``gogeta_cli.*``,
    so the wheel shipped ``gogeta_cli/*.py`` but dropped the ``dashboard_auth``
    and ``proxy`` subpackages. The dashboard then died on every install with
    ``ModuleNotFoundError: No module named 'gogeta_cli.dashboard_auth'``.
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (gogeta_cli/plugins.py) registers each bundled plugin by
