...
SQLite State Store for Gogeta Agent.
...
DEFAULT_DB_PATH = get_gogeta_home() / "state.db"
...
# gogeta_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``gogeta_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see gogeta_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple gogeta processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``gogeta_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `gogeta update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        ...
        try:
            ...
            cursor.execute("CREATE VIRTUAL TABLE temp._gogeta_fts5_probe USING fts5(x)")

            cursor.execute("DROP TABLE temp._gogeta_fts5_probe")
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Gogeta surface and jump
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Gogeta, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Gogeta session.



        A Gogeta session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        ...
        """Return True if a Gogeta session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``GOGETA_DISABLE_FTS_TRIGRAM`` or not yet created), so
