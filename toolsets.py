...
_GOGETA_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (GOGETA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `gogeta tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (GOGETA_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Gogeta toolsets (CLI + messaging platforms)
    #
    ...
    "gogeta-cli": {
        ...
        "tools": _GOGETA_CORE_TOOLS,
        "includes": []
    },

    "gogeta-cron": {
        # Mirrors gogeta-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `gogeta tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as gogeta-cli; gated by `gogeta tools`",
        "tools": _GOGETA_CORE_TOOLS,
        "includes": []
    },
    ...
    "gogeta-email": {
        "description": "Email bot toolset - interact with Gogeta via email (IMAP/SMTP)",
        "tools": _GOGETA_CORE_TOOLS,
        "includes": []
    },
    ...
    "gogeta-sms": {
        "description": "SMS bot toolset - interact with Gogeta via SMS (Twilio)",
        "tools": _GOGETA_CORE_TOOLS,
        "includes": []
    },
    ...
    "gogeta-webhook": {
        ...
        "tools": _GOGETA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "gogeta-gateway": {
        ...
        "tools": [],
        "includes": ["gogeta-telegram", "gogeta-discord", "gogeta-whatsapp", "gogeta-slack", "gogeta-signal", "gogeta-bluebubbles", "gogeta-homeassistant", "gogeta-email", "gogeta-sms", "gogeta-mattermost", "gogeta-matrix", "gogeta-dingtalk", "gogeta-feishu", "gogeta-wecom", "gogeta-wecom-callback", "gogeta-weixin", "gogeta-qqbot", "gogeta-webhook", "gogeta-yuanbao"]
    },

    "self_config": {
        "description": "Self-configuration tools — automatically install MCP servers, configure platform connectors (WhatsApp, Telegram, Discord), and handle generic setup tasks. Part of the Gogeta Autonomy Directive (Law 2).",
        "tools": ["self_configure"],
        "includes": []
    },

    "user_created": {
        "description": "Autonomously created tools — generated, tested, and wired at runtime by the ToolCreatorEngine. Part of the Gogeta Autonomy Directive (Law 4).",
        "tools": ["create_tool", "detect_missing_tools"],
        "includes": []
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (gogeta-<name>).
        # Gives them _GOGETA_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("gogeta-"):
            platform_name = name[len("gogeta-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_GOGETA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
